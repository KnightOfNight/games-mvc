# Shyland Project Instructions v23
...
**Multiple design chats per major version (v23+).** A major version runs roughly **one design chat per bucket**, planned that way from the start rather than discovered halfway through. Nothing carries between chats except the issue tracker and the committed documents — a chat that needs context reads issues and repo docs, never another chat. Each chat therefore verifies its own work from committed reports before handing off.
...
- **Brief numbers map to buckets, not execution order.** Brief N implements bucket BN; the order they *run* is an operator ruling made independently (v23 executed 2, 1, 3, 4, 5). Nothing may assume it is first — see the architecture-doc rule below.
- **A bucket may split across briefs by mechanism.** Where one bucket contains both a code change and the data that depends on it, splitting them into two briefs is preferred to one oversized brief: v23's B4 shipped as a code half (pool machinery) and a data half (the authored corpus), with the founding issue closed by the second. The split costs a brief slot and is usually worth it.
...
- **Deploy-time data-action rule:** any brief that defers actions to deploy time (seed reruns, data fixup commands, `make seed` itself) must (a) list them in a dedicated **PENDING DEPLOY-TIME ACTIONS** block in its closeout report, and (b) every subsequent brief/amendment in the same version carries a pre-flight line reporting whether prior pending actions have executed, until confirmed done. Design-chat verification treats unexecuted deploy-time actions as open items — never silently passed. **`make seed` is one of these** — a content brief's work is invisible in production until the seed runs, and a build does not run it
- **Code first, data second:** when a brief ships both a behavior change and the seed data that depends on it, the code deploys **before** the data action runs. Seeding content that only reads correctly under the new code would otherwise serve the very bug the brief is fixing, live, to players (v23 B5: narration greetings under the old render rule)
- **Seed-owned replacements delete rows — say how many.** Under enforce-exact seeding, replacing an authored row deletes the old one. A brief that replaces seed-owned content states the **expected deletion count as a number**, never "zero deletions expected" by reflex; the closeout reports the actual count against it
- **Test hygiene when a fixed string becomes a pool:** pre-existing tests that pin a literal the brief is pooling get converted to pool-membership assertions **with the original intent preserved as explicit assertions**, and the conversion is reported as a deviation in the closeout — never changed silently. Briefs that pool authored strings should anticipate this and say so
...
- **Architecture doc file handling — create-or-update, always:** every implementation brief of a **major version** carries the same conditional step: **create** `Shyland_Architecture_vN.md` if it does not yet exist (`git rm` the old, written header-first then one section at a time), **update it in place** if it does. Creation duty follows **first-to-execute, not brief number** — brief numbers map to buckets and the running order is ruled separately, so no brief may assume it is first (v23: B2 ran first and created the file). A **point-release** brief never creates a new file — it updates the current `Shyland_Architecture_vN.md` in place and bumps the stamp to N.M (hash moves only for architectural changes)
- **Standing implementation-brief requirements — never omit these.** Every implementation brief carries all three:
  1. **Version constant.** The *first* implementation brief of a major version bumps `SHYLAND_VERSION` in `django/src/apps/shyland/version.py` to `"N.0-DEV"`; the closeout ritual bumps it to `"N.0"`. Later briefs state that they leave it alone.
  2. **An operator-authorized in-session production deploy.** The invocation is **exactly `make build && make migrate`** — never `make prod`, never `make deploy`, with `DOCKER_HOST` verified in pre-flight. Any deploy-time data actions execute in the production container in the same session where authorized, with their results recorded in the closeout; the PENDING block is the fallback only when the deploy is not authorized.
  3. **A "Ready after deploy" operator playtest checklist**, itemized, so the operator can play the change immediately.
...
- **Operator stashes are apply-only.** A brief may permit `git stash apply` of an operator experiment as a starting point — only when the resulting diff is then verified line-for-line against the brief and any experiment markers are replaced with permanent issue-referenced comments. **Never `drop`, never `pop`**; the stash stack is the operator's, same as pruning
- **Binary documentation assets are never chased.** If a generated asset (the color chart PNG, any rendered twin) needs regenerating and no renderer is already present, CC reports it **stale in the closeout** and stops. It does not install tooling to produce it
...
- **Issue callouts:** whenever Claude believes something warrants a GitHub issue, it says so in a clearly visible **"Issue callout:"** and files it via brief unless the operator rules otherwise. Every CC-filed issue is assigned `--assignee "@me"` — the `KnightOfNight/@me` form is **not valid `gh` syntax** and has burned a filing step; do not reintroduce it
- **The triaged label means cold-start-ready:** an issue is `triaged` only when it carries enough diagnosis plus ruling for a brand-new design chat to pick it up with no other context. Symptoms-only filings are not triaged
