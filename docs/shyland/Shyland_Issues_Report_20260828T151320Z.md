...
| 304 | Two bots on one target: per-checkout pidfiles cannot see a same-target bot running from another checkout — enforce a per-(bot, target) singleton | KnightOfNight |  |  | 2026-08-28 |
| 305 | sudo bot: the conversation store records model prose as truth — a false action claim poisons the thread (persist receipts with the answer) | KnightOfNight |  |  | 2026-08-28 |
| 306 | sudo bot: everything the bot delivers should render deterministically — machinery composes and the game renders all data-shaped output; model prose is commentary only | KnightOfNight |  |  | 2026-08-28 |
