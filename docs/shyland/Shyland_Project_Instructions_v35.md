# Shyland Project Instructions v35

**Status: ACTIVE — approved by the operator 2026-08-15. Supersedes Shyland_Project_Instructions_v34.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v35 edition is the action-item pass (#249's proven pattern, operator-directed codification): a long-lived ticket whose work is deliberately partitioned across releases never joins a release milestone — its release-bound slice rides the release as ordinary brief scope tracked by a ruling comment on the ticket, and the ticket closes in an ops session when its arc completes; the litmus is closes-with-the-release (milestone, the dependency case) vs outlives-the-release (never the milestone). (v34 the interim-retirement pass; v33 the prod-verification ruling pass; v32 the gift-language pass; v31 the seed-prod pass; v30 the label model; v29 the release model; v28 the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
