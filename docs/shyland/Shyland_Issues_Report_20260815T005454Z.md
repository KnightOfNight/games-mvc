...
| 211 | Silver accessory tier: Mk 2 jewelry needs the tier-material ladder extended (silver definitions + Mk-mismatch ruling) | KnightOfNight | triaged, V24 | Version 24.28 | 2026-08-15 |
...
| 235 | New player setting: 'plunder' (default off) — auto-loot rights-held corpses at combat end | KnightOfNight | commands |  | 2026-08-15 |
...
| 245 | Tier-material ladder: rule its full extent (eight rungs, copper -> sphaerium) and its unbounded terminal rung | KnightOfNight | triaged, V24, itemization | Version 24.28 | 2026-08-15 |
...
## Design ruling — 2026-08-14, Version 24.28 design session

**#211 is the founding ticket of Version 24.28** (milestone `Version 24.28`, branch `version_24_28`). One founding ticket, one brief, per the release scope law. On theme for the V24 new-zone-prep major: the ladder's silver rung is prerequisite content for V25's Mk 2 zones (Z02/Z03).

### Ruling 1 — The ladder is one curve; the material is the rung's name

The silver definitions mirror the copper set **exactly** in stat authorship, differing only in identity (slug, name, description):

- `primary_stats`: `base 0.7, factor 2.1` (Mk 1 midpoint m1 = 2.8, authored as the #130 full lift `(0.25, 0.75) × m1`)
- `secondary_stat_pool`: two entries at `base 0.175, factor 0.525` (m1 = 0.7, same lift)
- `takes_durability_loss: False`, `durability_table: []`, `suppress_mk_suffix: True`, `item_type: accessory`, `genre_tag: fantasy`

Because `_roll_stat` computes `midpoint = base + factor × mk_tier`, the identical authorship already yields **2.8 at Mk 1 (copper) and 4.9 at Mk 2 (silver)** — correct tier progression with no double-dip and no new balance pass. The rejected alternative — an Mk-2-native curve of silver's own — would make copper-at-Mk-2 and silver-at-Mk-2 numerically different items, reintroducing exactly the name/tier incoherence this issue was filed about, and would make every future rung (gold, platinum) its own tuning exercise.

**Set shape:** 12 new ItemDefinitions mirroring the copper set one-for-one — `Silver Ring of <stat>` ×6 (`RING`) and `Silver Amulet of <stat>` ×6 (`NECK`) across str/dex/end/int/wis/per, each carrying the same primary stat and the same two-entry secondary pool as its copper counterpart. Names and descriptions are authored at implementation time under the standing creative-content policy.

**No drop tables.** Z02/Z03 do not exist yet (V25). The silver set ships as seeded definitions with zero LootTableEntry rows — this is prep work, which is the major's whole theme.

### Ruling 2 — `base_value` 30, same as copper

Valuation already multiplies `base_value` by `mk_tier`, so a silver accessory at Mk 2 is worth 60 against a copper accessory's 30 — the standard 2× every item type gets per tier. Pricing silver at the currency ladder's 10× (300) was considered and **rejected**: GDD §6 calls tier-material naming "a display alias, same math, zero engine change," and a 600-copper vendor sale off a routine Mk 2 accessory drop is a large economy change that has no business riding a naming ticket.

### Ruling 3 — Hard Mk-mismatch guard on a new `tier_material_mk` field

Name/tier agreement becomes **enforced, not conventional**:

- New field **`ItemDefinition.tier_material_mk`** — `PositiveSmallIntegerField(null=True, blank=True)`, help text to the effect of "When set, this definition is a rung on the tier-material ladder and instances may only be generated at this Mk tier." Migration required.
- **Copper set → `tier_material_mk=1`. Silver set → `tier_material_mk=2`.** Everything else stays `null`.
- **`generate_item_instance` raises `ValueError`** when a definition with a non-null `tier_material_mk` is asked for any other `mk_tier`. This binds every path — seed, drop generation, and the admin gift path that actually produced the bad instances.

Seed-verification-only and convention-only were both rejected: the failure this issue documents came in through an admin gift, which is precisely the path those options leave open.

**This also un-overloads `suppress_mk_suffix`, which is why the guard needs its own field.** Two of the fourteen definitions carrying that flag are not tier materials at all — `tarnished-band` and `cloudy-glass-pendant`, the v19 freebie kit — so a guard keyed on `suppress_mk_suffix` would wrongly bind them to a tier. With `tier_material_mk` carrying the ladder fact, `suppress_mk_suffix` goes back to meaning exactly and only "never print the Mk N suffix," and the freebie kit keeps its suppression with no rung. GDD §6's tier-material passage is amended accordingly in this release (GDD-first, landing with this ruling).

### Existing-instance disposition — verify, do not migrate

The guard governs **generation only**; it does not touch instances already in the database.

- **Dev:** the three off-seed silver clone definitions created for the V24.14 playtest are already gone. `ItemDefinition` is seed-owned and swept by `_sweep_all`, and `ItemInstance.definition` is `on_delete=CASCADE` — a prior reseed deleted the clones and cascaded Harley Stone's three Mk 2 instances with them. No cleanup owed.
- **Production:** copper accessories drop only in Z01 at Mk 1, so the expected count of mismatched instances is **zero**. The brief carries a read-only verification query, not a data action.

### Scope note

No `LootTable`/`LootTableEntry`/`VendorEntry` changes; no combat, valuation, or display-code changes beyond the generation guard. Seed data + one field + one migration + one guard + GDD §6 text.


**KnightOfNight** — 2026-08-15:
## Ruling amendment — 2026-08-14, same design session

The operator directed, in session, that **the whole tier-material ladder ships in Version 24.28** rather than the silver rung alone. The full ladder ruling — seven rungs (copper → silver → gold → platinum → rhodium → iridium → osmium), the terminus above Mk 7, and the currency-table decoupling — is recorded on **#245**, filed as this ticket's dependency (`#211 blocked by #245`).

### What changed from the ruling above

| | Original ruling | Amended |
|---|---|---|
| Rungs shipped | silver only (12 defs) | silver, gold, platinum, rhodium, iridium, osmium (**72 defs**) |
| `tier_material_mk` values in use | 1, 2 | **1–7** |
| Expected seed reconciliation | 12 created / 12 updated / 0 deleted | **72 created / 12 updated / 0 deleted** |
| Ladder definitions total | 24 | **84** |

**Unchanged:** rulings 1, 2 and 3 above stand exactly as written. One curve for every rung (identical stat authorship; the engine's `base + factor × mk_tier` supplies progression). `base_value` 30 at every rung. Hard guard in `generate_item_instance` on the `tier_material_mk` field, with `suppress_mk_suffix` un-overloaded back to a display flag. Existing-instance disposition is still verify-not-migrate.

### The scope-law position, stated plainly

This **is** a widening of this ticket's founding scope — #211 was filed for the silver rung, and it now carries six. It is recorded as such rather than folded in silently. #245 is the dependency ticket that makes the widened scope explicit and describes the same problem (extending the tier-material ladder), which is the form the scope law asks for when a release grows this way. Both tickets close together when the brief's verification passes.

The rungs above silver ship with **no drop-table and no vendor entries** — Z02 through Z07 do not exist yet. This is prep work, which is the V24 major's whole theme; the marginal build cost is near zero because every rung is copper's curve under a different name.

### Consequences for the brief

`docs/shyland/Shyland_V24.28_Brief_1_Silver_Accessory_Tier.md` is rewritten for the full ladder: the definition table covers all six new materials, the seed counts and ladder-total assertions move to the numbers above, the tests pin curve equality across every rung rather than silver alone, and the playtest checklist exercises a high rung as well as silver. The filename keeps its name — internal cross-references must match it.


**KnightOfNight** — 2026-08-15:
## Ruling amendment 2 — 2026-08-14, same design session

The operator named a terminal metal in session: **Sphaerium** — the ladder's eighth rung, **unbounded above**, covering Mk 8 and every tier beyond it forever. Named for the spheres, standing on the Primordial Sphere at the Heart. Full ruling on #245.

This **replaces** the previous terminus rule. There is no longer a fallback to flavor materials plus Mk suffixes above the ladder's top — the ladder now names every tier that can exist.

### What changed from amendment 1

| | Amendment 1 | Amended again |
|---|---|---|
| Rungs | 7 (copper → osmium), fallback above | **8** (copper → sphaerium), **no fallback** |
| Terminal rung | osmium, Mk 7 only | **sphaerium, Mk 8 and unbounded above** |
| Rungs shipped | 6 (72 defs) | **7 (84 defs)** |
| Ladder total | 84 | **96** |
| Membership field | `tier_material_mk` (single) | **`tier_material_mk_min` + `tier_material_mk_max`** (max null = unbounded) |
| Expected seed reconciliation | 72 created / 12 updated / 0 deleted | **84 created / 12 updated / 0 deleted** |

### Sphaerium shows its Mk suffix — the one rung that does

Every finite rung suppresses because the material *is* the tier. Sphaerium spans infinitely, so suppression would render a Mk 8 and a Mk 47 piece identically — a name carrying no tier signal across an unbounded range, which is the exact defect this ticket was filed about. `Sphaerium Ring of Strength Mk 15` reads correctly: the finite metals name your tier; past them the material says only that you are beyond the ladder, and the number says how far.

**This is the payoff for ruling 3 above.** Un-fusing `suppress_mk_suffix` from ladder membership was filed as freebie-kit housekeeping; it is now load-bearing. Sphaerium is on the ladder *and* shows its suffix — with both facts on one flag, the rung could not have been expressed.

### Membership becomes a bounded range

`tier_material_mk` is replaced before it ships by a nullable pair: `tier_material_mk_min` / `tier_material_mk_max`. Null `min` = off the ladder. Non-null `min` with null `max` = unbounded above (the Sphaerium shape). The guard refuses any `mk_tier` below `min` or above a non-null `max` — copper at Mk 2 still raises; sphaerium at Mk 47 passes.

**Unchanged:** rulings 1 and 2 above stand exactly as written. One curve for every rung. `base_value` 30 at every rung. Hard guard in `generate_item_instance`. Existing-instance disposition still verify-not-migrate.

The brief is `docs/shyland/Shyland_V24.28_Brief_1_Tier_Material_Ladder.md` — renamed from the silver-only draft, which was never committed. Amendment 1's note that the filename would stay is superseded by this line.
...
## Issue #235: New player setting: 'plunder' (default off) — auto-loot rights-held corpses at combat end
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/235

### Body

## Request

A new player setting in the `brief`/`echo` family: **`plunder`**, syntax `plunder [on|off]`, **default off**. When on: at combat end, the player automatically loots any corpses they have rights to.

## Grounding in current mechanics

- **Loot rights already exist and are exactly the needed predicate:** `corpse.killed_by_id == character.pk` (the same filter bare `loot` uses for its sweep, #189). Autoloot is effectively "run the bare-`loot` sweep automatically."
- **Settings pattern to follow:** `brief`/`echo` — toggle command with `[on|off]`, help-table entry ("Default: off."), per-character persistence. Note: unlike recent runtime-only releases, this needs a **migration** (new Character boolean).
- **Trigger point:** the combat-end moment. The natural hook is the engine's last-kill completion (the "Combat has ended." path) — which conveniently is where the corpse was just created and rights assigned. Flee also ends combat but moves the player away, and death ends combat with no rights — neither should plunder.

## Design-session considerations

- **Ordering vs. re-aggro:** in aggro-respawn rooms (The Choke), a fresh engagement can follow combat-end within a round; plunder should fire in the combat-end motion, before any new engagement can re-block looting (loot is combat-blocked, #29). Worth ruling whether plunder that loses this race simply doesn't fire (miss) or queues.
- **Output:** the loot sweep already emits per-item lines through the choke point (#63 bulk rule) plus the copper line; combat-end + plunder produces a burst — probably fine, but the design session may want the sweep's existing messaging reused verbatim so plunder output is indistinguishable from typed `loot`.
- **Full-inventory refusals:** reuse the existing loot refusal messages; plunder must not silently eat them.
- **Interactions on record:** #218 (zombie sessions would have suppressed plunder exactly as they block manual loot — its fix helps here); #220 (kill attribution = killing blow today; if multiplayer attribution is ever re-ruled, plunder rights follow automatically via the killed_by predicate); #189 (bare-loot sweep is the semantic plunder reuses).
- GDD: §9 command reference + wherever the settings family is documented, shipping with the release that ships this.
...
**KnightOfNight** — 2026-08-12:
Operator ruling input (2026-08-12): the re-aggro race is a non-issue — rule is MISS, never queue. Rationale in the operator's words: if you get more aggro, you're still in combat; only when all the monsters are dead are you out of combat. Autoloot fires only at a true out-of-combat transition. Note the self-correcting property this gives for free: rights-held corpses persist until decay, so a sweep skipped by re-engagement is simply picked up by the NEXT combat-end sweep — the 'lost race' costs nothing but a short delay, no queueing machinery needed.

**KnightOfNight** — 2026-08-12:
Operator ruling input, addendum (2026-08-12): autoloot must NOT fire between monsters within a fight — a mid-combat kill (one NPC down, others still alive in the session) never triggers a loot, even of that fresh corpse. The one and only trigger is the full combat-end transition: every monster in the fight dead. Combined with the previous comment, the trigger definition is now exact: autoloot runs the rights-scoped sweep at the moment the player transitions to genuinely out-of-combat, and at no other time — no per-kill loots, no queueing, and skipped sweeps self-correct at the next such transition.

**KnightOfNight** — 2026-08-12:
Rename (2026-08-12): the setting is 'plunder', not 'autoloot' — 'plunder [on|off]', default off. Suggested by GunnySam's IRL human. ❤️ Title and body updated; the two trigger rulings in the comments above apply unchanged. (Historical note: earlier comments say 'autoloot' — same feature.)

**KnightOfNight** — 2026-08-12:
## Consequence analysis: plunder completes the park-and-level loop (operator + session, 2026-08-12)

**Operator observation:** shipping plunder gives players park-and-level-until-inv-fills-up.

**Precise framing:** the AFK loop predates this feature. Auto-attack (the engine queues attacks for idle session members) + aggro-respawn re-engagement already form a zero-input XP loop in production today. What contains it is reward leakage: unlooted corpses decay and `copper_drop` never transfers. Plunder plugs exactly that leak — it connects the existing engine to the bank.

**Natural limits already in the design:**

- **XP self-caps:** the outleveled-decay ladder (−20%/level, 10% floor) collapses parked XP to a trickle once the player outgrows the room. Park-and-level is really park-and-level-a-little — the ladder was built for this.
- **Survival isn't idle:** healing is manual, so true AFK parking only works in outleveled content — which is exactly where XP already pays nothing. The safety curve and the reward curve cross correctly.
- **Income does NOT decay:** outleveled commons still sell, copper still transfers. Brakes: carry capacity (STR×10 + bags), corpse decay, low Mk 1 sell values. Slow, but infinite and free — this is the half with no designed ceiling.

**Constellation links:** the uncapped half lands in the healing-economy ledger — #164 (income vs draught cost) gains a new income line. If idle income ever needs a countermeasure, the lever is not nerfing plunder but making infinite combat non-free — which #70 (Longevity's first consuming mechanic) and #142 (in-combat acuity drift) already want for their own design reasons.

**Operator disposition (2026-08-12): no cause for concern yet.** Low-grade idle farming is genre-traditional; the XP half is already gutted by the ladder, the income half is bag-bounded. The design session that ships plunder should ratify this knowingly — suggested ruling shape: "accepted consequence; economy watch via #164."
...
## Issue #245: Tier-material ladder: rule its full extent (eight rungs, copper -> sphaerium) and its unbounded terminal rung

- State: open
- Author: KnightOfNight
- Labels: triaged, V24, itemization
- Milestone: Version 24.28
- Assignees: KnightOfNight
- Created: 2026-08-15 | Updated: 2026-08-15
- Blocked by: none
- Blocks: #211
- URL: https://github.com/KnightOfNight/games-mvc/issues/245

### Body

## Summary

The tier-material naming ladder has never had a ruled extent. GDD §6 established it at v18 with a single rung (copper, accessories only) and one forward-looking sentence — "later zones extend the ladder upward with the nobler metals as Mk tiers rise" — naming neither the metals, nor how far the ladder runs, nor what accessories are called above its top.

#211 forced the question by needing the second rung. This issue carries the full ruling and scopes the remaining rungs. It is a **dependency of #211, describing the same problem** — extending the tier-material ladder — not a widening into a different one.

## Ruling (2026-08-14, Version 24.28 design session)

### 1. The ladder is eight rungs, and the last one is infinite

| Mk tier | Material | Levels | Mk suffix | Note |
|---|---|---|---|---|
| 1 | Copper | 1–10 | suppressed | shipped v18 |
| 2 | Silver | 11–20 | suppressed | |
| 3 | Gold | 21–30 | suppressed | |
| 4 | Platinum | 31–40 | suppressed | last rung borrowed from the currency table |
| 5 | Rhodium | 41–50 | suppressed | platinum-group; costliest by weight |
| 6 | Iridium | 51–60 | suppressed | rarer, harder |
| 7 | Osmium | 61–70 | suppressed | densest natural element — last finite rung |
| **8+** | **Sphaerium** | **71+** | **shown** | **unbounded — covers every tier above osmium, forever** |

Rungs 1–7 are real metals, so the ordering carries some outside intuition rather than having to be taught from scratch. Rung 8 is **Sphaerium** (operator, 2026-08-14) — named for the spheres, and the Primordial Sphere at the Heart of the Convergence is the fiction it stands on: that sphere didn't predate the zone-end sphere pattern, *it started it*. A metal named for the spheres is the right thing to sit where the ladder stops being a ladder.

### 2. Sphaerium solves the infinity problem instead of deferring it

Mk tiers are infinite by ruling — the Wastelands scales forever and a level 150 character finds Mk 15 loot. An earlier draft of this ruling ended the ladder at osmium and fell back to flavor materials plus Mk suffixes above it. Sphaerium replaces that fallback entirely: **the terminal rung is itself unbounded**, covering Mk 8 and every tier above it without limit. There is no fallback rule, no naming discontinuity at the frontier, and no tier the ladder fails to name.

### 3. Sphaerium is the one rung that shows its Mk suffix

Every finite rung suppresses the suffix because the material *is* the tier — one material, one tier, and the name says it. Sphaerium spans infinitely, so suppression would render a Mk 8 and a Mk 47 piece identically: a name carrying no tier signal across an unbounded range, which is exactly the lying-name defect #211 exists to kill.

So the terminal rung shows its number — `Sphaerium Ring of Strength Mk 15` — and the fiction reads correctly. The finite metals name your tier. Past them, the material tells you only that you are beyond the ladder, and the Mk number says how far beyond.

This is expressible **only because this release un-fuses `suppress_mk_suffix` from ladder membership** (see ruling 6). Sphaerium is on the ladder *and* shows its suffix; with the two facts carried by one flag, this rung could not have been stated at all.

### 4. The ladder borrows the currency table's names; it does not track it

GDD §6 currently says the ladder "tracks the currency table." That is true for four rungs and then breaks, because the two ladders have unrelated pacing:

| | 1 | 2 | 3 | 4 | 5 |
|---|---|---|---|---|---|
| **Currency** | 1 cp | 10 | 1,000 | 1,000,000 | ~10^10 |
| **Mk tier** | lvl 1–10 | 11–20 | 21–30 | 31–40 | 41–50 |

Currency tier 5 is astronomically out of reach; Mk 5 is mid-game. The ladder therefore **borrows** the currency table's four names for its first four rungs and continues on its own authority past platinum. GDD §6 is re-scoped accordingly.

### 5. Every rung is the same curve

Per the #211 ruling, a rung mirrors the rung below it exactly in stat authorship — same primary entry, same secondary pool, same Mk 1 midpoints — because a midpoint already computes as `base + factor × mk_tier`. Rungs differ in name and description and nothing else, `base_value` included (30 for every accessory rung; the tier multiplier alone separates them). No rung is balanced against its neighbors; there is one curve and eight names for reading it.

### 6. Membership is enforced, and it is a bounded range

`ItemDefinition` carries **`tier_material_mk_min`** and **`tier_material_mk_max`** (both nullable). A null `min` means the definition is not on the ladder. A non-null `min` with a null `max` means the rung is **unbounded above** — the Sphaerium shape. `generate_item_instance` refuses any `mk_tier` below `min`, or above a non-null `max`.

| | `min` | `max` | `suppress_mk_suffix` |
|---|---|---|---|
| Copper | 1 | 1 | True |
| … | n | n | True |
| Osmium | 7 | 7 | True |
| **Sphaerium** | **8** | **null** | **False** |

`suppress_mk_suffix` remains a display flag only and is explicitly **not** the ladder predicate — the v19 freebie kit (`tarnished-band`, `cloudy-glass-pendant`) suppresses the suffix without standing on any rung, and Sphaerium stands on a rung without suppressing.

### 7. The ladder still covers accessories only

Weapons and armor are untouched. Extending tier-material naming to other item types is not ruled here and would be its own ticket.

## Scope in Version 24.28

Operator ruling (2026-08-14): **the whole ladder ships at once.** All seven unshipped rungs — silver, gold, platinum, rhodium, iridium, osmium, sphaerium — are seeded in this release as **84 new ItemDefinitions** (12 per rung: `<Material> Ring of <Stat>` ×6 and `<Material> Amulet of <Stat>` ×6), with the copper set gaining its `min`/`max` of 1. The ladder totals **96 definitions**.

The rungs above silver are **prep work with no drop tables and no vendor entries** — Z02 through Z08 do not exist yet. Seeding the ladder complete now means no future zone release has to stop and author jewelry before it can drop any; the marginal build cost is near zero because every rung is the same curve as copper.

Recorded as a deliberate widening of #211's founding scope, at the operator's direction, with this ticket as the dependency that makes the widened scope explicit rather than silent.

## Implementation

Covered entirely by `docs/shyland/Shyland_V24.28_Brief_1_Tier_Material_Ladder.md` on branch `version_24_28`. Closes with #211 when that brief's verification passes.
