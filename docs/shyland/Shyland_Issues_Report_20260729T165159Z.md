...
| 12 | Two-factor authentication via TOTP (django-otp) | KnightOfNight |  | Authentication | 2026-07-29 |
...
| 125 | player macro/alias system | KnightOfNight |  | Version 24 | 2026-07-29 |
...
| 139 | Healing consumables can't track vitality growth — the draught tier needs an evolution pass | KnightOfNight |  | Healing Economy | 2026-07-29 |
...
| 162 | Reduce the home command cooldown from 15 minutes to 5 | KnightOfNight |  |  | 2026-07-29 |
| 163 | Map: more information — starting with percentage of the current zone explored | KnightOfNight |  |  | 2026-07-29 |
| 164 | Healing economics: draught dependence outruns player income — sell values and money drops can't fund the staple | KnightOfNight |  | Healing Economy | 2026-07-29 |
| 165 | Out-of-combat vitality regen is too slow and doesn't scale | KnightOfNight |  | Healing Economy | 2026-07-29 |
| 166 | Top-level heal command — 'use infinity healing draughts' semantics | KnightOfNight |  | Healing Economy | 2026-07-29 |
| 168 | GDD §6/§9 say use-heal consumption is lowest-value first; shipped v23.3 behavior is oldest-first | KnightOfNight |  |  | 2026-07-29 |
...
- Milestone: Authentication
- Assignees: KnightOfNight
- Created: 2026-07-11 | Updated: 2026-07-29
- Blocked by: none
- Blocks: none
...
- Milestone: Version 24
- Assignees: none
- Created: 2026-07-20 | Updated: 2026-07-29
- Blocked by: none
- Blocks: none
...
- Author: KnightOfNight
- Labels: none
- Milestone: Healing Economy
- Assignees: KnightOfNight
- Created: 2026-07-22 | Updated: 2026-07-29
- Blocked by: none
- Blocks: none
...
## Issue #162: Reduce the home command cooldown from 15 minutes to 5
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/162

### Body

Reduce the `home` command cooldown from 15 minutes to **5 minutes** (300 seconds).

Current state: `Character.home_cooldown_seconds` (`models.py:277`, added v22 B3 / #57) defaults to `900`. Completion-only consumption — the clock starts only when the traveler actually lands at the Heart; interrupted countdowns don't consume it.

Implementation notes specific to this change:

- This is a **per-character override field**, not a global constant — every existing character row carries a stored `900`. Changing the model default only affects newly created characters; the change needs a ruling on how existing rows update (data migration flipping rows still at the old default, vs. touching all rows regardless of override). Any character an admin has deliberately overridden should presumably keep their override.
- The `AlterField` default change generates a migration; migrate runs regardless (standing rule), and the data-side update is a deploy-time action if done via command rather than migration.
- The cooldown refusal pool (`HOME_COOLDOWN_POOL`, `consumers.py:2627`) says "Give it a few minutes" — still accurate at 5 minutes, no wording change required.
...
## Issue #168: GDD §6/§9 say use-heal consumption is lowest-value first; shipped v23.3 behavior is oldest-first
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/168

### Body

Two GDD phrases describe `use` heal consumption as **lowest-value first**, but the shipped v23.3 behavior is **oldest-first**.

**Where:**
- `docs/shyland/gdd/section_09_player_command_reference.md` — Partial Fulfillment, the Heal sequences bullet (#61, #65; reruled by #151): "exactly the items the heal needs are consumed (capped by N and inventory, lowest-value first as ever)"
- `docs/shyland/gdd/section_06_economy_and_items.md` — Transactional aggregation bullet: "only the items the heal needs consumed (lowest-value first)"

**What actually shipped (v23.3 Brief 1, recorded deviation 1, operator-attested benign):** the brief's operative instruction was "walk `res.items` in resolution order, never re-sort," and the resolver's `use` policy selection is `'oldest'` (the `command_grammar.Policy` default — only sell/drop use `'lowest'`). Consumption order is therefore oldest-first, pinned by `tests/test_use_output_merge.py` and stated in a code docstring. The architecture doc and the GDD v23.3 changelog row both describe oldest-first; only the two section phrases above are stale.

**Fix needed:** design-session wording pass on those two phrases (GDD source is design-session territory — the v23.3 closeout's marker sweep was parenthetical-deletion-only and could not touch prose). Alternatively, if a design session prefers lowest-value-first as the actual behavior, that's a re-ruling with a code change — but the shipped, tested, attested behavior is oldest-first.


### Comments (1)

**KnightOfNight** — 2026-07-29:
Process ruling for this issue (operator, 2026-07-29 — codified in Instructions v27 / #169): this is a **GDD erratum** — the game is right, the docs are out of date. It is handled by ONE operator-directed **ops session on main**: correct the affected GDD section files to match SHIPPED behavior only, run `make gdd`, commit directly to main (no PR, no version, no stamp move, no changelog row), close this issue, run the issues report. The erratum test gates every edit: describing what shipped is in scope; anything not shipped, or any question needing a design ruling, stops the session — that part goes back to a design session. See the GDD Errata rule in `Shyland_Project_Instructions_v27.md`.
