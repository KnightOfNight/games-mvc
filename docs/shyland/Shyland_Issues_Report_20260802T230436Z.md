...
| 80 | Design: item identification visibility — knowledge by holding | KnightOfNight | V24, itemization |  | 2026-08-02 |
...
| 127 | Ranged proc damage ("between X and Y") — new weapon kind, midpoint-and-spread family | KnightOfNight | triaged, V24, itemization | Version 24.10 | 2026-08-02 |
| 130 | Secondary-stat curves vs Mk band growth — audit before Mk 2 content | KnightOfNight | V24, game-balance |  | 2026-07-30 |
| 134 | Repair kit not wired up yet | KnightOfNight | V24, itemization |  | 2026-08-02 |
...
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-16 | Updated: 2026-08-02
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-02, V24.9 design session): Phase 2 (itemization structure) release-by-release queue order ruled: **#129 → #127 → #80 → #134**. #80 is **third in the queue** — a singleton release (its ruled design direction from 2026-07-15 stands: knowledge by holding, examine reveals without pickup, pickup unlocks, plus the two ride-along cleanups).
...
- Milestone: Version 24.10
- Assignees: KnightOfNight
- Created: 2026-07-21 | Updated: 2026-08-02
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.9 design session): Phase 2 (itemization structure) release-by-release queue order ruled: **#129 → #127 → #80 → #134**. #127 is **second in the queue** — it ships as the release after Version 24.9 (whose founding ticket is #129).

Pairing note: #127 and #129 are a design pair — both restructure an item stat into an authored guarantee plus a roll above it (proc floor on the offense side here; authored armor base on the defense side in #129). They remain separate tickets and separate releases per the scope law: implementation-independent (#127 copies the shipped weapons midpoint-and-spread pattern), each closing with its own release. Shared doctrine cross-recorded on #129.


**KnightOfNight** — 2026-08-02:
Design ruling cross-record (2026-08-02, V24.9 design session — operator-confirmed on #129): **the pair doctrine is ruled.** An item stat of consequence is an **authored guarantee plus a roll above it** — the definition authors the floor the designer promises; the drop-time roll adds variance strictly on top.

- Defense side ships first: #129 (Version 24.9) — authored `armor_base` + rolled `physical_resist` bonus.
- Offense side is this issue, next in the queue: the authored proc floor ("between X and Y"), roll landing between floor and ceiling, copying the weapons midpoint-and-spread pattern.

This issue's own detailed design (generation, §5 stat table, display composition, rolled-stat structure) is ruled by its release's design session against the then-landed #129 structure.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.10 design session — operator-confirmed): **the floor/roll composition is ruled.** Option B — "authored floor lifts the range," the #129 pattern on offense:

- **(a) The floor X is authored, deterministic, rarity-blind.** The definition carries an authored pair (`proc_floor_base`, `proc_floor_factor`); at drop the instance's floor is `X = proc_floor_base + (proc_floor_factor × mk_tier)`. No rarity multiplier, no roll — the designer's promise, exactly as #129's `armor_base` is on defense.
- **(b) The ceiling is floor + the existing rolled proc factor V, unchanged machinery.** Same secondary-stat roll, same authoring curve family, same rarity spread at drop. Rarity buys ceiling, never floor (pair doctrine verbatim). Y = X + ⌈V⌉.
- **(c) On-success damage = uniform random X..Y. Proc chance is untouched:** V × 0.05, capped at 50%. The floor changes what a proc pays, never how often it fires.
- **(d) Existing proc weapons: zero behavior change, zero display change.** The shipped "up to N" procs (damage 1..⌈V⌉) are the no-floor case and keep that exact path — "no authored floor" routes to the shipped 1..⌈V⌉ formula, never through the new formula as X=0 or X=1. Floors exist only where authored, i.e. the new weapon kind. Examine display: floored procs read "between X and Y"; unfloored keep "up to N."

Rejected alternative recorded for history: the literal midpoint±spread transplant (authored proc midpoint curve + fixed spread width, roll in [M−S, M+S]) — it makes the floor move with rarity, contradicting the pair doctrine's authored guarantee. The midpoint-and-spread "family" resemblance is the two-number range identity; the composition is floor + roll-above.

Still open in this session: scope (what the new weapon kind is, seed definitions, primary-vs-secondary carriage of floored procs), generation details, display composition, GDD §5/§6 text.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.10 design session — operator-confirmed): **the scope is ruled.** Continues the composition ruling's lettering:

- **(e) `flame_factor` joins the proc family as its fourth member.** Identical wiring to `bleed_factor`/`stun_factor`/`poison_factor`: chance = V × 0.05 capped 50%, flavor-only name (no status effects, no damage types), payout sums into the existing one-parenthetical gear-bonus pool. This is the predicted stat-table change (GDD §6.4 wiring map gains the row; §5 status notes updated to name four proc flavors).
- **(f) On the new weapon kind, the floored proc is a PRIMARY stat, and floors are authorable only on primary proc entries.** Primary presence on every instance is what makes the floor a promise; a floor in a secondary pool is ruled a **seed defect** (seed-enforced invariant, same family as grid-adjacency and octagon-never-agro). The guarantee is presence + floor; the proc's V still rolls through the standard drop machinery, so rarity still buys ceiling on the same stat — composition rulings (a)–(d) unchanged.
- **(g) Two new Mk 1 ranged seed definitions, one per proof point:** a flame-floored cyber/wasteland RANGED weapon (working name *Flame Projector*) exercising the new `flame_factor` end to end, and a poison-floored RANGED weapon (working name *Dart Caster*) proving floors generalize over existing proc stats. Additive-only seed change — **expected deletion count 0**; both join existing weapon drop pools. The existing 8 weapon definitions are untouched (per (d)). Final names/descriptions under the creative-content policy.
- **(h) Rolled-stat structure:** the floor is authored as an optional pair on the definition's stat entry (shape: `{'stat': 'flame_factor', 'base': …, 'factor': …, 'floor_base': …, 'floor_factor': …}`); at drop the instance snapshots both the rolled V and the computed floor X = floor_base + floor_factor × mk_tier (drop-time snapshot semantics — held items never change retroactively). Exact model field names/types are the brief's to pin.

Rejected alternatives recorded: retrofitting floors onto existing weapons (contradicts (d)); a new `item_type` beyond `weapon` (nothing downstream reads a finer kind — scope law); floor as a standalone sibling stat (splits the guarantee from the stat it guarantees).

Still open in this session: display composition (examine line wording; hit line already flows through the existing parenthetical unchanged), GDD §5/§6 text, the brief.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.10 design session — operator-confirmed): **display composition is ruled.** Completes the design set (a)–(j) with the composition and scope rulings above:

- **(i) Floored proc entries render the standard stat line plus the promise as a parenthetical range:** `Flame Factor: 4.2 (between 12 and 17 damage)`. Label and rolled value V render exactly like every other stat line (chance driver visible, zeros-never-hidden untouched, one rendering pattern for all stats); the parenthetical is the authored promise made legible — `between X and Y damage`, computed from the snapshot floor X and Y = X + ⌈V⌉. Unfloored procs render no parenthetical, line byte-identical to today. `flame_factor` needs no STAT_LABELS entry (title-case fallback yields "Flame Factor").
- **(j) The hit line changes not at all.** Floored proc payouts join the existing gear-bonus pool inside the existing single parenthetical; no new combat output line, no per-proc callout. Verbose mode exposes the X..Y roll in the calculation chain as it does every roll.

Rejected alternatives recorded: replacing the value with the range (hides the chance driver, breaks stat-line consistency); a second output line per proc on hit (noise; the one-parenthetical law is settled).

Remaining in this session: GDD §5/§6 text with (v24.10, pending implementation) markers, then the implementation brief.


**KnightOfNight** — 2026-08-02:
V24.10 design session (2026-08-02): design complete — rulings (a)-(j) recorded above, GDD text landed on version_24_10 at 239159a with (v24.10, pending implementation) markers, Brief 1 (Shyland_V24.10_Brief_1_Proc_Floor.md) committed at 98859cd. Cold-start-ready: triaged applied. Next: implementation session on version_24_10, directed to Brief 1 by name.
...
- Milestone: none
- Assignees: none
- Created: 2026-07-22 | Updated: 2026-08-02
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.9 design session): Phase 2 (itemization structure) release-by-release queue order ruled: **#129 → #127 → #80 → #134**. #134 is **fourth in the queue** — a singleton wiring release (repair kit into the existing repair machinery); this stub gets fattened by its own release's design session.
