...
| 40 | Free repair messages repeat too often (Morra example) — research spike into other duplication cases | KnightOfNight | B4, triaged | Version 23 | 2026-07-25 |
...
| 144 | Six service NPCs have zero dialogue entries — they have never greeted anyone | KnightOfNight | bug, B4, triaged | Version 23 | 2026-07-23 |
...
- Assignees: KnightOfNight
- Created: 2026-07-12 | Updated: 2026-07-25
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-07-25:
**Design-chat amendments of 2026-07-24, operator-ruled** (carried here by V23 Brief 4 Step 1 so the tracker never lags the design chat):

- **The bucket splits into two briefs by mechanism.** Brief 4 (this session) is the code half — every site where a hardcoded single string becomes a pool plus a random pick, in the new `npc_voice.py` module. Brief 5 is the data half — the sites where pool machinery already exists and only seed rows are missing (#144's six silent NPCs, thin keyword entries, departure reactions, connectives). **Brief 5 closes this issue, not Brief 4.**
- **Boss `death_message` is CUT from scope.** It stays a scalar `TextField` — no migration, no child table, no seed change.
- **Report-category renderings are ruled OUT of the sweep**: the vendor list caption, the nothing-left-for-sale line, and the repair sweep summary all stay stable. The governing line: **speech gets pooled; renderings stay stable** — consistent with the standing timestamps-mark-events-not-renderings distinction.


**KnightOfNight** — 2026-07-25:
**The code half landed** — V23 Brief 4, commit 8ab074c on branch version_23. This issue stays OPEN; Brief 5 (the data half) closes it.

What Brief 4 shipped: the `npc_voice.py` pool module (`pick()` + kibitz-style random selection, per this issue's ruling — no novel selection architecture), with pools wired at every hardcoded-single-string site: sell acknowledgments (single/bulk/worthless), buy acknowledgments (the vendor speaks here for the first time), sold-out, all six paid-repair outcome sites, per-NPC pity-repair pools for all seven repairers (Morra, Pella, Ferwick, Repairbot Prime, Maro, Tavik, Old Brammel), a grown kibitz pool, and `AGGRO_ENGAGE` at all three engagement call sites (walk-in, flee-in, respawn — the respawn site keeps its #79 indefinite-introduction name composition).

Scope rulings honored (2026-07-24, operator): **`death_message` is cut from scope** — it stays a scalar `TextField`, no migration, in this brief and in Brief 5; report-category renderings (vendor list caption, nothing-left-for-sale, repair sweep summary) stay stable — speech gets pooled, renderings don't.

Remaining for Brief 5: the pure-data half — #144's six silent NPCs, thin keyword entries, departure reactions, connectives — then this issue closes.
...
## Issue #144: Six service NPCs have zero dialogue entries — they have never greeted anyone

- State: open
- Author: KnightOfNight
- Labels: bug, B4, triaged
- Milestone: Version 23
- Assignees: KnightOfNight
- Created: 2026-07-23 | Updated: 2026-07-23
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/144
...
- Maro the Mender (`maro-the-mender`)
- Tavik the Mender (`tavik-the-mender`)
- Old Brammel (`old-brammel`)
- Essa the Trader (`essa-the-trader`)
- Sona the Trader (`sona-the-trader`)
- Ridda the Trader (`ridda-the-trader`)
...
**KnightOfNight** — 2026-07-23:
Cross-link: found by the #40 research spike (see its findings comment for the full audit). Sibling issues from the same spike — #40 fixes repetition at sites that have lines; this issue gives lines to NPCs that have none. No dependency link; either can land first.
