...
| 301 | sudo bot: file GitHub issues from in-game — Q&A gathering like the artifact flow, filed on explicit confirm | KnightOfNight | triaged, monitoring-and-command | Version 25.10 | 2026-08-28 |
| 304 | Two bots on one target: per-checkout pidfiles cannot see a same-target bot running from another checkout — enforce a per-(bot, target) singleton | KnightOfNight | triaged, monitoring-and-command | Version 25.10 | 2026-08-28 |
| 305 | sudo bot: the conversation store records model prose as truth — a false action claim poisons the thread (persist receipts with the answer) | KnightOfNight | triaged, monitoring-and-command | Version 25.10 | 2026-08-28 |
| 306 | sudo bot: everything the bot delivers should render deterministically — machinery composes and the game renders all data-shaped output; model prose is commentary only | KnightOfNight | triaged, monitoring-and-command | Version 25.10 | 2026-08-28 |
...
**KnightOfNight** — 2026-08-28:
**Operator ruling (2026-08-28, V25.10 design session) — confirmed in full; founding ticket of Version 25.10:**

1. **The filing is bot-side.** The GitHub token lives in the agents host's `.secrets` under the #268 credential-custody standing law (named, never printed, never committed); the bot calls the GitHub REST API directly. The game box holds no GitHub credentials and the door stays game-scoped — no new door vocabulary for this feature.
2. **The Q&A gathering shape:** sudo collects **title + body** conversationally (artifact-flow precedent — building the thing up); machinery applies convention: assignee = the operator, **no labels** — operator filings are thin by doctrine, triage fattens them.
3. **The confirmation gate:** the draft is read back and the issue is filed **only on an explicit yes** — filing is an outward-facing act.
4. **The filing emits a machinery-composed receipt** carrying the real issue number and URL taken from the GitHub API response — never model-typed (the #306 doctrine applied at birth; a filing claim without a receipt is visibly unbacked, and per #305 the receipt persists with the stored answer).
5. **Provenance:** the filed issue's body carries a "filed via sudo (in-game)" footer.
6. **Audience:** admin-only by construction (sudo's audience); any future bot inheriting the capability re-opens this as a #261/#282 scopes question.

Carrier: V25.10 Brief 1.


## Issue #304: Two bots on one target: per-checkout pidfiles cannot see a same-target bot running from another checkout — enforce a per-(bot, target) singleton
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/304

### Body

## Incident (2026-08-28, V25.9 Brief 1 implementation session)

The brief's §7.4 step — "restart the dev bot via `agents/botctl.py` with the new code (dev target)" — was run from the **version_25_9 worktree** while a dev bot was already running from the **main checkout**. The worktree `botctl dev status` reported "not running" (truthfully, for its own checkout), went straight to start, and for a window **two dev bots were attached to the same door simultaneously**. The operator noticed and stopped the main-checkout bot by hand.

Two concurrent bots on one target means every sudo command is handled twice: double answers in the pane, and — worse — **double actions** (a gift request lands two items, a move fires twice). The door records both faithfully; nothing refuses the second actor.

## Mechanism

`#299`'s (bot, target)-scoped state files are **per-checkout by construction**: `pidfile(target) = AGENTS_DIR / '.sudo_bot.<target>.pid'` where `AGENTS_DIR` derives from the running file's own location, and botctl is deliberately self-locating ("the copy you run is the checkout it manages", #295). That scoping solved dev/prod coexistence *within* one checkout, but it makes the pidfile check blind **across checkouts**: main's `.sudo_bot.dev.pid` and the worktree's `.sudo_bot.dev.pid` are different files, so each checkout's "already running" guard sees only its own bot. The exact demonstration, minutes after the incident:

```
$ games-mvc-version_25_9/agents/botctl.py dev status
sudo bot (dev) running (pid 86286)
$ games-mvc/agents/botctl.py dev status
sudo bot (dev) not running
```

The collision surface is structural: implementation briefs *require* running the worktree copy against dev (the code under test), while the operator's standing dev bot may be running from main — the workflow itself steers toward this collision every release that touches the bot.

## Direction candidates (for a design session to rule; not decided here)

1. **Machine-scoped lock, checkout-independent** — the pid/lock file for a (bot, target) pair lives outside the checkout (e.g. `~/.shyland/` or `/tmp`), so any copy of botctl/sudo_bot sees any other copy's claim. Simple; per-host only.
2. **Process-scan guard in botctl/sudo_bot start** — before starting, scan for any running `<name>_bot.py run --target <target>` process regardless of path and refuse loudly, naming the pid and checkout. No state-file change; catches the exact incident shape.
3. **Server-side singleton (the structural fix, #302's spirit)** — the door/egress refuses or supersedes a second attached connection for the same agent account: one `agent-sudo` = one attach, enforced where trust lives. Covers multi-host too; needs a takeover-vs-refuse ruling (a stale half-dead connection must not lock the account out — cf. the player-door presence takeover pattern).

Options compose: (2) is a cheap immediate guard; (3) is the door-enforced invariant.

Filed from the V25.9 implementation session at the operator's direction ("we need to prevent this in the future"); recorded as a note in the V25.9 Brief 1 closeout.


### Comments (1)

**KnightOfNight** — 2026-08-28:
**Operator ruling (2026-08-28, V25.10 design session):**

1. **Door-enforced singleton, hard reject (direction candidate 3).** One agent account = one attached connection at `ws/shyland/mc/`. A second connection for an already-attached account is refused — accept-then-close with a dedicated conflict close code (**4409**, joining the door's 4403/4503 convention) and a reason naming the holding attach, plus a `shyland.mc` log line. **No takeover.**

2. **The operator's condition — "only if we are sure that killing the old bot will free up the slot" — verified against the code at ruling time** (mc_consumer.py, Dockerfile): no attach registry exists today; the singleton claims at `connect` and releases (guarded — only its own claim) in `disconnect`. The registry lives **in-process in the single Daphne process** (one `django` container, `CMD ["daphne", ...]`, no worker fan-out — every WebSocket lives there), which yields the guarantee on all three timescales: killing the old bot (clean stop or `kill -9`) closes its socket at the OS level → `disconnect` fires immediately → slot freed; a truly half-dead peer is reaped by Daphne's built-in WS ping (defaults in effect: interval 20s, timeout 30s) → freed within ~50s worst case; a Django restart destroys the registry together with every connection it guarded → a stale lock is structurally impossible. (This is why the claim is deliberately in-process, not a Redis key a crash could strand.)

3. **Local side: no process scanning** (direction candidate 2 rejected by the operator — too problematic). Instead, bot runtime state moves to a **machine-scoped central directory, `~/.shyland/`** — checkout-independent: the pidfiles, the logs, **and the conversation store** (operator-confirmed), keeping #299's per-(bot, target) filenames (e.g. `~/.shyland/.sudo_bot.dev.pid`). Any copy of botctl/sudo_bot then sees any other copy's claim, ending the per-checkout blindness; the conversation store gains one continuity per (bot, target) per host regardless of which checkout runs the bot.

Options compose as the issue anticipated: the door singleton is the trust-boundary invariant; the central directory is the client-side courtesy.

Joins **Version 25.10** (milestone); carrier: V25.10 Brief 1.


## Issue #305: sudo bot: the conversation store records model prose as truth — a false action claim poisons the thread (persist receipts with the answer)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/305

### Body

## Finding (2026-08-28, V25.9 Brief 1 operator playtest — closeout deviations 8 and 9)

The sudo bot's conversation store records each exchange as **plain text: request + the model's final answer** (the ruled 25.6 shape — deliberately provider-neutral, persisted across restarts). That makes the model's own prose the durable record of what happened — and prose can be false. During the 25.9 playtest this produced a two-stage failure on one waypoint name:

1. **The replay trap (deviation 8):** by the operator's third quick waypoint teach, the model's replayed history showed two prior `teach → "Saved waypoint X at …"` exchanges with **no tool calls visible** (tool_use/tool_result blocks are not persisted — only text is). Its own stripped transcript taught it that a text-only "Saved" is a complete response: it answered `Saved waypoint "one-up" at …` **without calling `remember`**. The v25.9 delivery machinery exposed the lie (no `sudo did:` line; the store had no row), which is how the operator caught it.
2. **The poisoned history (deviation 9):** after a prompt-order fix and bot restart, the re-teach of the *same name* failed again — this time with **zero tool calls**: the persisted history still contained the earlier false "Saved 'one-up'" claim, so the model treated the work as already done. The lie was self-reinforcing and survived the restart by design. Fresh names (`two-up`, `three-up`, `green-start`) all saved correctly with receipts under the new standing order; only the poisoned name kept failing, until the dev conversation file was deleted in-session.

In-force mitigations: the v25.9 receipts machinery (a false claim ships visibly unbacked — the admin can always see it), the deviation-8 standing order naming the replay trap, and — worst case — deleting the target's conversation file. None makes the *stored record* truthful.

## The structural question (needs a design ruling — changes the ruled 25.6 store shape)

The exchange as actually **delivered** now includes machinery receipts (`sudo did:` lines — bot-composed truth from door results, v25.9 #302). The stored record could match the delivered record. Direction candidates:

1. **Persist receipts with the answer** — the recorded answer text carries the turn's receipt lines (e.g. appended `[did: remembered waypoint 'one-up' …]`). Replayed history then distinguishes a backed "Saved" from an unbacked one, and stays a plain-text shape any future bot can reuse. Smallest change; makes the record self-describing.
2. **Persist tool activity structurally** — store tool_use/tool_result blocks (or a summary of calls made) in the exchange. Richer, provider-shaped, heavier; abandons the deliberately provider-neutral text shape.
3. **Suppress/mark unbacked action claims at record time** — the bot already knows the turn's receipts; an answer claiming an action with no matching receipt could be recorded with a warning marker (or not recorded). Content inspection of prose — fragile, likely the weakest option.

Option 1 is the natural fit with #302's "receipts are the record" doctrine: the conversation store should remember what the game confirmed, not what the model said.

Related: #302 (shipped v25.9 — the machinery that made this failure visible), the v25.8 closeout deviations 9–11 (the prompt-order arc this finding extends).

Filed from the V25.9 implementation session at the operator's direction; full incident timeline in `docs/shyland/Shyland_V25.9_Brief_1_Closeout.txt` deviations 8–9.


### Comments (1)

**KnightOfNight** — 2026-08-28:
**Operator ruling (2026-08-28, V25.10 design session) — confirmed: direction candidate 1, persist receipts with the answer.**

The conversation store's recorded answer text carries the turn's machinery-composed receipt lines (the `sudo did:` records, #302). Replayed history then distinguishes a backed "Saved" from an unbacked one — the model can no longer teach itself from its own stripped transcript that a text-only claim is a complete response, and a false claim no longer survives restarts as apparent truth. The store keeps the ruled 25.6 plain-text, provider-neutral shape (candidate 2's structural tool blocks rejected as provider-shaped; candidate 3's prose inspection rejected as fragile). This is #302's "receipts are the record" doctrine applied to the stored record: the store remembers what the game confirmed, not what the model said.

Joins **Version 25.10**; carrier: V25.10 Brief 1.


## Issue #306: sudo bot: everything the bot delivers should render deterministically — machinery composes and the game renders all data-shaped output; model prose is commentary only
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/306

### Body

## The principle (operator-directed, 2026-08-28, V25.9 playtest)

**Everything the bot delivers should render deterministically.** Any data-shaped output — lists, detail views, rosters, receipts, paths — should be composed by machinery (the door, from live data) and rendered by the game, never formatted by the model. Model prose is commentary; the machinery is the record and the rendering.

## The observation that prompted it

During the V25.9 playtest, the same "list waypoints" request rendered two completely different ways minutes apart:

- One turn: a tidy multi-line list, one `name — Zone: Area: Room` row per waypoint — which was actually the model *violating* its ruled voice ("Plain text only — no markdown, no emoji, no line breaks", 25.6).
- The next (fresh conversation): a single bland comma-separated line — the model *obeying* the rule.

Both were honest; neither was reliable. Model-formatted data output is inherently variable turn to turn — the format is a sampling artifact, not a rendering. (The specific waypoint listing is fine as-is for now — operator ruling; this issue is the general principle.)

## Where the codebase already points

The doctrine exists in shipped parts and keeps proving itself:

- **#296 (v25.8):** the `report` action — the game renders an admin's inventory view itself (door-composed leader + the player compositions), because hand-written rosters were unreliable. "Prefer report over hand-writing a roster" is already a standing order.
- **#302 (v25.9):** `sudo did:` receipts — action records are door-composed lines in a form model prose cannot produce; the model is told to never enumerate its own receipts.
- This issue is the generalization: **the report/receipt pattern should cover every data-shaped answer the bot gives** — memories/waypoints listings, memory detail, item detail, room search results, event search results, who-is-online, character sheets, and whatever the vocabulary grows next.

## Direction candidates (for a design session to rule)

1. **Grow the `report` family** — more `report` kinds (`memories`, `waypoints`, `items`, `events`, `who`, `character`, …), each a door-composed deterministic rendering delivered into the requesting admin's pane; the model's job shrinks to choosing the right report and adding a sentence of commentary. Follows the #296 shape exactly.
2. **Auto-render query results** — when the bot's turn ends after data-shaped queries, the machinery renders the result itself (the bot side composes deterministic lines from the door's structured result, like the receipt composer does), regardless of what the model wrote.
3. **Prompt-only** — order the model to always use a fixed format. Rejected by the evidence above: format-by-prompt is what just failed; this issue exists because prompts don't make rendering deterministic.

Option 1 keeps the trust boundary clean (the game renders, server-side, from live data) and each new kind is a small door change on the established pattern; option 2 keeps it bot-side and generic. A design session should rule the mechanism and the priority order of surfaces.

Related: #296 (the report action — the pattern's origin), #302 (structural receipts — the same doctrine for actions), #305 (conversation-store truthfulness — the same doctrine for the stored record).

Filed from the V25.9 implementation session at the operator's direction.
...
1. **The principle stands as standing doctrine and lands in the GDD with this release:** everything the bot delivers renders deterministically — machinery composes and the game renders all data-shaped output (lists, detail views, rosters, receipts, paths); model prose is commentary only. Doctrine's home is the GDD (§10.11 family text), not an open ticket.

2. **Mechanism: direction candidate 1 — grow the `report` family** on the #296 shape: each kind a door-composed deterministic rendering delivered into the requesting admin's pane, server-side, from live data; the model's job shrinks to choosing the right report and adding commentary. (Candidate 3, prompt-only, stays rejected by the evidence in the body; candidate 2 loses to keeping the trust boundary clean.)

3. **The 25.10 slice: `report waypoints` and `report memories` (with memory detail)** — exactly the surfaces that misrendered in the 25.9 playtest.

4. **Ordinary milestone member, operator-confirmed — deliberately NOT a v35 action item:** joins **Version 25.10** and closes with the release. Future data-shaped surfaces (`who`, `character`, `items`, `events`, and whatever the vocabulary grows next) are filed as thin tickets when wanted and judged against the GDD doctrine — no perpetually open ticket. That roster order is recorded here for later carriers, not for this release.
