...
| 302 | sudo bot: the model must never invent a value it should read from the database — make receipts structural, not behavioral | KnightOfNight | emergent, triaged, monitoring-and-command | Version 25.9 | 2026-08-28 |
...
## Issue #302: sudo bot: the model must never invent a value it should read from the database — make receipts structural, not behavioral

- State: open
- Author: KnightOfNight
- Labels: emergent, triaged, monitoring-and-command
- Milestone: Version 25.9
- Assignees: KnightOfNight
- Created: 2026-08-27 | Updated: 2026-08-28
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/302

### Body

## The principle

The sudo bot must never invent a value it should have read from the database. Every id, room, count, or state fact the bot acts on or reports must be traceable to a tool result — a receipt — from the current turn. This issue is the design home for making that a property of the system, not a behavior we ask the model for.

## Evidence (V25.8 Brief 1 playtest, 2026-08-27 — all three fixed in-session as prompt standing orders)

All door-side machinery behaved correctly in every case; every failure was the model inventing a value:

1. **Invented success** — teaching three waypoints in a row, the third turn ran `where_is` and answered "saved" with **no `remember` call** (2 of 3 stored; the bot reported 3). Fix: actions reported only on tool receipts (7752dd7's chain, commit fe3a296).
2. **Invented location** — a repeated `move X to battle-1` made **zero tool calls**, declining because the model "knew" the character was already there from its own earlier-turn move; the player had walked elsewhere. A follow-up also passed the hallucinated `to_room_id: 0` (door-refused `not-found`). Fix: a move request is a location decision — fresh `where_is` in the same turn (commit b2d8f11).
3. **Invented destination** — two waypoint moves teleported the player through a **wrong room first**: the model called `move` with a guessed id (once the waypoint memory's own id → room 4; once the digit peeled off the name "battle-1" → room 1, the Heart), then read the memory and issued a corrective second move. The player really makes every hop. Fix: `to_room_id` only from current-turn tool results; waypoint moves read the memory first, move once (commit 7752dd7).

Also observed: a string-typed `to_room_id: 'null'` (door-refused `bad-params`).

## Why prompt rules are not the end state

Three patches in one playtest arc, all variants of the same failure. Prompt standing orders reduce frequency but are behavioral, not structural — the model can still emit any integer into any id field, and the door will faithfully execute a well-formed wrong instruction. Confident false success reports are the worst of these: they corrupt the admin's picture of the world while leaving a clean audit trail nobody is reading in the moment.

## Candidate directions (for the design session to rule; not prescriptions)

- **Vocabulary that eliminates raw ids for taught things** — e.g. move-by-waypoint (`move {name, waypoint}` resolved door-side from the caller's own store), so the bot never handles a room id for a taught place at all. Generalizes: wherever the bot routinely round-trips an id it just looked up, consider a door verb that does the lookup-and-act atomically.
- **Bot-side receipts ledger** — machinery (not the model) tracks ids returned by tool results within the turn; a tool call whose id-typed argument was never returned by a prior result in the same turn is refused before it reaches the door, fed back as an error tool-result ("that id is not from a lookup — look it up first").
- **Schema/description tightening** — id fields documented as "must come from <tool> in this turn"; possibly dedicated param names that make provenance checkable.
- **Answer-side receipts** — the bot appends nothing; but machinery could refuse to deliver an answer claiming an action (remember/move/gift/…) when no matching successful action ran in the turn — the delivery-gate analog of the ledger.

## Relation to shipped work

The v25.8 prompt standing orders (receipts-not-intentions; move-is-a-location-decision; to_room_id-from-current-turn-results) remain in force and are the mitigation of record until this is ruled and built.


### Comments (3)

**KnightOfNight** — 2026-08-28:
**Operator ruling (2026-08-28, V25.9 design session): founding ticket of Version 25.9.**

This ticket anchors the release: the sudo bot must never invent a value it should have read from the database, made a property of the system rather than a behavior we ask the model for. The 25.8 prompt standing orders (receipts-not-intentions; move-is-a-location-decision; to_room_id-from-current-turn-results) remain the mitigation of record until this ships.

Branch `version_25_9` cut from main (e41f488) and pushed; milestone `Version 25.9` created with this ticket as its first member. Design rulings on the candidate directions (door-side id-free vocabulary, bot-side receipts ledger, schema tightening, answer-side receipts) follow in this session and will be recorded here as they are made.


**KnightOfNight** — 2026-08-28:
**Operator rulings (2026-08-28, V25.9 design session) — all four confirmed, and the release scope with them:**

1. **Door-side: `move` gains a `waypoint` destination form.** `a_move` accepts exactly one of `to_name` / `to_room_id` / `waypoint` (a memory name); the door resolves it from the **calling agent's own store** (`kind=waypoint`, case-insensitive — the same addressing law as the memory verbs), refusing `not-found` on a miss. The bot never handles a room id for a taught place; the lookup-and-act is atomic door-side. `to_room_id` stays for rooms-search destinations, covered by ruling 2.

2. **Bot-side: a typed receipts ledger at the tool-execution choke point** (`sudo_bot.py` `_execute_tool`). Machinery — never the model — harvests ids from successful tool results within the current turn, **typed by id-space**: room ids only from room-bearing result fields (`rooms` rows, `where_is`/`move` results, waypoint `data.room_id`), item instance ids only from `inventory`/`item`/`gift`/`create_artifact` results, memory ids from `memories`/`memory`/`remember`, stream_ids from `events`. A tool call whose id-typed argument is not in the matching ledger is refused before it reaches the door, fed back as an error tool result the model recovers from in the loop. Typing is load-bearing: the playtest's finding 3 passed a *receipted* integer (the memory's own id) into the *wrong* id-space — an untyped ledger would have licensed it.

3. **Delivery-side: structural action receipts on `answer` — in scope for 25.9.** The door's `answer` action gains a machinery-only `receipts` param; the bot fills it from the turn's actual successful action calls, never from model text; the game renders receipts as their own line(s) in a form model prose cannot produce. An answer claiming an action with no receipt line is visibly bare. This is the structural catch for the invented-success failure (finding 1), the worst of the three. Rendering wording/color is authored at brief time under the creative-content policy.

4. **Schema tightening rides along:** id-typed tool-schema fields gain the explicit "from `<tool>` in this turn" contract in their descriptions. No new machinery.

**Scope:** all four are this one ticket — the milestone stays at one member, no dependency filings, one implementation brief. GDD text lands in §10.11 (door vocabulary + the structural-receipts law) with this release. The 25.8 prompt standing orders remain in force as defense-in-depth beneath the structural layer.


**KnightOfNight** — 2026-08-28:
**Brief committed (2026-08-28, V25.9 design session):** `docs/shyland/Shyland_V25.9_Brief_1_Structural_Receipts.md` at 1dd31c4 on `version_25_9` (GDD §10.11 passage at e302f81). Every structural claim line-cited against the code at writing time (#252).

Two design details settled at brief-writing time, operator-notified in-session (recorded here for completeness):

1. **Receipts-only delivery:** a turn whose model goes silent but whose actions succeeded still delivers its `sudo did:` receipt lines — the admin always sees what was actually done; model prose is commentary, receipts are the record. A turn with neither text nor receipts stays silent as today.
2. **Gift receipt aggregation:** identical (slug, mk_tier, rarity, to) gifts in one turn aggregate to one `×N` receipt (the shipped v22 count form) — without it, a 25.8-style bundle replay (up to 50 gift calls) would flood or overflow the 20-receipt bound.
