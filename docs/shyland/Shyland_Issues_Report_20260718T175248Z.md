...
| 29 | Block looting (and related inventory commands) during combat | KnightOfNight | B2 | Version 22 | 2026-07-17 |
...
| 54 | consider how to simplify combat language and make it more human | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 57 | new command: home [now] | KnightOfNight | B3 | Version 22 | 2026-07-18 |
| 58 | vendor for-sale list changes | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 59 | some commands not logged in timestamped output | KnightOfNight | bug, B2 | Version 22 | 2026-07-17 |
| 61 | refuse to use a healing draught if player vitality is full | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 65 | 'use 3 heal' responds with 'You can't use everything at once.' | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 67 | tab completion doesn't work for 'equip' | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 70 | Feature: Longevity has no drain — the slow-burn design needs its first consuming mechanic | KnightOfNight |  |  | 2026-07-15 |
| 75 | repair all should retry, not need multiple manual tries | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 76 | inv should take filters like buy or sell | KnightOfNight | stub, B2 | Version 22 | 2026-07-17 |
...
| 88 | new command: last | KnightOfNight | stub, B3 | Version 22 | 2026-07-18 |
| 93 | command behavior inconsistencies when no arguments passed | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 95 | the ring needs an area | KnightOfNight | stub |  | 2026-07-18 |
| 96 | examine doesn't autocomplete on NPC name | KnightOfNight | stub, B2 | Version 22 | 2026-07-17 |
| 98 | command 'who' needs color output | KnightOfNight | stub, B2 | Version 22 | 2026-07-17 |
...
| 109 | Design ruling: mid-combat stat spend refills bars to new max (bankable free heal) | KnightOfNight | B5 | Version 22 | 2026-07-18 |
...
| 111 | command revamp | KnightOfNight | B2 | Version 22 | 2026-07-17 |
| 112 | new command: sudo | KnightOfNight | B3 | Version 22 | 2026-07-18 |
| 113 | new command: cancel | KnightOfNight | B3 | Version 22 | 2026-07-18 |
| 115 | Map pane: restore breathing room lost to v21 zone-colored borders | KnightOfNight | bug, B1 | Version 22 | 2026-07-18 |
| 116 | No single-session enforcement — two simultaneous logins on one character desync and race | KnightOfNight | bug, B1, emergent | Version 21.1 | 2026-07-18 |
...
- Assignees: KnightOfNight
- Created: 2026-07-12 | Updated: 2026-07-17
- Blocked by: none
- Blocks: #111
...
- Milestone: Version 22
- Assignees: none
- Created: 2026-07-14 | Updated: 2026-07-17
- Blocked by: none
- Blocks: #111
...
- Milestone: Version 22
- Assignees: none
- Created: 2026-07-14 | Updated: 2026-07-18
- Blocked by: none
- Blocks: none
...
- Milestone: Version 22
- Assignees: none
- Created: 2026-07-16 | Updated: 2026-07-17
- Blocked by: none
- Blocks: #111
...
- Milestone: Version 22
- Assignees: none
- Created: 2026-07-16 | Updated: 2026-07-18
- Blocked by: none
- Blocks: none
...
## Issue #115: Map pane: restore breathing room lost to v21 zone-colored borders
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/115

### Body

The 5px zone-colored pane borders from v21's display sweep ate the breathing room around the map — the 300×300 rendering now sits flush against the border with no visual margin.

When we open the bucket, we'll decide whether the fix is padding on the container, an inset on the map's drawable area, or shrinking the node layout bounds a touch — whatever preserves the fixed 300px pane geometry while getting the gap back.
...
## Issue #116: No single-session enforcement — two simultaneous logins on one character desync and race

- State: open
- Author: KnightOfNight
- Labels: bug, B1, emergent
- Milestone: Version 21.1
- Assignees: KnightOfNight
- Created: 2026-07-18 | Updated: 2026-07-18
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/116
...
**KnightOfNight** — 2026-07-18:
**Triage confirmation (read-only, against `django/src/apps/shyland/consumers.py` on `main`):**

(a) **No guard exists.** `connect()` (line 263) checks only `user.is_authenticated` and that a character exists — it then unconditionally accepts, joins `room_{id}` (line 295) and `player_{pk}` (line 298), and writes the presence key (lines 306–310). Nothing inspects an existing presence key or an existing connection for the character; no refuse, close, or supersede path exists anywhere in the connect flow. `player_message` (line 1988) handles `clear`/`text`/`status`/`fight` payloads plus the `dying` and `respawn` event types — there is no `superseded` (or equivalent) event type.

(b) **The v19 per-connection token exists and is available exactly where the ruling assumes.** `self.session_token = uuid.uuid4().hex` is minted at line 301, immediately after the `player_{pk}` group join (line 298), and is embedded in `presence_value` (lines 302–305) written to Redis before the heartbeat starts (line 311). The ruling's proposed supersede broadcast would slot in right after that write with the token already in hand. The normal-disconnect-path claim also holds: `disconnect()` (lines 330–345) does the guarded `PRESENCE_DELETE_LUA` delete keyed on `presence_value` (no-op against a newer session's key), the `player_group`/`room_group` discards, and the heartbeat cancellation.

The issue body's per-connection-state reading is also accurate as written: `self.room_group` is consumer-instance state updated only by that connection's own move (line 461), flee (line 1772), and respawn (lines 2017–2019) paths, and `last_direction` / `_character_is_dying` live on the instance (lines 281–282).

No contradictions found. No code changed.
