# Shyland Project Instructions v38

**Status: ACTIVE — approved by the operator 2026-09-13. Supersedes Shyland_Project_Instructions_v37.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v38 edition is the playtest-format pass (operator rule 2026-09-13, born in the V26.0 Brief 1 implementation session): every operator playtest checklist — in briefs and in closeout-report recipes — is written as explicit numbered run/expect/decision sequences, never compressed into prose; each step gives the command to run, the exact expected observation with its precondition, then an explicit stop-and-report vs continue fork. (v37 the daemon-inspection pass; v36 the technical-coherence pass; v35 the action-item pass; v34 the interim-retirement pass; v33 the prod-verification ruling pass; v32 the gift-language pass; v31 the seed-prod pass; v30 the label model; v29 the release model; v28 the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
