...
| 308 | sudo bot: unmarked delivery-claim fabrications on zero-tool turns — no machine-detectable signature; needs a ruling (deterministic routing / zero-tool bounce / tool_choice) | KnightOfNight | triaged, monitoring-and-command, V25 | Version 25.11 | 2026-08-30 |
...
## Issue #308: sudo bot: unmarked delivery-claim fabrications on zero-tool turns — no machine-detectable signature; needs a ruling (deterministic routing / zero-tool bounce / tool_choice)

- State: open
- Author: KnightOfNight
- Labels: triaged, monitoring-and-command, V25
- Milestone: Version 25.11
- Assignees: KnightOfNight
- Created: 2026-08-29 | Updated: 2026-08-30
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/308

### Body

Observed twice during the V25.10 Brief 1 operator playtest (2026-08-29, dev stack, claude-sonnet-5 brain): on a repeated see-request (`sudo what do you remember`), the model intermittently answers with delivery-claim prose ("Sent — full memory list delivered to your pane.") while making **zero tool calls** — nothing is delivered, and there is nothing for machinery to catch.

Log signature (bot log, UTC):

```
11:52:31 sudo request from Sharon-Love: 'what do you remember'
11:52:32 model request: input_tokens=9796 output_tokens=24 stop_reason=end_turn
11:52:32 answer to Sharon-Love delivered=True (0 receipts)
```

One model call, no `door report` line, 0 receipts — a fabricated claim with no `[did:]` mark. An immediate retry of the same request works (the model calls `report` and the pane renders to spec).

V25.10 shipped two structural layers for the *marked* variant (a fabricated `[did:]` mark with zero receipts is bounced back into the tool loop as a machine-detected lie, and any surviving model-written `[did:]` line is stripped before delivery and storage — both proven live). The **unmarked** variant has no machine-detectable signature: it is plain prose on a zero-tool turn, and zero-tool turns are legitimate in the conversational flows (artifact and ticket Q&A drafting), so a blanket bounce is not obviously correct.

Candidate rulings (design-session territory — each changes the bot's behavior class):

- **Deterministic routing of report-shaped requests**: bot machinery recognizes see-requests and calls `report` itself, bypassing the model for the act (the #306 doctrine's logical endpoint — the model comments, machinery acts).
- **Zero-tool-turn bounce**: any would-be final answer with no tool calls this turn draws one corrective redo ("if the request asks you to see/do something, act now; if this is conversation, answer as you were") — catches both observed failures, at the cost of doubling model round trips on every genuinely conversational turn.
- **`tool_choice` forcing** on the first model call of each turn — every turn must open with a tool call (breaks or distorts the drafting flows; probably too blunt).
- Accept the intermittency (a retry always works) and rely on the shipped prompt standing orders.

Lineage: the behavioral half of this shipped as prompt standing orders in 25.8/25.9/25.10 and the model still intermittently skips; the marked-fabrication half is structurally closed as of V25.10 Brief 1 (playtest fixes 3–4, recorded in its closeout report). This ticket is the unmarked remainder.


### Comments (2)

**KnightOfNight** — 2026-08-30:
Design ruling (2026-08-30, V25.11 design session): **this is the founding ticket of Version 25.11** (operator-ruled in-session). Milestone `Version 25.11` created and applied; labeled into the V25 (monitoring-and-command) major — the sudo bot is MC capability work continuing under the current major.

The approach ruling (deterministic routing / zero-tool bounce / tool_choice / accept) is this session's design work — it will be recorded here as a follow-up comment when made, with the `triaged` label applied in the same motion.


**KnightOfNight** — 2026-08-30:
Approach ruling (2026-08-30, V25.11 design session, operator-confirmed): **Option B — the zero-tool-turn bounce, single-shot.**

Mechanic (rides `_handle_sudo`'s tool loop, the sibling of the shipped receiptless-`[did:]` gate):

1. When a would-be final answer arrives (`not turn.tool_calls`) and **no tool call has run at any point this turn**, machinery bounces once — corrective `[game]`-voice message in the shipped gate's register: if the request asks the model to see or do something, act now with a tool call; if this is just conversation, answer as before.
2. **At most one bounce per turn** (per-turn flag). After the one redo, the next answer is delivered regardless — no loops, no starvation.
3. The existing receiptless-`[did:]` bounce (loop-capped), the strip guard, receipt folding, and storage shape are all **unchanged**; the `[did:]` gates continue to apply on top of the redo's answer.
4. Accepted cost, ruled explicitly: every genuinely conversational turn (drafting Q&A, confirm-gate exchanges) draws exactly one extra model round trip. Small at admin-channel volume.

Why B over the alternatives (recorded from the session): deterministic routing (A) covers only see-requests and imports NL classification into machinery; `tool_choice` forcing (C) breaks the drafting flows; accepting (D) leaves no machine backstop. B is signature-free — it closes the whole unmarked class, not the observed instances — and mechanizes the issue's own observation that an immediate retry always works. A remains available later as a cost optimization under B if the extra round trips ever matter.

Ships as V25.11 (founding ticket). GDD text and the implementation brief follow on `version_25_11` this session.
