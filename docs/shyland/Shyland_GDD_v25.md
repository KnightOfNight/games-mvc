...
| **v25.10** | **Point release — Closed** | **RELEASE — truthful delivery and the filing bot (#301 founding ticket; #304, #305, #306 riding the release).** One founding ticket, one brief, per the release scope law. §10.11's deterministic-delivery passage is now built exactly as ruled — the whole delivery surface, the stored record, and the bot's very singularity made structural, and the issue-first law's front door moved into the world. **The attach singleton (#304):** one agent account = one attached connection — a duplicate is accepted, sent the `already-attached` error frame, closed with the dedicated conflict code **4409**, never takeover; the claim registry (`ATTACHED`, agent username → `channel_name`) is a module-level in-process dict in the single Daphne process, deliberately not Redis — a crash destroys the registry together with the connections it guards, so a stale lock is structurally impossible; gate order fixed auth → membership (4403) → kill switch (4503) → singleton (4409) — a killed door reports killed, not conflict; `disconnect` releases under the guarded-presence-delete discipline, so a rejected duplicate's teardown cannot disturb the holder; bot-side `CLOSE_MEANINGS` gains 4409 and the refused bot backs off patiently. Bot runtime state leaves the checkout for machine-scoped **`~/.shyland/`** (pid, log, conversation store; filenames undotted by operator direction — `sudo_bot.<target>.pid` et al.), ending the per-checkout blindness behind the two-dev-bots incident. **The report family grows (#306):** `REPORT_KINDS` ×4 — `waypoints`, `memories`, and `memory` detail join `inventory` — kind-dispatched `a_report`, door-composed leaders and rows from live data, every listing/detail row a value-voice `{'v':}` entry (the client's render contract; a bare string renders blank — a playtest finding), delivered privately into the requesting admin's pane; receipts kind-aware (`report (waypoints) to … : delivered`). **The store keeps receipts (#305):** the recorded answer folds the turn's machinery-composed `[did: …]` lines after the text — still plain text, still provider-neutral — so a replayed thread distinguishes a backed claim from an unbacked one: the store remembers what the game confirmed, not what the model said. **The bot files the tickets (#301):** `file_issue` — title and body gathered conversationally on the artifact-flow precedent, filed only on an explicit confirm; bot-side GitHub REST with the token under the `.secrets` custody law (the game box holds no GitHub credentials, the door no new vocabulary); machinery applies the conventions (assignee = operator, no labels), the receipt carries the real issue number and URL from the API response, the filed body a filed-via-sudo provenance footer; tokenless hosts refuse legibly (`not-configured`) — **the first live bot-filed issue is #309**, filed during the operator playtest with every machinery mark verified. **Playtest-hardened in the implementation session (four findings, all fixed in place):** (1) a pre-hello error frame is a refusal announcement — the refused bot logs it, waits for the 4409 close, and re-enters the capped backoff instead of a protocol-check fatal; (2) report rows wrapped as `{'v':}` dicts door-side (the render-contract fix); (3) machinery strips model-written `[did:` lines — the form belongs to machinery, a model-written one is always a fabrication; (4) the strip guard grown into a **redo gate** — a would-be final answer carrying a `[did:]` mark while machinery holds zero receipts is a machine-detected fabrication, bounced back into the tool loop with a corrective (perform the real call or drop the claim; `TOOL_LOOP_CAP`-bounded, exhaustion falls through to the strip), proven live over the dev door with a scripted brain. The machinery-invisible remainder — a fabricated claim writing NO `[did:]` mark — observed live and filed design-lane as **#308** (unmarked zero-tool fabrications; candidate rulings on the ticket). Operator-directed additions: `botctl`'s dev target drops `--insecure` (the renewed wildcard cert is live on dev; both targets verify TLS fully). **Code-only release: no migration (`0054` remains head — migrate is a no-op), no seed, no client change.** Suite **877 → 891** (14 new tests, `tests/test_v25_10_brief1.py`; the brief's scenario 15 satisfied by the v25.8 report tests passing with zero edits). **Twelfth release under the technical pre-flight (#252): no mismatch recorded.** **Operator playtest successful against the dev stack** — all six checklist items confirmed: singleton refusal, deterministic listings, thread receipts, poison immunity, the live filing of #309, kill-switch precedence with unaided reattach. Architecture doc stamped 25.10 in place, hash moved to `66896ca` (architectural — the attach singleton, the report family, the persisted receipts, the filing action, the relocated bot runtime). One marker swept at this closeout: §10.11's deterministic-delivery parenthetical, to the shipped provenance form. **Pending deploy-time actions: none** — code-only release, migrate no-op, no seed, no data actions; the prod bot restart after deploy remains the operator's standing action — stop the old-code bot **from the main checkout that started it** (old-location pidfile), restart with post-25.10 code so its state lands in `~/.shyland/`; prod filing needs `agents/.secrets/github-token.sudo` on the bot host (operator-provisioned, any time before first use). |
| **v25.11** | **Point release — Closed** | **RELEASE — the zero-tool bounce (#308 founding ticket, sole milestone member).** One founding ticket, one brief, per the release scope law. §10.11's unmarked-lie passage is now built exactly as ruled — the marked fabrication's unmarked sibling closed structurally: delivery-claim prose on a turn with **zero tool calls** carries no `[did:]` mark, no receipts, and no machine-detectable signature (observed live twice in the 25.10 playtest — "Sent — full memory list delivered to your pane.", nothing delivered, an immediate retry always working). The shipped machinery is the **single-shot zero-tool bounce** in `_handle_sudo`'s tool loop: two per-request locals beside the ledger (`ran_tool`, set whenever tool calls execute — success or error result alike; `bounced`, the single-shot latch); a **nonempty** would-be final answer with no tool call executed at any point this turn and no prior bounce draws exactly one corrective `[game]`-register redo (mirroring the receiptless-`[did:]` gate's form), and the redo's answer is delivered without a second bounce — no loops, no starvation. Gate order fixed: the receiptless-`[did:]` gate evaluates first and is unchanged (loop-capped, multi-fire); the zero-tool gate sees only answers that pass it. Silence stays sanctioned and unbounced; the bounce consumes one `TOOL_LOOP_CAP` iteration, cap exhaustion falling through to the strip guard and delivery path unchanged. The ruled cost accepted explicitly: every genuinely conversational turn draws exactly one extra model round trip. **Playtest-hardened in the implementation session (one finding, operator-ruled in place):** the brief's corrective wording drew audience-confused redos on conversational turns — the model answered the machinery in the third person and single-shot delivery shipped the meta-commentary to the admin's pane; the conversation clause was reworded ("…give your answer again — your next message is delivered to the admin's pane, so speak to them directly, never about this exchange"), mechanism byte-identical, and the operator re-playtested the conversational case successfully. Change surface `agents/sudo_bot.py` only (host-side; the Docker image carries only the version stamp); `StubBrain`'s canned flow always answers after an executed tool call — no stub edits. **Code-only release: no model change, no migration (`0054` remains head — migrate is a no-op), no seed, no client change.** Suite steady at **891** (no new in-suite tests — the suite cannot see `agents/`; the scripted-brain driver pattern over the real dev door proved all four brief cases **15/15**, re-run 15/15 after the wording fix — precedent v25.9/v25.10). **Thirteenth release under the technical pre-flight (#252): no mismatch recorded.** **Operator playtest successful against the dev stack** — all checklist items confirmed, including the issue-drafting Q&A re-test against the reworded corrective. Playtest side-findings filed design-lane, pipeline not this release: **#311** (empty `durability_table` with `takes_durability_loss=True` falls through to the 1.0 full-penalty fallback — bot-created artifacts never author a durability posture; dev-mitigated in-session by operator direction) and **#312** (Django admin cannot re-save ItemDefinitions with legitimately empty JSON list fields — `blank=True` gap). Architecture doc stamped 25.11 in place, hash moved to `f520bde` (architectural — a new gate in the bot's tool loop; the final hash lands the playtest-ruled corrective rewording). One marker swept at this closeout: §10.11's zero-tool-bounce parenthetical, to the shipped provenance form. **Pending deploy-time actions: none** — code-only release, migrate no-op, no seed, no data actions; the prod bot restart after deploy remains the operator's standing action — the running prod bot predates the gate; restart with post-25.11 code (state already in `~/.shyland/` once restarted on post-25.10 code, else stop from the old checkout per the 25.10 note). |
| **v25.12** | **Point release — Closed** | **RELEASE — the durability-posture invariant (#311 founding ticket; #312, #314 riding the release).** One founding ticket, one brief, per the release scope law. §6.5's durability-posture passage is now built exactly as ruled — a definition's wear flag and its table are one coupled fact with exactly two legal postures: **wearing** (`takes_durability_loss` set, table fully covering every integer 0–100 — bands may touch or overlap, first-match-wins at read time unchanged, penalties 0.0–1.0) and **non-wearing** (flag unset, table `[]`); nothing between the two saves, anywhere. The defect family: bot-created artifact definitions inherited the model-default pair `True + []`, and `get_durability_penalty`'s no-band-matched 1.0 fallback treated pristine items as fully degraded — say-voice durability cells and fully-penalized strikes (#311); the manual repair path was itself blocked because the admin form refused legitimately-empty JSON lists (#312, the `blank=True` gap — fixed on exactly six fields); and the door's `durability_current` edit accepted fractional values no seed band covers (#314 — **durability is integral**: every fractional value refused, never stored, the game paths already moving whole points). The 1.0 fallback itself stays exactly as it was, deliberately: with the invariant enforced the state is unreachable, and softening it would mask future misconfiguration. **The machinery — one shared pure validator behind three enforcement layers:** `durability_posture_violations(takes_durability_loss, durability_table)`, module-level in `models.py` (placement avoids the `item_utils` import cycle), returning human-readable violation strings; behind it `ItemDefinition.clean()` (new — `ValidationError` keyed to `durability_table` so the admin form renders errors in place), door-side validation on the new `edit_item` definition keys (`takes_durability_loss` + `durability_table` validated as the post-edit coupled state), and **the repo's first `CheckConstraint`** — `durability_posture_coherent`, the database refusing the half-configured state outright. **Artifacts are born non-wearing:** `_create_artifact` authors `False + []` explicitly; the artifact spec vocabulary carries no durability keys — a wearing artifact would be a future ruling of its own. **Migrations `0055` + `0056`:** `0055_normalize_durability_posture` authored (reverse-noop, two UTC-Z-stamped count lines — dev printed 0 definitions flipped / 0 instances normalized; **the prod counts are read from the closeout tail's deploy output**), then generated `0056` (six field alters + `AddConstraint`, auto-depending on 0055). Suite **891 → 915** (24 new tests, `tests/test_v25_12_brief1.py`: validator, `clean()`, constraint, door creation/edit postures, integral rule, admin re-save). **Fourteenth release under the technical pre-flight (#252): zero mismatches in the brief's Section-2 claims — and the pre-flight's empirical arm caught a false premise elsewhere:** the brief's "zero modified tests" claim was wrong (139 fixture errors + 1 pinned old behavior + 3 artifact-test failures against the new constraint); hard stop taken, operator ruled the mechanical repair in-session — 13 test files' fixtures author an explicit all-worn band `[{'min': 0, 'max': 100, 'penalty': 1.0}]` (behavior-preserving by construction: exactly the empty-table fallback behavior they ran under, zero assertion churn), and `test_durability_zero_sets_broken`'s fractional-accepted pin converted to assert the #314 refusal with its nonzero-clears-broken intent preserved on a whole number (test-hygiene rule, reported never silent). **Playtest-hardened in the implementation session (one finding, fixed in place):** the bot's `edit_item` schema (doctrine: mirrors the door's param shapes) had not grown the new keys — the model had no vocabulary for the edit and warned that items "cannot be exempt from durability" while the door was authoring exactly that; fixed bot-side (`agents/sudo_bot.py`): the coupled pair with post-edit-state descriptions, `durability_current` noting the integral rule, `create_artifact` and the system prompt stating artifacts are born non-wearing. **Operator playtest successful against the dev stack** — all six checklist steps confirmed: admin re-save clean, form error rendered on the field, artifact born non-wearing with no durability cell, door edit coupling both directions on the restarted bot, Ludicrous Speed clean, 55.5 refused naming the integral rule / 55 accepted. Playtest side-finding filed design-lane, operator-directed: **#315** (the door's durability edit at 0 sets `is_broken` even on a non-wearing item — combat-honored, invisible, no in-game repair path; instance-side sibling of the #311/#314 family). Architecture doc stamped 25.12 in place, hash moved to `419a30a` (architectural — the shared validator, the three enforcement layers, the first CheckConstraint; the final hash lands the playtest-found bot-side schema mirror; §4.1, §4.7, §4.22, §4.23). One marker swept at this closeout: §6.5's durability-posture parenthetical, to the shipped provenance form. **Pending deploy-time actions: none beyond the ordinary** — migrations 0055/0056 ride `make deploy-prod`'s migrate step in the closeout tail (named executor), 0055's two printed prod counts recorded actual-vs-expected (expected small, likely 0/0); no seed, no data actions; the prod bot restart after deploy remains the operator's standing action (post-25.12 code carries the schema mirror). |
| **v25.13** | **Point release — Closed** | **RELEASE — over-capacity legibility (#275 founding ticket; #319 riding the release).** One founding ticket, one brief, per the release scope law. §6.10's over-capacity passage and §3's auto-swap displacement clause are now built exactly as ruled (Direction B): **the over-capacity state is legal and legible — the stranding action succeeds and warns, it is never refused, and the bag guard stays byte-identical.** The defect pair: unequipping (or auto-swap-displacing) stat-granting gear could drop carry capacity below the current load with zero output — the character discovered the stranding only through opaque acquisition refusals (#275) — and pickup/buy counted **equipped** items against capacity (`get_carry_capacity`, all owned instances), disagreeing with loot, the sweep, the inventory header, and the bag guard (unequipped-only) by the equipped count (#319). **The machinery, three legibility surfaces:** (1) **the stranding warn** — new consumer helper `_warn_if_over_capacity(char)` reads `get_carry_counts` and, strictly over only, appends the warn-category line `You're over your carry limit (N/M items) — you can't pick up, loot, or buy anything until you're under it.` after the success sentence and before `send_status_refresh()`, in exactly three completion paths: `cmd_unequip`, `cmd_equip`'s free-slot branch, and the auto-swap branch — firing on the stranding action and as a deliberate restatement on every later gear change made while still over, never at or under; (2) **honest refusals** — the four acquisition gates (pickup's outright gate, buy's new pre-check before its quantity check, single-corpse loot's in-loop gate, and `sweep_corpses`' in-loop gate in `loot_utils` — typed loot / loot all / plunder in one edit) refuse strictly-over with exactly `You're over your carry limit. ({current}/{max} items)`; existing refusal strings byte-unchanged at ordinary fullness, pickup's mid-sweep line untouched; (3) **standing visibility** — the inventory header's `(current/capacity)` count renders the over state as-is, no additional marker. **The count basis unified (#319):** `cmd_pickup` and `cmd_buy` moved onto `get_carry_counts` (unequipped-only — the basis loot, the sweep, the header, and the bag guard already used); `consumers.get_carry_capacity` deleted, guard grep zero hits over `django/src/`. Recovery always in hand: drop, sell, and (re-)equip remain un-gated by capacity. **Code-only release: no migration (`0056` remains head — migrate is a no-op), no seed, no client change.** Suite **915 → 927** (12 new tests, `tests/test_v25_13_brief1.py`, covering all eight brief scenarios — stranding warn on unequip and auto-swap with exact numbers, at-or-under silence, the four refusal sites, exact-full string preservation, the #319 count basis U < M ≤ U + E, bag-guard regression, recovery). **Fifteenth release under the technical pre-flight (#252): all 13 claims verified against the code, zero load-bearing mismatches.** Two deviations recorded, neither silent: the brief-directed `test_gear_combat.py` conversion duplicates an adjacent assertion (harmless, noted for a hygiene pass), and the new test bag fixture authors explicit non-wearing durability posture to satisfy v25.12's `durability_posture_coherent` constraint (fixture-only). **Operator playtest successful against the dev stack** — all eight checklist items confirmed. Emergent findings filed design-lane during playtest support, outside brief scope: **#320** (the agent door / sudo bot has no currency-grant action — copper cannot be gifted; workaround: admin console) and **#321** (bulk buy/sell at admin-scale quantities takes minutes with zero output — sell O(n) transactions, buy O(qty) serial INSERTs; design session to rule). Architecture doc stamped 25.13 in place, hash moved to `4b97966` (architectural — a consumer DB-helper retired; refusal semantics changed across the consumer and `loot_utils`). Two markers swept at this closeout: §6.10's over-capacity parenthetical and §3's auto-swap clause, to the shipped provenance form. **Pending deploy-time actions: none** — code-only release, migrate no-op, no seed, no data actions; the prod bot restart after deploy remains the operator's standing action from prior releases (no bot-side change this release; post-25.12 code already carries everything pending). |
...
**The unmarked lie draws one redo (v25.11, #308).** The 25.10 gates closed the *marked* fabrication — a `[did:]` claim with no receipt behind it bounces, and a surviving model-written mark is stripped — and the release's first playtest promptly showed the unmarked sibling: intermittently, on a see-request, the model answers with delivery-claim prose ("Sent — full memory list delivered to your pane.") having called no tool at all. Nothing delivers, and plain prose on a zero-tool turn carries no machine-detectable signature to catch. The structural answer is the **zero-tool-turn bounce, single-shot**: when a nonempty would-be final answer arrives and no tool call has run at any point this turn, machinery bounces it once — a corrective message in the shipped gates' `[game]` register: if the request asks the bot to see or do something, do it now with a tool call; if this is just conversation, answer as before. **At most one bounce per turn, and the redo's answer is delivered regardless** — no loops, no starvation — with the `[did:]` gates still applying on top. The bounce is signature-free by design: it catches any phrasing on any request family, mechanizing the observed fact that an immediate retry always works, and machinery stays dumb — no natural-language classification of requests moves bot-side (deterministic routing of see-requests remains available beneath this gate as a future cost optimization, judged if the round trips ever matter). The accepted cost is ruled explicitly: a genuinely conversational turn — drafting Q&A, a confirm-gate exchange — draws exactly one extra model round trip, small at the admin-only audience's volume. Model silence stays sanctioned and unbounced — abandoned exchanges expire silently by design; the fabrication class is prose claims, not quiet. Everything else — the receiptless-`[did:]` bounce, the strip guard, receipt folding, the store's shape — stands untouched.
