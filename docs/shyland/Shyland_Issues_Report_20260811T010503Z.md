...
| 215 | Bags don't do enough: flat carry_bonus is noise against the STR-scaling capacity base (Mk 2 bag = +4.3% at level 17) | KnightOfNight | game-balance |  | 2026-08-10 |
| 217 | 'last' should show current location for players who are currently logged in | KnightOfNight | output |  | 2026-08-08 |
| 218 | Zombie combat sessions: all NPCs dead but session stays active — loot blocked 'in combat'; stale sweep can't reap (engine refreshes last_tick_at unconditionally) | KnightOfNight | bug, triaged | Version 24.19 | 2026-08-11 |
| 219 | In-game release notes: new 'readme' command — format and content TBD | KnightOfNight |  |  | 2026-08-08 |
...
| 221 | Character creation screen: font color too hard to read (muted text ~3.2:1 contrast, below AA) | KnightOfNight | output |  | 2026-08-10 |
| 222 | Raise the game's default font size a couple of pitch points (base 14px + a scattered hardcoded scale) | KnightOfNight | output |  | 2026-08-10 |
...
| 225 | Acuity displays truncate to one decimal — 1.15 renders as 1.1; need two decimals or more | KnightOfNight |  |  | 2026-08-11 |
...
## Issue #215: Bags don't do enough: flat carry_bonus is noise against the STR-scaling capacity base (Mk 2 bag = +4.3% at level 17)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/215

### Body

## Observation (operator, dev playtest during V24.16)

A level-17 test character has an unassisted carry capacity of **470**; equipping the Mk 2 Satchel adds only **+20** (~4.3%). That's lame for a dedicated equip slot — a bag should feel like meaningful capacity, something more like **+20%** (operator's initial suggestion, unruled).

## Where the 470 comes from (diagnosis — the math is working as designed)

Carry capacity = **effective STR × 10 + equipped-bag `carry_bonus`** (#100). For the test toon (Harley Stone, level 17):

- Base `stat_str` 28 (level-up spends) + gear STR +19 (seven equipped Mk 2 pieces rolling `str`: Iron Sword +7, Silver Ring of Strength +5, Leather Vest +3, four +1 riders) = effective STR **47** → 470
- Mk 2 Satchel `carry_bonus` = 20 (flat, authored per definition ~10 × Mk) → total 490

## The actual problem

Bag `carry_bonus` is a **flat** number competing against a **level-and-gear-scaling** base. The base grows with level-up STR spends and with gear flats (which now track the Mk band per #130), so the bag's relative contribution shrinks continuously — at level 1 the starter bag was a meaningful fraction of ~100; by level 17 the best bag in the game is noise. Any fix likely wants the bag term to scale (percentage of the STR-derived base, STR-tracking flat, or a steeper per-Mk curve) — design-session ruling required; percentage-based would be the first non-additive term in the capacity formula.

Filed from the V24.16 implementation session (mid-build discovery, normal pipeline); numbers read live from the dev DB.
...
## Issue #218: Zombie combat sessions: all NPCs dead but session stays active — loot blocked 'in combat'; stale sweep can't reap (engine refreshes last_tick_at unconditionally)

- State: open
- Author: KnightOfNight
- Labels: bug, triaged
- Milestone: Version 24.19
- Assignees: KnightOfNight
- Created: 2026-08-08 | Updated: 2026-08-11
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/218

### Body

## Live occurrence

2026-08-08, prod (v24.16): player **GunnySam** in The Choke, all NPCs in his fight dead, but `loot` refuses with the in-combat message. Operator reports having seen this before. Player-side unstick: `flee` (see mechanism below).

## Mechanism

The in-combat gate is membership in a `CombatSession` with `is_active=True`. Sessions are deactivated in exactly one combat-flow path: the engine's attack round where the **last living NPC dies** (`run_tick_engine.py` ~585: `live_npcs` empties → `is_active=False`). Every other route to "all NPCs dead" leaks the session.

**The stale-session sweep cannot catch the leak.** `update_session_tick` (~line 259) bumps `last_tick_at = now` for every active session on every tick, unconditionally, before any liveness check — then non-round ticks `continue`. A zombie session (active, no living NPCs) therefore keeps its own pulse fresh forever; the `STALE_SESSION_SECS = 30` cutoff never trips. The safety net is neutralized by the thing it's meant to watch.

Candidate leak entries (how the happy path gets skipped — to be confirmed during fix):

- NPC death outside the attack round (proc/effect/DoT tick paths)
- An engine exception after `session.npcs.remove(npc)` but before the `is_active` save (the #135 crash family)
- Multi-member session edge cases in the per-character kill loop

## Player-side workaround (verified in code)

`flee`: if the session has no NPC rows attached, the handler ends the session immediately and cleanly (empty-npcs branch → `end_combat_session`); if dead NPC rows are still attached, the contest runs against them and success also ends the session. Cooldown applies between attempts.

## Suggested fix directions (design session to rule)

1. **Self-heal at the loop head:** when an active session is picked up with zero living NPCs, deactivate it right there (with the "Combat has ended." message) instead of ticking it. Catches every leak path regardless of origin — defense in depth.
2. Only refresh `last_tick_at` for sessions that actually have living NPCs, restoring the stale sweep's ability to reap zombies (weaker than 1 — 30s of stuck-ness — but repairs the net).
3. Audit the non-attack-round NPC death paths so removal and session-end stay atomic.

(1) + (3) together is the thorough shape; (1) alone likely suffices for the player experience.


### Comments (5)

**KnightOfNight** — 2026-08-08:
## Live field capture (2026-08-08, prod, operator-authorized read-only DB access)

Caught the zombie in the act. Player GunnySam, The Choke:

- **Exactly one zombie session** (id 1500) — not an accumulation. `started_at` 19:46:02Z, `first_attacker='npc'` (aggro engagement), `members=1`.
- **`npc_rows=1, live_npcs=0` — a DEAD NpcInstance still attached to the active session.** This is the key forensic fact: the engine's kill path removes the NPC row *before* deactivating, so an attached dead NPC proves the NPC died outside this session's kill path.
- Engine ticked the zombie normally (tick_counter 117+, last_tick_at advancing) until ~19:49:14, then the session closed by ~19:49:59 with **no player action** (GunnySam did nothing, didn't die, didn't flee — he was re-engaged by fresh aggro at about the closure time). So the leak **self-heals in minutes** (stale sweep, or a cleanup triggered by the new engagement — exact closer unidentified). The earlier "immortal zombie" reading was too pessimistic; observed outage ≈ 3.5 minutes of blocked looting, compounded in aggro-respawn rooms where the player is re-engaged before they can loot.
- Ticker logs clean — no tracebacks.

## Root-cause hypothesis (fits every observation): cross-session shared-NPC kill

`CombatSession.npcs` is M2M — one NpcInstance can belong to two players' separate sessions. The operator was fighting the same NPC as GunnySam and landed the kill. The engine's kill handling runs inside the killer's session only: it removes the NPC from *that* session and closes *that* session. The other session keeps the now-dead NPC attached, its own happy-path deactivation can never fire (its NPC list will never produce a kill), and its member is stuck "in combat" until the slow cleanup reaps it.

**Reproduction test available:** two players engage the same NPC in separate sessions; the player who does NOT land the kill is predicted to zombie for minutes. Conversely a self-kill ends cleanly. **Half confirmed live:** GunnySam's next fight ended by his own hand and loot worked immediately afterward — the clean path behaves as predicted. The cross-session half (operator steals the kill → zombie re-forms) remains untested.

## Fix note for the design session

The self-heal-at-loop-head fix (suggestion 1 in the issue body) also covers this cross-session case and shrinks the stuck window from minutes to one tick: any active session with zero living NPCs deactivates immediately with "Combat has ended." Additionally the kill path could remove the dead NPC from ALL sessions holding it (M2M `npc.combat_sessions`) and evaluate each affected session for closure — that is the semantically-correct fix; the loop-head self-heal is the belt-and-suspenders.


**KnightOfNight** — 2026-08-08:
## ROOT CAUSE CONFIRMED by deliberate live reproduction (2026-08-08, prod)

The operator ran the kill-steal test: engaged the same NPC GunnySam was fighting, from a separate combat session, and landed the kill. **GunnySam immediately stuck "in combat" again.** DB capture during the reproduction:

- Session **1509**: started 20:03:09Z, `first_attacker='character'` (GunnySam initiated — so the bug is independent of who engages first; occurrence 1500 was `'npc'`), `members=1`, **`npc_rows=1, live_npcs=0`** — the identical dead-NPC-still-attached signature — and the engine still refreshing `last_tick_at` (tick_counter 91 at capture).

**Confirmed mechanism:** `CombatSession.npcs` is M2M; one NpcInstance can belong to multiple players' sessions simultaneously. The engine's kill handling runs only in the killer's session: it removes the NPC from that session and closes it. Every *other* session holding that NPC keeps the dead row attached, can never fire its own last-kill deactivation, and its members are stuck behind the in-combat gate until the slow cleanup path reaps the session (~3.5 min observed in occurrence 1500).

Two occurrences in one hour with two players online — with a real multiplayer population fighting shared spawns, this fires constantly.

**Fix (for the design session), restated with confidence now:**
1. Kill path removes the dead NPC from **all** sessions holding it (`npc.combat_sessions`), then evaluates each affected session for closure (no living NPCs → deactivate + "Combat has ended."). The semantically correct fix.
2. Engine loop-head self-heal: any active session with zero living NPCs deactivates immediately. Belt-and-suspenders that also catches every other leak path (crash-mid-kill, effect-death, etc.) and caps any future stuck window at one tick.


**KnightOfNight** — 2026-08-08:
Design sibling filed: #220 — the multiplayer-combat model for shared NPCs (the same M2M root, gameplay side: parallel 1v1s, double NPC output, no aggro semantics, attribution undefined). This issue (#218) stays the mechanical bug: its fix (dead-NPC removal from ALL sessions + engine loop-head self-heal) ships independently and first; #220's ruling may later make the M2M semantics deliberate (e.g. shared sessions would structurally eliminate this orphan case).

**KnightOfNight** — 2026-08-11:
Design ruling (2026-08-10, V24.19 design session): **this issue is the founding ticket of Version 24.19** (milestone assigned). Bug fixes never wait for phase order (v29 release model) — this ships ahead of the Phase 4 (travel & world) queue. Fix-shape ruling to follow in this session; brief to be committed on `version_24_19`.


**KnightOfNight** — 2026-08-11:
## Design ruling — fix shape (2026-08-10, V24.19 design session, operator-confirmed)

Version 24.19 ships this fix as its founding (and only) ticket. Three parts, grounded in code readings this session:

1. **Kill path goes M2M-wide (the semantic fix).** At the single NPC death site (`run_tick_engine.py` ~534–596), after the NPC is marked dead: remove it from **every** active session holding it (`npc.combat_sessions`), not just the killer's. Each *other* affected session left with zero living NPCs closes on the standard pattern — `is_active=False`, `focus_npc=None`, `release_session_npcs()` (verified safe: filters `is_alive=True` and skips shared-live NPCs, so the disengagement-restore rule of GDD §5 is preserved), "Combat has ended." (`reward` category), fight-clear payload, fresh status to its members. The killer's own session keeps its existing flow untouched. If the dead NPC was another session's focus with other NPCs still live there, focus reassigns with the standard announcement.

2. **Loop-head self-heal, placed *before* `update_session_tick`.** Any active session picked up with zero living NPCs closes immediately (same messaging) instead of ticking. Because the check precedes the tick update, a zombie never refreshes its own `last_tick_at` — **the stale sweep is restored as a backstop for free** (the issue body's option 2 falls out of the ordering; no separate change). Stuck window for any leak path, present or future, caps at one engine tick.

3. **Path audit closes as "confirmed single site."** Exactly one place in game code kills an NPC (`run_tick_engine.py` ~535); the only other `is_alive=False` reference is the respawn reaper deleting long-dead instances. The issue body's option 3 needs no further work.

**Boundaries:** #220 (multiplayer combat model) untouched — no aggro, attribution, or session-sharing semantics change; this is purely orphan-session hygiene. **GDD:** no edit — §5 already describes intended behavior; mechanical defect only, so 24.19 carries no GDD content change and no pending-implementation marker.

Brief committed to `version_24_19` as `Shyland_V24.19_Brief_1_Zombie_Combat_Sessions.md`. `triaged` applied in the same motion as this ruling — diagnosis + ruling are cold-start-complete.
...
## Issue #221: Character creation screen: font color too hard to read (muted text ~3.2:1 contrast, below AA)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/221

### Body

## Report

The font color on the character creation screen (`/shyland/create/`, `character_create.html`) is too hard to read. (Operator report, 2026-08-08.)

## Ground facts

The screen has its own self-contained inline palette (independent of the game client's color system):

```
--bg / --surface: #16161a    --text:   #c8c8d4
--border: #2a2a35            --muted:  #6b6b80
--accent: #7b68ee            --room:   #a8d8a8
--error: #cc4444             --ok:     #4caf7d
```

Likely culprit: `--muted` (#6b6b80) on the dark surface computes to roughly **3.2:1 contrast — below the WCAG AA minimum of 4.5:1** for normal-size text. It's used for the subtitle, helper text, and the back link — i.e., much of the screen's explanatory copy. `--text` (#c8c8d4, ~10:1) and the accent-colored labels are comfortably readable by comparison; if the whole screen reads as dim, the small monospace size (14px Courier) may be compounding the muted issue.

## Scope note

Fix is contained to the one template's inline CSS — no shared-surface or game-client color-doctrine impact. Exact element(s) and replacement values to be confirmed at fix time (operator to point at the worst offenders on the live screen).
...
## Issue #222: Raise the game's default font size a couple of pitch points (base 14px + a scattered hardcoded scale)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/222

### Body

## Request

The default font size for the whole game needs to go up a couple of pitch points (operator, 2026-08-08). E.g. base 14px → 16px, with the rest of the type scale moving proportionally.

## Current state (game client, `game.html`)

Sizes are hardcoded px at ~10 separate sites rather than derived from one root:

| Element | Size |
|---|---|
| base (`html, body`) | 14px |
| output pane, prompt | 15px |
| input line | 14px |
| right pane (stats header, bars, map block) | 12px |
| stats name | 12px |
| fight-panel numbers, conn-ms | 11px |

## Implementation notes (fix-time decisions)

- Preferred shape: set the scale once at the root and convert the hardcoded px sites to relative units (`rem`/`em` or a `--font-*` variable set), so future size changes are one line. A straight +2px on every literal also works but leaves the next adjustment just as scattered.
- The right pane is width-budgeted (bars, map, fight panel, Kind 3 tables in the output pane) — larger type may need a look at pane widths/wrapping at common window sizes; the map's node-and-line rendering and table column budgets are the likely pinch points.
- The character creation screen carries its own inline 14px base — same bump should apply there for consistency (contrast issue on that screen is separately tracked in #221).
...
## Issue #225: Acuity displays truncate to one decimal — 1.15 renders as 1.1; need two decimals or more
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/225

### Body

Found during the V24.18 Brief 1 playtest (in-combat acuity drift pause, #142): after a Focus Tonic settles at the band edge, the stats pane shows "1.1" — the stored value is 1.15 (band edges are 2-decimal exact by #133). Acuity displays need two decimal places or more (operator).

Display surfaces that truncate to one decimal today:

1. Stats-pane number: the status payload already carries 2-decimal values (`consumers.py` `_build_status`, `round(..., 2)`), but the client renders `msg.acuity.toFixed(1)` (`templates/shyland/game.html`).
2. `stats` command render: `{character.acuity_current:.1f}` (baseline too) in `consumers.py`.
3. Tick-engine message suffixes: `(Acuity {...:.1f})` on the dot/hot/sharpens/wavers lines in `management/commands/run_tick_engine.py` (four sites).

Note for the ruling: #133 deliberately kept "round-to-0.1 display behavior" for the mid-climb values while storing band-edge arrival 2-decimal exact — a ruling here should say whether mid-climb rounding stays or the whole meter goes 2-decimal end to end.
