...
| 234 | Character deletion: items are silently orphaned (owner SET_NULL) — rule item disposition + hard-vs-soft delete model | KnightOfNight | triaged | Version 24.27 | 2026-08-14 |
...
- Author: KnightOfNight
- Labels: triaged
- Milestone: Version 24.27
- Assignees: KnightOfNight
- Created: 2026-08-12 | Updated: 2026-08-14
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-14:
## Ruling (design session, Version 24.27, 2026-08-13)

Operator-confirmed, all three points:

1. **Hard delete only.** No soft-delete model of any kind — no undo window, no name retirement, no preserved live FKs, no stats museum. The row is deleted, the schema cascades run, and the case-insensitive name frees for reuse. The soft-delete question is closed, not deferred.

2. **Items are deleted with the character.** Disposition = removal — the character's entire inventory (held and equipped, bound and unbound) is deleted as part of the same flow. No dropping in the room, no transfer. This arrives as a deliberate flow, not the current silent `SET_NULL` orphaning; the exact mechanism (`on_delete` change vs. pre-delete sweep) is specified in the brief.

3. **The Django admin console is the only deletion surface.** No in-game command, no management command, no player-facing self-delete. Deleting a character is an operator action performed in `/admin/`, full stop.

Milestone: Version 24.27 (founding ticket). Brief to follow on `version_24_27`.


**KnightOfNight** — 2026-08-14:
Ruling addendum (same design session, 2026-08-13): operator additionally confirmed the survivor detail — corpses the deleted character killed remain in the world with `killed_by` nulled, **lootable-by-nobody, decaying on their natural timer**. No corpse sweep on character delete. Recorded in GDD §3.8 (`50f9c48`); brief: `Shyland_V24.27_Brief_1_Character_Hard_Delete.md` (`9a46e01`, branch `version_24_27`).
