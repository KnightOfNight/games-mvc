...
| 145 | hot_acuity / dot_acuity announce no-op effect ticks (doctrine from #133) | KnightOfNight | triaged, game-mechanics, V26 | Version 26.0 | 2026-09-11 |
...
- Author: KnightOfNight
- Labels: triaged, game-mechanics, V26
- Milestone: Version 26.0
- Assignees: KnightOfNight
- Created: 2026-07-24 | Updated: 2026-09-11
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-09-11:
Design ruling (2026-09-11, V26.0 design session — operator-confirmed): **the #133 doctrine applies uniformly to the whole dot/hot family — all six branches.**

Code re-verified this session against the current engine (the issue's line numbers have drifted; the defects have not):

- `hot_acuity` (~1573): at baseline `step` computes to 0 — still saves and announces "Your mind clears… (Acuity N)" every tick. Confirmed as filed.
- `dot_acuity` (~1536) and `dot_longevity` (~1526): clamp-and-announce — pinned at the floor, they keep announcing an unchanging number. Confirmed as filed. (`dot_vitality`'s floor is the Dying event — terminal by nature, already correct.)
- **Scope addition, found in the same verification — the mirror no-op at max:** `hot_vitality` (~1549) and `hot_longevity` (~1561) at a full bar announce "You recover N…" every tick while recovering nothing. Same disease, same family, joins the scope.

**The ruling:**

1. All six dot/hot branches adopt the change-only + one-time-terminal-line pattern the shift branches have used since v23 B3: **announce only actual change** (the applied delta, never the nominal magnitude when they differ); **one terminal line at boundary arrival** (floor for DoTs, max/baseline for HoTs); **holding is silent**.
2. **Terminal-line wording for each branch is authored at fix time** (standing creative policy, per the issue).
3. **Sequencing (for the release-grouping step): lands before the curse engine (#330).** The floor-hold curse deliberately pins a bar and holds it — built after this fix it inherits silence-while-holding with one line at the hold; built before it, it would spam a no-op every tick.
4. **Scope:** one small release, one brief, code-only — no migration, no seed data, no GDD text beyond at most a §-note (the doctrine is already law from #133).


**KnightOfNight** — 2026-09-11:
Queue-order ruling (2026-09-11, V26.0 design session — operator-confirmed): **#145 is the founding ticket of Version 26.0** (milestone created and applied). The ruled V26 release order:

| Release | Founding ticket |
|---|---|
| 26.0 | #145 — effect-tick hygiene (the major opens on clean plumbing) |
| 26.1 | #70 — three-tanks doctrine + first Longevity drain |
| 26.2 | #330 — curse engine (Release A) |
| 26.3 | #297 — curses in the world (Release B) |
| 26.4 | #283 — greeting name anchors |
| 26.5 | #316 — K/M shorthand |
| 26.6 | #47 — effects pane (capstone) |
| 26.7 | #220 — shared-session multiplayer combat (deepest engine change, lands on a stable world; the promote-it-early alternative was considered and declined) |

Order lives in this ruling per the v30 model (labels/milestones don't encode order); later releases' milestones are born with their own design sessions. Brief 1 for 26.0 follows this session.
