...
| 166 | Top-level heal command — 'use infinity healing draughts' semantics | KnightOfNight | triaged, V24, healing-economy | Version 24.4 | 2026-08-02 |
...
| 188 | Process proposal: merged design+implementation session per point release, with automated playtesting (future Instructions v32) | KnightOfNight |  |  | 2026-08-02 |
...
- Milestone: Version 24.4
- Assignees: KnightOfNight
- Created: 2026-07-29 | Updated: 2026-08-02
- Blocked by: none
- Blocks: none
...
V24.4 design session (2026-08-01): this issue is the **founding ticket of Version 24.4** — milestone assigned, branch `version_24_4` created from main.

- **GDD text landed** on `version_24_4` at `083c043` (§9.1 chart row + grammar notes + Partial Fulfillment + state-gating matrix + aggregation cross-refs; §6.12 cross-ref), all carrying `(v24.4, pending implementation)` markers. The chart's "New in v22" column was retitled "Added" (operator-approved).
- **Operator confirmations this session:** the while-dying reading — `heal` is allowed while dying and behaves exactly as `use` (one restorative, the revival sequence); the column retitle; the GDD text as drafted.
- **Micro-ruling (design-session, this brief's scope):** gate order — the at-full refusal beats the no-draughts warn (at full vitality `You are already at full health.` fires even with an empty inventory). Case-2 wording authored: `You have no healing draughts.` (warn). The shortfall reuses the standing `You only had N.` — #161's family-wide wording question is deliberately not absorbed.
- **Brief committed:** `docs/shyland/Shyland_V24.4_Brief_1_Heal_Command.md` at `81292d2` — actionable when the operator directs an implementation session to it by name.
...
|---|---|---|---|---|
| 187 | Closeout tails need a sanctioned production-seed path: make seed-prod (Instructions v31) | KnightOfNight | triaged | 2026-08-02 |
