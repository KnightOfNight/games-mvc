...
| 30 | Travel network: should checkpoints (shards) also be travel senders? | KnightOfNight | V24, travel | Version 24.26 | 2026-08-13 |
...
| 38 | Obelisk attunement: player-set home spawn at checkpoint shards | KnightOfNight | V24, travel | Version 24.26 | 2026-08-13 |
...
| 240 | GDD §12 errata: stale "no Convergence commerce" note (shipped #95) + leftover "Document version 22.0" footer | KnightOfNight | errata |  | 2026-08-13 |
...
Design ruling (2026-08-13, V24.26 design session): **#38 is the founding ticket of Version 24.26**; this issue rides as the bundle's dependency (blocked-by link on #38, milestone `Version 24.26` on both) — same shape as the 24.25 Convergence bundle (#41 ← #95). The sender-model ruling (full mesh / relay-to-obelisks / status quo) follows in this session and will be recorded here when made.


**KnightOfNight** — 2026-08-13:
Design ruling (2026-08-13, V24.26 design session — bundle ruling A): **option 2 — shards relay to obelisks only.**

- **Obelisks (spheres) send anywhere:** unchanged — from an obelisk room, `travel` reaches any revealed node, shard or sphere.
- **Checkpoint shards become senders, but only to revealed obelisks:** from a shard room, the `travel` listing offers exactly the player's revealed sphere nodes (currently the Heart of the Convergence and the Verdant Crown; every future zone-end sphere joins as zones ship). A shard is a fragment of an obelisk, with a fragment of its power — the relay is diegetically native.
- **Status quo asymmetry is retired; full mesh is rejected** — obelisks keep their hub specialness and return-trip traversal survives (frontier shopping = shard → obelisk → shard, two hops).
- Independent of attunement (#38) per pending ruling B4 direction: sending is the shard's nature (world data), never gated by a player's bond.

GDD text (§2.11) lands with this release's design pass; exact listing/refusal wording follows at authoring time under the pooled-speech law.


**KnightOfNight** — 2026-08-13:
Ruling A callout (operator, 2026-08-13, V24.26 design session): **shard senders obey revelation, explicitly.** A shard offers only obelisks the player has actually revealed — a player halfway through the Verdant Reach can relay back to the Heart, but the Verdant Crown does not appear until they have stood in it. Not a new mechanic — the standing flat rule (*destination revealed? travel permitted*, revelation derived from `RoomVisit`) applied to the shard listing with a sphere-type filter — but pinned here because a new sender surface is exactly where an implementation could accidentally bypass the membership check. The brief's verification section must cover it: unrevealed obelisks absent from a shard's listing and refused on direct attempt. (Locked-zone rendering is unchanged where it applies: revealed-but-locked destinations list muted and draw the zone-lock refusal, §2.12.)
...
**KnightOfNight** — 2026-08-13:
Design ruling (2026-08-13, V24.26 design session): **this is the founding ticket of Version 24.26** — the shard bundle release (second Phase 4 travel-&-world release of the V24 series, per the 2026-08-12 bundle ruling). #30 (checkpoint shards as travel senders) rides as the bundle's dependency, linked blocked-by here, same shape as the 24.25 Convergence bundle (#41 ← #95). Milestone `Version 24.26` applied to both.

The bundle's design rulings (attunement mechanics, sender model, and their interlock) follow in this session and will be recorded here and on #30 as they are made.


**KnightOfNight** — 2026-08-13:
Design ruling (2026-08-13, V24.26 design session — bundle ruling C): **the recall scroll is retired** — killed, not deferred, not tracked (the directional-arrows disposition). `home` + attunement covers the entire command-driven-return need; an item duplicating a free command is dead design weight. The scroll was never implemented, so this is a pure design deletion: the §9.2 recall row and the §2.6 "Recall scroll" travel option come out of the GDD with this release. Rulings A (sender model, #30) and B1–B5 (attunement mechanics) are under operator study and will be recorded as they land.


**KnightOfNight** — 2026-08-13:
Design ruling (2026-08-13, V24.26 design session — bundle rulings B1–B5, operator-confirmed):

- **B1 — explicit `attune` command**, and the grammar is ruled tighter than proposed: **bare verb, no nouns, ever.** You attune to the node in the room you are standing in — presence is the argument. Behavior by case:
  - Room has no travel node → the command says so (world-declined warn: nothing here to attune to).
  - Already attuned to this room's node → it says that.
  - Otherwise → attunes and reports the new attunement and home location.
- **B2 — exactly one home.** Attuning to a new node replaces the old bond. Every character starts attuned to the Heart of the Convergence; existing characters default there at deploy (no behavior change until they choose).
- **B3 — any travel node is attunable** — shards and spheres alike, the Heart included (re-attuning to the Heart is how you go back to default). In-the-room-only, per B1.
- **B4 — independent of #30.** Sending is the shard's nature (world data); attunement is the player's bond (player data). Attunement never gates travel in either direction.
- **B5 — one home concept:** the attuned node is both where `home` delivers you and where death respawn wakes you. All travel nodes are safe rooms, so a frontier respawn is never a death trap.

Structural note (for the GDD text and brief): every attunable room is a safe room, so `attune`'s success path needs no combat gate — in-combat `attune` can only ever hit the "nothing here" warn. Refusal/success wording is authored at GDD/brief time under the pooled-speech and three-layer response doctrines.

With ruling A (#30, relay-to-obelisks) and ruling C (recall scroll retired), the bundle's design surface is fully ruled. GDD edits (§2.6, §2.11, §9; respawn text where it lives) follow in this session, marked "(v24.26, pending implementation)".


**KnightOfNight** — 2026-08-13:
Design ruling (2026-08-13, V24.26 design session — operator): **`stats` gains a `Home:` row** — the home-discoverability surface. Placement: the identity block, directly under the `Player:` line, before the blank line and the stat rows. Value: the effective home node's `travel_name` (a null bond renders the founding node's name, "The Convergence") — a rendering, exact and never varying, value-color under the standing key/value form. GDD §2.11/§9 text and Brief 1 amended in the same motion on `version_24_26`.
...
## Issue #240: GDD §12 errata: stale "no Convergence commerce" note (shipped #95) + leftover "Document version 22.0" footer
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/240

### Body

## Summary

GDD §12 (Future Systems) carries text that is wrong about **shipped** behavior — discovered during the V24.26 design session's GDD pass (2026-08-13). Two spots need an ops erratum pass on main; two more are recorded here for provenance only, already absorbed by the in-flight v24.26 GDD edits.

## Needs ops correction on main

1. **§12 "Battle Zones Beyond The Convergence" row** — says "the Convergence itself has no commerce yet — the Reach's checkpoint NPCs are the game's only vendors and repairers." False since **24.25 shipped #95** (The Everround / Morra's Smithy): the Convergence has commerce. The row's note should be corrected to match the shipped game (the architecture-doc §7 pointer may also need updating in the same motion).
2. **`section_12_future_systems.md` footer** — the section file ends with "*Document version 22.0 — Shyland, Closed*". The GDD's stamp of record lives in the index, `_00_header.md`, and the changelog; a per-section version footer is stale leftover (reads 22.0 against a shipped 24.25). Remove or correct.

## Absorbed by v24.26 (record only — no ops action)

3. **§12 "Zones-and-travel version (#30, #38, #41, #95)" row** — claimed all four items "belong to a future version"; #41/#95 shipped in 24.25 and #30/#38 ship with 24.26. The row is removed entirely on `version_24_26` (all four items now have dispositions). Moot when 24.26 merges; listed here because it was wrong on main from 24.25's merge until then.
4. **§2.11 "three ways out" paragraph** — omitted `home` (shipped v22) and leaned on the never-implemented recall scroll. Rewritten on `version_24_26` under this release's ruled edits (the scroll is retired by V24.26 ruling C on #38).

## The erratum test

Items 1–2 describe shipped behavior the docs got wrong — ops work on main, no version, no stamp move, no changelog row. No design ruling is required for either.
