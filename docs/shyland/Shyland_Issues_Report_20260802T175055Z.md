...
| 176 | Equipment paper-doll renders consumed hand slots as free (two-handed weapons) | KnightOfNight | triaged, output, V24, itemization | Version 24.7 | 2026-08-02 |
...
| 194 | Weapon handed-ness is invisible to the player — two-handed never surfaced in item display; equip expectations form wrong (V24.6 playtest) | KnightOfNight | triaged, output, V24, itemization | Version 24.7 | 2026-08-02 |
| 195 | Bare 'equip' should show current equipment — shared composition with 'inv' paper-doll | KnightOfNight | triaged, V24, itemization | Version 24.7 | 2026-08-02 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, output, V24, itemization
- Milestone: Version 24.7
- Assignees: KnightOfNight
- Created: 2026-07-30 | Updated: 2026-08-02
- Blocked by: none
- Blocks: #195
...
**KnightOfNight** — 2026-08-02:
V24.6 Brief 1 operator playtest (2026-08-02, dev stack): live confirmation of exactly this confusion, from the player side, while exercising the new composite strike (#177).

The operator, holding a two-handed Battle Axe, tried to add a Hunting Bow: "hunting bow is listed as ranged, my ranged slot is empty, I expected to be able to equip it and my battle axe. i understand why I cannot, and I don't want to change the game rules, I just want to improve the player experience."

The mechanics are ruled correct and unchanged (#178, confirmed as built in V24.6: a two-hander claims both hands from any slot it occupies — the bow needs both hands even though it sits in RANGED). This ticket's display-only framing stands, with one addition from the playtest: the paper-doll's free-looking consumed slots are half of the player-experience problem; the other half — nothing in the item's own presentation (inventory line, examine, refusal wording) discloses handed-ness *before* the expectation forms — is now filed as #194. The two tickets together are the full fix.

Urgency note: the composite strike (v24.6) makes every weapon slot contribute to damage, so players now actively try to fill Main hand + Off hand + Ranged as a normal part of play — this display gap gets hit far more often than in the single-attacking-weapon era.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.7 design session): joined to **Version 24.7** as a dependency of founding ticket #195 — the three equipment-display items (#176, #194, #195) ship together as one brief (same problem: equipment display doesn't communicate equip reality; operator-confirmed scope, no widening). Milestone `Version 24.7` assigned. Rendering specifics ruled in-session on #195's brief.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.7 design session — operator-confirmed specifics):

**Consumed hand slots name their consumer.** A hand slot consumed by a two-handed item equipped in another slot renders the consuming item's name-with-tier in the Name cell, **muted**, with `(two-handed)` muted in the Details cell:
...
Hunting Bow (two-handed, sits in Ranged): Ranged is the home row (normal rendering, details, flags); Main hand **and** Off hand both render the muted `Hunting Bow Mk 1` + `(two-handed)` rows. The true fact — "that weapon holds this hand" — is stated in words; muted distinguishes the informational rows from the home row so nothing reads as a second item. State carried by words, never color alone (the muted styling is reinforcement, not the carrier — the consumer's name and the `(two-handed)` word are the information).

Applies to the one shared paper-doll composition, which V24.7 extracts so `inv` and bare `equip` (#195) render identically. GDD §6 text lands on `version_24_7` with a (v24.7, pending implementation) marker.
...
## Issue #194: Weapon handed-ness is invisible to the player — two-handed never surfaced in item display; equip expectations form wrong (V24.6 playtest)
...
- Blocked by: none
- Blocks: #195
- URL: https://github.com/KnightOfNight/games-mvc/issues/194

### Body

Found in the V24.6 Brief 1 operator playtest (2026-08-02, dev stack), while exercising the new composite strike (#177).

## Symptom

Operator, playing: "hunting bow is listed as ranged, my ranged slot is empty, I expected to be able to equip it and my battle axe. i understand why I cannot, and I don't want to change the game rules, I just want to improve the player experience."

Nothing player-facing discloses a weapon's handed-ness before (or during) the equip attempt:

- The inventory line and examine output for a Hunting Bow present it as a Ranged weapon; `is_two_handed` is not surfaced anywhere in item presentation.
- The player sees an empty Ranged slot on the paper-doll and reasonably concludes the bow fits alongside a two-handed melee weapon. The refusal arrives only after the attempt, and doesn't explain the hands conflict.

## Diagnosis

Mechanics are correct and ruled (#178, confirmed as built in V24.6): a two-hander claims both hands from any slot it occupies — the bow needs both hands even though it sits in RANGED; a one-handed ranged weapon (Pulse Pistol) coexists with anything. **No rule change wanted** (operator, explicitly). This is a display/communication gap:

1. Item presentation (inventory/examine) never states two-handed.
2. The equip refusal doesn't say *why* (both hands are consumed by/needed for a two-hander).
3. The paper-doll half of the confusion is #176 (consumed hand slots render as free) — the two tickets together are the full player-experience fix.

How handed-ness should render (a flag-block word, an examine row, refusal wording) is a design ruling.

## Timing note

This gap matters more since v24.6: the composite strike makes every weapon slot contribute, so players now actively try to fill Main hand + Off hand + Ranged and hit this expectation mismatch as a normal part of play, not an edge case.

Related: #176 (paper-doll consumed-hands display), #178 (ranged-slot semantics ruling, closed), #177 (composite strike, closed).
...
**Handed-ness is disclosed before the expectation forms.** Three sites, one vocabulary (`One-handed` / `Two-handed`), zero mechanics changes:

1. **Examine:** every weapon gains a `Hands:` row (`One-handed` / `Two-handed`) beside Type/Genre/Damage. Both values always shown — the vocabulary is discoverable, not warning-only.
2. **Listing tables:** the shared Slot-cell composition appends the word for two-handers — `Ranged (two-handed)`, `Main hand (two-handed)` — so inventory and vendor tables inherit it from the one helper.
3. **Refusal wording:** when an equip refusal's displacement involves hands claimed by (or needed for) a two-hander, the sentence explains itself: `You'd have to unequip the Battle Axe first — the Hunting Bow needs both hands.` Plain same-slot swaps and the both-rings wording keep their current lines.

The paper-doll half of the fix is #176 (consumed slots name their consumer). GDD §6/§9 text lands on `version_24_7` with (v24.7, pending implementation) markers; ships with #195's brief.


## Issue #195: Bare 'equip' should show current equipment — shared composition with 'inv' paper-doll
...
- Blocked by: #176, #194
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/195

### Body

Operator-directed filing from the V24.6 Brief 1 playtest session (2026-08-02).

## Request

Bare `equip` (no arguments) should show the current equipment list instead of the fn-10 required-target prompt (`What do you want to equip?`).

The output should reuse/share the equipment display the `inv` command already composes (the paper-doll: every slot in anatomical order, occupants, muted `-` empties) — make it a shared composition rather than a second implementation, so the two commands can never drift apart.

## Notes

- Direction stated by the operator; **not triaged** — bare-verb semantics for `equip` (chart type, fn-10 departure, exact shared-output shape) are a design-session ruling. Precedent to weigh there: bare `loot` ≡ `loot all` (v24.5, #189) was ruled "the first and only verb whose bare form performs the `all` sweep — deliberately no precedent for `sell`"; bare `equip` proposed here is an *information* rendering, not a bulk action, so it sidesteps that concern, but the chart cell and GDD §9.1 footnotes need the ruling.
- Natural companions if bundled by design: #176 (paper-doll consumed-hands display), #194 (handed-ness invisible in item display) — all three are equipment-display player-experience items surfaced by the same playtest era.

Related: #189 (bare-verb precedent discussion), #176, #194.


### Comments (3)

**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.7 design session): **this is the founding ticket of Version 24.7** (V24 major, Phase 2 — itemization). Operator-confirmed scope: #176 (paper-doll consumed-hand slots) and #194 (handed-ness invisible in item display) ride as dependencies — three facets of the same problem, equipment display not communicating equip reality, all surfaced by the V24.6 playtest. One brief covers all three: the shared equipment composition, bare `equip` semantics, consumed-slot rendering, handed-ness disclosure, and refusal wording. Milestone `Version 24.7` assigned to all three; branch `version_24_7` open. Design specifics (chart cell for bare `equip`, §9.1 footnotes, rendering forms) ruled in-session; brief follows on the branch.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.7 design session — operator-confirmed specifics):

**Bare `equip` renders the equipment view.** Chart cell `equip (eq)` changes `4 · 10` → `4 · 21`; **new footnote 21** (append-only numbering): the target is optional — bare invocation is an *information rendering*: it reports instead of acting. For `equip`, the bare form renders the Equipment paper-doll — the identical composition `inv` shows, via one shared helper, so the two can never drift. Distinct from footnote 20: a bare information form performs no action and sets no precedent for bare bulk actions.

- Output is the paper-doll block **only**: the `Equipment...` header + 14-slot table. **No inventory table, no carry count, and no wallet line** (operator-pinned exclusion — `inv`'s trailing wallet section stays `inv`/`wallet` territory).
- Category `report` (a rendering: unstamped, never varies). `equip` stays action-type in the chart; only its bare form reports.
- The §9 parenthetical about the superseded `equipment` verb is rewritten: bare `equip` now carries the equipped-only view it declined to give a verb to.

Consumed-slot rendering and handed-ness disclosure ruled on #176/#194 (this release's dependencies). GDD §9.1/§6 text lands on `version_24_7` with (v24.7, pending implementation) markers; brief `Shyland_V24.7_Brief_1_Equipment_Display.md` follows.


**KnightOfNight** — 2026-08-02:
Design ruling addendum (2026-08-02, V24.7 design session — operator-ruled): **bare `equip` is allowed in combat.** The state gate follows the act, not the verb: the bare form is an information rendering (footnote 21), and the identical paper-doll is already visible in combat via `inv`. Targeted `equip` (any argument) keeps the standing combat refusal unchanged. §9.1 state-gating matrix amended on `version_24_7` with the (v24.7, pending implementation) marker.
