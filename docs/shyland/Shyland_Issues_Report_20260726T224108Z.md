...
| 150 | Protect players from 'sell all'-ing their potions — bulk sell needs a consumable guard | KnightOfNight | B1, emergent, triaged | Version 23.1 | 2026-07-26 |
...
- State: open
- Author: KnightOfNight
- Labels: B1, emergent, triaged
- Milestone: Version 23.1
- Assignees: KnightOfNight
- Created: 2026-07-25 | Updated: 2026-07-26
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-07-26:
**Design ruling (2026-07-26, Version 23.1 design session) — operator-confirmed.**

Direction: **Option 3 — grammar-level exclusion.** `sell all` and `sell all <rarity>` exclude the consumable item type by default; any noun-carrying form (`sell all draught`, `sell 5 draught`, `sell draught`) sells consumables deliberately, unguarded. No migration — pure resolver/selection policy, following the chart's existing precedent (`drop` excludes bound items from `all` by ruled design). Leaves options 2/4 (a player-managed `keep` flag) open as a later superset; confirmed in-session that `keep` requires schema changes (a per-instance boolean plus stacking-display design) and is out of scope here.

Sub-decisions (all operator-confirmed):

1. **Guarded forms:** exactly the noun-less bulk sells — `sell all` and `sell all <rarity>`. Every noun-carrying form bypasses the guard. `drop` is untouched.
2. **Excluded type:** `consumable` only. Materials, readables, and keys stay bulk-sellable. (Seed audit 2026-07-26: the guard touches exactly three definitions — Healing Draught, Focus Tonic, Repair Kit. Materials are only Animal Hide + Insect Carapace; no readables or keys are seeded. Everything else `sell all` reaches is unequipped gear.)
3. **Player feedback:** the exclusion is announced, not silent — when a bulk sell skips consumables, the sell output carries one note line saying they were kept and how to sell them intentionally (wording authored at brief/implementation time under creative policy). If the bulk sell matches *only* consumables, that is a world-declined response (warn layer), not a bare nothing-to-sell.
4. **#138 compatibility:** preserved — zero-value disposal is unaffected for non-consumable junk; consumable junk still exits via the named-noun form. The guard narrows default reach, not sellability.

Milestone: Version 23.1 (founding ticket — point-release scope law: one bucket, one brief, this issue). Brief 1 to be committed on branch `version_23_1`.


**KnightOfNight** — 2026-07-26:
**Ruling precision (2026-07-26, same session):** verified against GDD §6/§9 and `command_grammar.py` — bare `sell all` is *already refused* by the v22 grammar with teaching wording (fn 17; `'Sell all of what? …'`). The only noun-less bulk sell that executes is **`sell all <rarity>`**, so that is the form the consumable guard applies to. Bare `sell all` stays refused exactly as ruled in v22 — this point release does not legalize it. Sub-decision 1 reads accordingly: guarded form = `sell all <rarity>`; every noun-carrying form bypasses the guard; `drop` untouched.
