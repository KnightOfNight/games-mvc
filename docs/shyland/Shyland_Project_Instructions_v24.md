...
- **Standing implementation-brief requirements — never omit:**
  ...
  2. **An operator-authorized in-session production deploy** — **exactly `make build && make migrate`** (the legacy `make prod` no longer exists; never invent a `make deploy`), `DOCKER_HOST` verified in pre-flight. Production posture is required and guard-enforced: `DOCKER_HOST` set **and** `.env` a byte-for-byte copy of `.env.prod` (worktrees initialize in dev posture — the switch to `.env.prod` is a deliberate, operator-authorized act, made only for the deploy). The `crosscheck-env` guard stops the deploy on any mismatch; never copy an env file merely to satisfy the guard. Deploy-time data actions execute in the production container in the same session where authorized, results recorded in the closeout; the PENDING block is the fallback only when the deploy isn't authorized.
