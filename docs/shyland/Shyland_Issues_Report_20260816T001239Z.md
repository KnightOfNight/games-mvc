...
| 205 | Production deploy target gains a dangling-only image prune — stop the ~500MB/release root-volume growth | KnightOfNight | triaged, V24, deployments | Version 24.31 | 2026-08-15 |
...
| 255 | Erratum: architecture doc §2.2 Makefile table documents a nonexistent 'make reset' and omits the entire deployment/guard surface | KnightOfNight | triaged, errata, V24, deployments | Version 24.31 | 2026-08-16 |
...
**Technical finding at V24.31 design-session startup (2026-08-15) — the proposed dev flag no longer exists.**

Verified against Emma's local dev daemon just now:

- `docker --version` → **29.6.2-rd**
- `docker builder prune --help` offers **`--reserved-space bytes`**; there is **no `--keep-storage`** flag.

So the 2026-08-06 comment's proposed dev sweep — `docker builder prune -f --keep-storage 5GB` — would fail outright on the dev daemon as written. The LRU-cap idea survives; only the flag name changed (`--keep-storage` → `--reserved-space` in the Docker 28→29 era).

Unverified from a design session and therefore an open question for the ruling: **the production daemon's Docker version and its flag spelling.** Design sessions never touch the prod daemon, so this issue's implementation brief must either pin the flag after the operator confirms prod's version, or the sweep must be written so it cannot break a deploy if the flag is unrecognized.

Recorded at the founding of Version 24.31, for which this issue is the founding ticket.


**KnightOfNight** — 2026-08-15:
## Ruling (operator, 2026-08-15, recorded live) — Version 24.31 founding ticket

This issue is the founding ticket of **Version 24.31** (milestone created this session, branch `version_24_31`). The ruling below **supersedes the body's implementation shape**, which the 2026-08-11 field data overtook.

### What the two observations actually mean (reconciled)

The 2026-08-06 census (13 dangling image pairs, 4.35GB "reclaimable") and the 2026-08-11 finding (`docker image prune` reclaimed **~0B**; `docker builder prune -a` freed 7.3GB prod / 22.8GB dev) are **both true and not in conflict**. The dangling images' layers were *also* held by build-cache references, so pruning the image records released records, not bytes; the builder prune released the last references and the layers actually died.

**Therefore the sweep is both prunes, and the builder prune is the load-bearing one.** Image prune runs **first** (drops the dangling image records, releasing their layer refs), builder prune second (drops the cache records; layers with zero refs are then actually deleted). A dangling-only prune, as the body ruled, would have shipped and appeared not to work.

### The shape that ships

**Size-capped LRU eviction, flag spelled per daemon**, held in documented Make variables:

```make
# Build-cache sweep caps (#205). The flag name diverges by daemon version:
# --keep-storage was renamed --reserved-space in Docker 28. Prod runs
# Docker Engine 25.0.16 (linux/arm64); Emma's dev daemon runs 29.6.2-rd.
PROD_BUILDER_PRUNE_FLAGS := --keep-storage 5GB
DEV_BUILDER_PRUNE_FLAGS  := --reserved-space 5GB
```

LRU-to-a-cap rather than `-a`: the most recent build's layers are by definition kept, so the next deploy's pip layer is still a cache hit and **no deploy gets slower**. This is the property `-a` cannot offer.

### Both targets get the sweep (the body's "optional rider", now ruled in)

Operator nod given this session. Dev accumulates faster than prod (~2GB/build vs ~500MB/release) and has already required two manual catch-ups (35.04GB on 2026-08-06, 22.8GB on 2026-08-11). Same fix, second daemon, same brief — no scope widening under the release scope law.

### Two safety properties the recipe must have

1. **The sweep is non-fatal** — each prune line carries make's `-` error-ignore prefix. Housekeeping must never fail a deploy, and on `deploy-prod` a hard failure between migrate and the posture restore would strand production posture and block all dev work behind the guards.
2. **On `deploy-prod` the sweep runs AFTER the resting-posture restore**, not before it as the body proposed. The prune lines pin their own `DOCKER_HOST` and do not read `.env` (only `docker compose` substitutes from `.env`), so posture is irrelevant to them — while running them last means even an operator `Ctrl-C` during a long prune cannot strand prod posture. The completion echo moves after the sweep.

### The one-time catch-up needs no separate executor

The body called for a manual prod catch-up to reclaim the accumulated ~4.3GB. **It is now free:** once this ships, V24.31's own closeout-tail `make deploy-prod` performs the first sweep, reclaiming everything accumulated to date as part of an ordinary deploy. No `PENDING DEPLOY-TIME ACTIONS` block is required and no unrunnable prod-side step is written (executor checkpoint, #248, satisfied by construction).

### Scope

Entirely within the `Makefile` — **shared deployment surface, operator-confirmed this session** (CLAUDE.md Rule 2). No game code, no migration, no seed data, **no GDD text** (this release changes nothing player-facing, so it will produce no `(pending implementation)` markers to sweep).


**KnightOfNight** — 2026-08-15:
**Brief committed** — `docs/shyland/Shyland_V24.31_Brief_1_Deploy_Target_Build_Exhaust_Sweep.md` on branch `version_24_31` at `bcd6de9`.

Actionable per CLAUDE.md Rule 4 once an implementation session is directed to it by name. Carries #255 (arch doc §2.2 erratum) as its architecture-doc step, per the ruling on that issue.

Design-session rulings inside the brief worth surfacing here:

- **The prod prune line ships unexercised, by construction.** Dev's Docker 29 rejects `--keep-storage`, so verification can only exercise the dev spelling. Contained by the non-fatal `-` prefix — worst case the sweep silently does nothing; it can never fail or posture-strand a deploy. First exercised in this release's own closeout tail, which is instructed to read `deploy-prod`'s output and record the reclaimed bytes (or the `Error 1 (ignored)` line, which would mean prod was upgraded past Docker 28).
- **The architecture header hash moves** even though no game code changes — §2 documents the build/deploy machinery and this release changes the deployment procedure itself. (Contrast v24.24, presentation-only, whose hash did not move.)
- **No `PENDING DEPLOY-TIME ACTIONS` block.** The body's one-time prod catch-up is now free: the closeout tail's ordinary `make deploy-prod` performs the first sweep and reclaims the accumulated backlog. Executor checkpoint (#248) satisfied by construction — the only prod-side step lives inside the deploy target itself.
...
## Issue #255: Erratum: architecture doc §2.2 Makefile table documents a nonexistent 'make reset' and omits the entire deployment/guard surface

- State: open
- Author: KnightOfNight
- Labels: triaged, errata, V24, deployments
- Milestone: Version 24.31
- Assignees: KnightOfNight
- Created: 2026-08-15 | Updated: 2026-08-16
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/255

### Body

## Erratum — architecture doc §2.2 "Makefile workflow" table is stale

Found at the V24.31 design-session startup while verifying which doc sections #205's brief would touch. The doc is wrong about **shipped** behavior; the Makefile is right.

**File:** `docs/shyland/Shyland_Architecture_v24.md` §2.2 (table at lines 122–134)

### 1. It documents a target that does not exist

The table's last row reads:

| `make reset` | Drop all Docker volumes, rebuild, start, migrate, and run `seed_world` — full data wipe |

**There is no `reset` target in the Makefile.** Verified against the full target list at the branch tip. The nearest real target is **`nuke`** (line 126), which is materially different: it is `require-local`-gated (refuses to run when `DOCKER_HOST` is set), removes containers/volumes/images, and does **not** rebuild, start, migrate, or seed.

A reader following §2.2 would type a command that does not exist — and the row describes a full data wipe, so the misdescription points at the most destructive corner of the Makefile.

### 2. It omits the entire deployment and guard surface

The table predates the deployment structure work (2026-07-26) and never caught up. Missing entirely:

- **Deployment targets:** `deploy-dev`, `deploy-prod`, `seed-prod` (#187), `verify-prod` (#249) — the four posture-setting targets, the only ones permitted to write `.env`
- **Guards:** `require-local`, `crosscheck-env` — the mechanism enforcing the standing target rule, run automatically before every daemon-touching target
- **Others shipped and undocumented here:** `hooks`, `push-certs`, `nuke`, `createuser`, `gen-certs`, `seed`, `verify`, `check-secrets`, `gdd`, `help`

The irony is that §2.2's own "Critical workflow note" is correct and useful; the table above it is the stale part. (Note `verify` / `verify-prod` *are* described accurately elsewhere in the doc, at §1518 — only this table is behind.)

### Lane

Ops-lane erratum by the extended rule (#242, operator-ruled 2026-08-13): factual correction to the architecture doc, passing the technical-writer test — **no new technical content, no design change, header hash untouched**.

### Proposed handling — ride V24.31's brief instead of a separate ops session

**V24.31 (#205) already rewrites this exact table**, because it adds the build-exhaust sweep to `deploy-dev` and `deploy-prod` and those targets are missing from it. Correcting the stale rows in the same edit avoids a guaranteed merge conflict at closeout (an ops-lane edit on `main` versus the brief's edit on `version_24_31`, same table, same lines).

Recommend: the correction rides V24.31 Brief 1's architecture-doc step. Falls back to an ordinary ops session on `main` if the operator prefers the strict lane — in which case V24.31's brief must be told to leave the stale rows alone.


### Comments (1)

**KnightOfNight** — 2026-08-15:
## Ruling (operator, 2026-08-15, recorded live)

**This erratum rides Version 24.31 Brief 1 rather than a separate ops session on `main`.**

Rationale as filed: V24.31 (#205) already rewrites arch doc §2.2's Makefile table — it must, because the sweep lands in `deploy-dev` and `deploy-prod` and neither target appears in that table today. Correcting the stale rows in the same edit avoids a guaranteed closeout merge conflict (an ops edit on `main` against the brief's edit on `version_24_31`, same table, same lines).

Added to the **Version 24.31** milestone as a dependency of #205 under the scope law — it does not widen the release's problem, it is the same table the founding ticket's brief must touch. It closes with the release.

The `errata` label stays: this is still a factual correction to shipped behavior with no new technical content, no design change, and no header-hash movement (#242's technical-writer test). Only the lane is different, by this ruling.
