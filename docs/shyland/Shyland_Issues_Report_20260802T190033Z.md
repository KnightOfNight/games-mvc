...
| 197 | Listing Slot cell hides either-hand flexibility — dual-slot weapons should read 'Main hand/Off hand' | KnightOfNight | triaged, V24 | Version 24.8 | 2026-08-02 |
...
## Issue #197: Listing Slot cell hides either-hand flexibility — dual-slot weapons should read 'Main hand/Off hand'

- State: open
- Author: KnightOfNight
- Labels: triaged, V24
- Milestone: Version 24.8
- Assignees: KnightOfNight
- Created: 2026-08-02 | Updated: 2026-08-02
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/197

### Body

**Found during:** V24.7 Brief 1 implementation session (2026-08-02), while discussing the new handed-ness disclosure surfaces.

**Behavior:** the listing-table Slot cell (`_slot_cell` in `consumers.py`) renders `valid_slots[0]` only — the v22 #123 composition. A weapon valid in either hand (seed's only current case: **Combat Knife**, `valid_slots: ['MAIN_HAND', 'OFF_HAND']`) lists as just `Main hand` in the `inv` inventory table and vendor `list`. Its either-hand flexibility is undisclosed anywhere in listings — the same expectation-forming gap #194 closed for two-handers.

**Operator direction (stated at filing):** a dual-slot item's Slot cell should read `Main hand/Off hand`.

**Notes for the ruling:**
- Interaction with the v24.7 two-handed word: composition for a hypothetical dual-slot two-hander would need a ruling (none exist in seed today; all bows are RANGED-only).
- `examine`'s `Equipped:` row and the paper-doll are unaffected (they render the actual occupied slot); this is listing tables only.
- No mechanics involved — display composition only.


### Comments (1)

**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.8 design session): **#197 is the founding ticket of Version 24.8** (milestone assigned; branch `version_24_8` created from main at 3201d3f).

**Composition ruled:**

1. The listing-table Slot cell renders **all** valid slots — sentence-case labels joined with `/` in authored `valid_slots` order: `Main hand/Off hand` (seed's only dual-slot case: Combat Knife). Single-slot items and the slotless muted `-` unchanged.
2. Two-handed interaction (defensive — no seed case exists): the `(two-handed)` word appends once, after the full joined label — a hypothetical dual-slot two-hander reads `Main hand/Off hand (two-handed)`.
3. Scope: listing tables only, via the one shared `_slot_cell` in `consumers.py` (`inv` inventory table and vendor `list` inherit together). The paper-doll, examine's `Equipped:` row, and all mechanics are untouched. Runtime code only — no models, no migrations, no seed data.
4. Phase note: unphased ride-along in the V24 major (display composition, not itemization structure).

GDD §6.11 text lands with this release on `version_24_8` (marker carried until implementation lands). Brief: `Shyland_V24.8_Brief_1_Dual_Slot_Cell.md`. `triaged` applied in the same motion as this ruling — complete diagnosis (in the body, from the v24.7 filing) plus this ruling make it cold-start-ready.
