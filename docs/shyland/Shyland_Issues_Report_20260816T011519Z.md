...
## Issue #258: Sanctioned read-only daemon inspection for production (docker system df & co) — the #248-shaped gap on the daemon face
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/258

### Body

## The gap (spun out of #257's third ruling scope)

There is no **codified** read-only path for inspecting the production *daemon* — `docker system df`, `docker version`, image/cache listings. `make verify-prod` (#249) covers the ORM face (brief-shipped Django `VerificationCommand`s), but a daemon disk query is not a management command, and the posture rules rightly block `make shell`-style ambient access. #257 hit this concretely: the V24.31 closeout could not confirm whether prod's 0B builder prune meant "under the cap" or "silent failure."

## Working precedent from the 2026-08-15 ops session

Operator-directed, command-scoped queries worked cleanly and touched no posture state:

```
DOCKER_HOST=ssh://ec2-user@games.magrathea.com docker system df
```

- `.env` never changes; no make target runs; `crosscheck-env` is not in play (it guards make targets, and no ambient `DOCKER_HOST` persists past the one command).
- This resolved #257's open question in minutes (cache was 3.2GB under the 5GB reserve — prune working as coded).

## The ruling to make

Pick the shape for codification (next Instructions edition when ruled):

1. **Codify the precedent:** operator-directed, command-scoped, read-only daemon queries (`system df`, `version`, `ps`, `image ls`, `buildx du`-class) are sanctioned in ops sessions — no target needed, the operator's in-conversation direction is the gate. Cheapest; matches how it actually ran.
2. **A `df-prod`-style Make target:** the sibling posture contract minus the posture flip (a daemon query reads no `.env`), pinned `DOCKER_HOST`, fixed read-only command. More ceremony, more discoverable, guard-hookable.
3. **Both:** the target for the routine case (closeout tails recording sweep results), the codified precedent for ad-hoc diagnosis.

Not urgent: the sweep is now loud-on-failure (#257 fix), so the "0B — real or broken?" ambiguity that motivated this is largely gone; what remains is the general diagnosis capability.
