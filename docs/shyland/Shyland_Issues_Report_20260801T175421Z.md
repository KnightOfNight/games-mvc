...
| 164 | Healing economics: draught dependence outruns player income — sell values and money drops can't fund the staple | KnightOfNight | triaged, V24, healing-economy | Version 24.2 | 2026-08-01 |
...
| 181 | Loot-table enrichment: healing draughts (and other items) as NPC drops | KnightOfNight | triaged, V24, healing-economy | Version 24.2 | 2026-08-01 |
| 182 | Map changes/fixes (collecting issue) | KnightOfNight |  |  | 2026-07-31 |
...
- Milestone: Version 24.2
- Assignees: KnightOfNight
- Created: 2026-07-29 | Updated: 2026-08-01
- Blocked by: #180, #181
- Blocks: none
...
**Design ruling (2026-08-01, V24.1 design session — operator-confirmed). The income table is ruled, six points:**

Derived from the #180 fight-cost survey (`Shyland_V24.1_Fight_Cost_Survey.md`, §3.1) plus a full faucet audit of the seed (copper drops, loot tables, `get_sale_price` = value ÷ 3, item base values) at `version_24_1`.

**Audit headline:** bosses (k ≈ 2.3–11.8) and villagers (k ≈ 2.4–27) **already satisfy the k = 2 law**; the entire structural deficit is the aggressive grind population — every animal and insect pays ~0.7 cp/kill against targets of 7.2 (normal) / 26.2 (elite), a 10–37× shortfall. Side effect fixed by the same table: villager-killing is currently the game's mathematically optimal income (k ≈ 21–27 at Reedmere).

1. **The income law is KEPT at k = 2** (the fix-or-keep review, now with data): expected income ≈ 2 × expected draught cost, calibrated **per kill at solo cost**, with k degrading gracefully in multi-aggro rooms (drops pay linearly, grouped fights compound superlinearly). The signposted ×3 rooms (Lion's Watch, Bear's Throne) run k ≈ 0.8–1.0 — deliberately break-even prestige fights, not profit centers.

2. **Looted draughts are valued at 15 cp** (avoided purchase) in the income equation. Sensitivity floor: at sale value (5 cp) the table still clears break-even — a player who never needs healing profits on materials alone.

3. **Boss and villager copper numbers are UNCHANGED** — the pre-data "numbers go up" sketch is reversed on the measurement. (A boss jackpot bump stays available as pure feel, not need.)

4. **Trivial carve-out:** the seven trivial passives (river otter, black bear, young mountain lion, plains deer, plains rabbit, prairie dog, mountain squirrel) keep the current draught-free `animal-drops` table — no 1-minute-respawn rabbit-farm faucet.

5. **The enrichment table** (design shape: *draughts-in-kind cover roughly the consumption rate; materials are the profit* — keeps the vendor sink alive, in-kind supply runs below consumption):

| Table | Applies to | Draught | Materials | E[income]/kill | k solo |
|---|---|---|---|---|---|
| `animal-drops` (unchanged) | 7 trivial passives | none | hide 0.35 | ~1.4 | harmless |
| enriched normal (animal + insect variants) | combat normals incl. Verdant boss adds | 0.35 | common material 0.5, base → 12 (sells 4) | ~7.3 | ~1.6–2.0 |
| elite tables (animal + insect variants) | all 12 elites incl. delve adds | guaranteed (1.0) | new elite material base 36 (sells 12) + common material 0.5 | ~29 | 2.2 (1.6 room-blended) |

Supporting numbers: common materials hide 6 → 12, carapace 8 → 12 (both sell 4; global — trivials drift 0.7 → 1.4 cp, harmless). Two **new elite material definitions** (names are creative content, authored at brief time), base 36. New per-tier loot tables are required regardless (tables are shared across tiers today). Aggregate sanity check (20 normals + 6 elites + 1 boss): income ≈ 460 cp vs ~18 draughts consumed (270 cp) → k ≈ 1.7; in-kind supplies ~13 of 18 draughts, the rest purchased — deficit gone, sink alive. Mk invariance: draughts-per-fight is level-stable (Draught Law) and material values scale base × Mk, so the table's ratios survive into Mk 2 zones — Phase 3 inherits a shape, not stale constants.

6. **Insects may yield draughts** (the #181 per-table lore call, sanctioned): the delve economy requires it — elites are nearly all insects; lore cover authorable at brief time (devoured travelers' effects among the remains).

**Queue ruling:** this issue **founds Version 24.2**, with #181 as its dependency (same problem, loot-in-kind side; one brief ships both). The brief and the GDD text land with V24.2 — written by V24.2's design session after the V24.1 closeout. Per-table drop design detail is recorded on #181.


**KnightOfNight** — 2026-08-01:
**Design ruling (2026-08-01, V24.2 design session — operator-confirmed): the V24.2 plan is confirmed in full.**

GDD text landed on `version_24_2` as new §6.15 (The Healing Economy — the Income Law & Loot-in-Kind, `(v24.2, pending implementation)` marker) plus the §2 Loot & Drops bullet update. The implementation brief is committed as `Shyland_V24.2_Brief_1_Healing_Economy_Income_Table.md` on the same branch.

Confirmed refinements riding this release (operator-confirmed in-session):

1. **`healing-draught.base_value` 12 → 15, seed-owned.** The V24.0 closeout recorded the live value as 12 and unowned by the seed; the brief authors 15 into the seed so ruling point 2's arithmetic (15 cp avoided purchase, 5 cp sale) and the vendor price standard agree. Buy price unchanged at 15.
2. **Money is untouched, absolutely:** no copper changes anywhere (bosses 50–150/150–400/400–1000 cp, villagers 2–24 cp as today; beasts and insects stay at zero forever). The deficit closes entirely through loot-in-kind — operator re-confirmed this explicitly.
3. **Architecture doc hash moves** at 24.2 (seed logic + self-checks are code under `django/src/`).

Milestone `Version 24.2` holds exactly #164 (founding) + #181 (dependency). One brief ships both.
...
**KnightOfNight** — 2026-08-01:
**Design ruling (2026-08-01, V24.1 design session — operator-confirmed). The per-table drop design is ruled** (the loot-in-kind half of the #164 income table; full derivation and the six-point ruling recorded on #164):

**Design shape:** draughts-in-kind cover roughly the consumption rate; materials are the profit. In-kind supply deliberately runs below total consumption so the vendor sink stays alive.

**Per-table design:**

- **`animal-drops` — unchanged.** Stays draught-free at hide 0.35 and keeps exactly the seven trivial passives: river otter, black bear, young mountain lion, plains deer, plains rabbit, prairie dog, mountain squirrel (the carve-out — no rabbit-farm faucet).
- **Enriched normal tables** (animal and insect variants; new tables — today's tables are shared across tiers): all combat normals, including the Verdant boss adds (Matron's brood, Whistler's young, Dronemother's swarm). Healing Draught at **0.35**; common material (hide/carapace per lore) at **0.5**.
- **Elite tables** (animal and insect variants): all 12 elites, including the delve adds (Weaver's brood, King's skitterlings, Devourer's drones). Healing Draught **guaranteed (1.0)**; one **new elite material** guaranteed (base 36, sells 12); common material at 0.5.

**Values:** common materials rebased — animal-hide 6 → 12, insect-carapace 8 → 12 (both sell 4). Two new elite material definitions (one animal, one insect; names are creative content, authored at brief time), base 36 (sell 12). Drop entries remain Mk 1 in the Mk 1 band; material base × Mk scaling keeps the table's ratios valid for Mk 2 zones.

**Accounting (per #164 point 2):** a looted draught counts as 15 cp income (avoided purchase). Expected income per kill: combat normal ≈ 7.3 cp (k ≈ 1.6–2.0 solo), elite ≈ 29 cp (k ≈ 2.2 solo / 1.6 room-blended).

**Lore call (sanctioned):** insects yield draughts — the delve economy requires it (elites are nearly all insects); lore cover authorable at brief time (devoured travelers' effects among the remains). Bosses' and villagers' copper and gear tables are untouched — the audit shows both already at or above k = 2.

**Ships with:** Version 24.2, as a dependency of founding ticket #164 — one brief covers both. GDD text lands with that release.


**KnightOfNight** — 2026-08-01:
**Design ruling (2026-08-01, V24.2 design session — operator-confirmed): per-table design finalized for the V24.2 brief** (`Shyland_V24.2_Brief_1_Healing_Economy_Income_Table.md` on `version_24_2`; GDD §6.15 carries the design text).

Implementation-level points confirmed:

- **New table slugs:** `combat-animal-drops`, `combat-insect-drops`, `elite-animal-drops`, `elite-insect-drops` (all entries Mk 1–1, independent rolls, common 100).
- **`mountain-goat`** is passive but outside the exact seven-passive carve-out → it moves to `combat-animal-drops` (its lone user).
- **`insect-drops` is deleted** — zero users remain after the 22-NPC re-pointing (9 → combat-insect, 6 → elite-animal, 6 → elite-insect, 1 → combat-animal). Expected reseed deletions: 2 rows (table + its single entry).
- **Elite material names (creative content):** Pristine Animal Pelt (`pristine-animal-pelt`), Hardened Insect Chitin (`hardened-insect-chitin`) — base 36, sell 12.
- Commons rebased: `animal-hide` 6 → 12, `insect-carapace` 8 → 12 (both sell 4; both seed sites).
- Villager gear tables, all six boss tables (incl. their carapace 0.5 side entries), and every copper range are untouched.
