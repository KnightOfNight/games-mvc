...
| 266 | MC kill switch: a single lever that silences every AI actor at once — ships no later than the first live actor (HIGH PRIORITY) | KnightOfNight | monitoring-and-command, V25 | Version 25.4 | 2026-08-21 |
...
| 277 | MC persister: XREADGROUP BLOCK 5000 races redis-py 8.1 default socket_timeout=5s — idle-cycle reconnect churn | KnightOfNight | bug, monitoring-and-command | Version 25.4 | 2026-08-21 |
...
- Author: KnightOfNight
- Labels: monitoring-and-command, V25
- Milestone: Version 25.4
- Assignees: KnightOfNight
- Created: 2026-08-16 | Updated: 2026-08-21
- Blocked by: none
- Blocks: none
...
**Design rulings (2026-08-20, V25.4 design session) — the kill switch, ruled whole.** Founding ticket of Version 25.4 (milestone set); #277 rides the same release as ruled on that issue.

1. **Mechanism: a database singleton, read fresh at every enforcement point.** New minimal model (`MCKillSwitch` shape: singleton row, `killed` boolean, `flipped_at`, `flipped_by` as a loose name string — no FK, config not history). Postgres is the only store that survives restarts and reseeds (Redis has no volume — a Redis-key switch would silently reset to "alive" on restart, disqualifying). Readable from django and ticker trivially via the ORM. No cache: every enforcement point is a cold path (egress loop wake ≤2s per agent, connects, future per-action ingress checks), so fresh reads cost nothing and there is no cross-process invalidation story.
2. **Fail posture: fail closed.** Any error reading the switch = killed — egress refuses/severs, actuation (future) refuses. The game degrading to "no AI" is the shipped game. Capture is deliberately NOT fail-closed: it stays fire-and-forget and never checks the switch.
3. **What flipping kills (25.4 concrete surface):** (a) egress new connections — attach gate checks the switch after the `agents.shyland` check; accept-then-close with a distinct close code (killed ≠ not-authorized, e.g. 4503 vs the existing 4403); (b) egress live connections — the stream loop checks per wake, severing hung/rogue agents within ~2s (`LIVE_BLOCK_MS`) without their cooperation; (c) ingress — design-ahead doctrine only in 25.4: #261's actuation path checks the switch before honoring anything, binding on that design. Untouched: capture (both taps), the persister (monitoring infrastructure inside the trust boundary, not an actor), and everything players do.
4. **Three flip surfaces (operator-ruled, third added):** the admin command — verb **`mc`**, `admins.shyland`-gated and stealth-gated like `sudo`/`last`, subcommands `mc status` / `mc kill` / `mc restore`; a **documented shell helper** (one-liner via `make shell`, no game code in the path); and the **Django admin site** — the switch model registers editable in `admin.py`. Doctrine line drawn: §10.11's read-only admin posture governs MC *records* (append-only truth); the switch is *config*, not history — editable by design, no erosion.
5. **Legibility: every flip is an MC event.** Dedicated record kind (`mc_kill`, payload: new state, who, which surface) emitted through one shared choke point all three surfaces route through — the record shows when the world's AI went quiet and who did it, regardless of flip surface (the #273 lesson: state queryable, not inferred). `mc status` reports state on demand; the distinct close code makes the killed state legible to agents.
6. **Standing invariant (doctrine text, ships with 25.4):** *no actor goes live without kill-switch integration* — every future actor brief (#262, #263, #265, #259) states where its actuation checks the switch and what its degraded behavior is. Per-actor degraded behavior belongs to each actor's own design (silence is in-character for the watcher and responders; the pet-mid-combat hard case is #263's to answer at its session).

GDD text lands in §10.11 (plus the §9 chart row for `mc`) with the pending-implementation marker; the implementation brief follows on `version_25_4`.


**KnightOfNight** — 2026-08-21:
Brief committed: `docs/shyland/Shyland_V25.4_Brief_1_MC_Kill_Switch.md` at `7fa74c7` on `version_25_4` (GDD text at `b376a8c`). Covers #266 + #277; actionable when the operator directs an implementation session to it by name.
...
## Issue #277: MC persister: XREADGROUP BLOCK 5000 races redis-py 8.1 default socket_timeout=5s — idle-cycle reconnect churn

- State: open
- Author: KnightOfNight
- Labels: bug, monitoring-and-command
- Milestone: Version 25.4
- Assignees: KnightOfNight
- Created: 2026-08-20 | Updated: 2026-08-21
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/277

### Body

## Summary

`run_mc_persister`'s `XREADGROUP ... BLOCK 5000` races redis-py's client-side socket read timeout every idle cycle: **redis-py 8.1.0 — what the image's `redis>=5.0` pin now resolves to — defaults `socket_timeout` to 5 seconds**, the exact length of the server-side block. When the stream is idle, the server's "nothing new" nil reply at 5.000s and the client's read cap at ~5.000s finish in a photo-finish; each lost race raises `TimeoutError`, the persister logs `mc persister: redis unavailable (Timeout reading from socket); retrying`, tears down the connection, backs off, and reconnects.

**No data loss** — the loop catches `RedisError`, XREADGROUP redelivers unacked entries, acks only after commit (by design, v25.1). The cost is reconnect churn and log noise during idle periods: **107 timeout lines on the dev stack** as of filing. Prod's 25.2 image carries the same redis-py 8.1, so its quiet hours churn the same way; busy hours mostly don't (entries arrive before the block expires).

## Provenance

Found 2026-08-20 during the V25.3 Brief 1 operator playtest: the MC egress consumer had the identical 5s/5s race and its live tail died mid-playtest (~18 idle cycles in). Fixed for the egress in `b812afb` (`LIVE_BLOCK_MS` 5000 → 2000); the persister was explicitly outside that brief's scope (untouched by rule) and is filed here instead. Diagnosis details in `docs/shyland/Shyland_V25.3_Brief_1_Closeout.txt` (deviation 5 + related-finding block) and on #267.

## Suggested fix

Mirror b812afb: `BLOCK_MS` 5000 → 2000 in `django/src/apps/shyland/management/commands/run_mc_persister.py` — a server-side block comfortably inside the 5s client cap leaves no race window. One-line change plus whatever test pins the constant's relationship, if ruled worth pinning. (Alternative considered and not preferred: setting `socket_timeout` explicitly on the persister's client — touches connection behavior for all its commands, not just the blocking read.)

## Priority note

Operator directive at filing (2026-08-20, in the V25.3 Brief 1 implementation session): **"make sure we get to it soon"** — flagged for early pickup by the next design session.


### Comments (1)

**KnightOfNight** — 2026-08-21:
**Design ruling (2026-08-20, V25.4 design session): included in Version 25.4.**

The operator's at-filing directive ("make sure we get to it soon") is executed here: this fix rides the V25.4 (kill switch, #266) implementation brief and closes with the release — milestone member per the v35 litmus. Scope note: the operator explicitly ruled it into the release; it is a one-line same-subsystem fix (the MC persister), not a widening of the founding ticket.

Fix as suggested in the body: mirror `b812afb` — `BLOCK_MS` 5000 → 2000 in `run_mc_persister.py`, keeping the server-side block comfortably inside redis-py 8.1's default 5s client `socket_timeout`. The brief specifies the exact change and any constant-relationship test per the #252 technical-coherence rules.
