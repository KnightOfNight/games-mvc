...
> Authoritative technical reference as of commit 973a07d (v18 brief 5: The Verdant Reach world seed, part 1 — the zone row, 6 areas, 69 rooms across Fernwater Vale and the Sagewind Flats, the opened Verdant gate, 29 NPC definitions including three bosses and their gated minions, 7 loot tables, 57 spawns, the first two vendors and repairers, the Fordwatch and Stairhead checkpoint travel nodes, and a tick-engine respawn-timer fix).
...
Changed in v18 (brief 5): the partial world seed for **The Verdant Reach** (Z01), the first zone beyond The Convergence. All new steps follow the established conventions (coordinate-keyed `update_or_create` rooms via a registry of seed keys, two-pass exit wiring, content/balance `update_or_create` split for NPCs, `get_or_create` for spawn/loot/vendor/node rows) and the whole command remains idempotent — verified by double-run.
...
- **Verification pass extended** with 18 Verdant checks: zone/area/room counts, every VR edge bidirectional in the DB (including the gate), the F18 pending exit + message, exactly two safe rooms, 19 indoor cave rooms, the 29-definition roster, the passive/aggressive split, boss tier + death messages, repairer flags, 57 spawns, the three gated minion spawns, loot-table entry and guaranteed-group counts, pool message counts, and both vendors' entry counts. The brief 2 TravelNode check now expects 3 nodes.

Brief 6 seeds the remaining 81 rooms (Viridian Ridge, three delves, the Verdant Crown) and wires `vr-f18` north.
...
- Vendor and repairer NPCs in The Convergence (brief 5 shipped the first reachable services — Essa/Sona vend and Maro/Tavik repair at the Verdant Reach checkpoints — but no Convergence NPC has `VendorEntry` rows or `is_repairer` set)
- Vendor restock (`VendorEntry.sold_count` exhaustion is permanent until an admin intervenes; moot for now — both live vendors have unlimited stock)
- Obelisk Network obelisks beyond The Convergence (brief 5 added the Fordwatch and Stairhead **checkpoint** nodes — destinations only; the Heart remains the network's only travel source until a zone-end obelisk is seeded)
...
- Per-NPC unarmed message pools for Convergence NPCs (brief 5's cave insects, bosses, and minions carry the six species pools; all other NPCs fall back to the default pool)
...
- The second half of The Verdant Reach — Viridian Ridge, the three delves, and the Verdant Crown (brief 6; `vr-f18`'s north exit is reserved for it) — and all zones beyond Z01
