...
**Epic accessories can only roll 2 secondary stats, not the GDD's 3.** GDD §"Primary and Secondary Stats" says Epic rarity carries 3 secondary stats, and the brief 6 verification expected the Devourer's guaranteed Epic accessory to roll 3 — but the brief 1 copper accessories were authored with **2-entry** `secondary_stat_pool`s (primary stat + two themed secondaries), and `generate_item_instance` draws without replacement capped at pool size. An Epic copper accessory therefore rolls its full pool of 2 (which does put 3 stat lines total on the item, counting the primary — likely what the design intended to count). Verified at ship time and left as-is: enlarging the pools is a balance/design decision for the design chat, flagged for v18 closeout.
