...
| 262 | sudo AI watcher: an AI agent on the receiving end of sudo — reads the MC stream, and may or may not be allowed to answer | KnightOfNight | monitoring-and-command, V25 | Version 25.6 | 2026-08-23 |
...
| 279 | MC test-agent client: operator-side script that connects to the egress and tails the event stream (CLI: URL, username, password) | KnightOfNight | monitoring-and-command | Version 25.6 | 2026-08-23 |
...
| 284 | nginx login rate limit (zone=login 5r/m, burst=3) blocks agent-testing and bot reconnect login patterns — proposal: burst=10, rate unchanged | KnightOfNight |  | Version 25.6 | 2026-08-23 |
...
**KnightOfNight** — 2026-08-22:
**Operator ruling (2026-08-22, V25.5 design session) — sudo ships whole:**

A voice-only first slice (answers in the pane, fulfillment deferred to a later #261 slice) was proposed and **denied**. When the sudo bot ships, it ships **fully working — voice and fulfillment per the 2026-08-17 ruling — not half-assed**, and it doubles as the proof of concept for the whole future bot/agent family.

Release-shape consequence (not yet ruled, this session will settle it): the work may split into **25.5 = the complete agent/actuation infrastructure** and **25.6 = the fully working sudo bot**, rather than one overstuffed release. If that split is ruled, the founding-ticket/milestone assignments will be updated to match (this issue is currently the `Version 25.5` founding ticket).

Companion ruling recorded on #261 the same motion: agents never use the player door; the agent surface (MC endpoint family) is their front door for read and actuation; projection into the world must be lore-consistent.


**KnightOfNight** — 2026-08-22:
**Operator rulings (2026-08-22, V25.5 design session) — sudo's scope and day-one action vocabulary:**

General rules:

1. **sudo never replaces existing commands.** A request that maps to an existing admin command draws *"you don't need sudo for that"* (operator's example: `sudo mc kill`).
2. **Tier 3 (ops actions) is excluded by design, not deferred.** sudo will not prune disk, restart containers, or hold any permission outside the game world. **All agents/bots are internal-game actors only** — standing rule for the whole family.
3. **The sudo/Sirius spectrum, stated:** sudo does anything an admin says (within the ruled definition of "anything"); Sirius (#259) answers anyone but acts as his own personality and rules determine. Both strictly in-game activity.
4. **Knowledge (Tier 1) is in:** sudo answers game-state questions, reading from the MC stream or from the database (mediation through the agent surface — trust-boundary discussion in session; Redis-held presence under discussion as the third source).
5. **Item powers (Tier 2):**
   - **Artifact creation:** if the request carries the full spec (recipient, stats/bonuses, Mk level, material/name, everything needed) sudo creates and gifts it; if not, **sudo starts a Q&A session about the design** (who it's for, the level, the stats, any special lore, …) and gifts when complete.
   - **Generic gifting:** the item must already exist as a definition in the database — sudo never invents a new generic item kind that anyone can have.
   - Settled law applies unchanged: gifts become immediately soulbound to the recipient.

Day-one request examples (operator's own words, lightly normalized):

- `sudo give <player> an artifact, it will be a sword with these stats and bonuses, this mark level, this metal name, …` → create + gift
- `sudo let's talk about a new artifact, a chest piece` → Q&A, then gift
- `sudo strip <player>` → unequip everything to inventory (dev-stack testing helper — "nerf myself and make it possible for me to die")
- `sudo dress <player>` → restore the stripped items
- `sudo where is <player>`
- `sudo move <playerA> to <playerB>` / to <playerB>'s room
- `sudo wall <message>` → *"you don't need sudo for that"*

Sub-questions surfaced in-session, not yet ruled: the `wall` decline presumes #236 ships as a real admin command (sequencing question); `dress` needs an exact-restore mechanism (game-side snapshot at strip time is the session's working proposal); the artifact Q&A makes the sudo channel a multi-turn dialogue (conversation-state custody and failure posture to be ruled).


**KnightOfNight** — 2026-08-22:
**Operator rulings (2026-08-22, V25.5 design session) — decline model, outfit storage, bot persistence:**

1. **Command-knowledge decline model — no special conditionals for future commands.** The bot holds a list of the game's valid commands. A request that maps to an existing command draws "you don't need sudo for that"; a request the bot doesn't know how to fulfill draws the general handler's "I don't know how to do that" (today's `wall` case). When a command ships (e.g. #236 `wall`), the bot's list updates and the decline upgrades naturally — no per-command conditional ever written. Session proposal riding this (unruled detail): the bot fetches the live verb list through the agent query surface at attach, so the list is self-updating by construction.
2. **Outfit storage (strip/dress):** a player's 'outfit' gets stored as a JSON blob game-side for now — acknowledged as a design in its own right, deliberately minimal in this arc; the need is dev-stack testing. Operator accepts the #275 over-capacity state knowingly ("OK with the over-capacity bug for now that I'm aware of it") — strip producing it is the tool working as intended.
3. **Conversation and history live in the bot — nothing in the database, Redis, or the client.** The bot gets its own persistence layer for history: a structure reusable by other bots, bounded (never grows forever); a local JSON log file is an acceptable first version.


**KnightOfNight** — 2026-08-22:
**Operator ruling (2026-08-22, V25.5 design session) — bot rendering colors:**

General principle, operator's words: **bots have a talking color; when they do something, the effect lines use the standard game colors for that thing.**

Talking colors ruled so far:

- **sudo: error-color** (`#E24B4A`).
- **Sirius (#259): rare color** (`#B387E8` — the Rare rarity violet).

Effect lines, sudo examples:

- **Move-related: value-color** — e.g. `Harley Stone was moved to xyz.` / `Gunny was moved to Harley's location.` / (to the moved player) `An admin moved you to a new room.`
- **Gift-related: the actual giving line is loot-color.**

Session notes on mechanics (implementation shape, for the brief): the palette's own precedent is the v22 B2 Amendment 1/2 pattern — **new separable names** (`--sudo-color`, later `--sirius-color`) deliberately reusing existing hexes, so the chart-as-license set stays clean and any voice can diverge later without a hunt. Attribution is word-carried (`sudo:` prefix) per the words-never-color-alone law, so a shared hex never carries the meaning alone. Effect lines being ordinary world categories means agent-caused events render indistinguishably from any other world event — the pane shows the world; who-caused-it legibility lives in the MC stream record (#261 observability).

(Cross-noted on #259: Sirius' talking color = rare violet.)


**KnightOfNight** — 2026-08-22:
**Operator ruling (2026-08-22, V25.5 design session) — the carve is confirmed:**

- **V25.5 = the agent door** (game-side actuation infrastructure) — new founding ticket **#281**, which now holds the `Version 25.5` milestone. Operator: "the split is easy to see now — we get all the game engine done, then we have no blockers for the first bot."
- **V25.6 = the sudo bot** — this issue is the **founding-ticket-designate for Version 25.6** (milestone born with that release's design session, per the release model). The bot release focuses on bot code and bot infrastructure — start/stop, runtime, the likely repo move for agent code (#268 point of order, 2026-08-22).

This issue accordingly **leaves the `Version 25.5` milestone** in the same motion.

Also confirmed this round (recorded on #261): the identity model's two numbered points, the #273 resolution, and bot display names joining the player/NPC name invariant.


**KnightOfNight** — 2026-08-23:
**Operator rulings (2026-08-23, V25.6 design session) — the sudo bot's brain, model, and v1 rate posture:**

1. **Brain = Claude via the Anthropic API.** sudo v1 uses Claude for all human-language interaction and parsing, called from the bot process via the official `anthropic` Python SDK. **Future versions will use Ollama** (locally hosted models). Consequence, required in v1: the model client sits behind a **provider-agnostic "brain" interface** (parse request → door calls → compose reply) so Claude→Ollama is a configuration swap, not a rewrite — both providers expose the same chat-plus-tool-calling shape. This is the standing pattern for the whole bot family (Sirius, NPC responders): AI for language, never for world effects.

2. **Natural-language parsing rides tool use.** The agent door's vocabulary (the v25.5 queries and actions) is presented to the model as tools; the model parses the admin's natural language and emits structured door calls; the bot validates and executes them over its WebSocket to the door. The model never touches the game directly — composing with the door's deterministic end-to-end law (#281) and the command-knowledge decline model already ruled here.

3. **Model: sudo v1 starts with Sonnet 5** (`claude-sonnet-5`). The model ID is a per-bot configuration value, never hardcoded through the brain interface.

4. **v1 rate/cost posture — audience-bounded, not mechanism-bounded.** sudo's rate limit *is* its audience: the `admins.shyland` membership is the only population that can reach it, so v1 ships no additional technical rate limiter. For observability, the bot logs each request's token usage from the API's per-response `usage` stats into its own log/history. Harder knobs (per-request `max_tokens` caps, workspace spend limits/alerts in the Anthropic Console) are noted as available but deliberately not shipped in v1. Under a future Ollama provider, metered cost disappears entirely. Sirius' rate limiting is explicitly deferred to Sirius' own design (recorded on #259).


**KnightOfNight** — 2026-08-23:
**Operator ruling (2026-08-23, V25.6 design session) — multi-turn Q&A failure posture:**

Conversations **time out quietly**. An artifact-design Q&A (or any multi-turn exchange) abandoned mid-design simply expires after a bot-side timeout — no world effect, no error, no nag; the next `sudo` from that admin starts fresh. This closes the conversation-custody question the 2026-08-22 ruling left open (history bot-side, bounded) and composes with the standing posture that silence is always legal for sudo — an expired conversation is indistinguishable from the bot never having answered. Timeout duration is a bot configuration value chosen at brief time.


**KnightOfNight** — 2026-08-23:
**Brief committed (2026-08-23, V25.6 design session):** `docs/shyland/Shyland_V25.6_Brief_1_The_Sudo_Bot.md` is on `version_25_6` at dbe5ad9 — the release's one implementation brief per the scope law. Design-ahead GDD text landed the same session (3bee92f: §9.1 silence-reversal rewrite, §10.11 "The first live actor," §12 clause — all marked v25.6 pending implementation), and the `agents/` in-repo home was scaffolded at 9f64cf5. Every structural claim in the brief was verified against the code at writing time (#252); the Verified-Claims Ledger is §12 of the brief. Actionable when an implementation session is directed to it by name on `version_25_6`.
...
## Issue #279: MC test-agent client: operator-side script that connects to the egress and tails the event stream (CLI: URL, username, password)
...
- Milestone: Version 25.6
- Assignees: KnightOfNight
- Created: 2026-08-21 | Updated: 2026-08-23
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/279

### Body

**Need:** a simple, standing test-agent client for the MC egress (`wss://<host>/ws/shyland/mc/`, GDD §10.11). The 25.3 and 25.4 playtests each needed an agent client to exercise the endpoint (attach flow, live tail, kill-switch close codes), and the objective half of verification has been using ad-hoc in-container throwaway scripts — there is no operator-side client for the nginx/WSS half of the coverage.

**Spec (as built for the V25.4 Brief 1 playtest, 2026-08-21):**

- Plain Python script, everything via command-line arguments: base URL, username, password; optional `--after <stream-id>` (resume replay), `--insecure` (self-signed dev certs).
- Auth = the ordinary Django session: GET `/accounts/login/` for the CSRF token, POST credentials, carry the session cookie into the WebSocket handshake.
- Speaks the §10.11 wire protocol read-only vocabulary: prints `hello`, sends one `attach` (bare or `after`), prints every `event`/`gap` frame as it arrives.
- Reports close codes with their meaning — 4403 not authorized, 4503 killed (kill switch engaged) — since those codes are exactly what playtests need to observe.
- Dependencies: `requests` + `websockets` (operator-installed; nothing added to the repo's requirements).

**Status:** the script exists as an operator-side tool only — deliberately **not** checked into the repo and not part of any release (operator ruling in the V25.4 Brief 1 session). This issue records the tool and its spec so a future session can recreate or formalize it (e.g., as a committed `scripts/` utility or the seed of the 25.5 actor work) if ever ruled worthwhile.


### Comments (3)

**KnightOfNight** — 2026-08-21:
The definitive test harness now lives in `~/src/games-mvc-agents/` on the operator dev machine (operator, 2026-08-21): `mc_test_agent.py` plus `venvs/mc-agent` (requests + websockets). Still deliberately outside the repo — this directory is the standing home for agent-side tooling.

**KnightOfNight** — 2026-08-22:
"Point of order" recorded on #268 (2026-08-22, V25.5 design session): ~/src/games-mvc-agents/ is a temporary home — agent tooling moves into the repo around when the sudo bot ships. The "deliberately not checked in" status above was a mid-release convenience, not standing doctrine.

**KnightOfNight** — 2026-08-23:
**Formalized in-repo (operator-directed 2026-08-23, V25.6 design session):** `mc_test_agent.py` now lives at `agents/mc_test_agent.py` on `version_25_6` (9f64cf5), alongside `mc_door_agent.py`, a `requirements.txt` for the deps this issue's spec named, and a `.gitignore` covering `.secrets/` and `venvs/`. The "deliberately not checked in" status is fully superseded, per the #268 point of order. This issue **joins the `Version 25.6` milestone** as a dependency — it closes with the release that ships the in-repo home.
...
## Issue #284: nginx login rate limit (zone=login 5r/m, burst=3) blocks agent-testing and bot reconnect login patterns — proposal: burst=10, rate unchanged
...
- Milestone: Version 25.6
- Assignees: KnightOfNight
- Created: 2026-08-22 | Updated: 2026-08-23
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/284

### Body

## What happened (V25.5 Brief 1 playtest, 2026-08-22)

During agent-door playtesting, repeated runs of the operator-side door harness tripped the nginx rate limit on the login endpoint — the operator was blocked at login on the agent account after a few quick script reruns (each run costs **two** requests to `/accounts/login/`: the CSRF GET, then the credential POST). Kill-switch testing makes this routine: every reconnect after a 4503 sever is a fresh login.

**No change was made** (operator ruling at filing: change nothing now, file the issue). This ticket is the record and the proposal.

## Current configuration

`nginx/conf/rate-limit.conf` (landed at 385d830, the brute-force-protection commit):
...
`nginx/conf/default.conf.template:27-28`:
...
Net effect: keyed per client IP, ~3 requests back-to-back, then one every 12 seconds. Two script runs in quick succession exhaust it.

Behind nginx, **django-defender** is the second, independent layer (`base.py:132-137`): `DEFENDER_LOGIN_FAILURE_LIMIT = 5`, `DEFENDER_COOLOFF_TIME = 300`, keyed per username+IP, Redis db 1 — and it counts **failures only**. Successful-login frequency is limited solely by the nginx zone.

## Proposal (from the in-session discussion)

Raise the burst, keep the rate: `burst=3` → `burst=10` in `default.conf.template`. Absorbs a testing session or a bot reconnect loop (~5 back-to-back logins) while preserving the sustained 5r/m ceiling and defender's account lockout untouched. Raising the 5r/m rate itself was considered and recommended against — that is the actual brute-force protection.

## Interactions and notes

- **Shared surface:** `nginx/conf/` fronts all three games (CLAUDE.md Rule 2) — whichever release carries this needs the explicit shared-surface acknowledgment, and the change reaches production with that release's deploy (ordinary container bounce, no extra step).
- **The longer-term fix is bot-side, not server-side:** an agent runtime should log in once and reuse its session cookie across WebSocket reconnects instead of re-authenticating per connect — a note for the 25.6 sudo-bot runtime and the #268 operations arc. With cookie reuse, even burst=3 is generous in production; the tune mainly serves operator-side testing ergonomics.
- Related observation from the same playtest: #282 (answer delivery gate per-agent context). The kill-switch idle-connection observation may also get its own ticket.


### Comments (1)

**KnightOfNight** — 2026-08-23:
**Operator ruling (2026-08-23, V25.6 design session): #284 rides Version 25.6.** The burst tune (`burst=3` → `burst=10` in `nginx/conf/default.conf.template`, rate unchanged at 5r/m) ships with the sudo-bot release because it directly affects agent testing — the bot's own development and playtest loop hits this limit. **Shared surface explicitly acknowledged by the operator** (nginx fronts all three games, CLAUDE.md Rule 2); the change reaches production with the ordinary 25.6 deploy bounce. The bot-side mitigation stands as designed: the sudo bot logs in once and reuses its session cookie across reconnects. This issue **joins the `Version 25.6` milestone** as a dependency and closes with the release.
