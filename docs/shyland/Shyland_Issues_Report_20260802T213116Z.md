...
| 127 | Ranged proc damage ("between X and Y") — new weapon kind, midpoint-and-spread family | KnightOfNight | V24, itemization |  | 2026-08-02 |
| 129 | Authored per-item armor base — guaranteed minimum coverage under rolled resist | KnightOfNight | triaged, V24, itemization | Version 24.9 | 2026-08-02 |
...
**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.9 design session): **#129 is the founding ticket of Version 24.9** — milestone `Version 24.9` created and assigned in this motion.

Phase 2 (itemization structure) release-by-release queue order ruled: **#129 → #127 → #80 → #134** — each its own release per the scope law (one founding ticket, one brief).

Pairing note: #127 and #129 are a design pair — both restructure an item stat into an authored guarantee plus a roll above it (offense/defense sides of one structural family; #129's body already names the kinship) — but they remain separate tickets and separate releases: no shared code path, and each closes with its own release. The pair's shared doctrine is ruled in this design session and cross-recorded on #127.

Detailed design ruling, GDD text, and the Version 24.9 brief follow in this session.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.9 design session — operator-confirmed): **the #129 design is ruled.** This is Version 24.9's founding ticket; the release ships it.

**The shared doctrine (the #127/#129 pair, ruled once, cross-recorded on #127):** an item stat of consequence is an **authored guarantee plus a roll above it** — the definition authors the floor the designer promises; the drop-time roll adds variance strictly on top. Defense side = this issue; offense side = #127.

**The ruling, point by point:**

1. **Field:** `armor_base` on `ItemDefinition` (float, default 0). Base armor contribution = `armor_base × Mk tier` — the authored number replaces the *slot weight*, not the shape of the math (the graceful retirement promised in GDD §3.6).
2. **TAV** = Σ(`armor_base` × Mk over equipped, non-broken items) + Σ(rolled `physical_resist` over all equipped items). The hardcoded slot-weight table (CHEST 3 / HEAD 2 / LEGS 2 / OFF_HAND 2 / SHOULDERS 1 / HANDS 1 / WAIST 1 / FEET 1) is deleted.
3. **Equivalence at switchover — 24.9 is structure, not balance:** every seeded armor item's `armor_base` = its current slot weight. TAV numerically identical before and after; K=48 stays blessed; the v22 armored re-bless survives. Same-slot differentiation is deliberately deferred to Phase 3 (Mk 2 balance).
4. **Scope:** the field exists on every ItemDefinition, seeded nonzero only on armor; TAV sums over all equipped items with no slot gate. Nothing but armor uses it today.
5. **Unchanged by construction:** mitigation curve `TAV/(TAV+K)`, K=48, NPC→player-only mitigation, both floors, broken-contributes-nothing, rarity-blind base. `examine`'s armor confession and the `stats` Armor row keep their exact shapes, sourced from the field.
6. **Migration + reseed:** one migration (new field); seed-owned armor definitions updated in place — expected deletion count **0**.

GDD §3.6 text and `Shyland_V24.9_Brief_1_Authored_Armor_Base.md` land on `version_24_9` with this session.
