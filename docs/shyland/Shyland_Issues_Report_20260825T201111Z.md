...
| 268 | MC agent runtime & operations: hosting, supervision, credentials, cost governance for the AI actor fleet | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-23 |
| 275 | Unequipping +STR gear silently strands the character over carry capacity, blocking loot/pickup/buy | KnightOfNight | bug, emergent |  | 2026-08-20 |
| 282 | Agent door: 'answer' delivery gate must be per-agent context, not verb-global — sudo answers admins only, Sirius answers anyone | KnightOfNight | monitoring-and-command |  | 2026-08-22 |
...
| 286 | GDD erratum: §2.11 "three ways out" undercounts — the shard relay makes four | KnightOfNight | errata |  | 2026-08-22 |
| 287 | Agent door: no item-removal action — sudo can gift but not take | KnightOfNight | triaged, monitoring-and-command | Version 25.7 | 2026-08-25 |
| 288 | Agent door: no targeted equip/unequip actions — strip is all-or-nothing, dress only restores the snapshot | KnightOfNight | triaged, monitoring-and-command | Version 25.7 | 2026-08-25 |
| 289 | Agent door: no mutation action — created artifacts (and items generally) cannot be edited | KnightOfNight | triaged, monitoring-and-command | Version 25.7 | 2026-08-25 |
| 290 | Agent door: no room-directory query — rooms unreachable except through characters | KnightOfNight |  |  | 2026-08-25 |
| 292 | sudo bot: trailing slash on --url breaks login (double-slash POST draws a 302, looks like a credential failure) — normalize base_url with rstrip('/') | KnightOfNight | triaged | Version 25.7 | 2026-08-25 |
| 293 | Agent door: no character-inventory query — carried items and equipped gear invisible to sudo (read-side prerequisite for #287/#288) | KnightOfNight | triaged, monitoring-and-command | Version 25.7 | 2026-08-25 |
| 294 | sudo bot: durable admin-taught memory — named waypoints ('send shy-guy into battle') and item bundles ('the badass armor set') outlive conversation expiry | KnightOfNight |  |  | 2026-08-25 |
| 295 | agents/: adopt the operator's bot-management scripts (sudo-bot.sh / test-sudo-bot.sh) into the repo — one parameterized script, bash standards, status passthrough | KnightOfNight | triaged | Version 25.7 | 2026-08-25 |
...
## Issue #275: Unequipping +STR gear silently strands the character over carry capacity, blocking loot/pickup/buy
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/275

### Body

## What happened (operator playtest report, 2026-08-20)

Unequipped weapons → carry capacity dropped below current load → now at **443/408** and every acquisition path refuses ("You can't carry any more"). No items were lost — but loot, pickup, and buy are all blocked.

## Mechanism (verified against code on main)

1. **Capacity is derived from *effective* STR**, gear bonuses included. `item_utils.carry_capacity` (v24.23, #215) computes `floor(effective_STR × 10 × (100 + Σ bag_pct) / 100)`, and `combat_utils.effective_stats` (v22 B5, #100) explicitly feeds carry capacity from base + equipped-gear stat bonuses. So equipping +STR weapons raises capacity — by ~10 × bag-multiplier items per STR point.
2. **The unequip guard only covers bags.** `consumers.py` `_unequip_blocked_reason` refuses a bag unequip that would strand the character over the limit (v24.23, #215; GDD §6.3: "A bag cannot be unequipped if doing so would put the character over their carry limit"). Unequipping **stat-granting gear has no equivalent guard** — capacity silently drops below current load.
3. **While over capacity:** `cmd_pickup`, `_loot_single_corpse` / the loot sweep, and the buy path all check `current_count >= max_capacity` and refuse. Drop, sell, and (re-)equip are not capacity-gated.

## Player impact

Surprise soft-lock on acquisition. Recovery paths exist and work — **re-equip the +STR gear** (capacity returns instantly) or drop/sell down under the limit — but nothing in the game explains why capacity fell, and the refusal message gives no hint that unequipping caused it.

## Design ruling needed (not a straightforward code fix)

The two obvious directions pull against each other:

- **Extend the bag guard to all stat-affecting gear** — consistent with #215's precedent, but hostile in practice: a full-inventory character couldn't unequip (or auto-swap) weapons at all, which makes ordinary gear swapping fail at exactly the moment inventories are fullest.
- **Accept the over-capacity state as legal** (it already is, de facto — drop/sell/re-equip all still work) and make it *legible*: e.g. a warn line at unequip time when the action strands you over the limit, and/or an over-capacity indicator in `inventory`/status output.

Also worth noting during the ruling: GDD §3's auto-swap text says displacement "respects every existing unequip constraint … (bags)" — whatever is ruled here should be checked against the auto-swap path too.

Relates to #215 (percentage bags / the capacity formula and the bag-only guard).
...
## Issue #286: GDD erratum: §2.11 "three ways out" undercounts — the shard relay makes four
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/286

### Body

GDD §2.11 (Section 2, world model — the paragraph after The `travel` Command / hearth discussion) says:

> A player deep in a zone therefore has three ways out: walk, `home` (the hearth command, to their attuned node), or push forward to the summit obelisk — and once they reach any checkpoint, its shard relays them to a revealed obelisk (v24.26, #30).

The "three ways" headline undercounts shipped behavior. Since the shard relay shipped (v24.26, #30), reaching a checkpoint shard is a fourth, first-class way out — the sentence itself describes it while still claiming three. Correct count: **walk, `home`, reach the sphere, reach a shard.**

Erratum test: describes shipped behavior inaccurately (a count), no design ruling needed — ops-lane correction to the section file + `make gdd`.

Surfaced during the 2026-08-22 new-zones planning session (rule-of-three walkthrough, operator correction).
...
## Issue #287: Agent door: no item-removal action — sudo can gift but not take

- State: open
- Author: KnightOfNight
- Labels: triaged, monitoring-and-command
- Milestone: Version 25.7
- Assignees: KnightOfNight
- Created: 2026-08-24 | Updated: 2026-08-25
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/287

### Body

Found in the V25.6 Brief 1 operator playtest (2026-08-24), driving sudo against the dev stack.

The v25.5 agent-door action vocabulary (#281: `answer`, `gift`, `create_artifact`, `strip`, `dress`, `move`) has no action that removes an item from a character. sudo can put items into the world (gift, artifact create+gift) but cannot take one back out — there is no confiscate/remove counterpart on any write path.

Operator direction from the playtest: sudo should be able to remove an item from a player.

The door vocabulary is design surface (ruled 2026-08-17 → 2026-08-23 on #262/#261), so this needs a design ruling before implementation — param shape (instance id vs name resolution), what happens to the removed item (destroyed vs transferred), soulbind interaction, and the player-visible narration are all open design questions.


### Comments (3)

**KnightOfNight** — 2026-08-24:
Operator direction, refined in the same playtest conversation (2026-08-24):

Removal must be **complete** — sudo should be able to completely remove something from a character, **including a one-of-a-kind artifact**. For artifacts that raises the definition question explicitly: an artifact is a unique hand-authored ItemDefinition plus its single instance, so "completely remove" plausibly means destroying both (freeing the name for re-authoring), where removing an ordinary item touches only the instance. For the design session to rule.


**KnightOfNight** — 2026-08-25:
Read-side prerequisite filed: #293 (no character-inventory query) — removal needs to resolve/confirm the target instance the character is actually carrying.

**KnightOfNight** — 2026-08-25:
**Design ruling (2026-08-25, V25.7 design session)** — operator-confirmed in-session. Founding ticket of V25.7 (milestone member); read-side prerequisite #293 ruled first (inventory query supplies the instance ids this action targets).

**New action kind `remove_item`:**
- **Params:** character name (via `_resolve_character`, standard addressing) + `item_id` — an instance id from #293's `inventory` roster. The door verifies the named character owns that instance; a stale/mismatched id is a refusal, never a guess. **No string-based name resolution on the write path by design** — the bot resolves against the inventory query, then acts on an exact id.
- **Fate: destroyed, not transferred.** Hard `delete()`, the sell/consume precedent. No off-body storage exists to receive it, and transfer to another character would be admin-brokered item trading — the soulbind economy forbids it. Removal means removal from the world.
- **Artifacts: complete removal = definition + instance.** When the target instance's rarity is `artifact`, the **definition** is deleted — the `definition` FK CASCADE takes the instance with it, and the unique name/slug free for re-authoring (the operator's refinement on this issue). Ordinary items delete the instance only; the shared definition is untouched. Remove + re-create thereby becomes the working fix path pending #289.
- **Equipped targets allowed, admin-style:** deletion directly (no pre-unequip), then `rescale_bars_for_gear` — the #110 bar law binds every max-changing mutation, and deleting worn gear is one. Consistent with `_strip`: no player-side guard, no combat gate. Soulbind is irrelevant to deletion (not a transfer).
- **Cursed items: the curse ends with the item** (operator-ruled). Destroying a cursed item tears down its curse effect on the holder; the brief verifies the effect-teardown mechanics against the code at writing time.
- **Narration:** transparency doctrine, `_strip`'s shape — online holder gets one line naming the item via `item_ref` (unidentified items stay veiled by that helper's existing behavior), `system` category, `refresh_status` event when the removed item was equipped. Offline holder: silent removal. Exact wording authored at brief time (creative policy).
- **Result payload:** `{'item_id': N, 'definition_removed': bool}` — `definition_removed` true only on the artifact path.

GDD text and the implementation brief land this session on `version_25_7`.


## Issue #288: Agent door: no targeted equip/unequip actions — strip is all-or-nothing, dress only restores the snapshot
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/288

### Body

Found in the V25.6 Brief 1 operator playtest (2026-08-24), driving sudo against the dev stack.

The v25.5 agent-door `dress` action (#281) only restores the outfit snapshot taken by `strip` — it cannot equip an arbitrary item. There is no door action that equips a specific inventory item onto a character, so sudo cannot dress a player in anything outside the strip snapshot (e.g. a just-gifted weapon or artifact).

Operator direction from the playtest: sudo should be able to equip a specific item onto a player.

The door vocabulary is design surface (ruled 2026-08-17 → 2026-08-23 on #262/#261), so this needs a design ruling before implementation — item resolution (instance id vs name), slot selection, interaction with the equip guards (two-handed, slot validity, soulbind-on-equip), and whether it composes with or bypasses the strip/dress snapshot machinery.


### Comments (3)

**KnightOfNight** — 2026-08-24:
Operator direction, refined in the same playtest conversation (2026-08-24) — scope of this ticket broadened to targeted **unequip** as well (retitled to match):

- sudo should be able to **unequip any single item** — `strip` is all-or-nothing today; there is no per-item unequip on the door.
- sudo should be able to **equip any single item or more** without a dress list — i.e. targeted equip of arbitrary inventory items, not just restoring the strip snapshot.

Together with #287 these make sudo's gear powers fully granular: equip/unequip per item, and complete removal. For the design session to rule as a set.


**KnightOfNight** — 2026-08-25:
Read-side prerequisite filed: #293 (no character-inventory query) — targeted equip needs to resolve the specific carried instance before proposing the action.

**KnightOfNight** — 2026-08-25:
**Design ruling (2026-08-25, V25.7 design session)** — operator-confirmed in-session ("confirm all"). V25.7 milestone member; completes sudo's granular gear powers alongside #287 (removal) on #293's inventory-roster ids.

**Two new action kinds, `equip_item` and `unequip_item`:**
- **Addressing:** character name (via `_resolve_character`) + `item_id` from #293's `inventory` roster; ownership verified; stale/mismatched id is a refusal, never a guess. No string resolution on the write path (the #293 pattern).
- **Equip slot selection — optional `slot` param, player-algorithm fallback:**
  - `slot` absent: the same resolution `cmd_equip` runs — a free valid slot wins outright; an unambiguous one-for-one swap performs; genuine ambiguity (e.g. both rings full) refuses with a **structured error naming the options** so the bot can ask the admin or retry with an explicit slot.
  - `slot` given: equip into that slot, displacing whatever must come off (including a two-hander's both-hands claim).
- **Structural rules hold; protective guards yield.** `valid_slots` membership, slot capacity, and two-handed geometry are structural — always enforced (bypassing them is state corruption, not admin power). The protective guards (`_unequip_blocked_reason`: cursed, bag carry-capacity) are bypassed admin-style, exactly as `_strip` already does by ruling (2026-08-22, #275 acceptance) — displaced items come off admin-style too.
- **Soulbind-on-equip stands, no exception:** admin equip re-soulbinds to the wearer, byte-consistent with `equip_item`/`_dress` — the economy's one-way door is precisely for this case.
- **Unequip = the strip precedent, one item at a time:** both protective guards bypassed (a cursed item comes off; over-capacity accepted per #275). Curse *effects* untouched — unequip is not removal (#287 alone destroys the curse with the item); targeted unequip matches `_strip`'s curse behavior byte-for-byte, parity verified at brief-writing time.
- **Snapshot orthogonality:** neither action reads, writes, nor consumes `outfit_snapshot`; `_dress` already tolerates the interaction (already-equipped snapshot entries land in `missing`). No coupling.
- **Bars/status/narration:** `rescale_bars_for_gear` + `refresh_status` on every path; transparency lines (`system` category, `item_ref`) to an online holder, silent offline, wording authored at brief time.
- **Result payloads:** equip → `{'item_id': N, 'slot': S, 'displaced': [ids]}`; unequip → `{'item_id': N}`.

GDD text and the implementation brief land this session on `version_25_7`.


## Issue #289: Agent door: no mutation action — created artifacts (and items generally) cannot be edited
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/289

### Body

Found in the V25.6 Brief 1 operator playtest (2026-08-24), driving sudo against the dev stack.

The v25.5 agent-door vocabulary (#281) has no mutation action: `create_artifact` is create-only, and nothing on the door edits an existing item after creation. The operator authored an artifact through sudo, wanted it adjusted, and sudo has no way to do it — the only fix today would be complete removal (#287, itself unshipped) plus re-creation under a new name (the old name stays taken).

Operator direction from the playtest: sudo should be able to change an artifact after creation — and possibly other existing items too ("maybe can't change anything else").

Design questions for the ruling: what is mutable (rolled stats, damage keys, description/lore, name, Mk tier, the mystery pair) and on which side (the unique definition vs the instance); whether mutation is artifact-only or extends to ordinary instances; how a rename interacts with the unique-name law and slug; and what, if anything, the holding player sees when their item changes.

Sits with #287 (complete removal, incl. artifacts) and #288 (targeted equip/unequip) as one capability set: full granular item control for sudo.


### Comments (1)

**KnightOfNight** — 2026-08-25:
**Design ruling (2026-08-25, V25.7 design session)** — operator-confirmed in-session ("confirm all"). V25.7 milestone member; sits on #293's ids like the rest of the write pile.

**New action kind `edit_item` — one action, two permission tiers keyed to what's being edited:**
- **Instance-side fields — editable on any owned instance, any rarity:** `mk_tier`, `rarity`, `rolled_primary_stats`, `rolled_secondary_stats`, `damage_midpoint`, `damage_spread`, `durability_current`. Mutation is NOT artifact-only: the motivating example (operator, in-session) is raising the Mk of a player's ordinary item, and every gameplay read (display name suffix, armor TAV = slot weight × Mk, damage) reads live from these instance fields.
- **Definition-side fields — editable only when the target instance is an artifact:** `name`, `description`, `base_value`, `valid_slots`, `is_two_handed`, `armor_base`, `mystery_name`/`mystery_description`, `genre_tag`. An artifact's definition is one-of-a-kind, so editing it edits one item; an ordinary definition is a shared template — editing it would silently mutate every copy in the world, so the door refuses definition-side keys on non-artifacts.
- **Raw set, no re-roll:** the admin sets exactly the values stated; nothing re-derives from scaling formulas (artifact precedent: `scaling_base`/`scaling_factor` authored 0.0, meaningless). Validation reuses the `create_artifact` validators (`_validate_artifact_spec`/`_validate_stat_entries` family) where applicable — same bounds, same shapes.
- **Rename respects the unique-name law:** re-validate name + slug (`iexact`, as at creation), regenerate the slug, refuse collisions with `name-taken`.
- **Params:** character name + `item_id` (addressing identical to `remove_item`) + `changes` dict of whitelisted keys. Unknown or forbidden keys refuse the whole request — no partial application.
- **Bar law:** if the edited instance is equipped and a stat-bearing field changed — `rescale_bars_for_gear` + `refresh_status`, standard.
- **Holder narration:** transparency doctrine — one `system` line naming the item (`item_ref`) when online, silent when offline; wording authored at brief time.
- **Deliberately not editable via this action:** `owner`, `is_equipped`/`equipped_slot`, soulbind fields (gift/#288/removal territory), and curse/identification flags (a different power with no-leak implications; file separately if ever wanted).

GDD text and the implementation brief land this session on `version_25_7`.


## Issue #290: Agent door: no room-directory query — rooms unreachable except through characters
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/290

### Body

Found in the V25.6 Brief 1 operator playtest (2026-08-24), driving sudo against the dev stack.

The v25.5 agent-door vocabulary (#281) has no room-directory query. Rooms are only reachable through characters (`where_is`/`character` return the character's current room), and `move`'s `to_room_id` param has no discovery path — there is no way to resolve a room by name, so `sudo move Harley to the verdant sphere` is impossible: the bot (correctly) refused to guess and could only offer teleport-via-character.

Operator intent from the playtest: moving players to named rooms/places should work.

Design questions for the ruling: query shape (search by name substring? scoped by zone/area?), result shape (id + Zone: Area: Room path), ambiguity handling for duplicate room names across zones, and whether the result should be capped like `items` (50). Related detail worth ruling at the same time: the `move` action's result carries only the destination — including the origin room in the result would make "send them back" robust independent of bot memory.

Sits with #287/#288/#289 as the playtest's door-vocabulary set.


### Comments (1)

**KnightOfNight** — 2026-08-25:
Complementary arc filed: #294 (durable admin-taught memory — waypoints and item bundles). This issue lets the bot resolve authored room names on demand; #294 lets admins define their own names for places (and gift bundles). A ruling could fold the two into one arc.

## Issue #292: sudo bot: trailing slash on --url breaks login (double-slash POST draws a 302, looks like a credential failure) — normalize base_url with rstrip('/')
...
- Milestone: Version 25.7
- Assignees: KnightOfNight
- Created: 2026-08-25 | Updated: 2026-08-25
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/292

### Body

## What happened

First production start of the sudo bot (2026-08-25, right after the v25.6 deploy and `agent-sudo` provisioning): the bot was launched with `--url https://games.magrathea.com/` — trailing slash — and could not log in:

```
WARNING login refused for 'agent-sudo' (HTTP 302); retrying in 2s
...
WARNING login page unreachable: HTTPError; retrying in 32s
```

It presented exactly like a credential problem (the operator's password was fine and was never even checked).

## Root cause

`django_login` builds `f'{base_url}/accounts/login/'`. With a trailing slash on `--url`, that yields `//accounts/login/` (double slash):

- The **GET** follows redirects silently (`requests` default), so it lands on the real login page and picks up a csrftoken — no visible symptom.
- The **POST** uses `allow_redirects=False` and hits the double-slash path directly. `//accounts/login/` doesn't match Django's login route, so the request draws a 302 bounce to `/accounts/login/?next=//accounts/login/` with no `sessionid` cookie — which the bot correctly reads as a refused login. Verified live against prod: `GET //accounts/login/` → 302, `GET /accounts/login/` → 200.

Knock-on: the retry loop's GET+POST pairs then tripped the nginx login rate limit (`HTTPError` on the fifth cycle) — cosmetic here, but it muddies the log signature further.

The WebSocket URL is built the same way from `base_url` and would carry the same double slash.

## Fix

Normalize once at config time: `base_url = args.url.rstrip('/')` (or equivalent in `cmd_run`) before any URL is composed. One line, bot-side only (`agents/sudo_bot.py`); no game-side change.

Optionally, the `login refused (HTTP 302)` message could include the redirect `Location` — a 302-with-location line would have pointed straight at the URL problem instead of at credentials.

## Context

Found during the v25.6 closeout tail's first prod bot start; workaround (slash-free URL) confirmed working. Operator direction: rides the pile for the next bot point release.


### Comments (1)

**KnightOfNight** — 2026-08-25:
**Design ruling (2026-08-25, V25.7 design session)** — operator-confirmed in-session. V25.7 milestone member. Bot-side only; no game-side change.

- **Fix as filed:** normalize once at config time in `cmd_run` — `base_url = args.url.rstrip('/')` — before any URL composes. One line covers both the login URLs (`django_login` builds `f'{base_url}/accounts/login/'`, confirmed at the composition site) and the WebSocket URL built from the same `base_url`.
- **Diagnostic adopted:** the `login refused (HTTP 302)` warning grows the redirect `Location` header when present — a 302-with-location line points at URL problems instead of masquerading as a credential failure (the exact misdirection from the first prod start). Log lines UTC-stamped with trailing Z per the standing project rule.
- The #295 management script independently normalizes the URL it passes (belt and suspenders); that's recorded on #295.

Implementation brief lands this session on `version_25_7`.


## Issue #293: Agent door: no character-inventory query — carried items and equipped gear invisible to sudo (read-side prerequisite for #287/#288)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/293

### Body

Found on production (2026-08-25, first prod session of the sudo bot, v25.6 closeout tail): an admin asked `sudo show me gunnysam's inventory` and the bot correctly answered that no tool exists for it.

The v25.5 agent-door query vocabulary (#281: `commands`, `who_online`, `where_is`, `character`, `items`, `is_admin`) has no per-character inventory read:

- `query items` searches the **ItemDefinition catalog** (`_items_payload` — definitions only, built for the gift flow).
- `query character`'s payload (`_character_payload`) carries stats, bars, copper, XP, and room — no carried instances, no equipped gear.

So a character's carried items and worn gear are invisible to sudo by construction; the bot's only honest options are declining or telling the player to `inv` themselves.

**This is also a resolution prerequisite for the write-side pile:** #287 (remove an item) and #288 (equip a specific item) both need to name an item the character is actually carrying — without an inventory query the bot cannot resolve, disambiguate, or confirm the target instance before proposing the action.

Design questions for the ruling:
- Query shape: by character name (`iexact`, like `where_is`/`character`)?
- Result shape: instances with id, definition name, Mk tier, rarity, durability, soulbound state, equipped flag (and slot) — mirroring what the player-facing `inv`/composition helper shows, or a leaner id+name list?
- Cap and truncation like `items` (50), or uncapped (inventories are bounded by carry capacity)?
- Whether `query character` should additionally grow a gear-slots summary, or equipped state lives only in the inventory query.
- No-leak interactions: unidentified/cursed items — does the admin-facing read show true state (admin tool, presumably yes, like the Django admin) or the veiled player view?

Sits with #287/#288/#289/#290 as the door-vocabulary capability set (full granular item control for sudo); read-side sibling. The door vocabulary is design surface (ruled 2026-08-17 → 2026-08-23 on #262/#261) — needs a design ruling before implementation.


### Comments (1)

**KnightOfNight** — 2026-08-25:
**Design ruling (2026-08-25, V25.7 design session)** — operator-confirmed in-session. Ships in V25.7 (milestone member; read-side prerequisite for #287/#288).

**New query kind `inventory`:**
- **Target:** by character name through the existing `_resolve_character` (`name__iexact`) — identical addressing to `character`/`where_is`/`is_admin`.
- **Result:** an **uncapped** roster of **every instance the character owns — equipped gear included**, one row per instance, equipped rows distinguished by flags rather than segregated. Deliberately **unlike the player-facing `inventory` command**, which excludes equipped items: that command is a selection surface (feeding sell/drop pools with protective exclusions); this query is an admin **state report** — it reports what is, it doesn't select targets for a destructive verb. (Operator confirmed this exact distinction in-session.)
- **Row fields:** `id` (load-bearing — #287/#288 write actions name their target instance by it), definition `slug` + `name`, `item_type`, `mk_tier`, `rarity`, `durability_current`, `is_broken`, `is_soulbound`, `is_equipped`, `equipped_slot`. Rolled stats and damage are deliberately **not** roster fields (token weight per row in the bot's context; resolution doesn't need them).
- **Cap:** none. Inventories are bounded by carry capacity; a truncated inventory would defeat the query's purpose (resolve/disambiguate/confirm before a write). `ITEMS_CAP` remains a catalog-search (`items`) concern only.

**Companion query kind `item`:** one instance by `id`, full fidelity — rolled primary/secondary stats, damage midpoint/spread, curse state (`is_cursed`, `curse_identified`), identification state (`is_identified`, `is_unidentifiable`), plus the roster fields. The roster+detail pattern; also the verification read for #289 mutation once that ships.

**`query character` unchanged:** `_character_payload` grows no gear summary — equipped state lives in the inventory query only. One home, no drift.

**No-leak / veiling:** admin reads show **true state** — cursed, unidentified, artifact rarity, all of it, like the Django admin. Safe by construction: query results flow only to the bot, and `answer` delivery is already gated to live `admins.shyland` members (#273). The no-leak rule governs player-facing world speech, which queries never touch.

GDD text and the implementation brief land this session on `version_25_7`.


## Issue #294: sudo bot: durable admin-taught memory — named waypoints ('send shy-guy into battle') and item bundles ('the badass armor set') outlive conversation expiry
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/294

### Body

Found on production (2026-08-25, first prod session of the sudo bot): an admin taught sudo a waypoint in conversation —

> [19:08:50] "can you remember shy-guy's current location and send me back to it when I call for it by a certain name?"
> [19:09:19] "call it 'battle' like 'sudo send shy-guy into battle'"

— and sudo agreed. It works, but only as well as conversation memory can: threads expire after **600s of quiet** (`--convo-timeout`) and hold **20 exchanges** (`--history-max`). The "battle" waypoint survives a bot restart (JSON persistence) but a ten-minute lull erases it silently — "send shy-guy into battle" tomorrow draws a blank, indistinguishable from never having been taught. The shipped bounds are correct for *conversation* (an expired thread should look like never answering); they are the wrong store for *facts the admin intended to keep*.

The ask: **durable admin-taught named things**, operator-directed 2026-08-25. First-class examples:

- **Waypoints** — "battle" = a room captured at teach time; `sudo send <player> into battle`.
- **Item bundles / loadouts** — "this badass armor set" naming a gift set (e.g. the same session's ten-gift compound: full legendary Mk 1 leather + two copper rings of dexterity + copper amulet of strength), so `sudo give <player> the badass armor set` replays it without re-describing ten items.
- Possibly other aliases/notes an admin teaches in passing.

Design questions for the ruling:

- **Where it lives:** bot-side durable store (a second JSON/SQLite file beside the conversation file — no game-side change, but per-bot-host and invisible to the game/MC record) vs game-side (a door vocabulary addition — recorded, shared across bot restarts and future bots, but a model + door design like the rest of the pile).
- **Scope:** per-admin or shared across all admins? (Shy-Guy's "battle" — does another admin's "battle" collide? A shared bundle name is arguably a feature.)
- **What a waypoint stores:** room id captured at teach time (robust — survives renames) plus the Zone: Area: Room path for display; relation to #290 (room-directory query) — #290 lets the bot *resolve* named rooms on demand, a waypoint store lets admins *define* their own names; complementary, and a ruling could fold them into one arc.
- **What a bundle stores:** the resolved gift parameters (slug, mk_tier, rarity, quantity per line) — replay is N ordinary `gift` calls, every one still server-validated and on the record; no new write power, just memory.
- **Vocabulary:** teach/forget/list surfaces — natural language only (the model decides when to store, like the prod exchange) vs explicit forms; how "forget" works; caps on count/size.
- **What else is rememberable:** named things only, or arbitrary admin-taught facts/notes — and if the latter, the no-leak and audit posture (everything-on-the-record suggests taught facts should emit MC records if game-side).

Sits with the bot pile for the next point release: #287/#288/#289/#290/#293 (door vocabulary) and #292 (bot-side robustness). Bot-side vs game-side placement determines which half of the pile it joins.
...
## Issue #295: agents/: adopt the operator's bot-management scripts (sudo-bot.sh / test-sudo-bot.sh) into the repo — one parameterized script, bash standards, status passthrough
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/295

### Body

Operator-directed (2026-08-25, filed from the v25.6 closeout session): the operator wrote two bash wrappers for managing the sudo bot, currently living in `$HOME` outside the repo — `sudo-bot.sh` (prod: main checkout, `https://games.magrathea.com`) and `test-sudo-bot.sh` (dev: the v25.6 worktree, `https://emma.private.magrathea.com`). They work and are the right shape — `start`/`stop`/`restart`/`tail` subcommands, key sourced from `agents/.secrets/anthropic-api-key.sudo` at launch, `nohup` detach, post-action log tail. The ask: **improve them to repo standards and commit them to the repo** (`agents/` is the natural home, beside the bot they manage), as the management surface for the sudo bot and later bots like it.

The two scripts are line-identical except for two variables (`GAME` checkout path, `URL`) — copy drift waiting to happen; that's the core thing to engineer away.

Improvement inventory (a starting list, not a ruling):

- **One script, parameterized target** — `prod`/`dev` selected by flag, env, or a small config, collapsing the two copies; bot name as a parameter too (`agent-sudo` today, Sirius and friends later — this is the fleet's ops surface, per the #268 posture).
- **Path independence** — derive the repo root from the script's own location instead of hardcoded `/Users/ctg/...`, so the committed script works from any checkout/worktree (the dev copy points at a version worktree that will eventually be cleaned up).
- **Bash standards** — `set -euo pipefail`; quote every expansion (`test -z $command` and the unquoted `cd $GAME`/`tail $LOG` break on spaces; `$BOT` relies on word-splitting — make it an array or a function); handle `cd` failure; propagate the bot's exit codes instead of unconditional `sleep 2; tail`.
- **Expose `status`** — the bot already has the subcommand; the wrapper should pass it through (and `restart` could use it instead of a blind sleep between stop and start).
- **Codify the #292 lesson** — the script pins a slash-free URL today (good); keep that guaranteed (normalize/validate) until #292 lands bot-side.
- **Secrets posture preserved** — key read from the secrets file into the process environment only, never echoed, never in argv; committed script must contain no secret values (the `.secrets/` path convention already handles this).
- **Dev target detail** — the dev stack runs self-signed certs; the dev path may need `--insecure` plumbed through (the prod path must never get it by default).

Sits with the bot pile for the next point release: #287/#288/#289/#290/#293 (door vocabulary), #292 (bot-side URL fix), #294 (durable memory). Bot-side/ops-side only — no game-side change.


### Comments (2)

**KnightOfNight** — 2026-08-25:
Operator note (2026-08-25): the test script's target path is a moving target by design — it points at the current release's version worktree (games-mvc-version_25_6 today, _25_7 next, and so on every release). So the dev target can't be a baked-in path at all: the script needs either to derive the repo root from where it's invoked/located (run it from inside whichever worktree you're testing), an explicit --path/--target flag, or newest-worktree discovery. This makes path independence the load-bearing requirement of the rewrite, not just hygiene.

**KnightOfNight** — 2026-08-25:
**Design ruling (2026-08-25, V25.7 design session)** — operator-confirmed in-session, with one operator amendment: **the management script is proper Python, not bash**. V25.7 milestone member. Bot-side/ops-side only; no game-side change.

**One new script: `agents/botctl.py`** — stdlib-only Python (argparse/pathlib/subprocess, list-argv, never `shell=True`), executable with a `#!/usr/bin/env python3` shebang, runnable under the system `python3` (it must not depend on the bot venv — it's the tool that tells you the venv is missing). It replaces both `$HOME` wrappers (`sudo-bot.sh` / `test-sudo-bot.sh`), which the operator retires at their own convenience.

- **Path independence via self-location — the load-bearing requirement (operator note on this issue):** the script derives its repo root from its own file location (`Path(__file__).resolve()`), so **the copy you run is the checkout it manages** — main checkout's copy for prod, the current version worktree's copy for dev. No baked-in paths, no `--path` flag, no worktree discovery; the moving dev target is answered by the script moving with it. (Deriving the URL from the checkout's `.env` `DOMAIN` was considered and rejected: resting posture is dev everywhere, including the main checkout — it would misroute the prod bot.)
- **Target keyword for the URL half only:** `botctl.py <prod|dev> <command>`. `prod` → `https://games.magrathea.com`; `dev` → `https://emma.private.magrathea.com` **plus `--insecure`** (dev's self-signed certs — implied for dev, never available on the prod path). The two URLs live in one documented constants block; hostnames, not secrets.
- **Bot name parameterized, sudo default:** `--bot <name>` (default `sudo`) selects `agents/<name>_bot.py`, `agents/<name>_bot.log`, and `agents/.secrets/anthropic-api-key.<name>` — the fleet ops surface per the #268 posture.
- **Command surface `start | stop | restart | status | tail`:** `status` passes through to the bot's existing subcommand; `start`/`stop`/`restart` replace the blind `sleep 2; tail` with a short status poll and **propagate the bot's exit codes**; `start` detaches properly (`start_new_session`, output to the log) and finishes with a log tail so the launch outcome is visible.
- **Standing postures preserved:** the API key is read from the secrets file into the child process environment only — never argv, never echoed, no secret values in the committed script; the URL is `rstrip('/')`-normalized by the script too (belt and suspenders with #292); the bot runs under its per-checkout venv (`agents/venvs/mc-agent/bin/python`), and a missing venv is a clear error naming the fix (`python3 -m venv` + `requirements.txt`), never an auto-install.
