...
| 203 | Design: examine's 'Note: … you may drop it' line — weird on ground items, redundant with the flag block, key/value inconsistent | KnightOfNight | triaged, V24 | Version 24.20 | 2026-08-11 |
...
Design ruling (2026-08-10, Version 24.20 design session — **founding ticket** for Version 24.20):
...
Bound state is carried **solely by the headline's trailing flag block** (`[Rarity, Bound|Unbound]`), which renders on the same examine output two lines up — consistent with the standing binding-in-the-flag-block doctrine (GDD §6.11: the Details column reads binding, and binding sits in the stacking key precisely so the flag block never lies). One stroke resolves all three filed problems:

1. No misvoiced "you may drop it" on ground items (#80 knowledge-by-holding made examine render the full detail block without pickup).
2. No prose restatement of a fact already on screen in the flag block.
3. No key/value inconsistency — the two remaining conditional rows keep proper property keys.

**Explicitly untouched:** the `Equipped:` and `Curse:` rows stay — each carries a fact the flag block does not (equip slot; identified curse state).

GDD text lands with this release (§6.8 identification — the detail-block composition rule). Brief: `Shyland_V24.20_Brief_1_Examine_Binding_Rows_Removal.md` on `version_24_20`.
