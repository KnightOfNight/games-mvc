...
| 311 | Empty durability_table with takes_durability_loss=True: penalty lookup falls through to 1.0 — pristine items render yellow and fight fully penalized | KnightOfNight | bug, triaged | Version 25.12 | 2026-08-30 |
| 312 | Admin cannot re-save ItemDefinitions with legitimately-empty JSON lists: durability_table/primary_stats/secondary_stat_pool lack blank=True | KnightOfNight | bug, triaged | Version 25.12 | 2026-08-30 |
| 314 | Agent door: durability_current edit accepts fractional values — wear is integral and seed bands cover integers only, so 0.5 draws the 1.0 no-band penalty | KnightOfNight | bug, triaged, monitoring-and-command | Version 25.12 | 2026-08-30 |
...
## Issue #311: Empty durability_table with takes_durability_loss=True: penalty lookup falls through to 1.0 — pristine items render yellow and fight fully penalized

- State: open
- Author: KnightOfNight
- Labels: bug, triaged
- Milestone: Version 25.12
- Assignees: KnightOfNight
- Created: 2026-08-30 | Updated: 2026-08-30
- Blocked by: #312, #314
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/311

### Body

**Observed (dev, 2026-08-30, V25.11 playtest session):** the hand-authored artifact definition `Ludicrous Speed` has `takes_durability_loss=True` with an empty `durability_table` (`[]`). Result: its instance renders the durability cell say-colored (yellow/gold) at 100% durability in the inventory listing, and — worse — carries a full mechanical penalty while pristine.

**Mechanism (confirmed in code):** `get_durability_penalty` (`django/src/apps/shyland/item_utils.py:333`) walks `definition.durability_table` for a band containing `durability_current` and **returns the 1.0 fallback when no entry matches** — with an empty table the fallback always fires. `details_cell` (`item_utils.py:522`) derives the durability voice from that penalty (penalty > 0 → say voice), so the pristine item paints as penalized. The same function feeds combat's durability penalty, so the mispaint is a symptom of a real mechanical penalty. Standard seeded gear never hits this: the seed pairs `loss=True` with a full 0–100 band table (`WEAPON_DUR`/`RANGED_DUR`/`ARMOR_DUR`) and `loss=False` with `[]`, exactly.

**Operator direction at filing (2026-08-30):** an item definition with `takes_durability_loss=True` **must** have the durability table; and the bot should make sure the appropriate settings are done to the item (the bot-side item tooling should ensure/guard these settings on the items it touches).

**Immediate mitigation applied by the operator (dev):** flipping the definition to `takes_durability_loss=False` (table stays `[]`), which is coherent everywhere — no durability suffix or cell, penalty 0.0, no combat degradation, excluded from repair paths — the seed's exact convention for non-wearing items.

---
Filed from the V25.11 implementation session (playtest side-finding; out of Brief 1 scope).


### Comments (5)

**KnightOfNight** — 2026-08-30:
Vocabulary note for the ruling (asked in-session 2026-08-30): the sudo bot **cannot** currently change these settings. The door's `edit_item` whitelists (`mc_door.py:985` — `INSTANCE_EDIT_KEYS` / `ARTIFACT_DEFINITION_EDIT_KEYS`) include neither `takes_durability_loss` nor `durability_table`; whole-request semantics refuse the entire edit on an unknown key. So "the bot should make sure the appropriate settings are done to the item" implies either (a) adding the durability pair to `ARTIFACT_DEFINITION_EDIT_KEYS` (with the mandatory-table coupling validated door-side, mirroring the mystery_name/description coupling precedent), and/or (b) a door/model-level invariant that `takes_durability_loss=True` requires a non-empty `durability_table` (which would also protect the admin-form path). Interim path for the observed item: Django admin definition edit (artifact definitions are unique per item, so no ripple).


**KnightOfNight** — 2026-08-30:
Creation-path confirmation: `_create_artifact` (`mc_door.py:655`) never sets `takes_durability_loss` or `durability_table`, so **every** bot-created artifact definition inherits the model defaults `True` + `[]` — the exact broken combination. The artifact spec vocabulary (`_validate_artifact_spec`, §5.4 table) has no durability keys at all, so the model cannot author them either. The ruling's "bot ensures appropriate settings" most directly lands here: the builder should author a coherent durability posture at creation (simplest: `takes_durability_loss=False` for hand-authored artifacts, matching the seed's non-wearing convention; or spec-driven with the mandatory-table coupling).

Related but distinct (separate filing pending operator assent): the Django admin cannot re-save ANY ItemDefinition whose JSON list fields are legitimately empty — `durability_table`/`primary_stats`/`secondary_stat_pool` lack `blank=True` (`models.py:513,522,523`) and Django form validation treats `[]` as empty on a required field. This blocks the manual fix path for the observed item (and would equally block editing every seeded loss=False definition). Note the operator's fix attempt surfaced it 2026-08-30.


**KnightOfNight** — 2026-08-30:
**Design ruling (2026-08-30, V25.12 design session): #311 is the founding ticket of Version 25.12.**

- Milestone `Version 25.12` created and applied; branch `version_25_12` cut from main @ 5c43a04.
- **#312 joins the release as a dependency** (blocked-by recorded on this issue): same empty-JSON-list root, and it blocks this issue's manual fix path — the scope-law shape (dependency describing the same problem, not widening it).
- The fix-shape ruling (model/door invariant vs. creation-path posture vs. edit-key additions) follows in-session; the brief lands on the version branch.


**KnightOfNight** — 2026-08-30:
**Fix-shape ruling (2026-08-30, V25.12 design session — operator confirmed all five points in-conversation):**

1. **Artifacts don't wear.** `_create_artifact` (`mc_door.py`) explicitly authors `takes_durability_loss=False, durability_table=[]` — the seed's non-wearing convention. The artifact spec vocabulary stays durability-free in v1; a wearing artifact would be its own future ruling.
2. **The durability-posture invariant.** `takes_durability_loss=True` requires a table **fully covering 0–100%** (bands may touch at boundaries — the seed's own tables do, e.g. 75 in both `[75,100]` and `[50,75]`; lookup resolves first-match — so the rule is coverage, not strict non-overlap); `False` requires `[]`. Enforced in three layers: model `clean()` (admin form), door-side validation on the new edit keys, and a DB `CheckConstraint` on the cheap core (`NOT (loss=True AND table=[])`), preceded by a normalizing data migration flipping `loss=False` on any empty-table row (expected 0 on dev post-mitigation; count reported).
3. **Door edit keys.** `takes_durability_loss` + `durability_table` join `ARTIFACT_DEFINITION_EDIT_KEYS`, coupling validated on post-edit state (the mystery_name/description precedent).
4. **#312 widened by audit** — see the ruling there: six fields get `blank=True`, not three.
5. **`get_durability_penalty`'s 1.0 fallback stays** — with the invariant enforced the state is unreachable; softening it would mask future misconfiguration.

GDD §6.5 text landed on `version_25_12` @ 151a562 ("(v25.12, pending implementation)" marker). Brief follows on the branch.


**KnightOfNight** — 2026-08-30:
Dependency update (2026-08-30, V25.12 design session): **#314** (door `durability_current` edit accepts fractional values — the operator ruled the fix, require an integer, into this release) joins `Version 25.12` as this ticket's second dependency; blocked-by recorded above. Brief 1 amended in place at `ad4c655` (design rule 6, Step 3a's instance normalize, Step 4d, tests, playtest step 6, verification rows 4/7); GDD §6.5 gained the "Durability values are integral" sentence inside the marked block.


## Issue #312: Admin cannot re-save ItemDefinitions with legitimately-empty JSON lists: durability_table/primary_stats/secondary_stat_pool lack blank=True
...
- Blocked by: none
- Blocks: #311
- URL: https://github.com/KnightOfNight/games-mvc/issues/312

### Body

**Observed (dev, 2026-08-30):** the operator attempted to edit the bot-created artifact definition `Ludicrous Speed` in the Django admin and could not save — the form reported three fields "required" though legitimately empty: `durability_table`, `primary_stats`, `secondary_stat_pool`.

**Mechanism (confirmed in code):** the three `ItemDefinition` JSONFields are declared `models.JSONField(default=list)` with no `blank=True` (`django/src/apps/shyland/models.py:513,522,523`), which makes them **form-required** in the admin; Django's form validation treats an empty JSON list `[]` as an empty value, so any row whose lists are legitimately empty fails validation on save. This is not specific to bot-created artifacts:

- Every artifact definition **correctly** has empty `primary_stats`/`secondary_stat_pool` — hand-authored stats live on the instance (`rolled_primary_stats`), by the §5.4 builder's design.
- Every seeded `takes_durability_loss=False` definition (consumables, readables, keys, materials, bags …) **correctly** has `durability_table=[]` — the seed's standing convention.

So the admin cannot re-save a large, fully-valid class of existing rows; any single-field edit is blocked by unrelated "required" errors. Surfaced now because these rows had apparently never been edited through the admin before.

**Fix direction:** add `blank=True` to the three fields (display/validation-only; `default=list` already guarantees non-null values on every write path — no data migration needed, though Django will emit a no-op migration for the field-option change). `ItemInstance.rolled_primary_stats`/`rolled_secondary_stats` (`models.py:621` area) likely warrant the same check — non-weapon/simple instances legitimately carry empty rolled lists.

Related: #311 (the `takes_durability_loss=True` + empty-table combination this admin gap blocked the operator from fixing manually; interim mitigation executed via the container shell in-session).

---
Filed from the V25.11 implementation session (playtest side-finding; out of Brief 1 scope), operator-directed.


### Comments (2)

**KnightOfNight** — 2026-08-30:
**Design ruling (2026-08-30, V25.12 design session): joins Version 25.12 as a dependency of founding ticket #311** (blocked-by recorded there).

The `blank=True` gap blocks #311's manual fix path and shares the empty-JSON-list root — the scope-law dependency shape. Milestone applied. Fix-shape ruling follows in-session on #311/#312; one brief covers both.


**KnightOfNight** — 2026-08-30:
**Fix-shape ruling (2026-08-30, V25.12 design session — operator confirmed):** the `blank=True` set is **six fields, not three** (audit of every `JSONField(default=list)` in `models.py`):

- `ItemDefinition`: `durability_table`, `primary_stats`, `secondary_stat_pool` (the three filed), **plus `valid_slots`** — legitimately `[]` on every non-equippable definition, same admin re-save block.
- `ItemInstance`: `rolled_primary_stats`, `rolled_secondary_stats` (this issue's own "likely warrant the same check" — confirmed).

**Deliberately excluded:** `LootTableEntry.rarity_weights` — never legitimately empty (must sum to 100), so form-required is correct. `DialogueEntry.keywords` already carries `blank=True` (the in-repo precedent for the idiom).

Field-option-only change; Django emits a DB-no-op migration. Rides V25.12 Brief 1 with #311.


## Issue #314: Agent door: durability_current edit accepts fractional values — wear is integral and seed bands cover integers only, so 0.5 draws the 1.0 no-band penalty

- State: open
- Author: KnightOfNight
- Labels: bug, triaged, monitoring-and-command
- Milestone: Version 25.12
- Assignees: KnightOfNight
- Created: 2026-08-30 | Updated: 2026-08-30
- Blocked by: none
- Blocks: #311
- URL: https://github.com/KnightOfNight/games-mvc/issues/314

### Body

## Observation (V25.12 design session, 2026-08-30)

Found while pinning the durability-coverage rule for #311: the agent door's `durability_current` **instance** edit accepts any float in 0–100, but every game-path durability value is integral, and the seed's band tables cover integers only.

## Mechanism (verified against `version_25_12` @ 151a562)

- `mc_door.py` `_edit_instance_fields` (durability_current branch, ~line 1027): accepts any non-bool number with `0 <= value <= 100`, stores `float(value)`, sets `is_broken = (durability_current == 0)`.
- All game-path wear/repair arithmetic is integral: the only decrement is `DEATH_DURABILITY_LOSS = 10.0` (`models.py:12`, applied at `combat_utils.py:569`), repair kits restore `15 + 10 × Mk`, vendor repair sets `100.0` (`consumers.py:4513`), instances generate at the `100.0` default (`models.py:627`).
- Seed band tables (`WEAPON_DUR`/`RANGED_DUR`/`ARMOR_DUR`, `seed_world.py:34–56`) cover every **integer** 0–100 but not the fractional gap between bands: `[0,0]` then `[1,25]` leaves (0,1) uncovered.
- Consequence: an admin-set fractional value like `0.5` matches **no band**, so `get_durability_penalty` (`item_utils.py:333`) returns its no-band-matched fallback **1.0** — the item silently fights fully penalized, renders as `0%` (display rounds), and is **not** broken (`is_broken` requires exactly 0). The same misconfigured-looking state #311 fixes, reachable through a single door edit.

## Operator ruling (2026-08-30, in-session)

**The fix is to require an integer.** The door's `durability_current` edit accepts only integral values: an `int`, or a float whose value is integral (JSON clients may deliver `42.0`); booleans remain refused; fractional values draw `bad-params` naming the rule. Stored value normalized to the integral float. Range check and the `is_broken` invariant unchanged.

**Included in this release:** joins `Version 25.12` as a dependency of founding ticket #311 (same durability-coherence problem family) and rides Brief 1, amended in place by the design session before any implementation session is directed.
