...
| 56 | Timestamps display on renderings and state reports; should mark events only | bug | Version 20 | 2026-07-14 |
...
## Issue #56: Timestamps display on renderings and state reports; should mark events only
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/56

### Body

## Summary

Found in v20 Brief 2 playtesting. The timestamp prefix currently displays on look-section lines (Who's here?, occupant/item lines) with identical stamps, while other room-rendering lines (Exits:) arrive unstamped — the display rule was never categorized by content. Observed:
...
## Ruled (design chat, 2026-07-14)

Principle: TIMESTAMPS MARK EVENTS, NOT RENDERINGS.

- STAMPED (events): combat lines, damage/heals/XP/kills, chat/says, presence (arrivals, departures, connects), commerce transactions, errors, system/ambient notices (corpse decay, connection), and command echoes once the output & messaging brief ships them.
- UNSTAMPED (renderings): the entire room-rendering block — room name, prose, Exits:, Who's here?, What's here?, occupant and item lines — on both entry and look.
- UNSTAMPED (state reports, explicit ruling): inventory, equipment, stats, help, and similar command-response reports.

The envelope itself is unchanged: every outbound message still carries ts/seq at the delivery choke point (the firehose tap is untouched). This is display categorization only. Convenient side effect: room-rendering lines get their own message category, which the v20 output & messaging brief's look-section work (#14) wants anyway.

## Fix vehicle

v20 Brief 2 Amendment 1 (delivered after this issue's number exists, per the no-placeholder procedure).
