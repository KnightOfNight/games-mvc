# Shyland Project Instructions v36

**Status: ACTIVE — approved by the operator 2026-08-15. Supersedes Shyland_Project_Instructions_v35.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v36 edition is the technical-coherence pass (#252): briefs stop asserting unverified facts about existing code — every structural claim is verified against the code at writing time and recorded (recall is not a source), the design session gives the brief one end-to-end self-consistency read before commit, and the implementation pre-flight extends its diff from process assumptions to the brief's technical claims — a load-bearing mismatch is a HARD STOP back to the operator, never implemented as written and never silently worked around. Born from V24.29 Brief 1, whose four unverified claims produced #251. (v35 the action-item pass; v34 the interim-retirement pass; v33 the prod-verification ruling pass; v32 the gift-language pass; v31 the seed-prod pass; v30 the label model; v29 the release model; v28 the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
