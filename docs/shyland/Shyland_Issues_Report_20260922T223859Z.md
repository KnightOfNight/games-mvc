...
| 297 | Curse system live loop is unbuilt: apply active_curse on equip, unequip handling, seedable curse effects, acquisition + identification paths (needed for new zones; rule with #80) | KnightOfNight | triaged, game-mechanics, V26 | Version 26.3 | 2026-09-22 |
...
| 336 | examine should check equipped items first | KnightOfNight | bug, V26 |  | 2026-09-22 |
| 338 | Out-of-combat DoTs race passive regen — low-Mk drains cannot kill (oscillating pin); rule dot-vs-regen interaction with #297 Release B | KnightOfNight | emergent, triaged, V26 | Version 26.3 | 2026-09-22 |
...
- Author: KnightOfNight
- Labels: triaged, game-mechanics, V26
- Milestone: Version 26.3
- Assignees: KnightOfNight
- Created: 2026-08-26 | Updated: 2026-09-22
- Blocked by: #338
- Blocks: none
...
**KnightOfNight** — 2026-09-16:
Release B agenda handoff from the V26.2 closeout: **#338** (out-of-combat DoTs race passive regen — the F1 finding from the V26.2 Brief 1 playtest, recorded in that brief's closeout report). Low-Mk curse drains cannot kill through regen (Mk 1 Hollowing 8/round vs 9/round regen at a 614 bar = oscillating pin); ruling candidates are the dot-vs-regen interaction and curse kill tempo. Flagged for this ticket's Release B design session as its first agenda item.


**KnightOfNight** — 2026-09-22:
Cross-ref (2026-09-22, V26.3 design session — Release B): the first agenda handoff, **#338** (out-of-combat DoTs race passive regen), is **ruled**: an active `dot_vitality` component pauses passive vitality regen (bar-scoped, source-blind, silent; floor-hold unchanged). Full ruling on #338. It joins milestone Version 26.3 as a dependency riding the Release B brief. Note the corrected arithmetic recorded there: regen is 18/round at a 614 bar (constant 120, every tick), not 9 — the finding is stronger than the closeout's numbers suggested.


**KnightOfNight** — 2026-09-22:
Design ruling (2026-09-22, V26.3 design session — Release B, agenda item 2 of the Release B question set): **the cleansers of the shipped world** (operator-confirmed 2a–2f). This closes the Q3.1 work item — the complete NPC inventory.

**The inventory (verified in `seed_world.py`, 2026-09-22):** the shipped world's service NPCs are — Convergence: Morra (smithy; vendor + repair), Pella and Ferwick (gazebo; vendor + repair), Repairbot Prime (repair), Mother Tansy (remedy cart, east ring `r40`; draught vendor). Z01: three checkpoint trios, each Shard + Mender + Trader — Fordwatch `vr-v07` (Maro the Mender, Essa the Trader), Stairhead `vr-f01` (Tavik the Mender, Sona the Trader), Cragfoot `vr-c01` (Old Brammel the Mender, Ridda the Trader). Summit `vr-vc1`: Sphere + Obelisk. Villagers, hunters, and the fisher offer no services; Aldric, Info Prime, Seris, Veris, and the Obelisk offer none either.

- **2a. Convergence cleanser: Mother Tansy.** The hub's remedy-seller ("sold remedies on six worlds") gains the specialty; no new hub character.
- **2b. Z01 cleansers: the three Menders** — Maro, Tavik, and Old Brammel — gain the specialty. Every checkpoint trio therefore cleanses; "healers near the other NPCs" is satisfied without new characters, and a curse on gear is gear trouble a mender would know. (The fourth-NPC-per-checkpoint alternative was presented and declined.)
- **2c. Model and routing.** A new `is_cleanser` boolean on `NpcDefinition`, mirroring `is_repairer` (migration). The cleanse commands route to the living cleanser in the room (lowest pk, the repairer precedent). Cleansers are `attackable=False`, seed-verified like repairers and vendors.
- **2d. Voice.** Every cleanser gets pooled lines in their own voice — the quote, the success, the refusal, the sweep result — pools of at least three, the pity-repair pattern (`npc_voice.py`). Creative content, authored freely at brief/seed time.
- **2e. The presence sweep lives with the same NPCs** (the Q6 amendment): the professionals who remove curses are the ones who can tell you one is there.
- **2f. Sirius stays out** — nothing in this release touches #259.

Related: #179 (the new-zone cleanser standard — this is the shipped-world half of that rule), #259.


**KnightOfNight** — 2026-09-22:
Design ruling (2026-09-22, V26.3 design session — Release B, agenda item 3): **the cleanse price is authored per curse and scales with the item's Mk** (operator-confirmed 3a–3f, numbers included).

Anchors (verified in `seed_world.py`, 2026-09-22): Z01 gear vends at ~35–90 copper at Mk 1 Common; Ridge bosses drop 150–1000 copper per kill plus a guaranteed Rare/Epic item; all Z01 drops roll at Mk 1; the five Z01 curses run 180–360 s.

- **3a. The field.** `cleanse_price` (copper, the Mk 1 price) authored on each curse `EffectDefinition`. The charge is **`cleanse_price × item.mk_tier`** — a Mk 5 item's curse costs five times a Mk 1's. Curses only ever live on items, so the Mk is always present. "Tied to the curse itself" in both senses: which curse, and how hard it bites.
- **3b. Z01 expiring tier — pocket change for a Ridge-level player.** Mk 1 prices: **Curse of the Craven Edge 60 · Curse of the Moth-Eaten Ward 75 · Tithe of the Copper Court 90 · The Whisper in the String 120** copper. Each is a fraction of one boss kill's copper, and each curse would expire on its own inside six minutes — paying is a convenience, not a rescue.
- **3c. No-expiry tier — deliberately beyond a Z01 purse.** Mk 1 prices: **The Hollowing 5 gold (5,000) · Gravekeeper's Hold 8 gold (8,000) · The Threefold Ruin 15 gold (15,000)**, times Mk. Unreachable in live play this release (gift-only), but the scale is set now so Z02 authors into it; a low-level player wearing a gifted high-tier item meets the price the Q3 ruling wanted them to meet.
- **3d. Price transparency without a quote verb.** Once a curse is identified, `examine`'s revealed block gains one line — the cleansing price, through the tier formatter. Leaks nothing new (the block already keys on `is_cursed AND curse_identified`).
- **3e. Cleansing always succeeds.** No chance roll (unlike repair). The price is the whole cost, charged in one motion; insufficient funds is a world-declined refusal in the cleanser's voice that names the price — nothing spent.
- **3f. Plumbing.** All money through `currency.py` and the tier formatter; the teardown routes through the one `end_curse` path with a new cause **`cleansed`**, memorial stamped, and the curse-ending line spoken in the NPC's voice.

Recorded consequence of the #338 ruling (1a): at Mk 1 the Copper Tithe now drains ~180 vitality over its 180 s undiminished by regen, which can kill a very small bar. Ridge content is chapter 3, the wearer will be well past that size, and a curse-death cleans the item anyway — left as is, by ruling.

Related: #338 (1a), #330 (the fields this extends).


**KnightOfNight** — 2026-09-22:
Design ruling (2026-09-22, V26.3 design session — Release B, agenda item 4): **the `cleanse` grammar and the presence sweep `inspect`** (operator-confirmed 4a–4g).

- **4a. `cleanse <item>`.** §9.1 chart row: action, arguments `4 · 10` (item noun, target required; bare form draws the standard `What do you want to cleanse?` prompt). Resolution pool: **equipped items only** (the `unequip` pool). This is what makes the no-leak rule hold for free: an unequipped item misses the pool identically whether latent-cursed or clean, and every equipped cursed item is already sprung and identified. An equipped item carrying no curse draws a world-declined line in the cleanser's voice. (Edge, recorded: the documented admin bypasses — door dress/equip — can put an unsprung latent on a body; `cleanse` treats it as "nothing to cleanse" — active curses only, Q3.5 — and the admin surface is not a player leak.)
- **4b. Single target only.** No `all` form this release; one curse is one priced transaction. `cleanse all` simply misses the pool.
- **4c. Gating.** Requires a living cleanser in the room (world-declined warn otherwise — the "no repairer here" pattern; routing to the lowest-pk living `is_cleanser` NPC). Refused in combat and while dying, joining `repair` in the state-gating matrix.
- **4d. The sweep verb is `inspect`** — bare, footnote 2 (arguments ignored). Neutral across genres, reads as "have the professional look over my things," and stays distinct from `examine` (self-inspection of one item). (`check` / `appraise` were offered and declined.)
- **4e. Sweep price: per item carried.** **5 copper per instance carried**, inventory and equipped alike, charged in one motion — "look at all my stuff" costs more the more stuff you have. A typical Ridge pack of 20–40 items runs 100–200 copper, a real fraction of one boss kill, so the sweep is a deliberate purchase before a risky equip rather than a reflex after every drop. (Flat fee offered and declined: per-item scales with the work and self-limits reflex use.)
- **4f. Sweep answer.** Names every cursed instance carried (latent or already sprung) by its composed item line — **presence only, never the curse's name or effect**. Sets no flags (`curse_identified` untouched). All-clear line if nothing is cursed. Pooled lines in the cleanser's voice for both outcomes; insufficient funds is the standard refusal naming the price, nothing spent.
- **4g. Chart bookkeeping.** Both verbs join §9.1: chart rows, the state-gating matrix (refused in combat and while dying), the resolution-pools line (`cleanse → equipped only`; `inspect` takes no noun — no pool), and help entries. `cleanse` tab-completes the equipped pool; `inspect` completes the verb only.

Related: the Q6 amendment (2026-09-11) that created the sweep; Q3.5 (active curses only).


**KnightOfNight** — 2026-09-22:
Design ruling (2026-09-22, V26.3 design session — Release B, agenda item 5): **live acquisition goes on in Z01, Ridge-only, at one curse in three** (operator-confirmed 5a–5f, with 5b amended by the operator from the proposed 0.20 to an average of one in three).

Ground truth (verified in `seed_world.py` and `item_utils.py`, 2026-09-22): chapter-1 bosses (Silk Matron, Whistler Below, Dronemother) roll `uncommon: 100` and can never drop a curse under the Rare-and-up gate (`CURSE_RARITY_GATE`); the three Ridge bosses (Undercrag Weaver, Chittering King — `rare: 100`; Crowned Devourer — `epic: 100`) each guarantee one Rare/Epic item per kill on a 10-minute respawn; all 26 `CurseCandidate` rows sit on their loot definitions; `CURSE_WILD_CHANCE = 0.0` is wired and dormant.

- **5a. Ridge-only is blessed.** Curses begin in the zone's third chapter, where the player is strongest and the expiring curses are affordable lessons. Chapter-1 rarity weights are loot-economy tuning outside this ticket; untouched.
- **5b. The wild chance: one in three (operator-ruled).** `CURSE_WILD_CHANCE` becomes **1/3** — an average of one curse per three Ridge-boss Rare/Epic drops. A player farming the three bosses meets a curse roughly every third kill, each expiring inside six minutes and cleansable for 60–120 copper at the Cragfoot checkpoint.
- **5c. Nothing else drops curses.** Vendor buys are always Common; villager/hunter drops top out at Uncommon; chapter-1 bosses at Uncommon. Already true — recorded so the brief's verification pins it.
- **5d. Pools and gift path unchanged.** The 26 candidate rows and the force-curse shell path stay exactly as V26.2 shipped them.
- **5e. Where the number lives.** The existing `CURSE_WILD_CHANCE` constant in `item_utils.py` flips from `0.0` to `1 / 3` — a single tuning constant, the regen-constant precedent, not a model field.
- **5f. Playtest reality.** A one-in-three roll cannot be a playtest step's precondition. The brief's cleanse/inspect steps stage curses through the gift path; a unit test pins the roll with the random source mocked (both branches of the gate); a wild drop observed during play is recorded as a bonus, never required.


**KnightOfNight** — 2026-09-22:
Bookkeeping (2026-09-22, V26.3 design session — agenda item 6, operator-confirmed 6a–6c): **#297 is the founding ticket of milestone Version 26.3** and closes with the release ("curses work, in the world"). **#338 is the release's one dependency** (blocked-by relationship recorded on this issue); nothing else joins — one founding ticket, one brief. Queue note: **#336** (examine equipped-first) is ruled the next V26 point release after this one (26.4), shifting the previously ruled queue by one slot (26.5 #283 → 26.6 #316 → 26.7 #47 → 26.8 #220).


**KnightOfNight** — 2026-09-22:
Amendment to ruling 3b (2026-09-22, V26.3 design session, operator-confirmed): the seed carries **five** Z01 expiring curses, not four — ruling 3b omitted the **Curse of the Leaden Arm** (`curse-leaden-arm`: 25% STR cut, 300 s, pooled on iron mace and battle axe). Its Mk 1 cleanse price is **75 copper** (the Moth-Eaten Ward's — both single mild cuts of similar length). The complete Z01 expiring tier at Mk 1: Leaden Arm 75 · Craven Edge 60 · Moth-Eaten Ward 75 · Copper Tithe 90 · Whisper in the String 120. The no-expiry tier (3c) is unchanged. GDD §6.7 corrected in the same commit as the brief (`Shyland_V26.3_Brief_1_Curses_In_The_World.md`, born committed on `version_26_3`).


**KnightOfNight** — 2026-09-22:
Attestation (2026-09-22, V26.3 design session — closes discrepancy D1 from this session's V26.2 verification): **the V26.2 production seed execution is confirmed.** No committed artifact had recorded the closeout tail's `make seed-prod` run (2026-09-16), so the operator verified production directly via the Django admin: searching Effect definitions for `curse-` returns **all eight** curse definitions (Curse of the Leaden Arm, Curse of the Craven Edge, Tithe of the Copper Court, Curse of the Moth-Eaten Ward, The Whisper in the String, The Hollowing, Gravekeeper's Hold, The Threefold Ruin). Nothing from V26.2's PENDING DEPLOY-TIME ACTIONS block remains open. The V26.3 Brief 1 pre-flight cites this comment; the implementation session need not re-confirm.

Note for future closeouts: a `verify_*` command covering seed-set presence would let `make verify-prod` answer this without a manual admin check — filed separately if the operator wants it.
...
## Issue #336: examine should check equipped items first
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/336

### Body

Found during the V26.2 Brief 1 playtest (step 10, dev stack), operator-reported:

With a Rare leather cap **equipped** (carrying the live Gravekeeper's Hold test curse) and another leather cap in **inventory**, `examine leather cap` reported on the inventory copy, not the equipped one. Operator hypothesis: the inventory copy won the pick because it is higher rarity.

**Expected:** `examine` should check equipped items first — self-inspection should prefer what you're wearing over a same-noun copy in the bag.

Context for triage: `cmd_examine` builds its candidate pool as carried items + room items (`consumers.py`, `cmd_examine`; v22 B2 — "examine's pool is the union, resolved nearest-first where segments overlap") and hands it to the shared resolver; the pick order within the carried segment is what put the inventory copy ahead of the equipped one. During the playtest the operator worked around it with the numbered form.

Filed at operator direction from the V26.2 implementation session.


### Comments (1)

**KnightOfNight** — 2026-09-22:
Design ruling (2026-09-22, V26.3 design session — queue order): **this bug ships as the next Version 26 point release after 26.3** (operator-ruled). Labels `bug` + `V26` applied in the same motion; it takes its `Version 26.4` milestone when its own design session opens (milestones are born with their design session). The previously ruled V26 queue shifts one slot: 26.3 #297 → **26.4 #336** → 26.5 #283 → 26.6 #316 → 26.7 #47 → 26.8 #220.

Direction for that session, carried from the finding: `cmd_examine`'s candidate pool is carried + room, resolved by the shared resolver; within the carried segment, the pick order put a same-noun inventory copy ahead of the equipped one (rarity-aware selection picked the higher-rarity copy). Expected: self-inspection prefers what you are wearing — equipped before inventory within the carried segment, the rest of the pool order unchanged. Not `triaged`: the exact resolver change is that session's ruling.


## Issue #338: Out-of-combat DoTs race passive regen — low-Mk drains cannot kill (oscillating pin); rule dot-vs-regen interaction with #297 Release B

- State: open
- Author: KnightOfNight
- Labels: emergent, triaged, V26
- Milestone: Version 26.3
- Assignees: KnightOfNight
- Created: 2026-09-16 | Updated: 2026-09-22
- Blocked by: none
- Blocks: #297
- URL: https://github.com/KnightOfNight/games-mvc/issues/338

### Body

## Finding

Out-of-combat DoT ticks race passive vitality regen: both fire per round, and regen silently refunds part (or all) of every drain tick. At low DoT magnitudes the race inverts entirely — the DoT cannot kill, the bar oscillates at a pin, and the "drain" is theater.

Recorded as **finding F1** in the V26.2 Brief 1 closeout report (`docs/shyland/Shyland_V26.2_Brief_1_Closeout.txt`), surfaced at playtest step 9 (the Hollowing curse-death step, which failed as originally staged and had to be re-staged at Mk 5).

## Numbers from the run (character Sharon-Love, vitality_max 614)

- Passive regen: `ceil(614/300) × 3 = 9`/round out of combat (the v24.3 proportional regen law).
- **Mk 1 Hollowing: 8/round drain vs 9/round regen = net +1** — an oscillating pin, death impossible. The stuck curse had to be ended via shell `end_curse`.
- **Mk 5 Hollowing: 20/round vs 9 = net −11** — kills, but the tempo drags: regen quietly refunds nearly half the drain.

## Scope

The engine is behaving exactly as specified — this is not a v26.2 defect. The race predates the curse engine (v24.3 regen) and applies to **any** out-of-combat dot, curse-sourced or otherwise. In combat the regen pause hides it; out of combat every dot fights the regen tick for tick.

## Ruling candidates (design session)

- Dot-vs-regen interaction: should an active vitality dot pause/suppress passive regen (the in-combat precedent), or should authored curse magnitudes simply price the regen in?
- Curse kill tempo: what should a trial-by-ordeal drain feel like at each Mk once the regen question is settled?

Belongs with the **#297 Release B (curses-in-world) design session** — flagged in the V26.2 closeout as its first agenda handoff.


### Comments (1)

**KnightOfNight** — 2026-09-22:
Design ruling (2026-09-22, V26.3 design session — Release B of the #297 curse arc): **an active vitality drain pauses passive vitality regen.** Operator-confirmed 1a–1g.

**Corrected arithmetic first.** The finding's numbers assumed a 300-second regen constant. The code has been `VITALITY_REGEN_SECS = 120` since v14 (f4d212e; never 300), and Phase 4 runs every 1-second tick, so passive regen is `ceil(vitality_max / 120)` per tick = **3 × that per 3-second round**. At a 614 bar that is **18/round**, not 9: the Mk 1 Hollowing (8/round) nets **+10** (never dies), and the Mk 5 Hollowing (20/round) nets only **−2** (~15 minutes to die — the "tempo drags" observation). At a fresh 120 bar regen is 3/round. Regen is proportional to the bar (≈ max/40 per round) while drains scale with item Mk, so no authored magnitude can be correct at every level — "price the regen into the numbers" is ruled out on arithmetic alone. The finding stands, and is stronger than reported.

**The ruling:**

- **1a. Suppression rule.** While a character has any active `dot_vitality` component, passive vitality regen does not run — the same shape as the in-combat pause: nothing passively recovers while you are bleeding.
- **1b. Bar-scoped.** Only vitality regen pauses. Longevity regen and acuity drift are untouched (three independent tanks, GDD §4.4). Heal-over-time effects and consumables still work — they are effects, not passive regen.
- **1c. Source-blind.** Applies to every vitality dot: curse drains, NPC bleed/poison procs that outlive a flee, consumables. A player who flees carrying a bleed regens nothing until it expires or is answered.
- **1d. Floor-hold unchanged.** `floor_hold_vitality` is its own component type and already caps regen at the hold; it does not trigger 1a, so outside damage below the hold still regens back up to the hold.
- **1e. Silent.** Regen is silent by law; the pause adds no message — the bar simply stops climbing.
- **1f. Kill tempo that falls out.** Mk 1 Hollowing on a 614 bar becomes ~77 rounds (under 4 minutes); Mk 5 about 90 seconds. Trial by ordeal reads as an ordeal at every level. No magnitude retuning of the seeded curses.
- **1g. Bookkeeping.** This issue joins milestone **Version 26.3** as a dependency riding the Release B brief and closes with the release. GDD text lands in §4.1 (the regen law) with a cross-reference in §6.9. Implementation shape (brief-time detail, for orientation only): Phase 4's `get_regen_candidates` predicate gains an active-`dot_vitality` exclusion for the vitality arm; the longevity arm is unaffected.

Related: #297 (Release B founding ticket), #330 (the engine this rides on).
...
|---|---|---|---|---|
| 331 | Tick engine Phase 1: stacked same-character effects compute from stale per-row snapshots (duplicate arrival lines, last-writer-wins application) | KnightOfNight | bug, triaged, V26 | 2026-09-14 |
| 330 | Curse engine: schema, apply loop, lifecycle, families, seed set — acquisition admin-only (Release A of the #297 arc) | KnightOfNight | triaged, game-mechanics, V26 | 2026-09-14 |
