...
1. Hit check (v19 — contested d20 with independent critical):
   ...
   10 × (mk_tier − 1); DEX = round(18 + 2.5 × (level − 1)) + tier offset (normal +0 /
   elite +3 / boss +6 — yielding at-level hit rates of 55% / 40% / 25% at every level
   forever); STR/PER/INT = authored base + round(2.5 × (level − 1)), preserving species
...
### 6.13 Convergence Services, the Starter Kit, and Display Rules (v19)
...
### 7.5 NPC Dialogue — The Listening Model (v19)
...
Quests given by NPCs through the dialogue system (Section 7.5 — NPCs listen to `say`). Quest givers flagged in the client UI.
...
|Command     |Alias|Description                             |
|------------|-----|----------------------------------------|
|`say <text>`|—    |Speak to all players in the current room. **Some NPCs listen** (Section 7.5) and answer in the room|
...
|`wallet`|—   |Show your money (tier-formatted). `inventory` also shows a Wallet section|
...
**The output palette:** client-side styling driven by server-sent semantic categories (the server never sends hex for message text). Structural section headers share one chrome blue — *every* header, present and future. Room content near-white, prose dimmer; zone/area names in theme colors. Combat: outgoing `#C4453F`, crits `#E24B4A` bold, incoming `#E0724A` (direction readable at a glance — directional arrows were designed and **abandoned**), misses gray, kills/XP gold, success green, errors amber. Rarity colorizes the item flag block only (Common gray → Artifact red).
...
### 10.8 Standing Engineering Tenets (v19)
...
|**NPC Dialogue — deeper tiers**|The v19 listening system (Section 7.5) shipped keyword maps, greetings, and departure reactions for the Convergence roster. Future tiers: keyword vocabularies across battle zones, quest hooks, reputation-conditional responses, and Sirius-class persistent-memory entities.|
