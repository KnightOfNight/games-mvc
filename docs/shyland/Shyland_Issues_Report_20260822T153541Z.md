...
| 259 | Sirius: AI felis sapiens companion — callable, may help, gives only when he wants to (unfinished design) | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-22 |
...
| 262 | sudo AI watcher: an AI agent on the receiving end of sudo — reads the MC stream, and may or may not be allowed to answer | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-22 |
...
| 268 | MC agent runtime & operations: hosting, supervision, credentials, cost governance for the AI actor fleet | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-22 |
| 273 | MC stream carries no command outcome — readers infer refusals from adjacent out records (sudo observation) | KnightOfNight | monitoring-and-command, V25 | Version 25.5 | 2026-08-22 |
...
| 279 | MC test-agent client: operator-side script that connects to the egress and tails the event stream (CLI: URL, username, password) | KnightOfNight | monitoring-and-command |  | 2026-08-22 |
| 281 | V25.5 founding ticket: the MC agent door — query, action, and answer vocabularies (game-side actuation infrastructure) | KnightOfNight | monitoring-and-command, V25 | Version 25.5 | 2026-08-22 |
...
- Author: KnightOfNight
- Labels: monitoring-and-command, V25
- Milestone: Version 25.5
- Assignees: KnightOfNight
- Created: 2026-08-19 | Updated: 2026-08-22
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-22:
**Operator ruling (2026-08-22, V25.5 design session) — confirmed, closing with V25.5:**

The stream stays exactly as shipped — **no outcome field, no permission snapshot; the single-unconditional-tap purity stands.** Resolution is two-sided:

1. **Bot-side:** readers resolve permissions at interpretation time through the V25.5 agent-door **query vocabulary** (e.g. admin-membership resolution) — the issue body's original suspicion, realized through a mediated query rather than direct DB access (agents never hold DB credentials).
2. **Game-side, authoritative:** the **answer delivery gate** — sudo-channel answers deliver only to a currently-valid admin, whatever the bot concluded. A non-admin typing `sudo anything` gets byte-identical shipped behavior: echo, unknown-command error, bot silence.

Ruled at the named pickup point (the 25.3 deferral comment above: "the first actor session, 25.5, #262's orbit"). Joins the `Version 25.5` milestone as a dependency of founding ticket #281; closes when the release ships the delivery gate and query surface.
...
## Issue #281: V25.5 founding ticket: the MC agent door — query, action, and answer vocabularies (game-side actuation infrastructure)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/281

### Body

## Summary

The game-side half of the first-actor arc, carved from #262 by operator ruling (2026-08-22, V25.5 design session): **V25.5 ships the complete agent-facing game-engine surface — "the agent door opens." V25.6 ships the sudo bot itself (#262).** Operator's framing: get all the game engine done, then the first bot has no blockers and its release can focus on bot code and bot infrastructure (start/stop, runtime).

## Scope (per the session's rulings, recorded on #261/#262/#273)

- **Query vocabulary** on the agent surface (read-only asks, mediated game-side — agents never hold DB/Redis credentials): where-is, who's-online (presence), player detail, item-definition lookup, the live command list (feeds the bots' self-updating decline model), admin-membership resolution (#273).
- **Action vocabulary** (server-validated; every action emits an MC stream event attributed to the `agent-<name>` account): gift-existing-definition (soulbound on receipt, settled law), create-artifact, strip, dress (outfit snapshot — JSON blob storage + migration), move, answer-to-admin.
- **Answer delivery gate:** sudo-channel answers deliver only to a currently-valid admin, enforced game-side regardless of what a bot concluded (#273's authoritative half).
- **Kill switch extends over the new vocabularies** (#266 family): killed = refuse actions and queries, not just sever the tail.
- **Rendering:** the `sudo` output category + `--sudo-color` (error-color hex, deliberate-reuse pattern, separable name; set-equality test moves in the same motion).
- **Name invariant:** bot display names join the player/NPC name law — `sudo`, `Sirius` reserved against character creation's case-insensitive check; existing collisions verified at brief time.
- **Transparent effect narration** per the #261 projection ruling: value-color move lines, loot-color giving lines, standard game colors always.
- **Deterministically testable end to end via `agent-smith`** — no AI involved; the actuation path gets its test harness by construction.

## Relationships

- #261 — the arc (spine, identity, projection rulings live there)
- #262 — the bot; V25.6's founding-ticket-designate
- #273 — dependency, closes with this release
- #266 / #267 — the shipped substrate this extends
- #268 — runtime/ops arc (25.6 side)

Exact wire contracts, field names, and validation rules land in the release's brief (design session in progress; technical-coherence verification at brief-writing time per #252).
