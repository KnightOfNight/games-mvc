...
| 208 | 'inv' still shows equipment and wallet — trim to inventory only ('equip' and 'wallet' own those views now) | KnightOfNight | triaged, output, V24 | Version 24.16 | 2026-08-08 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, output, V24
- Milestone: Version 24.16
- Assignees: KnightOfNight
...
Design ruling (2026-08-07, V24.16 design session): **#208 is the founding ticket of Version 24.16** — milestone assigned in the same motion; branch `version_24_16` cut from main `be0ab01` (the 24.15 merge).

**The trim, as ruled (direction confirmed in the body and the 2026-08-07 operator comment):**

- `inv` / `inventory` renders the **Inventory table only**: the `Inventory (N/M)...` header plus the `Slot / Name / Quantity / Details` table, flat alphabetical by name, stacking rule (#18) unchanged. The Equipment paper-doll section and the Wallet line are dropped from the composite (the #90 three-part shape is retired).
- **The equipped-items query stays:** the header's capacity number is effective STR (base + gear, #100) × 10 + equipped-bag `carry_bonus` — the equipped set keeps informing the numbers without being displayed.
- **The paper-doll composition and its helper are untouched** — bare `equip` (#195) becomes the helper's sole consumer. `wallet` already owns the money line; its shared renderer is untouched.
- **Help text:** the `inventory (inv)` description becomes `Show your inventory.`
- **GDD text (ships with this release, per design-ahead):** §6.11 restructured — `inv` = the single Inventory table; the paper-doll paragraph re-attributed to bare `equip`; the wallet-line item dropped (its byte-identical rule is moot once nothing duplicates the line). §9.1 footnote 21 re-anchored (the "identical composition `inv` shows / can never drift" clause dies with the composite). §9 display-standards "Shipped surfaces" `inv` entry updated.
- **Release shape:** runtime-only — no migration, no seed changes, no data actions, expected deletions 0; the architecture doc hash moves (consumer output-composition change).

Cold-start-ready — `triaged` applied in the same motion.
