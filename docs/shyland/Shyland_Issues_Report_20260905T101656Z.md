...
| 321 | Bulk buy/sell at admin-scale quantities takes minutes with zero output — sell is O(n) transactions, buy O(qty) serial INSERTs | KnightOfNight | triaged, commands | Version 25.15 | 2026-09-05 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, commands
- Milestone: Version 25.15
- Assignees: KnightOfNight
- Created: 2026-09-01 | Updated: 2026-09-05
- Blocked by: none
- Blocks: none
...
Make the cap smart e.g. 1 sword or 25 healing draughts.

**KnightOfNight** — 2026-09-05:
**Design ruling (2026-09-05, V25.15 design session — founding ticket):**

The earlier "cheap cap" comment on this issue is **superseded by operator ruling in-session**: V25.15 ships **engine-side batching**, no quantity cap, no player-facing behavior change. The admin-scale inventories that surfaced this are off-design, but they point at real future scaling; batching removes the O(n) shape outright instead of fencing it off.

**Ruled direction (implementation brief to follow on `version_25_15`):**

- **Sell, bulk arm:** the per-item `do_sell` loop is replaced by one atomic transaction per bulk sale — every item's sale price computed in Python first, then one `SELECT FOR UPDATE` on the character row, one copper `UPDATE` with the summed total, one bulk `DELETE` of all sold instances. Crash semantics improve from per-item partial progress to all-or-nothing rollback. The single-item sell arm and its `do_sell` stay as shipped.
- **Buy:** `do_buy` keeps its single all-or-nothing transaction (#22) but splits generation from persistence — all N instances rolled unsaved via `generate_item_instance` (already built to return unsaved instances), then persisted in one `bulk_create`. Lock hold time drops from O(qty) serial INSERTs to milliseconds.
- **Output is unchanged byte-for-byte** — the aggregated count-form lines already compose after the work; only the work gets fast.

Scope per the scope law: vendor transactions (`buy`/`sell`) only — `drop`/`pickup`/`use` per-item loops are out of scope for this release.

Founding issue of **Version 25.15**; milestone assigned in the same motion.


**KnightOfNight** — 2026-09-05:
Operator confirmation (2026-09-05, V25.15 design session): no GDD edit for this release — batching is an architecture change only; §6 compensated-disposal text stands as written. The architecture doc (§4.1/§4.3, stamp 25.15, hash moves) is the documentation home, updated by the implementation session per Brief 1 step 4.8.
