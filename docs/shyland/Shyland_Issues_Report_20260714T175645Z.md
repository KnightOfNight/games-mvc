...
| 50 | map only displays one circle with gray lines when you enter a new room with agro | bug | Version 20 | 2026-07-14 |
...
## Issue #50: map only displays one circle with gray lines when you enter a new room with agro
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/50

### Body

The previous room you just came from isn't shown.  The map is missing pieces.

Maybe the problem is just combat or with the MagFrag containing 'The Whistle Throat'

Doesn't happen in 'The Entry Cleft' but I was there before.  The problems seen in Whistle were my first time visiting.

Revisiting the Whistle from the south, I see the "D" indicator is gray like I haven't been there before.


### Comments (1)

**KnightOfNight** — 2026-07-14:
Diagnosed (design chat, 2026-07-14) and confirmed by playtest screenshots. Root cause: RoomVisit recording is coupled to room-description rendering. The visit is created inside _resolve_room_rendering (consumers.py ~1983), which is only reached via send_room_description — and cmd_move's aggro branch deliberately skips the room description (ambush design) and calls start_combat instead. Result: ROOMS ENTERED UNDER AGGRO NEVER RECORD A ROOMVISIT.

While standing in such a room, the map's the-room-underfoot-is-always-known special case masks the bug; on leaving, the room drops out of fog-of-war as if never visited. Every symptom follows: holes in the map behind you in battle-zone caves (the Whistling Sink's first entries are almost all aggro entries), lines missing because lines only draw between two drawn rooms and the intermediate room is absent, and revisits reading as first visits. The screenshot pair in The Wind Gallery is the control case: mid-combat first pass shows holes; after peaceful re-walking of the same rooms, the chain renders connected — visits recorded only on the quiet entries.

Latent since v19 Brief 8 (which moved visit recording into the rendering path for the first-entry-always-long-description rule); invisible until the map made visit state visible. Also means aggro first entries never got the guaranteed long description.

Fix (v20 Brief 1 Amendment 2, delivered): decouple — record the visit unconditionally at arrival time (move, travel, respawn), make _resolve_room_rendering read visit state rather than create it, preserve the first-entry long-description behavior for peaceful entries. No data repair needed: missing visits self-heal on next entry to each room.
