# Shyland Project Instructions v28

**Status: ACTIVE — approved by the operator 2026-07-29. Supersedes Shyland_Project_Instructions_v27.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v28 edition is the playtest-disposition pass (#170): implementation sessions cannot end without an explicit operator playtest disposition recorded in the closeout report, the closeout entry gate reads dispositions from committed reports, and serial implementations are doctrine. (v27 added the GDD-errata rule; v26 the end rituals and session-end gate; v25 the closeout session type and closeout-tail deploy.)
