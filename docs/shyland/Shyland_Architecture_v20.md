...
> Authoritative technical reference as of commit 07c751f (v20 brief 5 + amendment 1: output palette, look sections (no bracket header, zone-colored separator), command echo, NPC article grammar with indefinite introductions, corpse-decay/empty-loot messaging; v20 brief 4 + amendment 1: UI layout skeleton, location bar, fight feed, connection indicator, stats bars, viewport-fit; v20 brief 3 + amendment 1: command grammar resolver, loot room sweep, bulk-sale message splitting, dispatch guard, combat movement block, timestamps toggle, rarity flag block, tab completion; v20 brief 2 + amendment 1: output message envelope, events-not-renderings display categorization; v20 brief 1: map system — commits 02758b2 model + verification, 9599e11 world re-lay, 4289902 map payload, 963ddf2 map client).
...
- **`get_item_flags(item)`** — the trailing flag block: `[<Rarity>, Bound|Droppable]` for identified items (rarity capitalized; `Bound` = soulbound by any route, `Droppable` = unbound), `[Bound]`/`[Droppable]` alone for unidentified items, which never reveal rarity.
...
**Item-line composition (#48).** See Section 4.6: `compose_item_line()` renders every item line as `<name with tier>[ xN]  <suffix>  [<Rarity>, Bound|Droppable]`. Inventory grouping (consumables by definition/tier/rarity) feeds its count through the helper's `xN`; the room render listed ground items under an interim `On the ground:` section, which v20 brief 5 absorbed into the `What's here?` section (Section 4.16) — composed lines, identical lines collapsed to `xN`, unchanged.
...
**The output palette (#13 — authoritative).** All output colorization is client-side styling keyed on server-supplied **semantic categories**; the server never sends hex for message text (the sole exception remains the zone/area `theme_color` values riding location updates and the room-render header, which come from model fields). The palette lives in one CSS block in `game.html`:

| Role | Category / class | Color |
|---|---|---|
| Structural section headers (#39) | `.msg-section strong` | `#7FB3D5` |
| Room content (names, occupants, items) | `.room-content` | `#E8E4D8` |
| Room/area prose | `.msg-room-render`, `.msg-report` | `#B8B4A6` |
