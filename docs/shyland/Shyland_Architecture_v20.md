...
| **Unstamped** | `room-render` | The **entire room-rendering block**, on both entry and `look` — since brief 5 one structured message: header object, prose, exits, players, `who`/`what` line arrays (Section 4.16) |
...
**Look-command output sections (#14/#39).** `look` and room entry render in this exact order — header `[ Area — Room ]`, prose, `Exits:`, `Who's here?`, `What's here?` — with a blank line between sections and **empty sections omitted entirely**. The room render is now **one structured `room-render` message**: `header` (`area_name`, `area_color` from the model — the same value the location bar shows — and `room_name`), `text` (prose only; the old text-embedded header line is gone), `exits` (comma list, one line after its label), `players`, and `who`/`what` string arrays. `Who's here?` lists living non-fixture NPCs, one composed uncapitalized reference per line (`the test vole is here.`, `a Verdant Shard is here.`); **`What's here?` is THE things/items section**: fixture NPCs, corpse `… lies here.` lines, and ground items as composed item lines (identical lines collapsed to `xN`) — the brief-3 interim `On the ground:` section is absorbed; exactly one items section exists. Every structural section-header label, present and future, renders `#7FB3D5` (`Exits:`, `Who's here?`, `What's here?`, `Players here:`, and any later label); labels and only labels use that color. The Verdant Shard was flagged `is_fixture=True` in the seed so it renders under `What's here?` per the brief's ruled example. Brief mode is unchanged; `look` always long (standing rule).
