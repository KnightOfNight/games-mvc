...
> Authoritative technical reference as of commit 3616fe5 (v20 brief 3: command grammar resolver, loot/sell all, dispatch guard, combat movement block, timestamps toggle, rarity flag block, tab completion; v20 brief 2 + amendment 1: output message envelope, events-not-renderings display categorization; v20 brief 1: map system — commits 02758b2 model + verification, 9599e11 world re-lay, 4289902 map payload, 963ddf2 map client).
...
**Command adoption (#3, #21).** All eleven object commands resolve through the module. `loot all` (and bare `loot`) takes everything on the targeted corpse, listing each item as a composed line; `sell all <noun>` / `sell N <noun>` / `sell all <rarity>` emit each sale line as before plus one total (`Sold 3 items for 2 silvers, 4 coppers.`), skipping zero-value items with a summary mention (`(2 worthless items skipped.)`); single-item sells keep the `That's not worth anything to me.` refusal. `buy N` checks funds and carry capacity up front for the whole quantity and `do_buy(entry, character, qty)` performs the entire purchase in **one transaction** — never partial. A verification-time fix rode along: `check_corpse_empty_and_delete` now queries the table directly instead of `corpse.contents.exists()`, which read a stale `prefetch_related` cache and left emptied corpses undeleted until a second loot or decay.
