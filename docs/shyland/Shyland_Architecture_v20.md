...
> Authoritative technical reference as of commit 963ddf2 (v20 brief 1: map system — commits 02758b2 model + verification, 9599e11 world re-lay, 4289902 map payload, 963ddf2 map client).
...
**Rendering semantics — a single `_resolve_room_rendering(character, room, force_long)` replaces the old `_check_and_record_visit`/`_record_visit` pair.** The old code recorded the `RoomVisit` and decided brief-vs-long in two different branches depending on `force_long` (`look`), and unconditionally prepended the area description regardless of which form rendered. The new function always records the `RoomVisit` first (`get_or_create`, the bookkeeping itself unchanged — travel revelation still depends on it), then returns whether the **long** form (long description + area description, if any) should render:
...
**Client message-type inventory** (for reference — the full set the client handles as of v20 brief 1): `output`, `status`, `clear`, `redirect`, `map`, plus the `quit` event.
