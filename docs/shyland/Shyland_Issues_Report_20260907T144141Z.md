...
| 326 | sudo bot: request failures are silent to the requesting admin — operator wants a legible in-pane line whenever the bot is running | KnightOfNight | triaged, monitoring-and-command | Version 25.17 | 2026-09-07 |
...
## Issue #326: sudo bot: request failures are silent to the requesting admin — operator wants a legible in-pane line whenever the bot is running

- State: open
- Author: KnightOfNight
- Labels: triaged, monitoring-and-command
- Milestone: Version 25.17
- Assignees: KnightOfNight
- Created: 2026-09-06 | Updated: 2026-09-07
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/326

### Body

Found during the V25.16 Brief 1 playtest (2026-09-06, dev stack): the Anthropic API began refusing the sudo bot's model calls with a billing error (`400 invalid_request_error: credit balance is too low`). The bot handled it as designed — caught the exception per request, logged `sudo request from <name> failed silently` with the traceback, stayed attached, and kept running — but the requesting admin got nothing in the pane. From the player side this is indistinguishable from the bot being down; the operator only diagnosed it by reading the host-side log.

The bot's silence doctrine ("silence is never an error") was designed for reconnect/kill-switch states, where quiet retry is correct. A hard model-API failure on a live request is a different case: the admin asked a question and the machinery knows it failed.

**Operator direction (2026-09-06, verbatim intent):** no more silent failure from the bot unless it is not running at all. If the bot is up and a request fails — model API error, billing, auth, whatever — the admin should get a legible in-pane line rather than silence.

Design questions for the ruling session: what the in-pane line says (and its category), whether transient model errors that succeed on retry stay quiet, and whether billing/auth failures should be distinguished from transient ones.

_Filed from the V25.16 implementation session; observed on the dev bot running `version_25_16` code (pid alive throughout, three silent failures logged 13:51–13:54 UTC)._


### Comments (3)

**KnightOfNight** — 2026-09-07:
**Founding ruling (2026-09-07, V25.17 design session):** this is the founding ticket for **Version 25.17** — milestone assigned, `monitoring-and-command` capability label applied (the sudo bot is MC surface). The session's design ruling on the open questions (line wording/category, retry posture, whether billing/auth failures are distinguished from transients) follows in this conversation and will be recorded here.


**KnightOfNight** — 2026-09-07:
**Design ruling (2026-09-07, V25.17 design session, operator-confirmed):** the three open questions are ruled as follows.

1. **Line + category:** when the bot is running and a request terminally fails, the requesting admin gets exactly one legible in-pane line, delivered through the door's shipped `answer` action, in sudo's established voice (error-color talking color, per the #262 ruling). The line is generic and legible — it names the failure class, never the raw API error detail; the detail stays in the host-side log as today.
2. **Retry posture:** a transient error that succeeds on retry stays quiet — the answer arriving is the report. Only a terminally failed request speaks. The bot's silence doctrine ("silence is never an error") survives intact for reconnect/kill-switch states — this ruling carves out exactly the case where the bot is up, an admin asked, and the machinery knows the request failed.
3. **Failure classes:** distinguished coarsely, two wordings only — **transient** (worth trying again) vs. **persistent/needs-operator** (billing, auth — retrying won't help; the operator's attention is required). No finer taxonomy; no secrets, account, or billing detail ever reaches the pane.

Scope note: the fix is expected to live entirely bot-side (`agents/` code) — catch → classify → deliver via `answer`. The #273 admins-only `answer` gate is never a limitation for sudo (its whole audience is admins); #282 is not implicated. V25.17 ships this ticket alone (scope-law review of all 38 open issues found no same-problem dependency).

Brief to follow on `version_25_17`. `triaged` applied in the same motion as this ruling.


**KnightOfNight** — 2026-09-07:
Brief committed: `docs/shyland/Shyland_V25.17_Brief_1_Legible_Bot_Failure_Reporting.md` on `version_25_17` @ 7c1506b (GDD §10 doctrine paragraph landed at 29c3b45). Playtest plan per operator direction (2026-09-07): chaos monkey by configuration, no code hacks — persistent class via an intentionally bad API key in `agents/.secrets/anthropic-api-key.sudo` (the bot only checks the key is set, so it attaches and fails on the first model call, reproducing the incident shape); transient class via `ANTHROPIC_BASE_URL` pointed at an unreachable endpoint.
