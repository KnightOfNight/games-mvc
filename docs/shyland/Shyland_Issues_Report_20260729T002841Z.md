...
| 149 | Using a consumable prints two lines — merge the use sentence and the effect message into one | KnightOfNight | B1, triaged, output | Version 23.3 | 2026-07-29 |
| 151 | Multi-use healing floods the pane — compute the heal once, consume what's needed, send one message | KnightOfNight | B1, triaged, output | Version 23.3 | 2026-07-29 |
...
- State: open
- Author: KnightOfNight
- Labels: B1, triaged, output
- Milestone: Version 23.3
- Assignees: KnightOfNight
- Created: 2026-07-25 | Updated: 2026-07-29
- Blocked by: none
- Blocks: #151
...
Design ruling (2026-07-28, Version 23.3 design session): **#149 is the founding ticket of point release Version 23.3** (milestone #17, branch `version_23_3`). #151 is included as a dependency under the point-release scope law — the existing blocked-by link stands; it describes the same use-command output-composition problem, not a widening.

Remaining design questions (merge point, merged-line shape, non-healing consumables) will be ruled in this session; the implementation brief will be committed on `version_23_3`.


**KnightOfNight** — 2026-07-29:
Design rulings confirmed by the operator (2026-07-28, Version 23.3 design session):

1. **Merge point and shape:** the effect layer returns its message as a *clause*; `cmd_use` composes one sentence, one envelope: `You use a Healing Draught Mk 1 and feel your body recover. (+25 Vitality)`. Connective merge. Timed-effect consumables (whose apply-time message list is empty — e.g. Focus Tonic) already print one line and are unchanged. Multiple instant components join their clauses in order.
2. **Category:** the merged line carries **success** (transactional green), replacing the old system-category effect line. Exception per #151 ruling 5: when a multi-use aggregate reaches full health, the single line is **reward**-category.
3. Single-item use keeps the `item_ref` indefinite-article form (no `×1`); the count form applies to aggregates only (see #151).

Aggregation rulings (3–7) are recorded on #151. Brief 1 of Version 23.3 will implement both issues together on `version_23_3`.
...
**KnightOfNight** — 2026-07-29:
Design ruling (2026-07-28, Version 23.3 design session): included in point release **Version 23.3** as a dependency of founding ticket #149 (point-release scope law — same use-output problem). The explicit reconciliation with the v22 B2 Amendment 5 per-line ruling ("use, repair, and loot stay per-line") will be ruled in this session before the brief is written.


**KnightOfNight** — 2026-07-29:
Design rulings confirmed by the operator (2026-07-28, Version 23.3 design session) — numbering continues from #149's merge rulings:

3. **Aggregation scope:** up-front computation applies to **instant-restore healing consumables only** (all effect components instantaneous, at least one `restore_vitality`). Timed-effect consumables keep the existing per-use loop — reapplication rules make multi-using them near-meaningless, and no aggregation math exists for them.
4. **Aggregate line composition:** Amendment-5 count form (plural-free until #126 ships): `You use Healing Draught Mk 1 ×3 and feel your body recover. (+75 Vitality)`. Mixed-Mk consumption (lowest-value-first, unchanged) joins groups with commas: `You use Healing Draught Mk 1 ×2, Healing Draught Mk 2 ×1 and feel your body recover. (+85 Vitality)`.
5. **Full-heal fold:** when the aggregate reaches full, the single line ends `… You are restored to full health.` and the whole line renders **reward**-category — no second message. The shortfall warn (`You only had N.`, #132 doctrine) stays a **separate warn line**, firing only when the request exceeded inventory *and* the deficit wasn't covered.
6. **Dying/revival:** unchanged — while dying, `use` consumes exactly one restorative and runs the v19 revival sequence; aggregation applies only to the non-dying path. No over-drinking past revival.
7. **Amendment 5 reconciliation (explicit):** the v22 B2 Amendment 5 per-line ruling is **superseded for `use` only** — multi-use is one computed transaction, one line, one status update. **Repair and loot remain per-line**; each of their iterations is still its own news.

The existing #61 full-health refusal (`You are already at full health.`, warn) is retained as the entry gate. Implementation: Brief 1 of Version 23.3 on `version_23_3`, together with #149.
