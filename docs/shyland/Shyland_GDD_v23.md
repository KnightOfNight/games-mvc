...
- **`home` (v22)** — the hearth command: a 10-second delayed return to the Heart of the Convergence, from anywhere, narrated as fog-motif atmosphere; broken by movement, combat, or `cancel`; 5-minute completion-only cooldown (timings retuned v23.4, #162, pending implementation). The Heart is the only destination until attunement (#38) ships. Full design in Section 2.11.
...
`home` is the way back when there is no obelisk near: a **10-second delayed return to the Heart of the Convergence** (v23.4, #162, pending implementation; retuned from 15 seconds — 5 was considered and rejected as near-instant escape), usable anywhere, in home's own fog-motif voice — a cousin of obelisk travel's machinery pattern, never its words.
