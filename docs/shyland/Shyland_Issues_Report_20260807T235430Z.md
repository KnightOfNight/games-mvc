...
| 130 | Secondary-stat curves vs Mk band growth — audit before Mk 2 content | KnightOfNight | triaged, V24, game-balance | Version 24.14 | 2026-08-07 |
...
- Milestone: Version 24.14
- Assignees: KnightOfNight
- Created: 2026-07-21 | Updated: 2026-08-07
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-07, V24.14 design session): **#130 is the founding ticket of Version 24.14** — the second release of Phase 3 (Mk 2 balance) of the V24 series plan (#139), following #104 (V24.13, NPC HP Mk scaling, shipped). Milestone `Version 24.14` (#34) assigned; branch `version_24_14` cut from main `d764738`.

Scope per this issue's body: audit the secondary-stat midpoint curves (`scaling_base + scaling_factor × mk_tier`) against Mk band growth — now concretely anchored by the shipped #104 lift (NPC HP × 1.75 per band) — and retune the seed curves so flat-value effects (lifesteal, proc factors, +N stat bonuses) hold their relevance across bands. Pure seed-data retune under the code-is-definitive rule; no code rework expected.

The audit tables, retune numbers, and the implementation brief follow in this session; the `triaged` label lands with the completed ruling, not before.


**KnightOfNight** — 2026-08-07:
**Ruling of record (2026-08-07, V24.14 design session — operator-confirmed): "gear flats track the band."**

Audit finding: every gear-rolled stat entry grows ~×1.28 per Mk band as seeded (factor 0.1–0.5 per tier) while the numbers gear competes against grow ~×1.75 per band (NPC HP by #104's shipped lift; NPC contest stats +25/band; player primary stats 28→53→78 at band mid-levels). Flat-value effects rot to noise by Mk 3 exactly as this issue predicted. The #104 lift is linear in `mk_tier`, so it is expressible **exactly** in the shipped midpoint formula `base + factor × mk_tier` — this is a pure seed-data retune, zero code, zero migration, and every Mk 1 midpoint is preserved byte-identical (live Mk 1 world untouched).

The transformation, per stat entry with Mk 1 midpoint `m1 = base + factor` (floors analogously with `f1 = floor_base + floor_factor`):

| Class | Stats | New curve | Effect |
|---|---|---|---|
| **Full band lift** | str, dex, end, int, wis, per, lifesteal, electric_damage_bonus, physical_resist + inert flats (spell_damage_bonus, mana_regen, radiation_resist, magic_resist) | `base = 0.25×m1`, `factor = 0.75×m1` | value = m1 × (1 + 0.75×(mk−1)) — tracks NPC HP growth exactly; stat-share drift <10% over three bands |
| **Half-power lift** | bleed_factor, stun_factor, poison_factor, flame_factor (V curves, pool or primary) | `base = 0.625×m1`, `factor = 0.375×m1` | V pays twice (chance = V×5% AND size = randint bound); half-power holds proc damage share constant vs √1.75-per-band (Mk 1 and Mk 3 exact, +4% at Mk 2) |
| **Full lift on #127 floors** | flame-projector floor 8/4 → **3/9**; dart-caster floor 5/3 → **2/6** | `floor_base = 0.25×f1`, `floor_factor = 0.75×f1` | floor is pure size (no chance duty); Mk 1 floors 12 and 8 preserved |
| **Exempt** | crit_chance | unchanged | probability stat — band-invariant value by nature; no rot exists |

Identity-proc V curves (2.0/1.0 on the #127 pair) take the half-power lift → 1.875/1.125, whose Mk 2 value 4.125 nearly re-derives the deliberately authored Mk 2 value of 4 — coherence confirmation, and this ruling supersedes #127's authored curve for those two entries.

Out of scope, deliberately: weapon damage curves (`scaling_base`/`scaling_factor` on definitions), `armor_base` (governed by #129), `ARMOR_MITIGATION_K`, and the composite-strike slot factors (flagged in code as a Phase 3 constants edit — separate ticket if pursued).

Ship shape: seed-data-only change to `seed_world.py` stat-entry literals (45 definitions, 146 entries, mechanical per the table above); reseed carries expected deletions 0; production execution is the closeout tail's `make seed-prod` (PENDING DEPLOY-TIME ACTIONS). GDD §6 text and implementation brief land on `version_24_14` in this session. `triaged` applied — cold-start-ready.
