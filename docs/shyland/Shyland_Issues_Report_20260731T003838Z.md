...
| 139 | Healing consumables can't track vitality growth — the draught tier needs an evolution pass | KnightOfNight | triaged, V24, healing-economy | Version 24.0 | 2026-07-31 |
...
| 164 | Healing economics: draught dependence outruns player income — sell values and money drops can't fund the staple | KnightOfNight | V24, healing-economy |  | 2026-07-31 |
| 165 | Out-of-combat vitality regen is too slow and doesn't scale | KnightOfNight | triaged, V24, healing-economy |  | 2026-07-31 |
| 166 | Top-level heal command — 'use infinity healing draughts' semantics | KnightOfNight | triaged, V24, healing-economy |  | 2026-07-31 |
| 174 | Admin command: uptime (container uptimes, disk free, reclaimable space) | KnightOfNight |  |  | 2026-07-30 |
| 176 | Equipment paper-doll renders consumed hand slots as free (two-handed weapons) | KnightOfNight | output, V24, itemization |  | 2026-07-30 |
| 177 | Combat ignores weapon slots — only one equipped weapon ever attacks; dual-wield contributes nothing | KnightOfNight | V24, game-balance, itemization |  | 2026-07-30 |
| 178 | Ranged slot semantics unruled — stat-swap flag only; equip asymmetry with two-handers; Pulse Pistol has no acquisition path | KnightOfNight | V24, itemization |  | 2026-07-30 |
| 179 | New-zone design rules (collecting issue) | KnightOfNight |  |  | 2026-07-30 |
| 180 | Post-gear-wiring fight-cost survey: measured HP loss, duration, and draughts-per-fight across NPC tiers | KnightOfNight | V24, healing-economy, game-balance |  | 2026-07-31 |
| 181 | Loot-table enrichment: healing draughts (and other items) as NPC drops | KnightOfNight | V24, healing-economy |  | 2026-07-31 |
...
**VapeRaiser** — 2026-07-29:
TOTP via django-otp is a solid choice — it integrates cleanly with Django's auth system. If you need a quick way to test TOTP code generation during development, https://2faco.com is a free client-side 2FA tool that runs entirely in the browser. Useful for validating that your django-otp setup is generating the right codes without needing a separate mobile app.
...
- State: open
- Author: KnightOfNight
- Labels: triaged, V24, healing-economy
- Milestone: Version 24.0
- Assignees: KnightOfNight
- Created: 2026-07-22 | Updated: 2026-07-31
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-07-30:
Design ruling (2026-07-30, V24.0 design session): the healing economy is part of the Version 24 (new-zone-prep) major plan — V24 label added alongside healing-economy. Intermediate zones (Mk 2, levels 11-20) cannot ship on the current healing math; this issue's design pass is on the V24 series path. Series order and release assignment to be ruled separately.


**KnightOfNight** — 2026-07-30:
Design ruling (2026-07-30, V24.0 design session) — **Version 24 series plan of record** (operator-confirmed):

**This issue is the founding ticket of Version 24.0**, the first release of the Version 24 (new-zone-prep) major.

The V24 queue (17 issues) drains in four ruled phases. A phase is a bucket of related issues spanning one or more point releases — the scope law (one founding ticket, one brief per release) binds every release; the phase says which bucket the next founding ticket is drawn from until it empties. Within a phase, release-by-release order is a per-design-session call (v30).

- **Phase 1 — Healing economy:** #139 (founding ticket, 24.0), #164, #165, #166. First: live player pain today, and every later balance number is tuned against whatever healing becomes. Design pass may rule all four together; implementation drains release by release.
- **Phase 2 — Itemization structure:** #127, #129, #80, #134. Deliberately before the balance phase: #129 changes armor/TAV composition and #127 adds a stat family — the curve audit and NPC tuning must run against final item structure.
- **Phase 3 — Mk 2 balance:** #104 (the ruled hard blocker for Mk 2 spawns), #130, #26, #105, #142. Last math phase so numbers are computed once, against settled healing and items, fresh for V25 zone authoring.
- **Phase 4 — Travel & world:** #30, #38, #95, #41. Independent of the math; needed by V25 ship time, not V25 design time.

Goal of the series: Version 25.0 ships new zones (Z02/Z03 — Intermediate, Mk 2) with no blockers.


**KnightOfNight** — 2026-07-30:
Plan-of-record addendum (2026-07-30, V24.0 design session, operator-confirmed): three gameplay-found issues join the V24 queue in **Phase 2 (itemization structure)** — #178 (Ranged slot semantics), #177 (combat ignores weapon slots / dual-wield inert; also labeled game-balance), #176 (paper-doll consumed-slot display). Queue is now 20.

Phase 2's internal design arc: slot semantics (#178) → multi-weapon damage model (#177) → ranged weapon family (#127) → armor base (#129), with #80/#134/#176 as smaller riders. The three new issues are one design conversation, likely three releases (#178/#177 separable as filed; #176 ships small after the rulings it depends on).

Ordering consequence: the itemization-before-balance rule hardens — #177's multi-weapon damage ruling moves player DPS, so Phase 3 (#104 NPC HP, #130 curve audit) must follow it. Phase 1 (healing) is unaffected and remains first; Version 24.0's founding ticket remains #139.


**KnightOfNight** — 2026-07-31:
**Design ruling (2026-07-30, V24.0 design session — operator-confirmed). The draught shape is ruled: percentage-of-max, with Mk buying speed.**

**The law:** a Healing Draught restores a percentage of the drinker's `vitality_max`:

> **heal = (15% + 5% × Mk) of vitality_max, minimum 25 HP flat**

- Mk 1 = 20% — five drinks from zero to full at every level. Percentage is of **max, not deficit** (deficit-based would recreate the regen slow-tail disease).
- **The Mk axis buys rounds, not raw HP:** Mk 2 = 25%, Mk 5 = 40% — higher tiers cost more (`base_value × mk_tier`, unchanged) and heal a bigger fraction per 3-second combat round. The premium buys action economy, which is what matters in a boss fight; per-copper efficiency deliberately does not improve with tier.
- **The 25 HP floor** keeps every player at least as well off as the current item (20 + 5×Mk); it goes dead around L4 and never matters again.
- No HoT variant now (stays open as future economy texture, to be weighed after the #165 regen ruling). No new named items — the Mk system is the ladder.
- Unchanged and binding: #61 full-vitality refusal, #151 single-message aggregate with consume-only-what's-needed, #168 oldest-first consumption.
- **Vendor stocking consequence:** no stocking change — all current zones are Mk 1 band; Mk 2 draughts enter the world with Mk 2 vendors/zones (V25).

**Rationale:** any fixed-number heal fails *within* a Mk band, not just across bands — vitality spans ~100→~600 inside the Mk 1 band alone (6×), so retuned absolute curves and named ladders serve the band edges badly forever. Percentage-of-max serves every level equally and can never diverge from the vitality formula again.

**Implementation surface (for the brief):** new `restore_vitality_percent` effect component type (magnitude as the Mk-scaled fraction, authored flat floor), draught EffectComponent seed change, GDD §6.9 vocabulary row. The #61/#151 code paths are untouched in behavior.

This is the founding ruling of Version 24.0. GDD §6 text lands on `version_24_0` with a "(v24.0, pending implementation)" marker.


**KnightOfNight** — 2026-07-31:
Plan-of-record addendum 2 (2026-07-30, V24.0 design session, operator-confirmed): two issues join **Phase 1 (healing economy)** from the #164 income ruling — **#180** (post-gear-wiring fight-cost survey; blocks #164; dataset also feeds Phase 3 #104/#130 and the boss-budget re-derivation) and **#181** (loot-table enrichment — draughts and other items as drops; the lore-clean income faucet after blanket copper on grind mobs was rejected). Queue is now 22.

Phase 1 sequence (amended): **24.0 = #139 (Draught Law, ruled — GDD §6.9 committed with v24.0 marker)** → next release = #180 (survey) → data-informed #164 table ruling in a design session → #164 release; #181 coordinates with #164's numbers; #165/#166 measurement-independent, slot per design-session call.


**KnightOfNight** — 2026-07-31:
Brief committed (2026-07-30, V24.0 design session): Shyland_V24.0_Brief_1_Draught_Law.md on version_24_0 at 4199017. Actionable when the operator directs an implementation session to it by name on this branch (CLAUDE.md Rule 4). Carries the version-start ritual (24.0-DEV bump + deploy-dev) as its opening act, the migration for the component-type choices change, the seed reconcile (0 deletions / 1 updated row), the PENDING DEPLOY-TIME ACTIONS block (production make seed at the closeout tail), and the Architecture v24 creation as the gated last step.
...
## Issue #164: Healing economics: draught dependence outruns player income — sell values and money drops can't fund the staple

- State: open
- Author: KnightOfNight
- Labels: V24, healing-economy
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-29 | Updated: 2026-07-31
- Blocked by: #180
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/164

### Body

## Problem

Players are dependent on healing draughts, but the economy cannot fund that dependence. The income faucets — selling combat materials (hides, carapaces) and NPC money drops — yield too little relative to draught cost, so the survival staple runs at a structural deficit.

Field evidence (operator, 2026-07-29):

- **Shy-Guy** was gifted 100 gold long ago and has been spending it down ever since — operator-gifted capital has been masking the deficit, not solving it.
- **Harley** started with nothing and it has been a struggle to even afford draughts — the from-scratch experience is underwater.

## Scope

Both sides of the ledger are in play, plus the mechanics that interact with it:

- **Income side:** material sell values (hides, carapaces, the broader material table) and NPC money-drop amounts (min/max × Mk formula).
- **Cost side:** draught pricing — likely folds into the #139 draught-tier evolution pass (draughts that can't track vitality growth and draughts that can't be afforded are two faces of the same redesign).
- **Time side:** out-of-combat regen is too slow and doesn't scale, so time can't substitute for money either — filed separately (see the Healing Economy milestone), but the balance target must consider both together.
- **UX side:** the top-level `heal` command (filed separately) and the multi-use output fix (#151).

## Goal

A player should be able to sustain normal draught consumption from normal play — no operator gifts required — while draughts stay a real cost (a meaningful sink, not free). The design session sets the actual balance targets.

Related: #139 (draught evolution — may absorb the cost side), #151 (multi-heal output), #125 (aliases — the heal command's origin story).
...
**KnightOfNight** — 2026-07-30:
Design ruling (2026-07-30, V24.0 design session): series phase order ruled — this issue is in **Phase 1 (healing economy)**, first of four (healing economy → itemization structure → Mk 2 balance → travel & world). Each phase spans one or more point releases; release-by-release order is ruled per design session. Full series plan recorded on #139 (Version 24.0 founding ticket).


**KnightOfNight** — 2026-07-31:
**Design ruling (2026-07-30, V24.0 design session — operator-confirmed):**

1. **The income law — provisional:** expected income per fight ≈ **2 ×** the expected draught cost of that fight (under the #139 Draught Law), at on-level even-split play. Confirmed "for now" — the multiplier is explicitly open to fix-or-keep once measured data is in.

2. **Faucets are lore-constrained (operator ruling):** blanket copper on aggressive mobs is **rejected** — beasts and insects don't carry coins, and copper drops there would change the zone lore. Instead:
   - **Bosses** keep dropping copper; their numbers go **up**
   - **Existing copper-dropping NPCs** (villagers/humanoids): numbers go **up**
   - **Loot in kind:** grind mobs feed the healing economy through enriched loot tables — healing draughts and possibly other items (#181, filed from this ruling). A looted draught counts in this issue's income equation (avoided purchase if drunk, sale value if sold).

3. **The numbers are deferred pending fresh data:** this issue is now **blocked by #180** (post-gear-wiring fight-cost survey) — the last feasibility dataset (#89, v21) predates the v22 gear wiring, and the operator ruled data-first. The per-tier sketch table from the design conversation is non-binding.

4. **Phase 1 sequence (amended, operator-confirmed):** 24.0 = #139 (Draught Law) → next release = #180 (survey) → data-informed table ruling in a design session → this issue's release. #165/#166 are measurement-independent and slot per design-session call; #181 coordinates with this issue's numbers.


## Issue #165: Out-of-combat vitality regen is too slow and doesn't scale
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/165

### Body

Out-of-combat vitality regeneration is **too slow, and it doesn't scale**.

Regen is the free alternative to draughts — trading time for money. Right now it fails on both axes: the wait is too long in absolute terms, and because the rate doesn't scale with vitality growth, a higher-level character with a deeper vitality pool waits even longer to refill. Time can't substitute for money, which feeds the healing-economics deficit (#164) — the two must be balanced together: regen speed, draught cost, and income form one system.

For the design session to rule:

- The regen rate law — likely proportional to `vitality_max` (percentage-per-tick rather than flat points) so refill *time* stays roughly constant across levels, or whatever curve the session prefers.
- The absolute target: how long should a full out-of-combat refill take at any level?
- Interaction with the tick engine (regen presumably rides `process_effects`/passive phases — same async-safety and round-boundary conventions bind).

Part of the **Healing Economy** milestone; balance targets coordinate with #164.
...
**KnightOfNight** — 2026-07-31:
**Design ruling (2026-07-30, V24.0 design session — operator-confirmed). The regen law is ruled: proportional-to-max.**

1. **The law:** out-of-combat vitality regeneration becomes `heal per second = vitality_max / T` — a full refill from zero takes exactly **T seconds at every level**, forever. (Current code is deficit-proportional — `ceil(deficit/120)` per tick — exponential decay plus a 1 HP/s tail: ~100 s at a 100-cap character but ~5.6 min at the L12/718 reference, with the last ~120 HP always crawling at 1 HP/s. Both filed complaints verified and killed by the new law.)
2. **T = 120 seconds.** The number `VITALITY_REGEN_SECS` always claimed; the implementation finally matches its comment. Two minutes of dead time prices the free alternative — draughts stay clearly worth buying.
3. **Longevity untouched:** its 3600 s constant and current shape stay as-is until something drains it (#70, deferred out of V24).

**Unchanged and binding:** the in-combat exclusion (the acuity half of that asymmetry is #142, Phase 3 — explicitly out of scope here), the dying-state exclusion, regen staying silent (status-pane update only), tick-engine async-safety conventions.

**Implementation surface (for the brief):** Phase 4 of the tick engine (`run_tick_engine.py:1313`) — vitality branch becomes `ceil(vitality_max / VITALITY_REGEN_SECS)` per tick, clamped at max; longevity branch untouched; constant's comment now true. Measurement-independent — this release can slot anywhere in Phase 1 per design-session call.

GDD §4 text lands on `version_24_0` with a "(v24.0, pending implementation)" marker.


**KnightOfNight** — 2026-07-31:
**Ruling amendment (2026-07-30, same session — operator-confirmed): Longevity joins the proportional-to-max law.**

Ruling point 3 is amended: instead of leaving Longevity untouched, the same brief gives it the proportional law at its own constant, `LONGEVITY_REGEN_SECS = 3600` — full recovery from zero takes about one hour at every level.

**Why:** the GDD's shipped-behavior claims ("at most one hour — 30× slower than Vitality") were discovered false in this session: because `longevity_max` (~274) is far below 3600, the deficit formula's ceil degenerates to a flat 1 point/second — Longevity actually refills from zero in ~4.6 minutes, faster than a L12 vitality bar. The accidental 1/s rate would gut the slow-burn identity the day #70 ships a drain; fixing it now, while nothing drains the bar, is the one moment the change is free (operator chose this over an erratum documenting the accident).

**Implementation surface — the ceil trap (for the brief):** `ceil(longevity_max / 3600)` is always 1 for any bar smaller than the constant — the vitality form cannot express the law here. Longevity must use the **interval form**: one point every `ceil(LONGEVITY_REGEN_SECS / longevity_max)` seconds (~14 s at a 274 bar → ~64 min from zero; the law's rate `longevity_max / 3600` in integer steps). Vitality keeps the simple per-tick form `ceil(vitality_max / 120)` — its bars exceed the constant almost immediately, so ceil generosity converges; the brief should state both forms explicitly.


## Issue #166: Top-level heal command — 'use infinity healing draughts' semantics
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/166

### Body

Add a top-level **`heal`** command: semantics of "use infinity healing draughts" — drink healing draughts until vitality is full or the supply runs out.

Origin: the operator keeps revisiting the alias system (#125) with exactly this use case in mind. It deserves to be a built-in rather than waiting for the macro machinery — it's the staple action of the whole healing loop, and a hardcoded command ships in one brief while #125 remains a designed system.

For the design session to rule:

- Exact stop conditions (vitality full, supply exhausted, and presumably the already-full case → world-declined refusal per the response doctrine).
- Output: one consolidated message, not a flood — this builds directly on #151 (compute the heal once, consume what's needed, send one message); the two probably ship together.
- Grammar: verb only, or does it admit nouns (e.g. a specific consumable)? §9.1 chart entry, footnotes, tab-completion pool, state gating (usable in combat?).
- Draught selection order when multiple tiers exist (interacts with the #139 evolution pass).

Part of the **Healing Economy** milestone. Related: #164 (economics), #151 (output), #139 (tiers), #125 (aliases — remains the general-purpose system; this is the shortcut that shouldn't wait for it).
...
**KnightOfNight** — 2026-07-31:
**Design ruling (2026-07-30, V24.0 design session — operator-confirmed). The `heal` command is ruled, five points:**

1. **Grammar: bare verb, no nouns.** `heal` takes no arguments in v24 — the draught family is the only healing consumable; noun scoping can be added compatibly later. Tab completion completes the verb; no noun pool.

2. **Semantics: exactly "use as many healing draughts as needed."** Compute the vitality deficit once; consume the minimum count that fills it (or all remaining, if short); one merged count-form message via the v23.3 aggregate machinery (#151) — mixed-Mk consumption uses the sentence form already specified in GDD §9 — and one status update. At full vitality: the existing heal-at-full world-declined refusal. No draughts in inventory: world-declined warn. Supply exhausted before full: shortfall report in the warn voice per the #132 consequence-must-be-seen doctrine; wording authored at brief time. (#161's family-wide shortfall-wording question remains its own unruled issue — adjacent, not absorbed.)

3. **State gating: allowed in combat, identical to `use`** — it is a use. No new power is granted; an aggregate `use N healing draughts` in combat is already legal.

4. **Cross-tier selection: oldest-first regardless of Mk** — the #168 law applied verbatim; each consumed item's heal is computed from its own Mk under the #139 Draught Law. Waste-minimizing selection deliberately rejected (it would break oldest-first); revisit only if play shows real pain, via a new filing.

5. **`heal` joins the reserved built-in verbs** — confirming the operator note on #125: the future alias system must refuse `alias heal <anything>` (built-ins are never shadowed).

**GDD text lands with this issue's own release** (§9.1 chart row, footnotes, state-gating matrix row, and the §6.12/§9 aggregate cross-references) — not on `version_24_0`, per the release model's text-ships-with-its-release law.

**Implementation surface (for the brief):** dispatch-table entry in `consumers.py` delegating to the `_use_aggregate` path (#151) with an infinity-count/deficit-driven request; the #61 refusal and #168 ordering paths are reused, not duplicated. Measurement-independent — slots anywhere in Phase 1 per design-session call; pairs naturally with the #165 release or ships alone.
...
## Issue #176: Equipment paper-doll renders consumed hand slots as free (two-handed weapons)

- State: open
- Author: KnightOfNight
- Labels: output, V24, itemization
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-30 | Updated: 2026-07-30
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/176

### Body

The equipment paper-doll renders every slot in anatomical order with a muted `-` for empties (v22 Brief 2 ruling: "every slot always shown"). But nothing marks a hand as **consumed**, so a slot blocked by a two-handed weapon renders identically to a genuinely free one.

## Symptoms

- Battle Axe (two-handed, sits in Main hand): **Off hand shows `-`** but equipping anything there is refused ("You'd have to unequip Battle Axe first"). The player reads it as a usable slot.
- Hunting Bow / Hunting Sling (two-handed, sit in Ranged): **both Main hand and Off hand render `-`** while neither is usable — the worst case, two misleading rows at once.

## Diagnosis

The equip mechanics are correct — the resolver treats a two-handed item as claiming both hands from any slot it occupies (`consumers.py` equip candidate logic) and refuses properly. The defect is display-only: the paper-doll loop (`consumers.py`, `cmd_inventory` doll rows) knows occupants per slot but has no concept of a slot consumed by an item shown on another row.

How a consumed hand should render is a design ruling (e.g. pointing at the consuming item vs. a "both hands" marker).

Related: #177 (combat-side slot wiring) — this ticket is display only.
...
## Issue #177: Combat ignores weapon slots — only one equipped weapon ever attacks; dual-wield contributes nothing

- State: open
- Author: KnightOfNight
- Labels: V24, game-balance, itemization
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-30 | Updated: 2026-07-30
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/177

### Body

Combat does not read weapon slots at all. The tick engine's player-attack path (`run_tick_engine.py`, ~445–470) selects the attacking weapon as: *all equipped, non-broken items of type weapon → take `[0]`*.

## Consequences

- **Dual-wield is inert.** A second equipped weapon (e.g. an off-hand Combat Knife) contributes nothing — no extra damage, no extra procs, nothing. Only `equipped_weapons[0]` ever swings.
- **Weapon choice is arbitrary.** With multiple weapons equipped, which one attacks falls out of queryset ordering, not player intent — equipping an off-hand weapon can silently change which weapon you fight with.
- **Slots carry no combat meaning.** Main hand, Off hand, and Ranged are indistinguishable to the damage path; `is_ranged` only swaps the stat bonus from STR to DEX.

The equip system (slot conflicts, two-handed claims, per-slot validity) models hands far more carefully than combat ever reads them — a gap left by the v22 gear-wiring scope law ("wire only what combat reads").

## Operator direction (2026-07-30, recorded verbatim-style)

Dual wield not doing more damage is the headline problem. Multi-weapon loadouts should compound: "when I have a knife, a mace, and a pistol, I want to hit *hard*." The damage model for multi-weapon attacks (contribution per weapon, procs, stat scaling) is a design-session ruling.

Related: #178 (Ranged-slot semantics) and #176 (paper-doll display); #127 (ranged proc damage, new weapon kind) is adjacent design space.
...
## Issue #178: Ranged slot semantics unruled — stat-swap flag only; equip asymmetry with two-handers; Pulse Pistol has no acquisition path
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/178

### Body

The Ranged slot has no ruled meaning, and its current behavior is an accident of the equip resolver's hand-claiming rule.

## Current behavior

- The two-handed flag is attached to the **item**, wherever it sits: an equipped two-handed ranged weapon (Hunting Bow/Sling) blocks both hand slots; a one-handed item only conflicts with two-handers when going **into** a hand slot.
- Result: with a Battle Axe (two-handed) equipped, the **Pulse Pistol equips freely into Ranged** — both hands full plus a pistol at the ready. Whether that's coherent depends on what Ranged means: **actively wielded** (then this is a mechanics hole) or **holstered/slung at the ready** (then it's correct and only display/combat need work). That's the ruling this issue needs.
- In combat, `is_ranged` does exactly one thing: swaps the damage stat bonus STR→DEX. No range, distance, or engagement mechanics exist (see the companion combat-slots issue).

## Pulse Pistol status

Defined since the item seed (cyberpunk genre, base value 90, slots Ranged or Main hand, one-handed) but present in **zero loot tables and zero vendor kits** — no acquisition path has ever existed. Operator direction (2026-07-30): acceptable for now (it's Neon Sprawl content), but the slot and weapon must *work* by the time pistols enter the world — the mechanics come first, under the V24 prep theme; the acquisition path lands with zone content.

Related: #127 (ranged proc damage, new weapon kind); #177 (combat slots) and #176 (paper-doll display).
...
## Issue #180: Post-gear-wiring fight-cost survey: measured HP loss, duration, and draughts-per-fight across NPC tiers

- State: open
- Author: KnightOfNight
- Labels: V24, healing-economy, game-balance
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-31 | Updated: 2026-07-31
- Blocked by: none
- Blocks: #164
- URL: https://github.com/KnightOfNight/games-mvc/issues/180

### Body

## Why

#164's income numbers must derive from **measured** fight costs, and the last feasibility dataset (#89, v21) predates the v22 gear wiring (#100 — effective stats, Option C armor mitigation, per-hit procs all moved player power). Operator ruled data-first (2026-07-30, V24.0 design session): the income table is not ruled until this survey reports.

## What to measure

Simulation at the combat-math level (the #89 / #128 precedent), at blessed on-level targets, even-split builds, current-era standard gear, per NPC tier (normal / elite / boss, including the signposted multi-aggro rooms):

- Expected HP loss per fight (mean and spread), as absolute HP and as fraction of `vitality_max`
- Fight duration in rounds
- Implied draughts-per-fight under the ruled Draught Law (heal = (15% + 5% × Mk) of `vitality_max`, min 25 — #139)
- Kill-feasibility sanity check against the #89 verdicts (flag any tier whose verdict flips post-gear-wiring)

## Consumers of the dataset

1. **#164 income targets (primary — that issue is blocked by this one):** the k = 2 income law's per-tier numbers derive from these measurements.
2. **Phase 3 baselines:** #104 (NPC HP scaling) and #130 (secondary-stat curve audit) want the same measured starting point.
3. **The v21 boss potion budgets** (≤8/encounter, zone-final ≤12) re-derived in drinks-per-encounter under the new Draught Law.

## Shape of the work

Implementation-session research brief: committed dataset + report document on the version branch; **no game-behavior changes**. Position in the plan: Phase 1 (healing economy), slotted as the release after 24.0 (#139) — see the series plan of record on #139.
...
## Issue #181: Loot-table enrichment: healing draughts (and other items) as NPC drops
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/181

### Body

## Operator direction (2026-07-30, V24.0 design session)

Review all NPC loot tables and add **healing draughts — and possibly other items** — to appropriate NPCs.

Context from the same ruling (recorded on #164): blanket copper drops on aggressive mobs were **rejected on zone-lore grounds** — beasts and insects don't carry coins. The income side widens instead as:

- Bosses keep dropping copper; **their numbers go up**
- Existing copper-dropping NPCs (villagers/humanoids); **their numbers go up**
- **Loot in kind (this issue):** grind mobs supply the healing economy through their loot tables rather than coin

## For the design pass

- Which tables get draughts (and at what `drop_chance`), and what "other things" means per table — lore-appropriate per NPC (an insect can plausibly yield a carapace; who plausibly yields a draught is a per-table call)
- Accounting: a looted draught is income in kind — avoided purchase (15 cp at Mk 1) if drunk, sale value (5 cp) if sold. The #164 income equation must count it either way; drop rates coordinate with the k = 2 law and the fight-cost survey data
- Interaction with stacking (consumables stack, v23) and the existing 0.35 material drop rates

Part of **Phase 1 (healing economy)** in the V24 series plan (plan of record on #139).
...
| 158 | Iteration 2: the closeout session — type, stamp whitelist, tail deploy, merge terms, instructions v25, -DEV CI gate | KnightOfNight | triaged | 2026-07-28 |
| 156 | Codify the standing release flow — prod runs main only, main never carries -DEV; must land before v23.1 closeout | KnightOfNight | triaged | 2026-07-26 |
