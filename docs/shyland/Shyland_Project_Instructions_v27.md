# Shyland Project Instructions v27

**Status: ACTIVE — approved by the operator 2026-07-29. Supersedes Shyland_Project_Instructions_v26.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v27 edition is the GDD-errata pass (#169, triggered by #168): documentation corrections to shipped behavior are ops work on main — deliberately not a new session type and not a conditional on design sessions. (v26 added the end rituals and the closeout session-end gate; v25 introduced the closeout session type and the closeout-tail deploy.)
