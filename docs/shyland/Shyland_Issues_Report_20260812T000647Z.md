...
| 225 | Acuity displays truncate to one decimal — 1.15 renders as 1.1; need two decimals or more | KnightOfNight | triaged, output | Version 24.22 | 2026-08-12 |
...
- Milestone: Version 24.22
- Assignees: KnightOfNight
- Created: 2026-08-11 | Updated: 2026-08-12
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-11, V24.22 design session — founding ticket): **the whole meter goes two-decimal, end to end.** Every acuity numeral renders fixed two decimals (`:.2f` server-side, `toFixed(2)` client-side), trailing zeros kept (`1.00`, never `1.0`) — the stats-pane number, the `stats` command (current **and** baseline), and all four tick-engine message suffixes (dot_acuity, hot_acuity, sharpens, wavers). This supersedes #133's "round-to-0.1 display" note for mid-climb values: display precision is uniform everywhere the meter speaks.

Rationale: band edges are 2-decimal exact by #133 and settled states sit at those edges — a one-decimal display lies exactly at the states the meter exists to show (1.15 → "1.1"). Mixed precision (0.1 mid-climb, 2-decimal at rest) would change the numeral's width as it moves; fixed two decimals gives one rule and stable width.

Scope: display-only — storage untouched; the status payload already rounds to 2. Seven format sites total: `game.html` stats-pane render (1), `consumers.py` stats command (2), `run_tick_engine.py` suffixes (4). Tests pinning `(Acuity N.N)` literals convert under the standing test-hygiene rule, reported as deviations in the closeout.

Ships in Version 24.22 (milestone assigned). Brief to follow on `version_24_22`.
