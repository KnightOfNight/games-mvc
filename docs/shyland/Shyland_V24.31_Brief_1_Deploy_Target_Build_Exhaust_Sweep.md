...
2. Eviction is **size-capped, not `-a`** — but the reason is narrower than first written. **Corrected during implementation (2026-08-15, operator-directed; see the closeout report):** `make build` runs `docker compose build --no-cache` **by design**, so no build ever reads the BuildKit cache. The cap therefore costs nothing in build time, and `-a` on the *builder* prune would cost nothing either; the cap is kept as a conservative, free bound, not because it preserves a cache hit. **The `-a` prohibition that is genuinely load-bearing is on the *image* prune:** `docker image prune -a` removes tagged base images and forces a cold re-pull of `python:3.12-slim` — that is the 2026-07-29 slowdown this sweep must not reintroduce. (The original wording justified the cap by "the next deploy's expensive layer (pip) is still a cache hit"; that was never true under `--no-cache` and is struck.)
...
3. **The cap holds.** `docker system df` after the sweep shows the Build Cache **RECLAIMABLE** figure at or below the reserve (`5GB`, which Docker parses as 5 GiB ≈ 5.37e9 bytes). **Corrected during implementation:** the reserve bounds the *reclaimable* cache, not the total — entries still in use by the running stack sit on top of it, so the reported SIZE legitimately runs above 5GB. Record before/after and the reclaimed delta in the closeout. **A reclaimed delta of ~0 is a valid pass**, not a failure, if the cache was already under the cap when the sweep ran; the assertion is the *ceiling on reclaimable*, not the delta.
4. **No deploy got slower.** Run `make deploy-dev` a **second** time immediately, with no source change. The build's `FROM` step must **resolve the base image without re-pulling its layers** (no image-layer download in the build output) — that is the property the image prune's bare form protects, and losing it is the 2026-07-29 slowdown. If the base image re-pulls, stop and report. **Corrected during implementation:** the pip/requirements layer always rebuilds and this is not a finding — `make build` passes `--no-cache`, so build-cache reuse is not a property of any deploy, swept or unswept (verified by two consecutive bare `make build` runs with no prune between them: pip rebuilt both times while the cache grew).
