...
| 66 | Balance: the Whistler Below is a difficulty cliff — contest scale doubles at boss #2 | KnightOfNight | B3 | Version 21 | 2026-07-17 |
...
Refined Whistler Below numbers from the #89 kill-feasibility survey (full method + table: `docs/shyland/Shyland_V21_Kill_Feasibility_Survey.md` §4, PR #99). Expected-value math from the code as implemented; Mk 1, Uncommon broadsword (expected midpoint 16.09), draughts heal 25 and are supply-limited only.

**The Whistler as seeded:** level 6, effective DEX 36 (curve 30 + boss offset 6), effective STR 28, HP 260. It hits a curve-tracking player at 85–95% for ~28/landed hit with zero mitigation. Young (gated, ×2): L4, DEX 26, STR 21, HP 50 each.

**Player-side hit chance vs. its defense (46):** DEX ≤ 22 → literally zero damage path; DEX 23–25 → graze-only (0% hits); every DEX point from 26 to 36 buys +5%. The reported L8 STR/DEX 25/25 build sits in the graze-only band: **0% hit chance, ~3 damage/round, ~97 potions solo / ~105 with young — mathematically unwinnable**, fully matching the playtest verdict.

**Encounter table (canonical full-split STR/DEX build):**

| Attacker | Solo TTK / dmg taken / potions | +2 young TTK / dmg taken / potions |
|---|---|---|
| L6 (STR 31/DEX 30, HP 203) | 17r / 454 / **13** | 19r / 555 / **17** |
| L8 (36/35, HP 228) | 8r / 165 / **0** | 10r / 210 / **2** |
| L8 operator build (25/25, HP 195) | 84r / 2551 / **97** | 89r / 2767 / **105** |
| L10 (41/40, HP 253) | 5r / 64 / **0** | 7r / 84 / **0** |

**Implications for the retune's design pass:**
1. At-level the fight hits its blessed 25% target exactly, but the *cost* of that target at 260 HP is 13–17 potions through a 27–45/round damage storm — CLIFF-grade at the level the placement implies, then trivial two levels later.
2. The cliff the playtest actually hit is **build sensitivity**, not (only) the boss's numbers: the 25% blessed target is real only for a build that banks every second point into DEX. A 10-point DEX lag turns the same fight from 0 potions into unwinnable. Deciding what build the calibration is allowed to assume is part of the retune.
3. The young are secondary: +~100 damage / +4 potions at L6. The ladder's difficulty step is dominated by the boss DEX+HP jump, not the escorts (contrast: Silk Matron + brood costs 3 potions at L3).
4. The survey's ladder numbers show the same shape getting monotonically worse above the Whistler — Dronemother 18/23 potions at-level, and all three delve bosses exceed the 20-potion bound even at band top (survey §3 G1). A retune scoped to this table's six rows should account for where the HP curve is allowed to cross the potion bound, or the cliff reappears one boss later.
