...
| 104 | NPC HP must scale with level/Mk tier before any Mk 2 spawn is authored | KnightOfNight | triaged, V24, game-balance | Version 24.13 | 2026-08-07 |
...
- Milestone: Version 24.13
- Assignees: KnightOfNight
- Created: 2026-07-17 | Updated: 2026-08-07
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-07, V24.13 design session): **#104 is the founding ticket of Version 24.13** — the release that opens **Phase 3 (Mk 2 balance)** of the V24 series plan (#139). Milestone `Version 24.13` created and assigned; branch `version_24_13` cut from main (post-V24.12).

Scope for this release: rule and implement NPC HP scaling with level/Mk tier so that Mk 2 spawn authoring is unblocked. The scaling design itself is being ruled in this session; the ruling and implementation shape will be recorded here before the brief is written.


**KnightOfNight** — 2026-08-07:
Design ruling (2026-08-07, V24.13 design session — operator-confirmed):

**Doctrine:** at-level time-to-kill is band-invariant — the HP counterpart of the blessed 55/45/45 hit targets. `base_vitality` remains the authored *within-band* (Mk 1) value; species identity and the #101 boss-ladder tuning are untouched.

**Mechanism:** instance HP is computed once at spawn: `vitality_max = round(base_vitality × (1 + 0.75 × (mk_tier − 1)))`, implemented as one helper beside `combat_utils.npc_level()` and called from the single `NpcInstance` creation site (the tick engine's respawn/refill sweep). Uniform across all combat tiers, bosses included. Mk 1 multiplies by exactly 1 — zero live impact, no reseed, no migration, expected deletions 0.

**The number (h = 0.75):** player at-level damage per landed hit is linear in level (weapon midpoint + primary stat), so the lift is linear, not exponential. Fitted at-level damage growth across the authored weapon set is ×1.72–1.77 for Mk 1→2 (h = 0.75 gives ×1.75 exact), ×~2.44 for Mk 1→3 (lift ×2.50), ×~3.15 for Mk 1→4 (lift ×3.25) — a hair above pure fit at Mk 3+, a deliberate gentle lean offsetting accumulated gear secondaries; #130's retune can trim later if it overshoots.

Examples: Mk 2 cave beetle 40 → 70 HP; Mk 2 Silk Matron 150 → 263 HP.

With the formula, site, and number pinned, this is cold-start-ready — `triaged` applied in the same motion. GDD text and the V24.13 brief follow on `version_24_13`.
