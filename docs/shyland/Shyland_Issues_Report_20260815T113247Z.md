...
## Issue #246: Playtest checklists: "admin-gift" is ambiguous where a step tests a generation-path guard (Django admin bypasses it by design)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/246

### Body

Found while preparing the V24.28 Brief 1 operator playtest. Not a code defect — the shipped behavior is correct and deliberate. The problem is that "admin-gift" is ambiguous in a way that makes a playtest step produce a **false failure report**.

**The two readings of "admin-gift"**

V24.28 Brief 1 §11 steps 3-6 say "Admin-gift yourself a `Silver Ring of Strength` at Mk 2", etc. There are two ways an operator can do that on the dev stack:

1. **The shell helper** — `generate_item_instance(definition, mk_tier, rarity, owner=character)`, the path the `stock-playtest-items` skill mandates ("Do not construct `ItemInstance` directly"). This **goes through the Mk-mismatch guard**.
2. **The Django admin `ItemInstance` add form** — direct ORM construction. This **bypasses the guard entirely**, by design.

Reading 2 is a natural one, especially since step 2 of the same checklist explicitly sends the operator into the Django admin ("In the Django admin, confirm all ninety-six ladder definitions are present").

**Why it matters**

§11 step 6 ("The lie is refused") asks the operator to attempt a `Copper Ring of Strength` at Mk 2, a `Gold Ring of Strength` at Mk 2 and a `Sphaerium Ring of Strength` at Mk 7, and to "confirm all three fail with the Mk-mismatch error rather than minting the item."

Through the Django admin add form, **all three will mint successfully**. The operator would correctly report the guard as broken, and it isn't — direct ORM construction is the documented, deliberate residual gap (Brief 1 §4; Architecture v24 §4.6). A save-time guard was explicitly rejected because it would make any pre-existing mismatched instance unsavable.

So the checklist as written can be run in a way that reports a failure against behavior that is working exactly as ruled.

**Scope of the confusion**

`ItemInstanceAdmin` is a plain `ModelAdmin` with no custom gift action, and there is no gift management command. The only two production callers of `generate_item_instance` are the loot-drop roll (`item_utils.py`) and `do_buy`'s vendor path (`consumers.py`). "Admin-gift" therefore has no single canonical implementation in the repo — it is a convention carried by the `stock-playtest-items` skill, not something the codebase names.

**Suggested direction (design call, not made here)**

Any one of these would close it; picking one is a ruling:

- Standardize brief language on "gift via the shell helper" (or similar) wherever a playtest step depends on generation-path behavior, and say so in the brief-writing rules.
- Add an explicit admin action or management command named for gifting that routes through `generate_item_instance`, making the natural reading the correct one.
- Leave the behavior alone and add a standing note to playtest checklists that the Django admin add form bypasses generation guards by design.

**Not blocking V24.28.** The guard is verified working through its real paths — automated tests cover it across every rung including the unbounded one, and the shell smoke test confirmed the three refusals live on dev. This issue is about the checklist wording and the recurring trap, which will surface again on any future release that guards a generation path.
