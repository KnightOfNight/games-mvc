# Shyland Project Instructions v34

**Status: ACTIVE — approved by the operator 2026-08-15. Supersedes Shyland_Project_Instructions_v33.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v34 edition is the interim-retirement pass (#249 Part 3): `make verify-prod` is live and proven — Part 1 shipped the target pair and guard (28db69e), Part 2 shipped the forced-rollback `VerificationCommand` harness and `verify_ladder` in V24.29, and the first live production exercise ran clean in that release's closeout tail (settling V24.28's handed-off survey) — so v33's interim rule routing read-only prod inspection to the operator is retired; the executor checkpoint now names the live target. (v33 the prod-verification ruling pass; v32 the gift-language pass; v31 the seed-prod pass; v30 the label model; v29 the release model; v28 the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
