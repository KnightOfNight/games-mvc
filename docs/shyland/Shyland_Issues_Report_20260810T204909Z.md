...
| 142 | Finish the acuity design: in-combat drift is unruled | KnightOfNight | triaged, V24, game-balance | Version 24.18 | 2026-08-10 |
...
| 218 | Zombie combat sessions: all NPCs dead but session stays active — loot blocked 'in combat'; stale sweep can't reap (engine refreshes last_tick_at unconditionally) | KnightOfNight | bug |  | 2026-08-08 |
...
- Milestone: Version 24.18
- Assignees: KnightOfNight
- Created: 2026-07-23 | Updated: 2026-08-10
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-10:
Design ruling (2026-08-10, V24.18 design session — founding ticket, milestone Version 24.18):

**Acuity drift pauses during combat — option (a), symmetric with the other two bars.** Tick Phase 2 (passive acuity drift) gains an in-combat exclusion using the **same combat-membership predicate** Phase 4 (Vitality/Longevity regen) already uses: active `CombatSession` membership (`combat_sessions.filter(is_active=True).exists()`). "Nothing passively recovers in combat" is now one law across all three bars.

**Rationale (recorded):** the tonic/shift family is already drift-exempt (#133 — a running shift owns the value), so in-combat drift's main real effect today is erasing *enemy-inflicted* acuity states at 0.03/combat round — exactly the mechanic Z02/Z07 acuity-warfare NPCs will need (`dot_acuity`, eldritch pushes). Pausing drift makes a mid-fight mind-crash a problem the player solves (consumable, future Warden tools), not one that auto-fades. Option (b) — keep drift running for "active upkeep" — was rejected: its upkeep pressure lands almost entirely on world-inflicted states, nerfing the interesting content. Option (c) — combat-specific drift rates — rejected as unearned complexity ahead of any shipped stress mechanics.

**Companion sub-rulings (same motion, all confirmed):**

1. **Predicate identity:** Phase 2 and Phase 4 share one definition of "in combat" — if #218 (zombie sessions) later changes combat membership, both phases move together. No second definition may be introduced.
2. **Shift interaction unchanged:** shift-active and in-combat are *independent* pause conditions; the #133 rules stand verbatim. A character with a running shift is paused regardless of combat state.
3. **Quit-in-combat:** combat continues after quit, so drift stays paused for a logged-out fighter until their session ends — consistent with "tab-closing and quitting are identical in cost."
4. **Post-combat:** drift resumes normally at combat-session end — ordinary rate, no burst correction, no refill (a 0.5 deviation settles in ~50 s).

**Scope:** runtime-only — no migration, no seed, no data actions; architecture-doc hash MOVES. GDD §4.2 text lands on `version_24_18` with the pending-implementation marker; brief to follow on the same branch.

Code facts at ruling time (branch `version_24_18` @ 90734e7): drift = `ACUITY_DRIFT_RATE` 0.01/s toward baseline, snap within 0.01, Phase 2 `run_tick_engine.py` ~1195–1234, no combat check; Phase 4 exclusion at ~1333; combat reads `acuity_current` fresh each player round via `acuity_damage_modifier`.
