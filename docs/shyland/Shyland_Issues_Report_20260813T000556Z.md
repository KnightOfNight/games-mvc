...
| 30 | Travel network: should checkpoints (shards) also be travel senders? | KnightOfNight | V24, travel |  | 2026-08-12 |
...
| 38 | Obelisk attunement: player-set home spawn at checkpoint shards | KnightOfNight | V24, travel |  | 2026-08-12 |
| 41 | Lock battle-zone access until a new player has visited all of The Convergence | KnightOfNight | triaged, V24, travel | Version 24.25 | 2026-08-13 |
...
| 95 | the ring needs an area | KnightOfNight | stub, triaged, V24, travel | Version 24.25 | 2026-08-13 |
...
| 205 | Production deploy target gains a dangling-only image prune — stop the ~500MB/release root-volume growth | KnightOfNight | triaged, deployments |  | 2026-08-11 |
...
| 217 | 'last' should show current location for players who are currently logged in | KnightOfNight | output |  | 2026-08-08 |
| 219 | In-game release notes: new 'readme' command — format and content TBD | KnightOfNight |  |  | 2026-08-12 |
...
| 231 | GDD §4 erratum: tick-suffix example "(Acuity 1.15)" — a suffix can never carry an exact x.x5 value | KnightOfNight | errata |  | 2026-08-12 |
| 234 | Character deletion: items are silently orphaned (owner SET_NULL) — rule item disposition + hard-vs-soft delete model | KnightOfNight |  |  | 2026-08-12 |
| 235 | New player setting: 'plunder' (default off) — auto-loot rights-held corpses at combat end | KnightOfNight |  |  | 2026-08-12 |
| 236 | Admin 'wall' command: broadcast to all connected players (pending-restart warning) | KnightOfNight |  |  | 2026-08-12 |
...
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-12 | Updated: 2026-08-12
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-12, V24.25 design session): Phase 4 bundle structure ruled by the operator — this issue and its companion form the **shard bundle** (#30 travel senders + #38 attunement), ruled and shipped together as one release. Release order: the Convergence bundle (#41 + #95) ships first as Version 24.25; the shard bundle queues as the following Phase 4 release. Founding-ticket choice and the bundle's design rulings happen at that release's design session.
...
- State: open
- Author: KnightOfNight
- Labels: triaged, V24, travel
- Milestone: Version 24.25
- Assignees: KnightOfNight
- Created: 2026-07-12 | Updated: 2026-08-13
- Blocked by: #95
- Blocks: none
...
**KnightOfNight** — 2026-08-12:
Design ruling (2026-08-12, V24.25 design session): this issue is the **founding ticket for Version 24.25** — the first Phase 4 (travel & world) release. Bundle ruled by the operator: #95 (ring area) rides as a dependency in the same release — the gate's player-facing messaging leans on every Convergence room belonging to a named area, and the ring is the gap. Phase 4 release order ruled: the Convergence bundle (#41 + #95) ships first; the shard bundle (#30 + #38, senders + attunement) follows as a later release. Rulings on enforcement point, completion semantics, and permanence follow in this session.


**KnightOfNight** — 2026-08-12:
Design rulings (2026-08-12, V24.25 design session, partial — discussion continuing):

- **Completion semantics (R2):** ruled — completion means all rooms of the zone, matched against `RoomVisit` (recorded at arrival on every path). No curated subset.
- **Existing characters (R4):** ruled — grandfather everyone: the deploy backfills completion for every character existing at deploy time; the gate binds only characters created after 24.25.
- **Travel surface (R5):** ruled — locked destinations stay visible in the `travel` listing, rendered in the muted font; the travel attempt itself is refused with the world-declined voice.
- **Gate shape (R1/R3):** generic per-zone prerequisite model accepted in direction (a seed-authored zone entry requirement + a permanent per-character zone-completion record; nothing keyed to danger level — supports multiple safe zones and future zone-to-zone chains, e.g. new zones requiring the Verdant Reach). Final framing (locks/keys) under discussion.
- **Messaging (R6):** under discussion.

Area rulings recorded alongside on #95: the ring area is named **The Everround** (operator-confirmed); Morra's Smithy (exterior + interior) becomes its own two-room area named **Morra's Smithy** (operator-confirmed), not part of the ring; the Heart stays area-free.


**KnightOfNight** — 2026-08-12:
Design rulings (2026-08-12, V24.25 design session) — messaging ruled; ruling set for this issue now complete:

- **Gate model (final):** locks and keys. The lock is world data: `Zone.entry_requires_zone` (nullable FK, seed-authored) — "to enter this zone you must have fully explored that zone." The key is player data: a `ZoneCompletion` record (character, zone, completed_at; unique together), minted when the character's `RoomVisit` count for a zone reaches that zone's room count, **never revoked**. Keys are permanent; locks are authored. Completions are recorded for every zone, not just currently-required ones (future-proofs V25 gates with real data). 24.25 ships one lock: the Verdant Reach requires the Convergence. Nothing keys off danger level.
- **Refusal message:** world-declined warn; states the requirement and names exactly one area still holding unseen rooms (the machinery picks the area with the most); no counts (zone-exploration counts are #163 territory). One **generic pool** (zone-name and area-name slots) serving every gate until more is needed; **door-agnostic** wording — the same pool serves the walk exit and the travel attempt.
- **Unlock announcement:** yes — the completing room-entry delivers a pooled announcement in the **green** ("went your way") family. It celebrates the completed zone but does **not** name the unlocked zone(s) — allusive only.
- Pool lines (≥3 each, pooled-speech law) are creative content, authored at brief time.

Next: GDD text (operator review before commit), then the implementation brief.
...
- State: open
- Author: KnightOfNight
- Labels: stub, triaged, V24, travel
- Milestone: Version 24.25
- Assignees: none
- Created: 2026-07-17 | Updated: 2026-08-13
- Blocked by: none
- Blocks: #41
...
**KnightOfNight** — 2026-08-12:
Design ruling (2026-08-12, V24.25 design session): joins **Version 24.25** as a dependency of founding ticket #41 — the ring area ships in the same release so the completion gate's messaging can name it. Stub fattening happens this session as the design settles (area name/lore under the creative-content policy; theme color and location-bar behavior surfaced for systemic review).


**KnightOfNight** — 2026-08-12:
Design rulings (2026-08-12, V24.25 design session) — stub fattened:

- **Scope:** the 40 ring rooms (r01–r40) form one new area; Morra's Smithy (smithy exterior + interior) forms a second, separate two-room area — it is an establishment off the ring, not part of it. The Heart stays area-free (the singular center, not a district). Result: every non-Heart Convergence room belongs to a named area.
- **Names (operator-confirmed):** the ring area is **The Everround**; the smithy area is **Morra's Smithy**. NPC dialogue's colloquial "ring road" / "ring street" stands untouched — a street with an official name and a nickname.
- **Theme colors:** both areas get authored `theme_color` values, proposed for operator review at GDD/brief time (harmonized with the four existing park-path area colors; chart discipline observed).
- **Reseed shape:** area assignment only — no geometry changes (no rooms, exits, or coordinates move), expected deletion count 0, no character relocation. Location bar on affected rooms gains the Area segment (display-only; Area truncates first).

Ships with founding ticket #41 in Version 24.25 so the gate's messaging can name every unvisited area.


**KnightOfNight** — 2026-08-13:
Late-catch ruling record (2026-08-12, V24.25 design session end ritual): the operator approved the area theme colors in-session — **The Everround `#C9AE7A`** (worn-brass amber), **Morra's Smithy `#C0855C`** (ember copper). Both are specified in the committed Brief 1 (`Shyland_V24.25_Brief_1_Zone_Locks_And_Convergence_Areas.md`), with a contrast-verification step (≥4.5:1 against the client background, computed ratios reported in the closeout).
...
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-08-07 | Updated: 2026-08-11
- Blocked by: none
- Blocks: none
...
## Issue #231: GDD §4 erratum: tick-suffix example "(Acuity 1.15)" — a suffix can never carry an exact x.x5 value
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/231

### Body

## What the GDD says

`docs/shyland/gdd/section_04_the_three_bars.md` (the acuity display-precision paragraph, swept at the V24.22 closeout) illustrates the tick-engine message suffixes with the example `(Acuity 1.15)`:

> Every acuity numeral the game renders shows **fixed two decimals, trailing zeros kept** (`1.00`, never `1.0`): the stats-pane number, the `stats` command's current and baseline values, and the tick-engine message suffixes (`(Acuity 1.15)`).

## What shipped

The two-decimal format itself is correct everywhere — but a tick suffix can never carry an exact x.x5 value. Per the V24.22 Brief 1 closeout's recorded test-construction deviation (`docs/shyland/Shyland_V24.22_Brief_1_Closeout.txt`):

- Every engine acuity mutation except the band-edge stop stores `round(..., 1)`, so suffix values are always one-decimal quantities rendered at two decimals (`1.10`, `1.20`, `0.20`).
- The band-edge stop — the only path that stores an exact x.x5 like 1.15 — deliberately prints the numeral-free terminal line ("Your focus settles at its keenest.") per #133 doctrine, which the V24.22 brief explicitly preserves.

So `(Acuity 1.15)` is an example no player can ever see in a tick suffix. The value 1.15 renders on the stats sheet and stats pane, not in suffixes.

## Why this is an erratum, not design work

The game is right; the doc's illustration is wrong about shipped behavior — the erratum test (Instructions v31, GDD Errata rule). Fix is ops work on main: swap the example for one a suffix can actually produce (e.g. `(Acuity 1.10)`), or attribute the 1.15 example to the stats surfaces. Zero design decisions required.

Found at the V24.22 closeout (2026-08-11); the closeout session's edit surface excludes GDD prose, so it was flagged rather than fixed.
...
## Issue #234: Character deletion: items are silently orphaned (owner SET_NULL) — rule item disposition + hard-vs-soft delete model
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/234

### Body

## Problem

Deleting a character today orphans their entire inventory. `ItemInstance.owner` is `on_delete=SET_NULL`, so every item the character held becomes an ownerless row — no owner, no room, unreachable by any game path, permanent DB flotsam (the #137 orphan family, new entry point). A mid-level character carries dozens-to-hundreds of instances, so each deletion strands a meaningful pile.

Everything else about deletion is already correct by schema design: room visits, active effects, own combat actions, dialogue state all CASCADE; kill attribution and targeted-actions survive with nulled references (name snapshots preserved); the auth `User` is untouched (the cascade runs User→Character, not the reverse); the case-insensitive name frees for reuse.

## Ruling needed 1: item disposition on delete

When a character is deleted, their items should be **removed** (operator lean, 2026-08-12, for the hard-delete case) — or possibly dropped/transferred; the design session rules the exact disposition. Whatever is ruled should arrive as a deliberate flow (pre-delete sweep or an `on_delete` change), not the current silent orphaning.

## Ruling needed 2: hard vs soft delete

- **Hard delete:** row gone, cascade runs, items removed per ruling 1. The obvious model for dev/test characters.
- **Soft delete:** an open question — the operator notes it's unclear what soft deletion even buys here ("it's not like we charge people to play"). Candidate meanings if any survive scrutiny: undo-window for accidental/regretted deletion, name retirement vs. reuse policy, preserving `killed_by` rows as live FKs instead of nulls, stats/history museum. The design session may legitimately rule "hard delete only" — the simplest honest answer for a free game.

## Context / non-goals

- Motivating case: test-character hygiene on dev (level-appropriate playtesting means creating and eventually culling fresh characters). Prod player deletion has no current demand; the ruling should still define the flow so the first real request isn't improvised.
- Operator note for the record: deleting Harley (level 21) is "maybe premature — but I don't want to be afraid of it. He can take it. 🙂" — i.e., the flow should be trustworthy enough that deleting a veteran character is boring.
- Admin-only surface initially (Django admin or management command); no player-facing self-delete implied by this issue.
...
## Issue #238: GDD §2.9 erratum: "35-room ring street" — the shipped ring is 40 rooms (v20 Brief 1, #43)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/238

### Body

GDD §2.9 (Infinity City — The Ring Street) opens: "A 35-room ring street surrounds the park…"

The shipped ring has been **40 rooms** since v20 Brief 1 (#43): 10 steps per compass direction, delta sum (0,0), five corner rooms (r36–r40) replacing the old diagonal corner steps. The seed's `RING_WALK` table (40 entries, r01–r40) is the authority, and the Convergence's 60-room total (40 ring + Heart + 17 park-path rooms + 2 smithy rooms) only reconciles with 40.

Erratum test: the game is right, the doc is stale — shipped behavior, no design ruling needed. Ops lane (main) per the GDD Errata rule.

Coordination note: discovered during the V24.25 design session, which is editing the same §2.9 subsection on `version_24_25` (the ring's area naming, #95). The branch edit necessarily writes "40-room" in its rewritten text; operator may rule whether an ops session corrects main first (branch then merges main forward) or whether the correction simply rides the 24.25 release. Either way this issue tracks the erratum to closure.


### Comments (1)

**KnightOfNight** — 2026-08-13:
Second stale count in the same subsection, found during the V24.25 design session's room-count reconciliation: the §2.9 Convergence Park path table lists **Bamboo Run: 3 rooms** — the shipped path has **4** (v20 Brief 1: "the Bamboo Run gains BR-4 so it reaches the ring at (4, 0)"; seed `PATH_EDGES` has br1–br4). Same erratum family and same fix lane as the ring count: with 40 ring + 17 path (4+4+5+4) + Heart + 2 smithy, the zone's 60-room total reconciles.

Scope of this issue is now both §2.9 counts: ring 35→40 (the branch's rewritten paragraph already carries 40) and Bamboo Run 3→4 (table untouched on the branch — needs the erratum fix).
