...
| 177 | Combat ignores weapon slots — only one equipped weapon ever attacks; dual-wield contributes nothing | KnightOfNight | triaged, V24, game-balance, itemization | Version 24.6 | 2026-08-02 |
| 178 | Ranged slot semantics unruled — stat-swap flag only; equip asymmetry with two-handers; Pulse Pistol has no acquisition path | KnightOfNight | triaged, V24, itemization | Version 24.6 | 2026-08-02 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, V24, game-balance, itemization
- Milestone: Version 24.6
- Assignees: KnightOfNight
...
Design ruling (2026-08-02, V24.6 design session): **#177 is the founding ticket of Version 24.6.** Milestone `Version 24.6` created this session; branch `version_24_6` cut from main at feedefd (post-24.5). #178 (Ranged-slot semantics) is included in the release as a dependency — what the Ranged slot *means* is required input to this issue's multi-weapon damage model, so it is ruled first and **one implementation brief settles both** (scope law: same problem — what equipped weapon slots mean to combat). Substantive design rulings (damage model, procs, stat scaling) follow in this session and will be recorded here.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.6 design session — operator-confirmed): **the composite-strike multi-weapon damage model.**

**Correction to this issue's premise, first:** "no extra procs, nothing" is slightly overstated — `roll_gear_bonus_damage` already iterates **all** equipped items, so an off-hand/ranged weapon's proc factors, flat electric bonus, and `crit_chance` already contribute on landed hits today. What extra weapons contribute nothing to is **base damage and stat scaling**; that is what this ruling fixes.

**The model (one brief settles this + #178):**

1. **One hit roll per round, unchanged** (`resolve_hit`, d20 + effective DEX vs. NPC defense). A miss is still one miss line.
2. **Every equipped, non-broken weapon contributes to one composite strike.** Total base-and-stat term = Σ over weapons of `factor_w × (damage_roll_w + stat_w) × durability_w`, where:
   - `damage_roll_w` = uniform(midpoint ± spread) per weapon, as today;
   - `stat_w` = the weapon's own governing stat (effective STR melee, effective DEX ranged);
   - `durability_w` = that weapon's own durability penalty factor;
   - `factor_w` = **1.0 for the primary weapon, 0.5 for every other weapon** (slot factors: Off hand 0.5, Ranged 0.5). First-pass numbers — Phase 3 (Mk 2 balance) retunes them.
3. **Primary weapon** = the occupant of the highest-priority weapon slot, priority **MAIN_HAND → RANGED → OFF_HAND**. Predictable and player-controllable; a bow-only or off-hand-only loadout fights at full strength. This also kills the queryset-ordering arbitrariness — `equipped_weapons[0]` is dead.
4. **Acuity modifier and the graze/crit multiplier apply once, to the composite** — as today.
5. **Procs, lifesteal, and gear crit-chance: unchanged** (already all-gear, full strength, no slot factor).
6. **Unarmed path untouched.**
7. **Output stays one sentence** — the damage number grows; no per-weapon breakdown (the paper-doll carries loadout).

Net effect: knife + mace + pistol ≈ the mace's full hit plus half a knife plus half a pistol — the multi-weapon loadout hits hard, per the recorded operator direction, while two-handers keep their identity as the biggest single contribution and may still add a holstered ranged weapon on top (#178's "at the ready" ruling).

GDD text lands with this release (V24.6); brief: Shyland_V24.6_Brief_1, committed to `version_24_6` this session.
...
**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.6 design session): included in **Version 24.6 as a dependency of #177** (the release's founding ticket). The Ranged-slot meaning (wielded vs. holstered) is required input to #177's multi-weapon damage model, so this ruling lands first, and **one implementation brief settles both issues**. Milestone applied; the semantics ruling will be recorded here when made this session.


**KnightOfNight** — 2026-08-02:
Design ruling (2026-08-02, V24.6 design session — operator-confirmed): **the Ranged slot means "at the ready."**

- A ranged weapon in the Ranged slot is holstered/slung and ready — equipping it is legal alongside anything, **including two-handed weapons**. The current equip-resolver behavior (a two-hander claims both hands from any slot it occupies; the Ranged slot itself is never claimed by hand logic) is hereby confirmed **correct as built** — Battle Axe + Pulse Pistol is coherent loadout, not a mechanics hole. No equip-resolver changes.
- In combat, the ranged weapon **fires every round** as part of the attack routine, contributing to the composite strike under #177's damage model (slot factor 0.5 unless it is the primary weapon — see #177 ruling). The 3-second round is abstract enough for a shot woven between swings.
- `is_ranged`'s stat swap (STR→DEX) continues, now applied **per weapon** within the composite: each weapon's contribution uses its own governing stat.
- Pulse Pistol acquisition path: unchanged disposition (zone content, lands with the Neon Sprawl) — this release makes the slot and the weapon *work*, per the recorded operator direction.

One implementation brief settles this issue together with #177 (Shyland_V24.6_Brief_1, committed to `version_24_6` this session).
...
## Issue #193: GDD erratum: §5.4 claims unarmed base_damage = 0 — shipped code rolls uniform(1, 3)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/193

### Body

Found 2026-08-02 (V24.6 design session) while grounding the #177 composite-strike rewrite of §5.4.

## Doc claim (two sites in `docs/shyland/gdd/section_05_combat_system.md`)

- §5.4 step 2 of the calc block: "If no weapon is equipped, base_damage = 0 (only stat_bonus applies)"
- §5.4 unarmed paragraph: "`base_damage` is 0 — there is no weapon damage roll"

## Shipped behavior

The unarmed branch of the player-attack path rolls `base_damage = uniform(1, 3)` (`django/src/apps/shyland/management/commands/run_tick_engine.py:474` at time of filing), then adds effective STR — there **is** an unarmed damage roll, small but real.

The game is right and the doc is wrong about shipped behavior → erratum per the v27 rule (#169): an operator-directed ops session on main corrects the section file to match shipped behavior, runs `make gdd`, commits directly to main, and closes this issue. No version, no stamp move, no changelog row.

## Coordination note

The V24.6 branch (`version_24_6`) rewrites §5.4 step 2 for the composite strike; its new text no longer repeats the claim in the calc block — the durable erratum site is the **unarmed paragraph**, which V24.6 does not touch. If this erratum lands on main while V24.6 is in flight, the version branch picks it up via the standard merge-forward.
