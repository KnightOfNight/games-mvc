...
| 165 | Out-of-combat vitality regen is too slow and doesn't scale | KnightOfNight | triaged, V24, healing-economy | Version 24.3 | 2026-08-01 |
...
Design ruling (2026-08-01, V24.3 design session — operator-confirmed): **this issue is the founding ticket of Version 24.3.** Milestone `Version 24.3` assigned; branch `version_24_3` created from main (ee17171).

Scope per the ruled law above (2026-07-30 ruling + amendment, both operator-confirmed): the release implements the proportional-to-max regen law for **both bars** — Vitality in the per-tick form `ceil(vitality_max / VITALITY_REGEN_SECS)` (T = 120 s), Longevity in the interval form (one point every `ceil(LONGEVITY_REGEN_SECS / longevity_max)` seconds, constant 3600) per the ceil-trap analysis.

GDD §4 text lands on `version_24_3` this session with "(v24.3, pending implementation)" markers — the original "lands on version_24_0" note was superseded by the text-ships-with-its-release law (v29/v30); §4's false shipped-behavior claim about Longevity ("at most one hour") is corrected in the same edit, as the ruling amendment chose over an erratum.

Brief to follow this session: `Shyland_V24.3_Brief_1_Proportional_Regen_Law.md`.


**KnightOfNight** — 2026-08-01:
V24.3 design session (2026-08-01): artifacts landed on `version_24_3` — GDD §4.1/§4.3 regen-law text with "(v24.3, pending implementation)" markers (f5cb16a) and the implementation brief `Shyland_V24.3_Brief_1_Proportional_Regen_Law.md` (a4a39c7). Ready for an implementation session directed at Brief 1 on `version_24_3`.
