# Shyland Project Instructions v37

**Status: ACTIVE — approved by the operator 2026-08-16. Supersedes Shyland_Project_Instructions_v36.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v37 edition is the daemon-inspection pass (#258): operator-directed, command-scoped, read-only queries against the production daemon are sanctioned in ops sessions — `DOCKER_HOST` on the single command only, `.env` untouched, no make target involved; the operator's in-conversation direction is the gate. Codifies the precedent that resolved #257's open question. (v36 the technical-coherence pass; v35 the action-item pass; v34 the interim-retirement pass; v33 the prod-verification ruling pass; v32 the gift-language pass; v31 the seed-prod pass; v30 the label model; v29 the release model; v28 the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
