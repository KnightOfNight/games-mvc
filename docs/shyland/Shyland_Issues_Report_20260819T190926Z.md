...
| 33 | Shyland: persist detailed combat logs for balance analysis | KnightOfNight | monitoring-and-command, V25 | Version 25.2 | 2026-08-19 |
...
| 273 | MC stream carries no command outcome — readers infer refusals from adjacent out records (sudo observation) | KnightOfNight | monitoring-and-command |  | 2026-08-19 |
...
- Milestone: Version 25.2
- Assignees: KnightOfNight
- Created: 2026-07-12 | Updated: 2026-08-19
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-19:
**Design rulings (2026-08-19, V25.2 design session) — instrumentation surface (agenda item 1 of 5):**

1. **Granularity: per-action records + encounter bookends.** One record per resolved attack action (the atomic unit — where the rolls live), plus an encounter-start record and an encounter-end record (outcome, duration). The round number rides each action record as a grouping key; there is **no separate round tier** — rounds are derivable from action records.

2. **Roll depth: full interiors.** Action records carry the numbers the helpers currently discard: the to-hit contest (attack total, defense, crit chance, graze margin), the damage decomposition (base/weapon term, stat bonus, acuity modifier, hit multiplier, gear bonus, TAV and pre-/post-mitigation), and lifesteal. This requires an **additive refactor** of `resolve_hit` / `calculate_damage` (and kin) to return richer structures — with every caller's outcome identical and **no new random calls** (the roll sequence is untouched). The #33 body's law stands: capture is never load-bearing; a logging failure never breaks or blocks a fight.

3. **Adjacent scope — IN:** initiative rolls (the per-round ordering contest, currently rolled and discarded); effect applications (`apply_npc_effects` on landed NPC hits — which effect, magnitude, duration); the flee contest and the dying/death path (fatal blow, dying window, death execution) — these are the loss/flee outcomes, captured so the encounter-end record can say *why*, not just *what*. **OUT (explicitly deferred):** per-tick DoT/HoT effect internals in `process_effects` — volume-heavy, effects are lightly used in play today; a later slice if the data proves wanted.

4. **Context snapshot at encounter start, lean actions.** The encounter-start record carries the full identity/stats snapshot: character (level, archetype, origin, effective stats, bars, TAV) and each NPC (definition, mk tier, level, stats, vitality max), plus zone/room. Action records carry pks and the round's live interiors only. A participant joining a live encounter mid-fight (aggro engaging a respawned NPC into an active session) gets a supplemental join record with its own snapshot. Analysis joins on the combat-session pk.

Grounding: rulings made against the code at `version_25_2` tip (1db89f8) — `execute_actions` (`run_tick_engine.py:457–846`), `resolve_hit`/`calculate_damage`/`roll_initiative`/`composite_weapon_term` (`combat_utils.py`), and the shipped v25.1 MC machinery (`mc.py`). Notable code fact informing ruling 2: **graze is currently indistinguishable in player output** (prints as an ordinary hit at half damage) — the stream will record things no player ever sees.

Remaining agenda in this session: record shape/kinds on the stream, volume & retention, queryability, then GDD text + the committed brief.


**KnightOfNight** — 2026-08-19:
**Design rulings (2026-08-19, V25.2 design session) — record shape (agenda item 2 of 5):**

Stream placement was already ruled on #269 (order-of-record, 2026-08-17): combat internals go **onto the same stream**, `mc:events`. Today's rulings shape the records themselves:

5. **Kind vocabulary: a `combat_*` family**, joining the shipped set ({cmd, connect, disconnect, out}): `combat_start`, `combat_join`, `combat_round`, `combat_action`, `combat_flee`, `combat_death`, `combat_end`. All fit `MCEvent.kind`'s `max_length=16`; the shipped `(kind, ts)` index makes every analysis query a clean kind filter. **Effect applications are not a kind** — they ride the `combat_action` record that landed them (they are part of that action's resolution). Naming note: `combat_round` carries **only** the initiative/ordering contest (ruled IN under item 1) — it is *not* the rejected round-rollup tier; actions are never aggregated into it. Exact names may be polished at GDD-text time within the 16-char limit.

6. **Envelope semantics preserved exactly.** `actor` = the acting entity: player-acted records carry `actor_id` = character pk + `actor_name` = character name; NPC-acted records carry empty `actor_id` + the NPC's display name, with npc instance/definition pks in `data`. **`audience` = `[]` always** — internals are addressed to no one, keeping the shipped meaning ("pks the event was addressed to") unforked. `room_id` = the session's room. Every combat record carries the combat-session pk in `data` as the encounter join key.

7. **Ordering contract: internals first, in action order.** When a round returns from `execute_actions` (sync, under `database_sync_to_async` — records must accumulate and emit on return to async context, mirroring the round's existing message pattern), all its combat records emit **before** the round's player-facing sends fire, in the exact order actions resolved. This extends v25.1's emit-before-send precedent to the round level: a reader always sees *why* (the internals) before *what the players saw* (the out records) — the adjacency direction #273's inference pattern relies on is now guaranteed for combat.

Grounding: `MCEvent` model confirmed at `models.py:1240-1260` (kind max_length=16, the three indexes); `cmd_flee` contest at `consumers.py:2804-2842`; the round's accumulate-then-flush message pattern at `run_tick_engine.py:826-846`.

Next agenda items: volume & retention, queryability, then GDD text + the committed brief.


**KnightOfNight** — 2026-08-19:
**Design rulings (2026-08-19, V25.2 design session) — volume & retention (agenda item 3 of 5):**

Grounding arithmetic (verified against the code): combat already writes ~1-2 records/sec per fighter today — `send_fight_payloads` fires every 1-second engine tick through the tapped `send_to_player`, so every fight-feed update is an `out` record. The new internals add ~2 actions + 1 initiative per 3-second round in a 1v1 — **~1 record/sec, comparable to what combat already emits**, with fatter `data` payloads (interiors + snapshot; roughly 0.5-1 KB for `combat_action`, vs ~300 B typical today).

8. **Hot tier: `MC_STREAM_MAXLEN` default rises 100,000 → 250,000** (operator ruling; Claude had recommended keeping 100k). ~2.5× today's window; at combat-era record sizes roughly 100-200 MB RAM worst case — inside the #267 soft-budget posture on the single box, and a many-hours window even during heavy combat. This is a code change (the `base.py:80` default) and ships in the 25.2 brief. The env knob remains per-deployment tunable.

9. **Durable tier: combat records inherit the shipped §10.11 posture — unbounded, pending trigger.** No pruning, no policy sized before data exists: this release *produces* the combat volume numbers the 25.1 closeout's volume note identified as the missing data point. Any future retention policy is its own ruled slice once a real trigger fires.

Next agenda items: queryability, then GDD text + the committed brief.


**KnightOfNight** — 2026-08-19:
**Design ruling (2026-08-19, V25.2 design session) — queryability (agenda item 4 of 5):**

10. **Rows only — analysis tooling deferred.** The structured records are the release's deliverable. Analysis runs ad-hoc: ORM/SQL over `MCEvent` (`data` is JSONB on Postgres; the shipped `(kind, ts)` index covers the primary access pattern), plus the read-only admin's kind filtering. No canned reporting command, no new indexes, no admin additions in 25.2 — a reporting surface gets designed later against real accumulated data and real questions, most naturally in #191's orbit (the watcher's whole job is mining the stream).

**This completes the design rulings for Version 25.2** (rulings 1-10 across the four agenda items above). Remaining session work: the GDD text (§10.11 combat-record family — GDD-first, operator-reviewed) and the committed implementation brief.


**KnightOfNight** — 2026-08-19:
Brief committed (2026-08-19): `docs/shyland/Shyland_V25.2_Brief_1_Combat_Instrumentation.md` on `version_25_2` @ f23b20c (GDD §10.11 combat-family text landed at 40cba4f). Implements rulings 1-10 above: seven `combat_*` kinds onto `mc:events`, per-action records with full roll interiors via delegation-pattern `_detailed` helper variants (all existing callers and the 8 helper-referencing test files untouched), encounter bookends with identity snapshots, `combat_end` at all eight verified end sites, flee/death paths, `MC_STREAM_MAXLEN` default → 250000. No migration, no seed, no persister changes. #252 technical claims verified at writing time against the branch tip; self-consistency read done. Milestone `Version 25.2` holds this ticket alone per the scope law; the release awaits its implementation session.
...
## Issue #273: MC stream carries no command outcome — readers infer refusals from adjacent out records (sudo observation)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/273

### Body

## Observation (operator, 2026-08-19, during the V25.1 Brief 1 playtest)

A non-admin character typing `sudo anything` produces a clean `cmd` record (`{raw, verb, args}` — captured before the dispatch gate, so stealth gating never hides it from MC) followed by the refusal `out` record (the unknown-command error, since `sudo` is stealth-gated). Both are in the stream, but **an MC reader has no structural way of knowing the command was refused, or that the actor lacked the permission**:

- The `cmd` record carries no outcome field — by ruling (#37, V25.1 brief §5.4): the tap is one unconditional emit, and rejections are the stream-adjacent `out` records that follow.
- There is no join key linking a `cmd` to its consequent `out`s — adjacency (same `actor_id`, stream order) is the inference mechanism, and the refusal prose is the only evidence. A reader must *interpret* outcomes, not *query* them.
- Nothing snapshots the actor's permission state at capture time.

## Operator's initial direction

**Not a blocker.** Suspicion recorded: the MC egress/agent layer (#267/#266/#262) will likely need database access for other things anyway, so agents can read permissions (e.g. `admins.shyland` membership) from the DB at interpretation time rather than the stream carrying an outcome field. This issue records the observation for a design session to rule when the reader/agent work is designed — options at that point include: leave as-is (reader interprets, DB for perms), an outcome/correlation field on the `cmd` record, or a permission snapshot in `data` — each softening the single-unconditional-tap purity to a different degree.
