...
> Authoritative technical reference as of commit fa3215b (v19 brief 4: death and dying — fall sequence, effect cancellation, lore ladder, revival via `use`, death declaration).
...
**The `'clear'` event** — `player_message` now checks `event.get('event') == 'clear'` first, ahead of the existing `text`/`status` delivery, and sends a bare `{'type': 'clear'}` to the client whenever it fires (independent of whether the same engine message also carries text — the tick engine's fall sequence sends it as its own zero-text message, see Section 4.9). Client-side (`game.html`), a `type: 'clear'` message runs `output.innerHTML = ''` — the exact same primitive the room-entry path already used for `msg.enter`, just exposed as its own message type so the engine can trigger it outside a room render. This clears the **contents** of the `#output` div only; the div itself (`role="log" aria-live="polite"`) is never removed or rebuilt, so a screen reader's live-region subscription survives the clear and the very next appended line — the room title, then the fatal-blow line — is announced normally. `aria-live="assertive"` is never used for any of this; politeness is unchanged.

One existing client rule had to be corrected to make the post-clear room title visible at all: `renderRoom()` strips any line that looks like `[ ... ]` (the bracketed room-header format) before appending, because that text is redundant with the header bar on a normal move/look. But the fall sequence's title line is a bare `category: 'room'` output with no `exits` field — it never came from `send_room_description` — and *needs* to render, since the header bar alone isn't enough context immediately after a pane wipe. The client dispatch for `category === 'room'` output now also checks `msg.exits !== undefined` before routing into `renderRoom()`; a `'room'`-category message without an `exits` key (the fall's title line, and the pre-existing `"{npc} is here."` / `"The corpse of {x} lies here."` one-liners, which had no brackets to strip anyway) falls through to the plain `append()` path instead, rendering its text — brackets and all — as an ordinary `.msg-room` line. No visible change for any pre-existing message; the fall's title line is the only text this newly reveals.
...
Two changes in v18 (brief 3), one in brief 5, three in v19 brief 2, two in v19 brief 3, and the fall/dying/expiry rework in v19 brief 4 below; everything else *(unchanged from v15)*.
...
4. Three `send_to_player` calls, in order: an `event='clear'` message with no text (see Section 4.3 for the client-side effect); the room title alone, formatted exactly like the standard header (`_room_title(room)`, a module-level helper — `'[ {area} — {room} ]'` or `'[ {room} ]'`), `category='room'`; then the personal fatal-blow line, `category='error'`, riding `event='dying'` (no separate send for the flag sync — Section 4.3's consumer handling is unchanged from brief 2). No hit line, kill line, or "Combat has ended." for this character is generated after this point while `is_dying` is true — the NPC-attacks-character branch's existing early guard and the new character-attacks-NPC guard together make "no combat output of any kind reaches the dying player" absolute, not just "no damage."
