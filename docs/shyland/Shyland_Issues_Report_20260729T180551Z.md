...
| 162 | Redo home timings: cooldown 15m to 5m, countdown 15s to 10s | KnightOfNight | B1, triaged | Version 23.4 | 2026-07-29 |
...
## Issue #162: Redo home timings: cooldown 15m to 5m, countdown 15s to 10s

- State: open
- Author: KnightOfNight
- Labels: B1, triaged
- Milestone: Version 23.4
- Assignees: KnightOfNight
- Created: 2026-07-29 | Updated: 2026-07-29
- Blocked by: none
- Blocks: none
...
Redo both `home` command timings (broadened from the original cooldown-only scope; rulings recorded 2026-07-29, operator in conversation):

1. **Cooldown: 15 minutes → 5 minutes (300 seconds).** `Character.home_cooldown_seconds` (`models.py:277`, added v22 B3 / #57) defaults to `900`. Completion-only consumption is unchanged — the clock starts only when the traveler actually lands at the Heart; interrupted countdowns don't consume it.
2. **Countdown: 15 seconds → 10 seconds.** `HOME_CADENCE` (`consumers.py:2763`) goes from `(7.0, 5.0, 3.0)` to `(3.0, 4.0, 3.0)`: start line at t=0, mid-pool line at t=3s, late-pool line at t=7s, arrival at t=10s. All four authored beats (start / mid / late / arrival) are retained — no pool content is removed.

## Rulings

- **Existing characters: update all rows unconditionally.** `home_cooldown_seconds` is a per-character override field, so the model-default change alone only affects new characters. A data migration sets every existing row to `300` — including any admin-overridden values ("then we know they're right"). The earlier hedge about preserving deliberate overrides is superseded.
- **Countdown stays a class constant** — code-only change, no per-character data work.
- **Balance:** 5 seconds was considered and rejected as too short (near-instant escape); 10 seconds is the ruled value. `home` remains refused in combat and broken by combat entry mid-countdown.

## Implementation notes

- The `AlterField` default change (900 → 300) generates a schema migration; the all-rows update is a data migration alongside it. Migrate runs regardless (standing rule).
- The countdown beats are also the engine-side combat-interrupt checkpoints (`broken_by_violence`); the new cadence keeps all three checks, now at t=3/7/10.
- The cooldown refusal pool (`HOME_COOLDOWN_POOL`, `consumers.py:2758`) says "Give it a few minutes" — still accurate at 5 minutes, no wording change required.
- The remaining-cooldown stamp formatting (`Nm`/`Ns`) needs no change.


### Comments (1)

**KnightOfNight** — 2026-07-29:
V23.4 design session (2026-07-29): milestoned to Version 23.4 and bucketed B1 (point-release scope law — sole founding ticket). GDD §2 / §2.11 updated with the ruled timings under `(v23.4, #162, pending implementation)` markers (commit 473778c on `version_23_4`); implementation brief committed as `docs/shyland/Shyland_V23.4_Brief_1_Home_Timings.md` (commit f8a133e, same branch).
