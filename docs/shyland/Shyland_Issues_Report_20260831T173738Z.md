...
| 275 | Unequipping +STR gear silently strands the character over carry capacity, blocking loot/pickup/buy | KnightOfNight | bug, emergent, triaged | Version 25.13 | 2026-08-31 |
...
| 317 | Sudo bot: file_issue cannot apply labels — extend the filing vocabulary to carry operator-named labels | KnightOfNight |  |  | 2026-08-30 |
| 319 | pickup/buy count equipped gear against carry capacity — load-counting basis disagrees with loot, the inventory header, and the bag guard | KnightOfNight | bug, triaged | Version 25.13 | 2026-08-31 |
...
- State: open
- Author: KnightOfNight
- Labels: bug, emergent, triaged
- Milestone: Version 25.13
- Assignees: KnightOfNight
...
**KnightOfNight** — 2026-08-31:
**RULING (design session, 2026-08-31 — Version 25.13, founding ticket):**

**Direction B confirmed: the over-capacity state is legal; the release makes it legible.** No new unequip guard for stat-granting gear. Three pieces:

1. **Warn at the moment of stranding.** When an unequip — or an auto-swap displacement — drops carry capacity below current load, the action still succeeds and the output gains a warn-category line naming the new state with the numbers (over-limit load vs. new capacity) and the consequence (acquisition blocked until back under).
2. **Honest refusals.** The over-capacity refusals in pickup, loot, and buy state *why*: over the carry limit, with the numbers — not the bare "You can't carry any more." This is where the stranded player actually stands; the refusal is the primary legibility surface.
3. **Standing visibility.** The `inventory` header's `(current/capacity)` count already renders the over state (e.g. `443/408`) — no additional marker added; the numbers stand on their own.

**The bag guard stays, unchanged (operator: "keep the guard on bags since it is 100% consistent").** A bag unequip is *always* and *only* a capacity operation — refusing it when it would strand is coherent with the bag's identity; a weapon/armor swap is almost never about capacity, so it warns instead of refusing. GDD §3's auto-swap sentence ("a displacement that would violate the carry limit (bags) refuses instead") remains literally true: bags refuse, stat gear proceeds-with-warn.

Recovery paths (unchanged, now discoverable via the warn/refusal text): re-equip the stat gear, or drop/sell down under the limit. Drop, sell, and equip remain un-gated by capacity.

Milestone: Version 25.13 (assigned). GDD: §6.10 gains the over-capacity paragraph; §3 auto-swap text touched for the warn case. Brief to follow on `version_25_13`.


**KnightOfNight** — 2026-08-31:
**Ruling refinement (same session, operator-approved):** the stranding warn's precise semantics — the warn renders whenever a completed equip/unequip/auto-swap leaves the character strictly over capacity: the stranding action itself and every later gear change made while still over (deliberate restatement — the state must be unmissable); never at or under capacity. GDD §6.10 sharpened accordingly (b8ae390); Brief 1 (Shyland_V25.13_Brief_1_Over_Capacity_Legibility.md, 3be7648) specifies it.
...
## Issue #319: pickup/buy count equipped gear against carry capacity — load-counting basis disagrees with loot, the inventory header, and the bag guard
...
- Blocked by: none
- Blocks: #275
- URL: https://github.com/KnightOfNight/games-mvc/issues/319

### Body

## The bug (verified against `version_25_13` @ `4c10a98`, discovered during the 25.13 design session's #252 verification pass for #275)

The game has **two different definitions of "current load"** feeding the carry-capacity gates:

- `loot_utils.get_carry_counts` (the **loot** path via the consumer wrapper), the **`inventory` header** (`len(unequipped)`), and the **bag-unequip guard** (`unequipped_count + 1` vs. new limit) all count **unequipped items only**.
- `consumers.get_carry_capacity` — used by **`cmd_pickup`** and **`cmd_buy`** — counts `ItemInstance.objects.filter(owner=character).count()`: **all items, equipped included**.

## Impact

A character with 20 items equipped sees `Inventory (400/408)` but pickup refuses with `(420/408)` — the refusal's numbers disagree with the header's, and pickup/buy block ~20 acquisitions earlier than loot does. Worn gear silently eats carry space on two of the four surfaces.

## Intended semantics

GDD §6.10's bag guard ("carrying too many items to remove the bag" — unequipped + the bag itself vs. the reduced limit) and three of the four code surfaces agree: **worn gear does not count against carry; load = the unequipped pool.** Pickup and buy are the outliers.

## Ruling (design session, 2026-08-31)

Folded into **Version 25.13** as a dependency of #275 — same problem, not a widening: #275's "honest refusals with the numbers" is incoherent while the numbers have two meanings. The 25.13 brief unifies pickup/buy onto the unequipped-only counting (`get_carry_counts`), retiring `consumers.get_carry_capacity`. Strictly player-favorable: usable capacity rises for geared characters.
