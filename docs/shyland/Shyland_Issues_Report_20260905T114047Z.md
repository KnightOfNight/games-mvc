...
| 315 | Door durability edit can silently break a non-wearing item: 0 sets is_broken (combat-honored) with no display cue and no in-game repair path | KnightOfNight | triaged, monitoring-and-command | Version 25.16 | 2026-09-05 |
...
- Milestone: Version 25.16
- Assignees: KnightOfNight
- Created: 2026-08-30 | Updated: 2026-09-05
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-09-05:
**Ruling (design session, 2026-09-05) — founding ticket of Version 25.16.**

**Shape A, confirmed by the operator: the door refuses `durability_current` edits on non-wearing definitions entirely.** When the target instance's definition has `takes_durability_loss=False`, an `edit_item` request naming `durability_current` refuses (`DoorError`, bad-params voice) — whole-request semantics as always (#289): no key applies. Rationale: on a non-wearing item the value is inert dormant data — nothing reads it (the penalty short-circuits on the flag, display is gated on the wearing definition, both repair paths filter on `takes_durability_loss=True`) — so any accepted write is a no-op dressed as admin power, and the 0 case arms the silent combat-honored `is_broken` landmine this issue documents. Refusal matches the door's exact-or-refused doctrine (the #320 `deduct_copper` precedent) and extends the durability-posture invariant's "nothing sits between the two postures" law (v25.12, #311) to the instance side.

**Flip-path sub-ruling (same problem, same release — operator-ruled in the same conversation): a wearing → non-wearing posture flip on an artifact definition resets its instance to healthy.** The door's artifact definition-edit vocabulary permits flipping `takes_durability_loss` (posture invariant enforced on the definition, `mc_door.py` #311 block), but today never reconciles the instance — a wearing artifact at 0%/broken flipped to non-wearing strands the identical silent-broken state through a different door, and under shape A the durability edit could no longer even fix it without flipping posture back. Ruled: the flip authors the non-wearing posture completely — `durability_current` resets to 100, `is_broken` clears, in the same atomic edit, narrated like any door write. (The reverse flip, non-wearing → wearing, inherits healthy state by construction and needs no special handling.)

Out of scope, deliberately: the Django admin instance form (direct ORM construction, the documented deliberate bypass — v25.7 residual-gap doctrine) and player-path writes (all posture-correct already).

Carrier release: **Version 25.16** (milestone set). Brief to follow on `version_25_16`.
