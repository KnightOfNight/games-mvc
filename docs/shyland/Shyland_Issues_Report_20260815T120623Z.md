...
## Issue #248: Closeout tail: a brief can specify a production-side verification step that no session can run (read-only prod queries have no sanctioned path)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/248

### Body

## The gap

A brief can write a **production-side verification step** into its PENDING DEPLOY-TIME ACTIONS block that **no session type is able to execute**. There is no sanctioned path to run a read-only query against production.

The closeout tail's only posture-setting targets are `make deploy-prod` and `make seed-prod`. Both are self-contained: they pin `DOCKER_HOST`, flip posture, do their one fixed action, and restore resting dev posture. Neither can be asked a question. Meanwhile:

- `make shell` is guarded by `crosscheck-env`, so from resting posture it targets **dev**, not production.
- Setting `DOCKER_HOST` by hand makes `crosscheck-env` fail, because `.env` is still the dev copy.
- Hand-flipping `.env` to prod posture is **forbidden** — switching posture is the operator's deliberate act, never something a session does to make a guard pass.

So the step is unrunnable, and the closeout inherits a promise it cannot keep.

## How it surfaced — V24.28 (2026-08-15)

`Shyland_V24.28_Brief_1_Closeout.txt` listed, under PENDING DEPLOY-TIME ACTIONS:

> **Expected production mismatch survey** (verification step 8 repeated against prod): **0 rows.**

Verification step 8 is a read-only `ItemInstance` query — count the rows sitting on a tier-material ladder definition outside their rung. It ran on dev during implementation (3 rows on ladder definitions, 0 outside their rung). It **could not run on production**, and the V24.28 closeout reported it as **unverified rather than assumed clean**.

Everything else in that tail was clean: `make deploy-prod` applied migration `0048`, `make seed-prod` reconciled **84 created / 12 updated / 0 deleted** exactly as expected, and every ladder assertion passed on production.

**Partial cover, for the record.** The V24.28 risk is low: `seed-prod`'s own on-prod verification asserted *"Every loot and vendor entry on a ladder definition sits inside its rung — 0 violations"*, so nothing on production can **mint** a mismatched instance, and the `generate_item_instance` guard is now live there. This issue is about the **missing path**, not about V24.28's data.

## Why it will recur

Nothing in the brief-writing process checks that a prod-side verification step is executable before it gets written down. Any future release that ships a data-shape invariant will want to confirm the invariant on production after the seed — the same shape of ask, the same dead end. It is also the second time the tail has been boxed out by posture rules: #187 (`make seed-prod`) closed the *data-action* case; this is the *read-only inspection* case, still open.

## The design question — not answered here

This is a process ruling, deliberately not made in a closeout session. Directions worth weighing:

1. **A sanctioned read-only prod target** — something in the `deploy-prod` / `seed-prod` contract shape (pins its own `DOCKER_HOST`, refuses an ambient one, flips posture, runs, restores) that executes a **read-only** query on production and prints the result. Requires ruling what "read-only" means and how it is enforced, since a Django shell is arbitrary code.
2. **Fold verification into the seed's own verification pass.** V24.28's ladder assertions rode `seed-prod` and did run on production. If a brief's prod-side check can be expressed as a seed assertion, it executes for free — but this only works for checks the seed can naturally make, and it re-runs on every seed forever.
3. **Rule the step out at design time.** A brief may not write a prod-side verification step unless a path exists to run it; anything else is reasoned about, stated as an expectation, and explicitly marked unverifiable. Cheapest option, and it removes the false promise rather than the gap.
4. **Leave it to the operator.** The step stays in the brief, but is addressed to the operator rather than the session — with the closeout reporting it as handed off, not done.

Whichever way this lands, the **brief-writing checkpoint** is the part worth keeping: at design time, confirm that every step written into a PENDING DEPLOY-TIME ACTIONS block has a session or a person who can actually execute it.

## Suggested outcome

A ruling plus, if it changes process rules, the next-numbered `Shyland_Project_Instructions_vN.md` edition — v31 is current and codifies the `seed-prod` half of this same problem (Deployment Law step 6).

---

Filed from the V24.28 closeout session at the operator's direction, for review in an ops session.
