...
| 68 | broadsword has a lifesteal value of zero | KnightOfNight | bug, B3 | Version 21 | 2026-07-17 |
...
| 93 | command behavior inconsistencies when no arguments passed | KnightOfNight |  |  | 2026-07-16 |
| 95 | the ring needs an area | KnightOfNight | stub |  | 2026-07-17 |
| 96 | examine doesn't autocomplete on NPC name | KnightOfNight | stub |  | 2026-07-17 |
| 98 | command 'who' needs color output | KnightOfNight | stub |  | 2026-07-17 |
| 100 | Wire equipped item stats into combat (armor mitigation, stat bonuses) | KnightOfNight |  | Version 22 | 2026-07-17 |
| 101 | Retune the Z01 boss ladder: HP curve and boss DEX offset, balanced for even-split builds | KnightOfNight | B3 | Version 21 | 2026-07-17 |
| 102 | Rule the ×3 aggro-elite rooms deadly-by-design: signpost them, ×2 rooms are the solo ceiling | KnightOfNight | B3 | Version 21 | 2026-07-17 |
| 103 | Make placeholder roster NPCs unattackable (Aldric, Info Prime, Seris, Veris) | KnightOfNight | B3 | Version 21 | 2026-07-17 |
...
**KnightOfNight** — 2026-07-17:
Scope narrowed by operator ruling (2026-07-17): v21 fixes the roll math only (a proc stat must never roll a dead value). Proc-stat SEMANTICS — what lifesteal means in combat — are deferred to the v22 combat wiring issue #100, since the stats are currently combat-inert (survey G7).


**KnightOfNight** — 2026-07-17:
V21 rulings (design chat, 2026-07-17) on the investigation's open questions (Q1 — proc-stat semantics — was previously deferred to #100):

- **Q2, fix mechanism — ruled:** raise the four broken stat curves to `base 0.5, factor 0.2`, matching the working proc family (crit_chance/bleed_chance/stun_chance). Affected pool entries: `lifesteal` (Iron Sword, Broadsword, Iron Shortsword), `poison_chance` (Combat Knife), `electric_damage_bonus` (Pulse Pistol), `mana_regen` (Apprentice Staff). Pure seed-data change; Mk 1 rolls become ≥1 at every rarity; no floor, no re-draw, no pool gating.
- **Q3, display — ruled:** zero-value stats are NOT hidden. A rendered zero is a bug signal and stays visible. No display changes.
- **Q4, existing data — ruled:** one-time cleanup re-rolls every zero-value entry stored in `rolled_secondary_stats` on existing ItemInstances, using the corrected curves. The item keeps the stat it drew; the entry gets a real value.

Implementation lands in the B3 brief.
...
## Issue #93: command behavior inconsistencies when no arguments passed
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/93

### Body

[17:01:25.04] > brief
Brief mode is off. Usage: brief on | brief off
[17:01:29.67] > timestamps
[17:01:29.67] Usage: timestamps on|off
[17:01:53.50] > timestamps off
Timestamps are now off.
> brief shit
Usage: brief on | brief off
> timestamps on
[17:02:07.84] Timestamps are now on.
[17:02:09.83] > brief shit
[17:02:09.83] Usage: brief on | brief off
[17:02:17.64] > timestamps
[17:02:17.64] Usage: timestamps on|off
[17:02:21.69] > brief
Brief mode is off. Usage: brief on | brief off


not all output is timestamped even when timestamps are on.


timestamps should report its setting the way brief does and not just report the usage.


research spike: check all boolean settings controls, they should all work the same.  'brief' is the current standard.
...
## Issue #101: Retune the Z01 boss ladder: HP curve and boss DEX offset, balanced for even-split builds
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/101

### Body

Survey findings G1 + G3 (#89, docs/shyland/Shyland_V21_Kill_Feasibility_Survey.md): at the blessed 25% at-level boss hit rate, feasibility decays linearly in boss HP, and the seeded HP ladder (120→260→320→500→650→850) crosses the 20-potion bound at the Undercrag Weaver — the three delve bosses are INFEASIBLE at any attainable level. Separately, the blessed calibration is only real for max-DEX builds: the d20 bridges 20 points, so an even-split build (the operator's 25/25) does zero damage to the Whistler at L8. #66 was the first symptom of both.

Operator rulings (2026-07-17), the design parameters for the retune:

1. **Balance target:** a player who splits points evenly across their two primary stats beats every Z01 boss at the intended level. The boss DEX offset shrinks accordingly.
2. **Potion budgets** at intended level, playing decently: normals ~0, elites ≤3, bosses ≤8, zone-final boss may touch ~12.
3. **Boss fight length:** 8–12 rounds.
4. **Intended kill levels:** Matron L3, Whistler L6, Dronemother L6, delve trio L8 / L9 / L10.
5. Escort compounding (G2) is budgeted INSIDE the numbers above — the encounter with adds, not the solo boss, meets the potion/length budgets.

Exact numbers (per-boss HP, DEX offset, any escort adjustments) come from the B3 design pass, derived from the survey's expected-value model against the even-split reference build. Elite-tier numbers are re-checked against the same budgets (elites ≤3) while in there. G5's even-level −5% rounding drift is noted as calibration noise; the retune's math should account for it but it needs no separate fix.

Absorbs #66 (the Whistler row of this ladder; its refined numbers are recorded there).


### Comments (1)

**KnightOfNight** — 2026-07-17:
Retune ruled (design chat, 2026-07-17). Full derivation: `docs/shyland/Shyland_V21_B3_Retune_Proposal.md`. All four proposal items confirmed: (1) the boss table, (2) the elite table, (3) flattened tier offsets 0/+2/+2, (4) the accepted consequence that the delve trio remains reference-build content until #100 ships gear.

**Boss table (authoritative):**

| Boss | Kill L | DEX → | STR → | HP → | Escorts → |
|---|---|---|---|---|---|
| Silk Matron | 3 | 25 | 17 | 150 | 2× brood (unchanged) |
| Whistler Below | 6 | 32 | 28 | 240 | 2× young (unchanged) |
| Dronemother | 6 | 32 | 30 | 260 | 2× swarm (unchanged) |
...
Boss DEX is implemented as the tier offset (+6 → +2); the per-boss values are the resulting numbers for verification. Delve escorts drop from 3 to 2 and keep elite tier with the reduced STR/HP shown.

**Elite table (authoritative):** offset +3 → +2 for the whole tier, plus HP trims: elder-cave-spider 110→95, elder-cave-centipede 130→100, elder-cave-beetle 150→110, prowling-mountain-lion 150→110, territorial-brown-bear 170→120. All other elites change via the offset only.

**Unchanged:** all normal-tier NPCs, villagers, ×3 room spawn counts (#102 owns signposting), the Whistler's intended kill level, potion price/healing, all combat formulas (data + two tier-offset constants only).

Budget verification (even-split reference at intended level, Uncommon band weapon): every boss encounter, escorts included, runs 8.7–13.0 rounds and costs 0/6/8/7/7/10 potions against budgets of ≤8 (final ≤12).


## Issue #102: Rule the ×3 aggro-elite rooms deadly-by-design: signpost them, ×2 rooms are the solo ceiling
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/102

### Body

Survey finding G4 (#89): the ×3 aggro-elite rooms (vr-m38 prowling mountain lions, vr-m39 territorial brown bears) breach the 20-potion feasibility bound even at band top with the best band weapon (20 and 27 potions at L10). The ×2 versions (vr-m11, vr-m24) sit just inside feasibility.

Operator ruling (2026-07-17): the ×3 rooms stay deadly — deliberately, as signposted "don't" content — and the ×2 rooms are the ceiling of intended solo content. Implementation: authored warning text (room descriptions and/or villager dialogue) must make the danger unambiguous before entry; no stat changes to the rooms themselves. Warning prose is authored creative content (Claude-authored, compass-audited per the geography rule).
...
## Issue #103: Make placeholder roster NPCs unattackable (Aldric, Info Prime, Seris, Veris)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/103

### Body

Survey observation G7 (#89): four Convergence roster NPCs are attackable with placeholder stats (base_vitality=999, all stats 1) — a patient level-1 player can slowly kill named service NPCs for 10 XP each.

Operator ruling (2026-07-17): set attackable=False on all four, matching the rest of the service roster. Data fix (seed + reseed), v21.
