...
- **`home` (v22)** — the hearth command: a 10-second delayed return to the player's attuned home node (default: the Heart of the Convergence — attunement is v24.26, pending implementation, #38), from anywhere, narrated as fog-motif atmosphere; broken by movement, combat, or `cancel`; 5-minute completion-only cooldown (timings retuned v23.4, #162). Full design in Section 2.11.
- **The Obelisk Network** — the game's fast-travel system: obelisks send anywhere; checkpoint shards relay to revealed obelisks (v24.26, pending implementation, #30); all nodes are destinations, revealed per-character by visiting them. Free, global, and command-driven (`travel`). Full design in Section 2.11.
...
A player deep in a zone therefore has three ways out: walk, `home` (the hearth command, to their attuned node), or push forward to the summit obelisk — and once they reach any checkpoint, its shard relays them to a revealed obelisk (v24.26, pending implementation, #30). (The once-planned recall scroll is retired — v24.26, #38: `home` plus attunement covers the entire command-driven-return need; killed, not deferred.) Conquering a zone's obelisk is what turns that zone from a place you trek through into a place you command.

#### The `travel` Command

Travel is a simple command — no dialogue system required:

- `travel` — lists the destinations the room's node offers: at an obelisk, every revealed node; at a checkpoint shard, revealed spheres only (v24.26, pending implementation, #30). In a room with no node it explains that travel requires an obelisk or its shard.
...
- **Shards relay (v24.26, pending implementation, #30).** A fragment of an obelisk carries a fragment of its power: from a Shard's room, `travel` reaches the player's revealed spheres — never other shards. The Network Shape rules above govern; the Shard is the diegetic face of the relay.
...
#### Attunement — Home Is a Choice (v24.26, pending implementation, #38)
...
`home` is the way back when there is no obelisk near: a **10-second delayed return to the player's attuned home node** (destination generalized from the fixed Heart by attunement — v24.26, pending implementation, #38; delay v23.4, #162, retuned from 15 seconds — 5 was considered and rejected as near-instant escape), usable anywhere, in home's own fog-motif voice — a cousin of obelisk travel's machinery pattern, never its words.
