...
**The detail block states binding once (v24.20, pending implementation — #203).** Bound state lives solely in the item line's trailing flag block (`[Rarity, Bound|Unbound]`), which heads the detail block; no prose row restates it. The block's conditional tail rows are exactly `Equipped:` and `Curse:` — each carrying a fact the flag block does not. The former `Note: This item is not yet bound — you may drop it.` and `Bound: This item is bound to you.` rows are deleted: the first read misvoiced on ground items once examine stopped requiring pickup, and both restated an on-screen fact under inconsistent key styles.
