...
**The invariant extends to the instance side (v25.16, #315).** A non-wearing definition's instances carry no durability semantics at all — `is_broken` never sets on them, and no write path may arm it. The agent door refuses `durability_current` edits outright when the definition doesn't wear: the value is inert dormant data there (the penalty short-circuits on the flag, display is gated on the wearing definition, both repair paths filter on it), so a write that stores what nothing reads is a trap dressed as admin power — and the 0 case armed a combat-disabled, display-invisible, in-game-unrepairable item. A wearing → non-wearing posture flip on an artifact definition resets its instance to healthy — durability 100, broken flag cleared — in the same atomic edit: authoring the non-wearing posture includes the instance.
...
Some items carry a hidden curse. The full engine is ruled on #297 (the Q1–Q7 system rulings) and #330 (the engine shape set); the apply-time admission doctrine its ticking components ride is Section 6.9's admission policy (#331). The engine shipped in v26.2 with acquisition **admin-only**; **v26.3 — Release B of the arc (#297) — puts curses in the world** (v26.3, pending implementation): the `cleanse` service, the `inspect` presence sweep, and live generation-time drops in Zone 1.
...
- A curse `EffectDefinition` is marked as such (`is_curse`) and is built from **real components** — the cuts, drains, and multipliers of Section 6.9. The old `curse_generic` placeholder type is retired in place. Each curse also carries its authored **`cleanse_price`** — the copper an NPC charges to lift it from a Mk 1 item (see Removal) (v26.3, pending implementation).
...
- **The one sanctioned presence check: `inspect`** (v26.3, pending implementation) — the cleansers' separate paid sweep of everything the player carries, inventory and equipped alike. Priced **per item carried (5 copper each)**, charged in one motion; the answer names every cursed instance carried by its item line — *that* it is cursed and which one, never what the curse is or does — or gives the all-clear. It sets no flags (`curse_identified` is untouched); pooled lines in the cleanser's voice; insufficient funds is the standard refusal naming the price, nothing spent. "Look at all my stuff, is it OK?" costs more the more stuff you have — a deliberate purchase before a risky equip, not a reflex after every drop. Sirius's nose stays outside both rules, whim-gated (#259).

#### Removal

- **Only an NPC can cleanse — never a potion or consumable.** Low-level expiring curses may simply be waited out.
- **The `cleanse` service (#297) (v26.3, pending implementation).** `cleanse <item>` at a cleanser lifts a live curse from an **equipped** item: the pool is equipped items only (the `unequip` pool), so an unequipped item misses the pool identically whether latent-cursed or clean — the no-leak rule holds by construction — and only active curses are ever cleansed (no pre-equip defusing; see Knowledge). One target per transaction (no `all` form). Cleansing **always succeeds** — no chance roll; the price is the whole cost, charged in one motion — and ends the curse through the one teardown (cause `cleansed`): the item clean forever, the memorial stamped, the curse-ending line spoken in the cleanser's voice. An equipped item with no curse, no cleanser in the room, or a purse short of the price each draw a world-declined line (the price named, nothing spent). Refused in combat and while dying.
- **Price is tied to the curse itself, never the player:** the charge is the curse's authored `cleanse_price` × the item's Mk tier. The Z01 expiring tier is pocket change for a Ridge-level player — Curse of the Leaden Arm 75 · Curse of the Craven Edge 60 · Curse of the Moth-Eaten Ward 75 · Tithe of the Copper Court 90 · The Whisper in the String 120 copper at Mk 1; the no-expiry tier is deliberately beyond a Z01 purse — The Hollowing 5 gold · Gravekeeper's Hold 8 gold · The Threefold Ruin 15 gold at Mk 1, times Mk — so a low-level player who wears a high-tier item meets the price of that gamble in copper. Once a curse is identified, `examine`'s revealed block shows the cleansing price, tier-formatted.
- **The cleansers of the shipped world:** `NpcDefinition.is_cleanser` marks them (the `is_repairer` pattern; `attackable=False`, seed-verified). Mother Tansy cleanses at the Convergence; in Z01 the three checkpoint Menders — Maro (Fordwatch), Tavik (Stairhead), Old Brammel (Cragfoot) — gain the specialty, so every checkpoint trio cleanses. Commands route to the living cleanser in the room; every cleanser speaks its quotes, successes, refusals, and sweep results in pooled lines of its own voice. Every new zone ships its own cleanser (#179).
...
- **v26.3 (Release B, #297) — live in Zone 1** (v26.3, pending implementation). The wild chance goes to **one in three** (`CURSE_WILD_CHANCE = 1/3`): each Rare-or-better drop from a curse-pooled definition rolls a latent curse a third of the time. As seeded, only the three Viridian Ridge bosses roll Rare+ (chapter-1 bosses roll Uncommon; vendor buys are always Common; villager drops top out at Uncommon), so curses begin in the zone's third chapter, where the player is strongest and the expiring curses are affordable lessons — blessed as progression, chapter-1 weights untouched. The candidate pools and the gift path are unchanged from v26.2.
...
|`dot_vitality`      |Duration, ticking  |Vitality damage per combat round — pauses passive vitality regen while active (Section 4.1, #338) (v26.3, pending implementation)|
...
- **Commands (v22 grammar — Section 9.1 is authoritative):** `list` (vendor stock), `buy [<N>] <item>` (numeric quantity only; `all` refused), `sell [<N>|all] <item>` and `sell all <rarity> [<item>]` (bare `sell all` is refused with wording that teaches the noun form), `repair <item>`, and `repair all` — which **loops**: passes over what is still damaged until everything is repaired, funds run out, or 5 passes (#75), each mend line printing as it lands. Bare `buy`/`sell`/`repair` prompt for a target (the v18 bare-repair convenience retired to the standard prompt). Commands route automatically: buy/sell/list to the living vendor in the room, repair to the living repairer, cleanse/inspect to the living cleanser (v26.3, pending implementation) — killed service NPCs are out of business until they respawn. The noun-less `sell all <rarity>` excludes the consumable type by default (v23.1, #150) — consumables sell only when named (`sell all draught`); a skipped stack is announced with one note line, and a bulk sell matching only consumables is a world-declined refusal.
...
**The Convergence gearing-up story:** the hub clothes new players. Morra (smithy) vends weapons and armor and repairs anything; Pella and Ferwick (the gazebo) vend trinkets and bags and repair — one shared stock, two voices; Repairbot Prime repairs only; Mother Tansy's remedy cart on the east ring vends draughts and, from v26.3, cleanses curses and sweeps packs for them (Section 6.7) (v26.3, pending implementation). All vendors, repairers, and cleansers are `attackable=False` by rule (seed-verified, hard failure).
