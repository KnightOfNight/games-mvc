...
**Recovery:** Longevity recovers passively out of combat under the same proportional-to-max law as Vitality (v24.0, pending implementation) with a much slower time constant (`LONGEVITY_REGEN_SECS = 3600`): the rate is `longevity_max / LONGEVITY_REGEN_SECS` points per second, implemented as **one point every `ceil(LONGEVITY_REGEN_SECS / longevity_max)` seconds** — integer bars cannot carry fractions, and the per-tick ceil form would degenerate to 1 point per second for any bar smaller than the constant. Full recovery from zero takes approximately one hour at every level — 30× slower than Vitality. (Pre-v24, that degenerate 1-point-per-second behavior *was* the shipped behavior: Longevity accidentally refilled in minutes; the law is corrected while nothing yet drains the bar, so no player feels the change.) Warden abilities can accelerate this. It is the hardest bar to restore and the one players are most likely to mismanage over a long dungeon run.
