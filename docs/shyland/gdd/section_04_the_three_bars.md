...
**Recovery:** Healing spells, medkits, potions, and passive natural regeneration. Passive regen is always active when not in combat and not in the Dying state — no rest command required. **The regen law: proportional to maximum.** The rate is `vitality_max / VITALITY_REGEN_SECS` points per second, applied per tick as `ceil(vitality_max / VITALITY_REGEN_SECS)` and clamped at max. At the constant of 120 seconds, a full refill from zero takes exactly **120 seconds at every level** — the deeper the pool, the faster the points return; refill time never grows with vitality growth, so time remains a real substitute for draught money at any level. Regen is silent — no message is sent; players observe recovery through the status bar. **An active vitality drain pauses passive vitality regen (#338) (v26.3, pending implementation).** While any `dot_vitality` component is active on the character, the vitality arm of passive regen does not run — the same shape as the in-combat pause: nothing passively recovers while you are bleeding. Bar-scoped (Longevity regen and Acuity drift are untouched — three independent tanks), source-blind (curse drains, NPC bleed and poison procs that outlive a flee, and consumables alike), and silent (the bar simply stops climbing). Heal-over-time effects and consumables still work — they are effects, not passive regen — and the floor-hold (Section 6.9) is its own component type: it caps regen at the hold and does not trigger the pause. The reason is arithmetic: regen is proportional to the bar (about `vitality_max / 40` per combat round) while drains scale with item Mk, so no authored magnitude could be right at every level; without the pause a low-Mk drain is theater (at a 614 bar, regen refunds 18 a round — a Mk 1 curse drain of 8 can never kill).
