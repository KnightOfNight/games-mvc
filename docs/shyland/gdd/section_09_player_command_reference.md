...
- **Heal sequences (#61, #65; reruled by #151) (v23.3, pending implementation):** `use N` on an instant-restore healing consumable is **one computed transaction** — the deficit is measured up front, exactly the items the heal needs are consumed (capped by N and inventory, lowest-value first as ever), and the heal lands as one act: one line, one status update. A heal that reaches full folds `You are restored to full health.` into the end of that line, the whole line reward-color; any heal attempted at full still fails (warn); the shortfall warn (`You only had N.`) keeps its own line, firing only when the request exceeded inventory and the deficit wasn't covered. While dying, `use` consumes exactly one restorative — the v19 revival sequence, never over-drunk. Timed-effect consumables keep the per-item sequence.
