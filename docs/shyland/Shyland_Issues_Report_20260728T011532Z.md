...
| 155 | shyland_issues_report.py crashes on Python 3.9 — require 3.14+ via env line and explicit version guard | KnightOfNight | bug, B1, triaged | Version 23.2 | 2026-07-28 |
...
## Issue #155: shyland_issues_report.py crashes on Python 3.9 — require 3.14+ via env line and explicit version guard

- State: open
- Author: KnightOfNight
- Labels: bug, B1, triaged
- Milestone: Version 23.2
- Assignees: KnightOfNight
- Created: 2026-07-26 | Updated: 2026-07-28
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/155

### Body

## Problem

`scripts/shyland_issues_report.py` crashes when run with the system Python (3.9.6 at `/usr/bin/python3`). It works correctly on Python 3.14 (`/opt/homebrew/bin/python3.14`).

Cause: the script uses PEP 604 union annotations (`str | None`, `tuple[...] | None` — lines 73, 103, 130), which are evaluated at function-definition time and raise `TypeError: unsupported operand type(s) for |` on anything below 3.10. The shebang is `#!/usr/bin/env python3`, which resolves to the 3.9 system interpreter on macOS, so an operator running the script bare gets a crash instead of a report.

## Fix (ruled)

Require Python 3.14 or newer at runtime, two layers:

1. **Shebang**: point the `env` line at `python3.14` instead of bare `python3`, so direct execution finds a working interpreter.
2. **Explicit version guard**: a `sys.version_info >= (3, 14)` check that exits with a clear message naming the required version and the interpreter that was actually used.

Implementation notes:

- The guard must be placed **before the first annotated `def`** and use only 3.9-compatible syntax, otherwise the annotation `TypeError` fires before the guard ever runs and the friendly error is never shown.
- The guard makes the script fail loudly and self-diagnosingly on any future wrong-interpreter invocation (e.g. `python3 scripts/...` from the usage line in the docstring — update that line too).


### Comments (2)

**KnightOfNight** — 2026-07-28:
**Design ruling (Version 23.2 design session, 2026-07-27).**

The in-body ruling is confirmed as final, with the following made explicit:

1. **The 3.14+ floor is deliberate, operator-confirmed.** The technical syntax minimum is 3.10 (PEP 604), but the ruled floor is 3.14 — the interpreter the script is actually run and tested on. Guard for the supported environment, not the syntax floor.
2. **Diagnosis verified against the script on branch `version_23_2`:** the three PEP 604 union annotations at lines 73, 103, 130 are the only ones in the file (all other `|` occurrences are markdown table strings; remaining annotations are PEP 585 builtins, fine on 3.9). No `from __future__ import annotations` present. Docstring usage line (line 10) reproduces the failing invocation and updates with the fix.
3. **Guard placement ruled precisely:** top of module, immediately after the import block — before any annotated `def` — in syntax that parses and runs on old interpreters, so the friendly message always wins the race against the annotation `TypeError`.

**Disposition:** founding ticket of Version 23.2 (point release — scope law: one bucket B1, one brief, one ticket). Labels `B1` + `triaged` applied in the same motion as this ruling. Implementing brief: `Shyland_V23.2_Brief_1_Issues_Report_Python_Guard.md` on branch `version_23_2`.

Release note: no GDD changes and no game-code changes beyond the version stamp — the release still runs the full version-increment + dev-deploy + closeout rhythm by operator direction (v23.2 is the end-to-end process test under Instructions v25).


**KnightOfNight** — 2026-07-28:
**Ruling addendum (Version 23.2 design session, 2026-07-27) — two deltas the brief carries beyond this issue's body:**

1. **Executable bit.** Design-session verification found `scripts/shyland_issues_report.py` is not executable (`-rw-r--r--`), so the shebang layer of the ruled fix has never been reachable by direct execution. The brief adds setting the executable bit (`chmod +x`, mode change committed) as part of the fix, with a mode-line verification step.
2. **Architecture doc: ruled no-op for this brief.** Nothing architectural changes (repo tooling only) — the 23.2 stamp moves at closeout as version bookkeeping with the header hash unmoved (doc-only point release).

Both are specified in `Shyland_V23.2_Brief_1_Issues_Report_Python_Guard.md` (branch `version_23_2`, commit `37e4e77`).
