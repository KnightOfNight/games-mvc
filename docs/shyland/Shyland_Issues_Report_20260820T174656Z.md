...
| 267 | MC egress: how remote consumers attach — Streams inside the trust boundary, WebSocket across it (auth, backpressure, resume) | KnightOfNight | monitoring-and-command, V25 | Version 25.3 | 2026-08-20 |
...
| 273 | MC stream carries no command outcome — readers infer refusals from adjacent out records (sudo observation) | KnightOfNight | monitoring-and-command |  | 2026-08-20 |
...
- Author: KnightOfNight
- Labels: monitoring-and-command, V25
- Milestone: Version 25.3
- Assignees: KnightOfNight
- Created: 2026-08-16 | Updated: 2026-08-20
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-20:
**Design ruling (2026-08-20, V25.3 design session): this is the founding ticket for Version 25.3.**

- Milestone `Version 25.3` created and applied; branch `version_25_3` cut from main (8cd54d3, the V25.2 merge) and pushed.
- Queue order follows the ruled ladder (recorded on #269's orbit): 25.3 = #267 egress, ahead of #266 (kill switch, 25.4) and the first live actor (25.5+). No actor goes live in this release — egress ships before anything attaches to act.
- The session now rules this issue's open questions (identity/authn, authorization granularity, wire format, duplex vs read-only, replay limits, resume semantics under volumeless Redis) and writes the release's one brief. Rulings land here as comments as they're made.
- Binding inputs carried in: the volumeless-Redis consequence (2026-08-18 comment — stream IDs don't survive restart; deep replay = Postgres) and the soft-RAM-ceiling note (2026-08-16 comment) shape the resume/replay contract.


**KnightOfNight** — 2026-08-20:
**Design rulings (2026-08-20, V25.3 design session) — the six open questions, operator-confirmed.** Recommendations served against code verified at ruling time (`mc.py`, `run_mc_persister.py`, `MCEvent`, `game_mvc/routing.py`, `settings/base.py`).

**R1 — Identity & authn.** Agents are ordinary Django users — **one service account per agent** (attribution). Access gated by a new group **`agents.shyland`**, on the exact `admins.shyland` pattern (live group check at connect). Auth is **session auth over WSS, same as players** — programmatic login at `/accounts/login/`, session cookie through `AuthMiddlewareStack`. No token machinery in 25.3. `is_active` and group removal are natural cut levers for #266.

**R2 — Authorization granularity.** **Full stream for any `agents.shyland` member.** Per #260, everything is data; access is the grant and the grant is group membership. No scoped subscriptions in 25.3 (agents filter client-side; scope law — build nothing for absent systems).

**R3 — Wire format.** Typed JSON frames. One server **hello** frame at connect carrying `protocol: 1` (schema versioned per-connection, not per-event). Event frames: `{"type": "event", "id": <stream-id>, "kind", "actor_id", "actor_name", "room_id", "audience": [...], "data": {...}}` — server decodes the JSON-encoded stream fields; agents receive one clean object per event.

**R4 — Read-only.** The MC endpoint is **read-only**: inbound frames are protocol control only (resume); everything else refused/ignored. Zero actuation surface in 25.3 — actuation is #261's design, and the ladder ships the kill switch (25.4) before any actor (25.5). Duplex, if ever, is a future #261 ruling.

**R5 — Resume/replay contract.** Default attach = live from now. Resume = client presents its last-seen stream id; server replays after it from the hot window (batched reads), then goes live. **Gaps are announced, never silent:** requested id older than the window's oldest entry (trim or Redis restart — same symptom, same handling) → an explicit **gap** frame (requested id, oldest available), then stream from oldest. Deep replay is not served over this socket — durable-store territory, future consumer-side work. Implements the 2026-08-18 volumeless-Redis comment. **Remote readers do not use consumer groups** — the agent owns its cursor and resumes by id; server-side reads are stateless. Consumer groups remain the persister's mechanism (it needs ack/pending; a reconnecting agent doesn't). Deliberate simplification of the original sketch.

**R6 — Replay depth.** **The hot window is the limit** — no artificial cap below it (~3–5 days at measured 0.6–1.03 records/sec against the 250k `MAXLEN`). Bounded by construction. Backpressure stays player-grade: a slow client is nginx/Daphne's problem; the sink never waits on egress.

**Shared-surface approval (CLAUDE.md Rule 2), operator-granted this session:** the implementation adds **one line to `game_mvc/routing.py`** registering the MC consumer route — the same registry motion `SkylandConsumer` used. The brief will scope it exactly.

**#273** is explicitly **deferred to the 25.5 actor session** (comment recorded there).

Next: GDD text (§10.11 orbit, marked `(v25.3, pending implementation)`), then Brief 1, born committed on `version_25_3`.


**KnightOfNight** — 2026-08-20:
**Ruling addendum (2026-08-20, V25.3 design session, recorded at session end):** MC egress connections are **not captured as stream events** — an agent attaching is not an in-world event, and the `connect`/`disconnect` kinds mean player presence (GDD §10.11). Attach/detach produce `shyland.mc` logger lines carrying the agent username; agent-fleet observability belongs to #268's arc. Settled in Brief 1 §2 rule 7 (`Shyland_V25.3_Brief_1_MC_Egress.md`, committed 0543d6e); surfaced to the operator in-session with no objection. Recorded here so the ruling lives on the tracker, not only in the brief.
...
**KnightOfNight** — 2026-08-20:
**Design ruling (2026-08-20, V25.3 design session): deferred to the first actor session (25.5, #262's orbit).**

V25.3 designs and ships the MC egress (#267) — transport only. This issue is reader semantics ("how does a reader know a command was refused"), and per its own text it belongs to the session that designs the reader/agent work. The 25.3 egress rulings change nothing here: the stream still carries no outcome field, and the endpoint is read-only, so the options enumerated in the body (reader interprets + DB perms / outcome-correlation field / permission snapshot) all remain open for the actor session to weigh.

Not forgotten — parked with a named pickup point.
