...
## Issue #189: Bare 'loot' behaves like 'loot all' — target becomes optional, defaulting to the sweep
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/189

### Body

## Ruling (operator, 2026-08-01, recorded live)

**Bare `loot` behaves exactly like `loot all`** — verbatim, in every case. The target becomes optional, defaulting to the room sweep.

## Diagnosis (current behavior, verified in code)

- Bare `loot` never reaches the handler today: `loot` sits in the `PROMPT_VERBS` table (`consumers.py` ~273), so the central fn-10 gate in `_dispatch` (~536) answers **"What do you want to loot?"** as a CLI error — the §9.1 footnote-10 standard for required-target commands (v22 Brief 2, DD §1).
- `loot all` (`cmd_loot`, ~1766): sweeps the room's corpses **killed by you** (`killed_by_id` gate); "That is not your kill; you may not loot it." if none are yours.
- `loot <NPC>` / `loot N.<NPC>`: single-corpse loot via `parse_corpse_noun` with bad-index and absent-corpse refusals.
- No corpses at all: "There is nothing to loot here." — emitted before argument parsing, so it already covers the bare form identically.

## Implementation shape

1. Remove `loot` from `PROMPT_VERBS` so the bare invocation reaches `cmd_loot`.
2. In `cmd_loot`, treat empty args as `'all'` (e.g. `args.strip().lower() in ('', 'all')`).
3. Everything downstream is inherited unchanged: kill-gating, copper transfer on first loot, sweep output, empty-corpse honesty, not-your-kill refusal, corpse-noun forms.

## Doc and doctrine consequences (ship with the release, GDD-first)

- §9.1 chart row: `loot all | <NPC>` → `loot [all] | <NPC>`, description unchanged in spirit ("Loot every corpse here, or one named corpse"). In-game help line at `consumers.py` ~1004 matches.
- Footnote 10's scope: loot simply leaves the required-target set — the footnote's rule is untouched for every other verb. **This makes loot the first verb whose bare form performs the `all` sweep.** Deliberately NOT a precedent for `sell` — #150 ruled bare sell-all stays refused; the distinction is that loot is kill-gated and value-safe, sell is destructive of inventory.
- Tests: any literal assertion on "What do you want to loot?" converts to the new behavior; the brief names this conversion explicitly (test-hygiene rule). New test: bare `loot` ≡ `loot all` (sweep path, kill-gate refusal, no-corpse warn).

No open sub-questions — the ruling covers all cases by inheritance.
