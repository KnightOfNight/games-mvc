...
| 290 | Agent door: no room-directory query — rooms unreachable except through characters | KnightOfNight | triaged, monitoring-and-command | Version 25.8 | 2026-08-27 |
| 294 | sudo bot: durable admin-taught memory — named waypoints ('send shy-guy into battle') and item bundles ('the badass armor set') outlive conversation expiry | KnightOfNight | triaged, monitoring-and-command | Version 25.8 | 2026-08-27 |
| 296 | sudo inventory reports are unreadable comma prose — answer path needs a game-rendered report delivery (leader line in sudo voice + the shared inv/equip item-line rendering) | KnightOfNight | triaged, monitoring-and-command | Version 25.8 | 2026-08-27 |
...
| 299 | botctl: one pidfile/log/conversation store per checkout — dev and prod bots can't coexist from main, and status/stop are target-blind | KnightOfNight | bug, triaged | Version 25.8 | 2026-08-27 |
| 300 | Agent door: time-windowed MC-history query — generic game-log search over the durable record (MCEvent) | KnightOfNight | triaged, monitoring-and-command | Version 25.8 | 2026-08-27 |
| 301 | sudo bot: file GitHub issues from in-game — Q&A gathering like the artifact flow, filed on explicit confirm | KnightOfNight | monitoring-and-command |  | 2026-08-27 |
...
**KnightOfNight** — 2026-08-27:
**Design ruling (2026-08-27, V25.8 design session): joins milestone `Version 25.8`** as a dependency of founding ticket #294 — operator-confirmed. The complementary-arc fold suggested in the earlier comment is happening: #290 (resolve authored room names on demand) and #294 (admins define their own names) ship together. This ticket's design questions (query shape, result shape, ambiguity handling, cap, `move` result carrying the origin room) will be ruled in this session.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — the query batch, confirmed:**

- **Query `rooms`:** case-insensitive name-substring search, optional zone filter, mirroring `items`' shape.
- **Result rows:** room `id` + the live `Zone: Area: Room` path, capped at 50. Duplicate names across zones are multiple rows, not an error — the model disambiguates conversationally.
- **`move` result gains `from_room`** alongside the destination — "send them back" works from the door's own receipt, independent of bot memory. (Complements #294's waypoints and #300's record search.)
...
**KnightOfNight** — 2026-08-27:
**Design ruling (2026-08-27, V25.8 design session): this is the Version 25.8 founding ticket.**

Milestone `Version 25.8` = {#294 (founding), #290, #296, #299} — operator-confirmed in-session. Branch `version_25_8` cut from main at 5c7f21d. Release theme: the sudo bot's usability arc from its first week of real use — durable memory (this ticket), room resolution (#290), readable reports (#296), and target-scoped bot state (#299). This extends the "places and memory" arc reserved at the 25.7 design session (#290+#294) with the two subsequent playtest/closeout findings.

Design questions in this ticket's body are the session's main ruling surface; rulings will land here as they settle. Not `triaged` yet — that comes with the rulings and the brief.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — placement, and the generic-store direction:**

1. **Taught memory lives game-side** — models + door vocabulary, not a bot-side file. Rationale accepted in-session: #299 demonstrates the bot-side state failure class; game-side teach/forget emits MC records (everything-on-the-record); survives bot restarts, conversation expiry, and checkout moves; shared by future bots by construction.
2. **The model is generic bot storage, not sudo-specific** (operator, verbatim in substance: moving state to the database will be useful for other bots too — make sure the model is not tied to a specific bot, but rather generic for any bot data storage). Waypoints and bundles are the v1 *kinds* stored in it, not the model's shape.

Model shape, scope (per-admin vs shared), payload shapes, and door vocabulary are the session's next ruling batch — landing here as they settle.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — scope: shared, not per-admin.** Taught memory is one namespace per (agent, kind) across all admins — any admin's "battle" is every admin's "battle". `taught_by` carries the audit trail; per-admin partitioning is not part of the design.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — waypoint payload: room PK only.** No path snapshot, nothing duplicative that can go stale (operator: we have to fetch the room any time we use it — why store data that is duplicative and can go stale?). The `Zone: Area: Room` path always renders live at use/list time (list via one joined query); a waypoint whose room no longer exists (world reseed, deletion) draws a legible world-declined refusal — no stored history behind it, the admin re-teaches or forgets. Waypoints dying across a world reseed is accepted behavior.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — bundle payload: resolved gift parameters, fresh generation on every replay.** A bundle stores `[{slug, mk_tier, rarity, quantity}, …]`; replay is N ordinary `gift` calls through the existing action — each server-validated, each MC-recorded, no new write power. The operator explicitly understands and wants fresh generation each time: same definitions/tiers/rarities, newly rolled instances per replay (rolled secondaries vary within the rarity's slots). No stat snapshotting — that would be duplicative, stale against definition changes, and a write power (`dictated-stat instance creation`) the door deliberately does not have.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — door vocabulary: generic verbs, kind-aware validation.** Three new door surfaces: query `memories` (list, filterable by kind/name; live-rendered summaries — waypoint's current Zone: Area: Room path, bundle's lines), action `remember` (kind, name, payload — payload validated per kind at teach time: waypoint room must exist, every bundle line must resolve to a real definition with valid tier/rarity/quantity; re-teach overwrites and the result says so), action `forget` (kind, name; not-found is a distinct legible result). Bot-side these are three tools; the model decides when to invoke from natural admin conversation (shipped tool doctrine — no teach syntax). Teach/forget emit MC records like every door action.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — Q4 amended: `forget` takes the memory PK, never a name.** Rationale: eliminates model-layer ambiguity (a guessed name could hit the wrong row; PK forces read-before-delete — query `memories`, see the row, destroy exactly that one), and matches the door's shipped mutation pattern (`remove_item`/`edit_item`/`equip_item` all target PKs resolved by a prior query). Alignment details: `memories` returns each row's `id`; `remember`'s result carries the `id` plus a created-vs-replaced flag. `remember` stays name-keyed upsert — teach is conversational and replacement is deliberate.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — Q5, caps and sizing (final):**

The operator's sizing intent: bots get a lot of database space — **1 GiB per bot** — decomposed into row-size × row-count since Postgres has no per-tenant byte quota. Final numbers, operator-confirmed:

- Bundle lines stored **positional** (`[slug, mk_tier, rarity, quantity]`) — ~2.2–2.7 KiB for a typical 50-line bundle, ~3.7 KiB absolute worst case (all slugs at the 50-char field max). The stored form is internal; `memory` detail rendering stays labeled/legible.
- **Payload ≤ 4 KiB serialized** (guarantees any legal 50-line bundle fits, no asterisk; operator chose 4 KiB over 3 KiB on that guarantee)
- **≤ 262,144 rows per agent** (= the 1 GiB budget at 4 KiB)
- **Bundles ≤ 50 lines** (bounds a replay to 50 gift calls — a burst control, not a byte concern)
- **Names ≤ 60 chars**
- **`memories` list capped at 50 summary rows** (id, kind, name, one-line gist), filter-first; **full payload only via `memory` detail by PK** — the items/item split, applied to memory. The context-window bottleneck, not Postgres, is what forces summary-first: 1 GiB fits in the DB; ~4 KB at a time is what a tool result can usefully carry.
- Every cap hit is a legible teach-time refusal, never silent.

Vocabulary is therefore four surfaces: `memories`, `memory`, `remember`, `forget`.


**KnightOfNight** — 2026-08-27:
**Operator rulings (2026-08-27, V25.8 design session) — time-window lookup doctrine, and the record/memory split:**

1. **`memories` gains `since`/`until` filters and returns newest-first.** The default query is the recent window; walking backwards is repeated calls with older windows (operator: starting from the present, moving backwards only as needed — most lookups stay recent, but the bot can be told when to look). Index on (agent, created_at).
2. **"What happened" questions are answered from the durable record, not taught memory** — operator ruled option (a): a time-windowed MC-history query on the door (filed as #300, joins `Version 25.8` as a dependency). Bots journaling their own actions into `AgentMemory` was considered and rejected as duplicative of what MC already records. Operator framing: sudo gains generic game-log searching abilities.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — Q6: two memory kinds only.** v1 rememberables are `waypoint` and `bundle` — no free-text `note` kind (deferred to its own ticket if ever wanted; the no-leak/audit questions free text opens stay unopened, and #300's record search covers much of what notes would have). Ruled in the same arc: **conversation threads stay bot-side** (recorded in full on #299) — the generic store holds taught facts, not working context.

This closes #294's design-question list: Q1 shared scope, Q2 room-PK-only waypoints, Q3 fresh-generation bundles, Q4 generic vocabulary (`memories`/`memory`/`remember`/`forget`, forget by PK), Q5 sizing (4 KiB / 262,144 rows / 1 GiB per bot), Q6 two kinds, plus the time-window lookup doctrine and the #300 record/memory split.


## Issue #296: sudo inventory reports are unreadable comma prose — answer path needs a game-rendered report delivery (leader line in sudo voice + the shared inv/equip item-line rendering)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/296

### Body

## Finding (V25.7 Brief 1 operator playtest, 2026-08-26)

`sudo show me <char>'s inventory` works, but the answer is barely readable/usable: the model flattens the roster into one comma-separated prose line, and the report omits the detail an admin actually needs (Mk tier, durability, rarity, flags).

The data is not the problem — the v25.7 `inventory` query (#293) returns `mk_tier`, `rarity`, `durability_current`, `is_broken`, `is_soulbound`, `is_equipped`, `equipped_slot` for every instance, uncapped. The bottleneck is the **answer delivery path** (v25.5/v25.6, all deliberate at the time):

1. The bot voice rules require plain text with **no line breaks** — 59 items collapse into comma prose.
2. `answer` caps at **2000 characters** (`MAX_ANSWER_LEN`, ellipsis-truncated) — a 59-item roster at inv-line detail is ~3–4k chars and physically cannot fit in one answer.
3. An answer renders as **one sudo-colored pane line**. The `inv`/`equip` rendering — the shared item-line composition (`compose_item_line`: name with Mk, durability suffix, `[Rarity, Bound|Unbound]` flag block, the colors) — lives in the consumer's report machinery, which the answer path never touches.

## Desired shape (operator, verbatim intent)

The sudo reply stays a one-line leader in its own voice/color:

> `sudo: Harley Stone (59 items total)`

followed on the next lines by the report **in the same format and detail as the `inv` and `equip` commands** — the game's own item-line rendering, colors and flag blocks included.

## Direction sketch (for the design session to rule)

A game-rendered report delivery on the door: instead of the model hand-writing a roster, the door renders the target character's inventory into the requesting admin's pane on the bot's behalf — leader line in `sudo` category, body through the same report machinery / `compose_item_line` path the player commands use. Bot-side prompt composition is a dead end here (the 2000-char cap alone kills it); prompt tuning was deliberately not applied as an interim (operator: "change nothing for now").

Refs: #293 (the inventory query), #262 (the bot), #281 (the door / `answer` path).


### Comments (2)

**KnightOfNight** — 2026-08-27:
**Design ruling (2026-08-27, V25.8 design session): joins milestone `Version 25.8`** as a dependency of founding ticket #294 — operator-confirmed. The desired shape in the body (one-line sudo-voiced leader + body through the shared `compose_item_line` report machinery) is the starting point for this session's ruling on the door's report-delivery mechanism.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — the mechanism batch, confirmed in full:**

- **New door action `report`** (recipient admin, target character, `kind`; v1 kind: `inventory`). Renders both sections as a player sees their own — equipped in paper-doll order (the `equip` composition), then carried (the `inv` composition) — through the shared `compose_item_line` machinery, colors and flag blocks included. The door reuses the player commands' report path pointed at another character; nothing new invented.
- **The door composes the leader line** (`sudo: <Name> (<N> items total)`, sudo voice/color) from live data — never model-supplied, so counts cannot lie. Bot commentary stays in `answer`.
- **Delivery private to the requesting admin's pane** (like `answer`, never room-broadcast), **gated admins-only** as `answer` is — verb-global gate, generalized later by the #282 scopes arc if ever needed.
- **`answer` untouched** — its 2000-char cap and no-line-breaks voice rules stand; `report` removes the roster load that was breaking them.
- New action kinds emit MC records for free via the mc_consumer pattern (25.7 receipt).
...
## Issue #299: botctl: one pidfile/log/conversation store per checkout — dev and prod bots can't coexist from main, and status/stop are target-blind
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/299

### Body

**Reported by the operator in the V25.7 closeout session (2026-08-26), immediately post-deploy:** botctl can't start the dev and prod bots from the same checkout — so running both from main (the between-releases resting state, when no version worktree exists) is impossible.

## Mechanism (verified at main tip 5804a79)

All three per-bot state files are keyed per checkout (or per bot name), never per target:

- `agents/sudo_bot.py:61` — `PIDFILE = AGENTS_DIR / '.sudo_bot.pid'` — a module constant, one per checkout, shared by every target (and not even `--bot`-derived).
- `agents/sudo_bot.py:62` — `CONVO_FILE = AGENTS_DIR / '.sudo_bot_conversations.json'` — one conversation store; dev and prod threads would interleave.
- `agents/botctl.py:65` — `self.log = AGENTS_DIR / f'{name}_bot.log'` — per bot name only; both targets append to the same file.

## Observed behavior

1. `botctl.py prod start` → prod bot up, owns the one pidfile.
2. `botctl.py dev start` → `cmd_start` polls the bot's own `status`, which reads the shared pidfile → "sudo bot already running — nothing to do", exit 0. No dev bot.
3. `stop` and `status` ignore the target argument entirely (`botctl.py:224-228` — `cmd_stop(paths)` / `cmd_status(paths)` never receive it). Operator-observed: `botctl.py dev status` and `botctl.py prod status` in the same directory both find the same daemon and report it OK — the target is accepted and then meaningless. Sharper edge of the same fact: `botctl.py dev stop` SIGTERMs whichever bot is running — including the prod one.

## Root design assumption

`botctl.py:9-11` records it: self-locating, "the main checkout for prod, the current version worktree for dev" — one bot per checkout. Between releases (worktree cleaned up, everything resting on main) that assumption leaves no home for the dev bot.

## Fix shape (for a design session to rule)

Target-scoped state derivation — pidfile, log, and conversation file keyed by (bot, target), not by checkout: e.g. `.sudo_bot.<target>.pid`, `sudo_bot.<target>.log`, `.sudo_bot_conversations.<target>.json`. Requires bot-side participation (PIDFILE/CONVO_FILE are `sudo_bot.py` module constants — they'd need to become CLI-parameterized or instance-suffixed) plus botctl passing the target through to `run`/`status`/`stop`. Gitignore patterns (`*.pid`, the conversation file) may need widening to match the suffixed forms.

## Workaround (immediate, no code)

Run the prod bot from the main checkout and the dev bot from the retained `version_25_7` worktree (`~/src/games-mvc-version_25_7`) — its agents/ copy is identical to the merged 25.7 code, and the self-locating design makes the two checkouts fully independent (separate pidfiles, logs, conversations). Worktree needs its own venv + `.secrets` (the V25.7 implementation session already provisioned both there, on operator approval).


### Comments (3)

**KnightOfNight** — 2026-08-27:
**Design ruling (2026-08-27, V25.8 design session): joins milestone `Version 25.8`** as a dependency of founding ticket #294 — operator-confirmed. The fix shape in the body (target-scoped state: pidfile/log/conversation store keyed by (bot, target); botctl passing the target through to run/status/stop) is the starting point for this session's ruling. Until 25.8 ships, the body's workaround stands: prod bot from the main checkout, dev bot from the retained `version_25_7` worktree.


**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session): the fix shape is confirmed as ruled direction.** All three state files become **(bot, target)-scoped** — `.sudo_bot.<target>.pid`, `sudo_bot.<target>.log`, `.sudo_bot_conversations.<target>.json` — with botctl passing the target through to `run`/`status`/`stop`, so prod and dev bots coexist in one directory (the main checkout, between releases). Gitignore patterns widen to match the suffixed forms.

Ruled in the same round: **conversation state stays bot-side, not in the database** (operator-confirmed while designing #294's generic store) — a thread is the model's working context, per-process by nature; its content is already substantially on the MC record (#300 makes it searchable); and a 20-exchange thread doesn't fit the 4 KiB memory-payload cap. A game-side conversation home, if ever wanted, is a future #268 arc slice.


**KnightOfNight** — 2026-08-27:
**Verification note (2026-08-27, V25.8 design session, brief pre-flight — late catch for the record):** `agents/.gitignore` already wildcards `*.pid` and `*.log`, so the suffixed pid/log forms are ignored as-is; only the literal `.sudo_bot_conversations.json` needs widening (to `.sudo_bot_conversations*.json`). The brief (`Shyland_V25.8_Brief_1_Bot_Memory_And_Record_Search.md`, §4.2) records this against the body's hedged "may need widening" sketch.


## Issue #300: Agent door: time-windowed MC-history query — generic game-log search over the durable record (MCEvent)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/300

### Body

## Operator direction (2026-08-27, V25.8 design session)

Ruled in-session while designing #294 (durable admin-taught memory): when an admin asks sudo about something that *happened* — the motivating example, verbatim in substance:

> "sudo 3 days ago I gave someone a sword, find that information and tell me about it"

— that is not taught memory; it is the record of actions, and it already exists durably as MC records. The operator chose **searching the durable record** over the rejected alternative (bots journaling their own actions into `AgentMemory` — duplicative of what MC already records, stale-able, and blind to anything before journaling started). Operator framing: **this gives sudobot generic game log searching abilities.**

Joins milestone `Version 25.8` as a dependency of founding ticket #294 — operator-confirmed in-session.

## Verified base (models.py:1275, `MCEvent`, checked at branch tip)

Append-only, deliberately FK-free (survives reseeds): `stream_id` (unique), `ts` (indexed), `kind` (16 chars), `actor_id`/`actor_name`, `room_id`, `audience` (JSON), `data` (JSON) — with composite indexes `(kind, ts)`, `(actor_id, ts)`, `(room_id, ts)`. Time-window-first search is what this table's indexes were built for.

## Design questions for the ruling

- **Query shape:** `since`/`until` window (windowed-first, newest-first — walk backwards only as needed, per the #294 lookup doctrine), plus filters: `kind`, `actor` (id or name), `room_id`, and some form of text match against `data`. Text match is the open design point — `data` is unindexed JSON, so the search semantics should be "filter by window and indexed fields first, then match within the bounded scan," and the window should be required or defaulted, never unbounded.
- **Result shape:** capped summary rows (50, matching `items`/`memories`), newest-first; whether a detail fetch by `stream_id` accompanies it (the items/item, memories/memory split).
- **Volume math for the ruling:** the stream runs ~1 combat-heavy record/sec (V25.2 measurement) — a multi-day window is hundreds of thousands of rows, which is exactly why indexed filters + caps come first and text match runs only inside the bounded window.
- **Audience/no-leak posture:** total capture (#260) means the record contains everything — chats included. For sudo this is ruled territory (admin audience; nothing is private to admins). But this query must be flagged to the per-agent authorization scopes arc (#261/#282): a future player-facing bot (Sirius) must not carry generic log search, or must carry a scoped form. Same shape as #282's answer-gate question.

Refs: #294 (founding; the lookup-doctrine rulings), #37 (the record), #260 (total capture), #261/#282 (agent scopes arc).


### Comments (1)

**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session) — the query batch, confirmed in full:**

- **Query `events` + detail `event`** (by `stream_id`) — the door's plural/singular split. `events` returns capped summary rows (ts, kind, actor_name, room_id, compact gist of `data`), newest-first, cap 50.
- **Filters:** `kind` (exact), `actor` (name case-insensitive, or id), `room_id`, `text` (case-insensitive substring against the serialized `data` payload — generic by design).
- **Window rules:** `until` defaults to now, `since` defaults to 24 hours back (recent-first doctrine, #294); explicit windows reach arbitrarily far back (walk-backwards pattern). **When `text` is present, window span caps at 7 days per call** — unindexed matching runs only inside a bounded window; indexed filters are safe at any span (composite indexes + newest-first + cap 50).
- **Audience:** admins-only via sudo; the #261/#282 scopes-arc flag stands — a player-facing bot never inherits generic log search as-is.

Numbers (24h default, 7-day text cap, 50 rows) operator-ratified as proposed.


## Issue #301: sudo bot: file GitHub issues from in-game — Q&A gathering like the artifact flow, filed on explicit confirm
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/301

### Body

## Operator direction (2026-08-27, V25.8 design session, filed at operator direction)

> make sudobot file tickets — Q&A style like building up an artifact

— so the operator can create GitHub issues **right from the game, at the moment of discovery**. This is the issue-first law's front door moved into the world: today a playtest finding means leaving the game to file; with this, "sudo, file a ticket about X" captures it thin on the spot, and the pipeline (triage → ruling → brief) proceeds as always.

The named precedent is the artifact-creation flow: sudo gathers what it needs conversationally (Q&A, building the thing up), reads the draft back, and executes on explicit confirmation.

## Design questions for the ruling session

- **Where the filing happens:** presumably bot-side (a GitHub token in the bot host's `.secrets`, GitHub API call) — the game box holds no GitHub credentials and the door stays game-scoped. Credential custody lands under the #268 arc's standing law (named, never printed, never committed).
- **The Q&A gathering shape:** what sudo must collect before filing (title, body) vs what it applies by convention (`--assignee "@me"`; labels probably left for triage — operator filings are thin by doctrine).
- **The confirmation gate:** filing is an outward-facing act — the bot should read the draft back and file only on an explicit yes, artifact-flow style.
- **Audit/transparency:** the conversation itself is on the MC record (sudo requests and answers emit records), but the filing is an external act — whether it deserves an explicit record/log line, and how the filed issue links back (e.g. body notes it was filed via sudo).
- **Audience:** admin-only by construction (sudo's audience); no new gating needed unless a future bot inherits the capability — then it's a #261/#282 scopes question.

Refs: #262 (the bot), #268 (runtime/credentials arc), #294 (the 25.8 usability arc this idea rides alongside).


### Comments (1)

**KnightOfNight** — 2026-08-27:
**Operator ruling (2026-08-27, V25.8 design session): not in Version 25.8** — queues for a future release. The 25.8 milestone stays at five ({#294 founding, #290, #296, #299, #300}); this ticket is cleanly separable (bot-side only) and its GitHub-credential design deserves its own pass.
