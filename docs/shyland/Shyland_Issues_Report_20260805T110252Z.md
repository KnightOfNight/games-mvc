...
| 80 | Design: item identification visibility — knowledge by holding | KnightOfNight | triaged, V24, itemization | Version 24.11 | 2026-08-05 |
...
- Milestone: Version 24.11
- Assignees: KnightOfNight
- Created: 2026-07-16 | Updated: 2026-08-05
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-04, V24.11 design session): #80 is the founding ticket of Version 24.11 (milestone assigned). The 2026-07-15 direction stands in full — knowledge is a property of holding; ground items show mystery names to all observers; `examine` reveals real details without pickup; pickup flips `is_identified` True permanently; drop re-veils; `is_unidentifiable` interplay preserved. Open sub-points ruled today:

**A — Examine reveal presentation:** an unidentified item's `examine` output is the full identified detail block, byte-identical to examining the same item identified — no special framing, no state change. Curse state stays hidden regardless (separately gated on `curse_identified`, which remains identification-service territory). Exception: `is_unidentifiable` items keep the mystery block.

**B — No info suffix on unidentified lines (operator-modified):** unidentified item lines show **no info suffix at all** — durability percentage, BROKEN, and bag carry-bonus are all hidden; the line is mystery name + `[Bound|Unbound]` only. Operator rationale: not all items have durability (or a carry bonus), so the suffix's mere presence partially reveals what an item might be.

**C — Pickup-flip choke point:** `is_identified = True` is set in `transfer_to_character`, mirror-symmetric with the drop-side re-veil in `transfer_to_room`, with the same `is_unidentifiable` guard — every holding path (pickup, corpse loot) becomes an unlock automatically, no per-command wiring.

**D — The authored-mystery middle tier collapses (confirmed consequence):** since any pickup identifies, builder-set `is_identified = False` survives only until first pickup; lasting mystery is exclusively `is_unidentifiable`. The mystery name/description fields' working roles are (a) the dropped-item veil and (b) unidentifiable items.

**E — Ride-along cleanups (mechanical):** the redundant double cannot-determine line dies with the old unidentified-examine branch; the surviving `is_unidentifiable` branch prints one mystery line plus the no-method line (wording authored at implementation under the creative-content policy). Fallback mystery name for items with no authored mystery name stays `an unidentified <item type>`.

GDD §6.8 text lands on `version_24_11` with the intra-branch pending-implementation marker; Brief 1 follows.


**KnightOfNight** — 2026-08-05:
Ruling addendum (2026-08-05, V24.11 design session — end-ritual late catch; decision was recorded in the committed brief but belongs here too): **the veil is the drop mechanic, not a ground-state invariant.** Seeded ground items (e.g. Convergence newbie gear) ship identified and stay identified; corpse decay deletes contents with the corpse (v23, #137), so player drops are the only source of veiled ground items. Consequence: the release is code-only — no seed changes, no deploy-time data actions. Flagged to the operator in-session with no objection; brief of record: `Shyland_V24.11_Brief_1_Knowledge_By_Holding.md`.
