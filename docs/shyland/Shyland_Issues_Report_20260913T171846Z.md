...
| 70 | Longevity has no drain — the slow-burn design needs its first consuming mechanic | KnightOfNight | triaged, game-mechanics, V26 | Version 26.1 | 2026-09-13 |
...
| 297 | Curse system live loop is unbuilt: apply active_curse on equip, unequip handling, seedable curse effects, acquisition + identification paths (needed for new zones; rule with #80) | KnightOfNight | triaged, game-mechanics, V26 |  | 2026-09-11 |
...
| 330 | Curse engine: schema, apply loop, lifecycle, families, seed set — acquisition admin-only (Release A of the #297 arc) | KnightOfNight | triaged, game-mechanics, V26 |  | 2026-09-11 |
| 331 | Tick engine Phase 1: stacked same-character effects compute from stale per-row snapshots (duplicate arrival lines, last-writer-wins application) | KnightOfNight | bug, V26 |  | 2026-09-13 |
...
## Issue #70: Longevity has no drain — the slow-burn design needs its first consuming mechanic

- State: open
- Author: KnightOfNight
- Labels: triaged, game-mechanics, V26
- Milestone: Version 26.1
- Assignees: KnightOfNight
- Created: 2026-07-15 | Updated: 2026-09-13
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-09-11:
Design ruling (2026-09-11, V26.0 design session — full ruling set for the first Longevity drain, plus the three-tanks doctrine it produced):

## The doctrine (operator-ruled, supersedes §4.4's paper design)

**The three bars are three independent fuel tanks. No bar ever affects another bar.** Code audit first (this session): nothing reads Longevity as an input to any calculation; §4.4's two causal connections (critically-low Vitality → Acuity panic spike; severely-low Longevity → Vitality/Acuity regen slowdown) were **never built** — the code has been independent tanks since day one. This ruling makes the docs match reality and cancels the unbuilt promises:

- **Vitality and Longevity are pure fuel tanks: they gate, they never scale.** Empty Vitality = Dying (shipped, stays). §4.1's unbuilt scaling bullets (melee damage scaling with current Vitality, movement/resistance degradation) die on paper — also never built.
- **Acuity is not a fuel tank and keeps its shipped identity** (operator-corrected explicitly: "I do not want to change how acuity works") — the v19 band gauge whose deviation modifies the player's own output stays exactly as is, in code and in doctrine.
- **Canceled in-session before recording** (ruled then superseded the same day): the §4.4 regen-slowdown read and the DoT/HoT window-scaling read. Longevity does not control the power or duration of any effect, in or out — power comes from player/NPC/item level, never from fuel remaining. Fuel controls only whether the action fires at all.
- Compatible survivors in §4.4: multi-bar eldritch effects (one effect, per-tank components — drainers, not connections) and the Warden's party-wide management (fillers).

## The first drain — flee exertion

- **Flee costs 25% of `longevity_max` per use, at every level** (proportional by construction). Use = attempt; success and failure both exert — the "still recovering from your last flee attempt" fiction finally has its cost.
- **Under 25% fuel: the flee attempt fails — not enough fuel** (operator-ruled). No fuel, no flee: the drained player fights, quaffs, or dies. Refusal wording/layer at brief time under the three-layer doctrine.

## Recovery and the potion

- **Regen retune: `LONGEVITY_REGEN_SECS` 3600 → 900** — zero-to-full in 15 minutes at every level (interval form preserved), one flee's worth back in ~3¾ min. Still 7.5× slower than Vitality (120 s), no longer an hour. (900 is the working number within the operator's ruled range "faster than an hour, slower than Vitality"; final blessing rides the GDD §4 edit review.)
- **New consumable: the mental-health potion — instant full Longevity restore**, `restore_longevity`'s first customer. Seeded into Z01 (vendor stock; per the major-wide direction all V26 mechanics land in Zone 1). Name, lore, price at brief time.

## Recorded direction (future, not this release)

**Longevity is mana fuel.** Future player abilities will draw it at much smaller percentages than flee's 25% — which is why the instant-refill consumable earns its place now.

## Display

**Longevity moves to second position — Vitality, Longevity, Acuity** — in player-facing bar ordering (stats pane and kin). GDD §4 subsection numbering stays as-is (§-references remain stable); the reorder is display-side.

## Scope

One release, one brief: flee drain + fuel gate, regen constant, potion (seed), display reorder, and the GDD §4 rewrite (§4.1 scaling bullets, §4.3 fuel-tank framing, §4.4 inversion to the independence doctrine) landing GDD-first on the version branch. Zero code exists to remove — the cancellations are docs-only; the additions are the release's build.


**KnightOfNight** — 2026-09-11:
Doctrine addendum (2026-09-11, operator, same session — the one-line form of the ruling above): **"the fuel tanks don't control power, they're just 'money'."**

Vitality and Longevity are currencies you spend to act — enough buys the action, not enough refuses it, and the balance never changes what the purchased action does. Acuity is the not-a-fuel-tank: its band deviation modifying your own output is its shipped identity and stands untouched. This is the sentence GDD §4's rewrite should be built around.


**KnightOfNight** — 2026-09-13:
Design rulings (2026-09-13, V26.1 design session — Q&A pass over the recorded V26.0 ruling set; items settled so far):

## Regen constant — blessed

**`LONGEVITY_REGEN_SECS = 900` is final** (zero-to-full in 15 minutes at every level, interval form preserved, one flee's worth back in ~3¾ min). The working number from the 2026-09-11 ruling stands.

## Flee fuel-gate mechanics

- **Gate position:** the fuel check runs after the dying check and the cooldown check, before the DEX contest. A player still recovering hears the cooldown line, not the fuel line.
- **Refusal is free:** an insufficient-fuel refusal costs nothing and does **not** set the flee cooldown — a refused purchase leaves no charge, per the fuel-is-money doctrine. Only a fueled attempt (contest actually rolled) exerts and starts the cooldown.
- **Out of combat, flee stays a no-op** — verified in code: `cmd_flee` already refuses with `"You are not in combat."` (warn) before any other check. That shipped behavior stands, entirely outside the fuel system: nothing to flee from, does nothing, costs nothing.
- **Boundary and rounding:** cost = `ceil(longevity_max / 4)`; gate is `longevity_current >= cost`. At exactly 25% the flee fires and lands the player at zero fuel — "under 25% fails" means strictly under.

## The potion — SUPERSEDES the 2026-09-11 "instant full restore" ruling

**Full restore is dead, on the doctrine's own logic:** a flat-priced fill-the-tank potion sells fuel at a price that ignores how much fuel is delivered — a financial win scaling with the buyer's level (operator: "if it restores the whole bar for any player at any level, that's a huge financial win for high level players"). It failed the fuel-is-money model it was meant to serve.

**Ruled replacement — consistency and sameness: the potion works exactly like the Healing Draught:**

- Percent-of-max restore under the Draught Law shape, at the draught's exact fractions: **`0.15 + 0.05×Mk` of `longevity_max`** (20% at Mk 1).
- **Priced at the draught standard: 15 cp** (seed-owned base_value, matching the draught's vendor treatment). The 30 cp premium priced against the full restore dies with it.
- **Common rarity** — verified in code this is mechanically inert on a consumable (the effect-application path reads only Mk; rarity on a consumable is display, stacking key, and drop-distribution only). Mk is where the item's value scales, like the draught.
- **Vendor stock only, Z01, no loot-table entries** (confirmed this session).
- **The Mk-1 mismatch against flee is deliberate:** one Mk-1 potion (20%) does not fully refund one flee (25%). Operator: "i like the mismatch between potion and flee, that's a real impact to the player." Fuel costs what it costs.

**Technical note for the brief (verified against the code):** the existing `restore_longevity` component type is flat-amount; the percent shape requires a new `restore_longevity_percent` component type mirroring `restore_vitality_percent` (#139). The 2026-09-11 ruling's "`restore_longevity`'s first customer" line is satisfied in spirit by the family's percent member — a verified deviation from the ruling's letter, to be recorded in the brief.

Name and lore remain brief-time creative content. Still open in this session's Q&A: the empty-session flee disengage's fuel treatment, and the GDD §4.3/§9 edit-scope confirmations.


**KnightOfNight** — 2026-09-13:
Design ruling (2026-09-13, V26.1 design session — flee fuel gate, final piece):

**The empty-session disengage is exempt from both the fuel cost and the fuel gate.** A flee issued inside an active combat session whose living opposition is already zero ends combat as the trivial disengage it is today (`flee_empty` — no contest rolled) at no Longevity cost, with no fuel check. Nothing to flee from, nothing to exert against. The 25% cost and its gate bind **contested attempts only** — where success and failure both exert.

The flee fuel-gate ruling set is now complete: gate position (after dying and cooldown checks, before the contest), free refusal (no cost, no cooldown), out-of-combat no-op (shipped behavior stands), ceil(longevity_max / 4) boundary at strictly-under-25%, and this exemption.

Also ruled (same session):

**GDD §4.3's mechanical-effects list dies entirely.** "Controls stamina duration," "governs sustained-effect durations," "controls buff/debuff windows," and "sustained spells collapse early" are all power/duration reads the independence doctrine cancels — none were ever built. Replacement text: Longevity is pure fuel; its one current consumer is flee; the recorded future direction is mana-fuel for abilities at much smaller percentages than flee's 25%.

**GDD §9 joins the edit scope (one line beyond the ruled §4 scope):** the flee entry in the command reference notes the fuel cost, and the insufficient-fuel refusal joins the three-layer doctrine as the world declining (warn layer, matching "too close to death" and "still recovering").

With this, the session's Q&A is complete — every open item from the 2026-09-11 ruling set is resolved. Next: the GDD-first §4 rewrite on `version_26_1` (operator review in-conversation before commit), then the brief.


**KnightOfNight** — 2026-09-13:
Design ruling (2026-09-13, V26.1 design session — regen future-proofing, operator-directed):

**The Longevity regen law gains a regime rule so refill time stays ~15 minutes at every bar size.** The rate is `longevity_max / LONGEVITY_REGEN_SECS` (900) points per second, with the numerical form chosen by regime:

- `longevity_max < 900` — **interval form** (as shipped): one point every `ceil(900 / longevity_max)` seconds.
- `longevity_max ≥ 900` — **per-tick form**, exactly Vitality's shape: `ceil(longevity_max / 900)` points per second.

Without this, the interval form's 1-second floor would make refill time grow linearly with any bar past 900 (a 1800 bar would take 30 minutes) — the 3600-era constant had a 13× safety margin over known bars; 900 has ~3.3×, so the rule ships now rather than as a someday-bug. The crossover is seamless: both forms approximate the same `max / 900` rate.


**KnightOfNight** — 2026-09-13:
Design rulings (2026-09-13, V26.1 design session — brief-writing pass, final two items):

## The Convergence carts stock the potion (operator-confirmed)

The two ring-street carts (VND-9, Mother Tansy) stock **every** consumable `ItemDefinition` automatically at seed time — deliberate v20 design (#43; the Focus Tonic is cart-only). The potion **rides that automatic stock unchanged**: the #70 ruling's "seeded into Z01 (vendor stock)" is about where the mechanics land, not a restriction carving the first-ever exclusion into the #43 cart query. Authored `VendorEntry` rows go to the three Z01 draught stockists (Essa, Sona, Ridda); the carts pick the potion up by law.

## Stop-at-full — derived from "make this potion work the same as healing"

The use path refuses a vitality restore at full vitality (`You are already at full health.`, warn — DD §7/#61). The potion mirrors it: a longevity restore at full Longevity refuses the same way (authored line at brief time), item not consumed. Derived from the operator's consistency ruling rather than separately ruled; called out here so it is on the record.
...
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-08-26 | Updated: 2026-09-11
- Blocked by: none
- Blocks: none
