...
| 105 | Elite even-level −5% hit calibration drift (rounding parity) | KnightOfNight | triaged, V24, game-balance | Version 24.17 | 2026-08-10 |
...
| 215 | Bags don't do enough: flat carry_bonus is noise against the STR-scaling capacity base (Mk 2 bag = +4.3% at level 17) | KnightOfNight |  |  | 2026-08-08 |
...
| 221 | Character creation screen: font color too hard to read (muted text ~3.2:1 contrast, below AA) | KnightOfNight |  |  | 2026-08-08 |
| 222 | Raise the game's default font size a couple of pitch points (base 14px + a scattered hardcoded scale) | KnightOfNight |  |  | 2026-08-08 |
...
- Milestone: Version 24.17
- Assignees: KnightOfNight
- Created: 2026-07-17 | Updated: 2026-08-10
- Blocked by: none
- Blocks: none
...
Design ruling (2026-08-10, V24.17 design session — founding ticket): **floor the curve.**

`get_npc_stats`'s DEX curve changes from `round(NPC_CONTEST_BASE + NPC_CONTEST_STEP × (L − 1))` — Python `round()`, banker's rounding — to `NPC_CONTEST_BASE + floor(NPC_CONTEST_STEP × (L − 1))`: the floor applied to the growth term, mirroring the reference player's own floor-share DEX accrual (5 points/level across two primaries = 2.5/level, floor-accumulated). NPC DEX then equals the reference build's attainable primary at every level of every band, and the blessed at-level hit targets (55% / 45% / 45%) become exact at all levels — the architecture doc's "exact and constant" claim becomes literally true.

**The defect (G5, #89):** banker's rounding sends the band's `.5` levels to the nearest even integer, overshooting the player at exactly L4 (25.5 → 26 vs. attainable 25) and L8 (35.5 → 36 vs. 35) — 1 DEX = one d20 pip = −5% hit. L2/L6/L10 round down and align. Mk-band lifts add integer multiples of 25, so the pattern repeats identically in every band. HALF-UP (the #104 precedent) was considered and rejected: it moves the drift to L2/L6/L10 — three misaligned levels per band instead of two. Floor removes it entirely.

**Sentinel:** elite L4 DEX 28 → 27; at-level reference hit 40% → 45%. Aligned levels (L2/L6/L10 and all odd levels) are byte-identical before and after.

**Scope:** the DEX curve only. STR/PER/INT `growth = round(...)` stays exactly as ratified by v19 Brief 7 Amendment 1 (±1 damage-side artifacts accepted; no hit% effect) — not re-litigated. `npc_max_vitality`'s HALF-UP (#104) stands.

**Release shape:** runtime-only — no migration, no seed, no data actions, deletions 0; architecture-doc hash MOVES. Milestone Version 24.17; brief `Shyland_V24.17_Brief_1_DEX_Curve_Rounding_Parity.md` on branch `version_24_17`.
