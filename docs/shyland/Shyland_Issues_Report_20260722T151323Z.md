...
| 30 | Travel network: should checkpoints (shards) also be travel senders? | KnightOfNight |  |  | 2026-07-22 |
...
| 38 | Obelisk attunement: player-set home spawn at checkpoint shards | KnightOfNight |  |  | 2026-07-22 |
| 40 | Free repair messages repeat too often (Morra example) — research spike into other duplication cases | KnightOfNight |  | Version 23 | 2026-07-22 |
...
| 125 | player macro/alias system | KnightOfNight |  |  | 2026-07-20 |
| 126 | pluralization subsystem — natural-English plurals for aggregate output | KnightOfNight |  |  | 2026-07-20 |
| 127 | Ranged proc damage ("between X and Y") — new weapon kind, midpoint-and-spread family | KnightOfNight |  |  | 2026-07-21 |
| 129 | Authored per-item armor base — guaranteed minimum coverage under rolled resist | KnightOfNight |  |  | 2026-07-21 |
| 130 | Secondary-stat curves vs Mk band growth — audit before Mk 2 content | KnightOfNight |  |  | 2026-07-21 |
...
| 134 | Repair kit not wired up yet | KnightOfNight |  |  | 2026-07-22 |
| 135 | Tick engine crashes (SynchronousOnlyOperation) on every full timed-effect expiry — unwrapped ORM call in the async expiry-message path | KnightOfNight | bug | Version 23 | 2026-07-22 |
...
## Issue #135: Tick engine crashes (SynchronousOnlyOperation) on every full timed-effect expiry — unwrapped ORM call in the async expiry-message path
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/135

### Body

Production tick engine crash, observed 2026-07-22 14:15:55Z: the ticker
container died with an unhandled exception and was auto-restarted ~1s
later by the compose restart policy (RestartCount 1). Found while
investigating an unexpected ticker uptime discrepancy; full traceback
preserved in `docker logs`.

TRACEBACK (tail; full trace in the container log at the timestamp):

    File "/app/apps/shyland/management/commands/run_tick_engine.py", line 51, in tick_loop
      await self.process_tick(tick_number)
    File "/app/apps/shyland/management/commands/run_tick_engine.py", line 58, in process_tick
      await self.process_effects(tick_number)
    File "/app/apps/shyland/management/commands/run_tick_engine.py", line 1230, in process_effects
      msg = _expiry_message_for_effect(parent)
    File "/app/apps/shyland/effect_utils.py", line 163, in _expiry_message_for_effect
...
CODE FACTS (branch state ~f17039c; cite-checked):

- `process_effects` (run_tick_engine.py ~1230) calls
  `_expiry_message_for_effect(parent)` DIRECTLY in the async tick loop
  when a timed effect's components all expire together
  (`all_expiring_now`). The helper is documented "Synchronous — call
  from within @database_sync_to_async" territory (effect_utils.py) and
  runs a real ORM query:
  `effect_instance.definition.components.order_by('order').first()` —
  always a fresh query, never satisfied by prefetch. Unwrapped ORM in
  async context -> SynchronousOnlyOperation, unconditionally, on every
  full expiry of a timed effect.
- The sibling per-component path at ~1225
  (`_expiry_message_for_component(ci, parent.definition.name)`) does
  not crash only because it reads FK attributes already loaded by
  `get_expiring_component_instances`'s select_related — it runs no new
  query. The two paths look symmetric but only one is async-safe.
- The exception propagates process_effects -> process_tick -> tick_loop
  -> asyncio.run -> process exit. Docker restarts the container; the
  crash is silent to players except as a skipped tick.

WHY IT WAS LATENT SINCE v19: instant effects (healing draughts — nearly
everything players use) close immediately at application
("no duration components") and never enter the expiry path. Only TIMED
effects reach it: the shift_acuity consumables (Focus Tonic / the fog
family), hot_* components, timed stat_bonus/stat_penalty, curses. These
are rarely used in practice. The trigger this time was the #133 Focus
Tonic playtest: the tonic's shift_acuity_high effect (~35s at Mk 1)
fully expired ~14:15:55 and the expiry-message path fired — possibly the
first full timed-effect expiry on this container generation. Earlier
occurrences on prior container generations would have been masked by the
auto-restart (restart counts die with each make build's recreation).

BLAST RADIUS:

- The crash aborts the ENTIRE tick mid-flight for the whole world —
  combat rounds, respawns, corpse decay, dialogue, regen all skip at
  least one beat; effect-expiry work for that tick is left partially
  done (component instances already expired via `expire_ci`, parent
  close and player message not reached; the close is retried next tick
  by the remaining==0 sweep, but the expiry message is lost).
- Recurs on EVERY full expiry of any timed effect until fixed — 100%
  reproducible: use a Focus Tonic, wait ~35s, watch the ticker restart.
- NPC on-hit timed effects (poison etc. via apply_npc_effects) also
  expire through this path — combat with effect-applying NPCs can kill
  the engine mid-fight.

FIX SHAPE (for the brief that takes this): wrap the
`_expiry_message_for_effect(parent)` call in the same
`@database_sync_to_async` pattern process_effects already uses for its
other DB work (a one-wrapper change), OR prefetch
`definition__components` in `get_expiring_component_instances` and make
the helper read the prefetched cache — the former is the minimal,
obviously-correct form. An audit of the sibling path's reliance on
select_related (fragile-by-coincidence) belongs in the same pass.

RELATED: #133 (the Focus Tonic band/clamp bug — same play session
triggered both; independent defects), and the B5 Amendment 3 closeout's
untouched-'system'-sends list (the expiry messages speak in the muted
voice — future ruling material).
