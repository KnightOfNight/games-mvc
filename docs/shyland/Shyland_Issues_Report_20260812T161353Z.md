...
| 221 | Character creation screen: font color too hard to read (muted text ~3.2:1 contrast, below AA) | KnightOfNight | triaged, output | Version 24.24 | 2026-08-12 |
| 222 | Raise the game's default font size a couple of pitch points (base 14px + a scattered hardcoded scale) | KnightOfNight | triaged, output | Version 24.24 | 2026-08-12 |
...
| 233 | Readability pass: raise the default type scale and fix below-AA contrast (umbrella for #221 + #222) | KnightOfNight | triaged, output | Version 24.24 | 2026-08-12 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, output
- Milestone: Version 24.24
- Assignees: KnightOfNight
...
Design ruling (2026-08-12, V24.24 design session): ships in **Version 24.24**, bundled with #222 under founding/umbrella issue #233 — one readability problem, two axes (contrast here, type size in #222). Operator confirmed the bundling in-session.

- This issue is a dependency of #233 per the scope law (same problem, not widening it).
- Milestone `Version 24.24` applied; branch `version_24_24`; implementation via the release's single brief, committed by this design session.
- Fix surface confirmed contained to `character_create.html`'s inline CSS — no shared surface, no game-client color-doctrine impact (self-contained palette). Exact replacement values are specified in the brief; #222's base-size bump applies to this screen's inline 14px base in the same pass.
...
**KnightOfNight** — 2026-08-12:
Design ruling (2026-08-12, V24.24 design session): ships in **Version 24.24**, bundled with #221 under founding/umbrella issue #233 — one readability problem, two axes (type size here, contrast in #221). Operator confirmed the bundling in-session.

- This issue is a dependency of #233 per the scope law (same problem, not widening it).
- Milestone `Version 24.24` applied; branch `version_24_24`; implementation via the release's single brief, committed by this design session.
- The bump also applies to the character-creation screen's inline 14px base (same pass as #221's contrast fix — overlapping file surface is part of why the two are bundled).
- The release's main verification weight sits here: the right pane is width-budgeted (map rendering, bars, fight-panel numbers, Kind 3 table column budgets), so the brief carries explicit reflow checks at common window sizes. Approach (one-root scale vs per-site literals) is ruled in the brief.
...
## Issue #233: Readability pass: raise the default type scale and fix below-AA contrast (umbrella for #221 + #222)
...
- Blocked by: #221, #222
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/233

### Body

## Founding ticket — Version 24.24 (umbrella)

One problem, ruled as one release: **the game's text is hard to read.** Two axes, each already diagnosed in its own issue, ride as dependencies of this umbrella:

- **#222 — legibility by size:** the default type scale is too small. Base 14px (`html, body` in `game.html`) with a scattered hardcoded px scale across ~10 sites (output pane 15px, input 14px, right pane 12px, fight-panel numbers/conn-ms 11px). Goes up a couple of pitch points, scale moving proportionally.
- **#221 — legibility by contrast:** the character-creation screen's `--muted` (#6b6b80 on #16161a) computes to ~3.2:1, below the WCAG AA 4.5:1 minimum for normal-size text, and carries much of the screen's explanatory copy.

## Why bundled (design ruling, 2026-08-12, V24.24 design session)

- The two issues cross-reference each other: #221 notes the small 14px monospace compounds the muted-contrast problem; #222 notes the creation screen's inline 14px base takes the same bump "for consistency."
- Overlapping file surface: `character_create.html`'s inline CSS block is touched by both — bundling makes it one coherent pass instead of two sequenced edits.
- Same layer, same label: pure client presentation inside `apps/shyland/` templates (`game.html`, `character_create.html`); no shared surface, no models/migrations/seed data, no game-client color-doctrine impact (#221's screen has its own self-contained palette).

Scope-law fit: dependencies describing the same problem (legibility), not widening it.

## Scope

- In: type-scale raise per #222 (including the creation screen's base), contrast fix per #221, and the layout verification #222's bump forces (width-budgeted right pane: map rendering, bars, fight-panel numbers, Kind 3 table column budgets at common window sizes).
- Out: everything else. No color-doctrine changes, no new commands, no server-side changes.

## Process

- Milestone: **Version 24.24** (this issue + #221 + #222 only).
- Branch: `version_24_24`. One implementation brief, committed by the design session.
- This umbrella closes when both dependencies close and the brief's verification passes.


### Comments (1)

**KnightOfNight** — 2026-08-12:
Design ruling (2026-08-12, V24.24 design session) — the release's design decisions, operator-confirmed in-session:

**Mechanism (D2):** every hardcoded type site in `game.html` and `character_create.html` converts to `rem` off a single px root per template (`html, body`), current size ratios preserved exactly. Future size changes become a one-line root edit — which is also what makes the tuning loop below cheap.

**Starting values:** root 14px → **16px** (both templates); creation-screen `--muted` #6b6b80 (measured **3.47:1** on #16161a, below AA) → **#85859c** (**5.01:1**, AA with headroom, stays subordinate to `--text` at 10.89:1).

**Operator tuning loop (chicken/egg resolution):** the starting values are deliberately NOT eyeball-confirmed — the brief grants the implementation session bounded tuning authority: after the first `make deploy-dev`, the operator judges the live screens and directs adjustments in-conversation; the session applies, redeploys, and iterates until operator approval. **Band (operator-ruled): base 12–24px; `--muted` must keep ≥ 4.5:1 AA contrast on #16161a and stay visually subordinate to `--text`.** Ratios always move via the root, never per-site. Anything outside the band is a design-level finding — filed and ruled, never tuned silently. Final values are recorded in the closeout report and echoed onto this issue.

**Exclusions (D4):** the right pane stays fixed at 300px; the map SVG is untouched — including `.map-updown`'s `font: bold 14px` (a `font:` shorthand, the 11th type site): the badge tuck-rule offsets are derived from the measured ink of 14px bold Courier, so that literal deliberately does not scale.

**GDD (D5):** no section edits this release — the GDD documents no type sizes and not the creation-screen palette; nothing written becomes wrong. Architecture doc: stamp-only move to 24.24, hash unmoved (presentation-only change).

Brief: `docs/shyland/Shyland_V24.24_Brief_1_Readability_Pass.md`, committed on `version_24_24` by this session.
