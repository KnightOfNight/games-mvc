...
| 180 | Post-gear-wiring fight-cost survey: measured HP loss, duration, and draughts-per-fight across NPC tiers | KnightOfNight | triaged, V24, healing-economy, game-balance | Version 24.1 | 2026-08-01 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, V24, healing-economy, game-balance
- Milestone: Version 24.1
- Assignees: KnightOfNight
...
**Design ruling (2026-07-31, V24.1 design session — operator-confirmed): #180 is the founding ticket of Version 24.1.**

Per the Phase 1 plan of record on #139 (24.0 = #139 Draught Law → next release = #180 survey → data-informed #164 table ruling → #164 release):

- Milestone **Version 24.1** created and assigned; branch `version_24_1` created from main at 79b2568 (the V24.0 merge).
- Shape as filed: an implementation-session **research brief** — committed dataset + report document on the version branch, **zero game-behavior changes**. The brief (`Shyland_V24.1_Brief_1_Fight_Cost_Survey.md`) follows on the branch this session.
- #164's income table stays blocked on this survey's report; the dataset also feeds Phase 3 baselines (#104, #130) and the v21 boss potion-budget re-derivation.


**KnightOfNight** — 2026-08-01:
Brief committed (2026-07-31, V24.1 design session): `Shyland_V24.1_Brief_1_Fight_Cost_Survey.md` on `version_24_1` at 7ac7227. Actionable when the operator directs an implementation session to it by name on this branch (CLAUDE.md Rule 4).

Carries the version-start ritual (24.1-DEV bump + `make deploy-dev`) as its opening act, then a zero-game-change research survey: Monte Carlo harness (`scripts/shyland_fight_cost_survey.py`) driving the shipped combat code in-container, `Shyland_V24.1_Fight_Cost_Survey.md` report + CSV dataset committed on the branch. Reuses the #89 verdict scale, encounter model, and reference build unchanged for comparability; headline loadout adds current-era standard armor (post-#100 gear wiring); implied draughts computed under the #139 Draught Law. No PENDING DEPLOY-TIME ACTIONS; expected playtest disposition "No playtests for this brief."
