...
| 149 | Using a consumable prints two lines — merge the use sentence and the effect message into one | KnightOfNight | output |  | 2026-07-25 |
| 150 | Protect players from 'sell all'-ing their potions — bulk sell needs a consumable guard | KnightOfNight |  |  | 2026-07-25 |
| 151 | Multi-use healing floods the pane — compute the heal once, consume what's needed, send one message | KnightOfNight | output |  | 2026-07-25 |
| 152 | V23 output-color pass: narration/system to value-color, direction-split miss colors, copper-loot line to reward | KnightOfNight | B5, triaged, output | Version 23 | 2026-07-25 |
...
## Issue #149: Using a consumable prints two lines — merge the use sentence and the effect message into one
...
- Blocked by: none
- Blocks: #151
- URL: https://github.com/KnightOfNight/games-mvc/issues/149

### Body

Observed (v23, post-B5) — using a consumable emits two messages:

```
You use a Healing Draught Mk 1.
You feel your body recover. (+25 Vitality)
```

The first is `cmd_use`'s DD §6 transactional sentence; the second is the effect layer's instant-component message (`restore_vitality` via `effect_utils`). Two separate sends, two envelopes, two timestamps.

Operator direction (2026-07-25): make this one line.

Note for the implementing brief: `use` is also the revival path (v19 brief 4 — a dying character's revival sequencing hangs off these messages) and the effect message text varies by component type, so the merge point needs choosing deliberately (compose at `cmd_use` after the effect result returns, or have the effect layer return its clause for embedding).
...
## Issue #150: Protect players from 'sell all'-ing their potions — bulk sell needs a consumable guard
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/150

### Body

**Target: point release immediately after V23 ships** (operator direction, 2026-07-25).

`sell all` (and `sell all common`, the inventory-dump workhorse) happily includes every potion you own. It is far too easy to walk out of a vendor stop with zero Healing Draughts — the exact items you are about to need — and the risk makes inventory dumps needlessly tense. The v23 stacking work (#18) made hauling large hoards tidier, which only increases the dump temptation; and sell resolution is deliberately stacking-agnostic (#22), so display grouping offers no protection.

**Candidate directions** (operator seeds + additions; design chat to rule):

1. **Separate inventory for consumables** (operator) — a belt/pouch that `sell all` never reaches. Biggest hammer: new container concept, display changes, capacity questions.
2. **A protected flag on consumables** (operator) — per-instance or per-type "keep" state that bulk sell skips. Cheapest schema shape is a boolean on `ItemInstance` (the soulbind pattern), set/cleared by a player command.
3. **Grammar-level exclusion** — `sell all` / `sell all <rarity>` simply exclude the consumable type by default, with an explicit `sell all draught` (named-noun form) still selling them intentionally. Precedent already in the chart: `drop` excludes bound items from `all` by ruled design. No migration, pure resolver policy; probably the smallest honest fix.
4. **A `keep <item>` / `keep all <noun>` command** — player-managed protection generalizing beyond consumables (heirloom weapons, quest items someday). Migration for the flag + a grammar verb + flag-block display state (`[Common, Kept]`?). Superset of option 2.
5. **Confirmation prompt on consumable-inclusive bulk sells** — "That includes 12 Healing Draughts — 'sell all' again within 10s to confirm." The central fn-10 prompt machinery (PROMPT_VERBS) is adjacent precedent, but a stateful confirm is a new interaction pattern for the game.

Options 3 and 2/4 compose (default exclusion now, player-managed keep later). Whatever ships must keep #138's worthless-junk disposal working — bound kit junk still needs to leave via bulk sell.
...
## Issue #151: Multi-use healing floods the pane — compute the heal once, consume what's needed, send one message
...
- Blocked by: #149
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/151

### Body

Observed (v23): `use 25 healing draught` iterates 25 individual swallows — each one a use sentence plus an effect message (see #149), so a big top-up floods the pane with up to ~50 lines, scrolls the fight out of view, and sends a WebSocket message (own envelope/seq/ts, plus status riders) per swallow.

Operator direction (2026-07-25): don't tick N times. **Calculate up front** — current deficit vs. heal-per-potion, consume only as many potions as the heal actually needs (capped by the requested N and by inventory), apply it, and send the player **one message** and one status update. One heal, one line, no scroll, no WebSocket flood.

Notes for the ruling/implementing brief:

- The per-swallow stop-at-full check already exists in the use loop; this moves that arithmetic before consumption instead of discovering fullness one swallow at a time.
- **This supersedes half of the v22 B2 Amendment 5 ruling** ("use, repair, and loot stay per-line — each iteration is its own news; use never pre-announces a count") for the multi-use case — the design chat should reconcile the two rulings explicitly rather than leaving them in conflict.
- Sibling of #149 (merge the single-use sentence + effect message into one line); whatever composes the merged single-use line should compose the aggregated multi-use line too.
- Edge cases to keep: revival-by-use while dying happens on the first restorative swallow (v19 brief 4 — no over-drinking past revival); mixed-Mk draughts in one resolution consume lowest-value first per the sell/use selection order; non-healing consumables (effects that aren't instant vitality) may not be aggregatable the same way and can stay per-use if ruled so.
...
## Issue #152: V23 output-color pass: narration/system to value-color, direction-split miss colors, copper-loot line to reward

- State: open
- Author: KnightOfNight
- Labels: B5, triaged, output
- Milestone: Version 23
- Assignees: KnightOfNight
- Created: 2026-07-25 | Updated: 2026-07-25
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/152

### Body

**The V23 output-color pass** — verified live in play 2026-07-25 (deployed as an operator-directed experiment; changes currently stashed at `428bc16c`, commit pending design-chat review). **Scope is CLOSED: exactly the changes below, no additions.**

## The changes

**1. Ambient/narration categories lift from muted to value-color** (`game.html`):
- `.msg-system` → `var(--value-color)` (presence arrivals/departures, effect ticks, ambient system lines)
- `.msg-room` → `var(--value-color)` (dialogue narration — greetings, departures, connectives — plus decay lines and all ambient room events)

Rationale: with the #147 render rule landing dialogue narration at category `room`, muted made the new content nearly invisible — a witness watching a full NPC exchange saw everything but the say lines vanish into dim gray. Value-color restores the read while keeping `say` gold distinct. (`.msg-echo` stays muted; the `--muted` declaration keeps its other users — seg-muted, placeholders, map unknowns, combat-miss legacy.)

**2. Misses split by direction** (`run_tick_engine.py` + `game.html`):
- "You miss {target}." now sends category **`combat-miss-out`** → `var(--warn-color)` (your whiff is a warning)
- "{attacker} misses you." now sends category **`combat-miss-in`** → `var(--success-color)` (their whiff is good news)
- The legacy `.msg-combat-miss` rule (muted) is retained with zero senders; the client's combat-family handling is prefix-based (`indexOf('combat') === 0`) so both new categories inherit assertive announcement and stamping with no client-JS change.

**3. The copper-loot line joins loot-color** (`consumers.py`):
- "You loot {amount} from {corpse}." moves category `success` → **`reward`** — parity with the item-loot lines, which have been reward/loot-color since v22 B2 Amendment 1 (#124). This half is arguably a pre-existing inconsistency fix.

## Play verdict

Combat readability is dramatically better: gold = speech, green = things that went your way (their misses, all loot), yellow = your whiffs, reds = damage, parchment = the world, muted = true chrome only.

## Commit checklist (when the design-chat FYI clears)

- Apply stash `428bc16c` (`v23-color-experiment-2026-07-25`), strip the EXPERIMENT comment markers into permanent issue-referenced comments.
- No test currently pins the `combat-miss` category (verified by grep) and no hex literals changed, so the chart-as-license conformance test is unaffected — but the **palette chart gains two category rows** (`combat-miss-out`/`combat-miss-in`) and the §4.16 arch-doc palette section needs the update at commit time.
- Arch doc: §4.16 category table (miss split + the room/system recolor + the loot-line recategorization); the v20 B2 stamped/unstamped table is unaffected (categories changed color/name, not stamping class).
- `make build` redeploy is a no-op for players (production already runs these bits) but re-syncs the image with git.
