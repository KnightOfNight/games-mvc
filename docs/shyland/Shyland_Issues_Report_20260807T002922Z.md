...
| 134 | Repair kit not wired up yet | KnightOfNight | triaged, V24, itemization | Version 24.12 | 2026-08-07 |
...
- Milestone: Version 24.12
- Assignees: KnightOfNight
- Created: 2026-07-22 | Updated: 2026-08-07
- Blocked by: none
- Blocks: none
...
The seeded Repair Kit consumable (cart-vendor stock at 15 cp) has no wired effect: its ItemDefinition carries `effect: None`, so `use repair kit` prints the generic "Nothing happens." refusal. The `durability_restore` EffectComponent type has existed in the vocabulary since migration 0004 with zero uses; its v23.3 clause is an authored fizz placeholder that never fires. Meanwhile the full repair machinery (vendor repairers, cost/success math, durability bands) is live — the kit just isn't connected to any of it.

Wire the repair kit into the existing repair machinery as the field-repair consumable. Complete design ruled 2026-08-06 (V24.12 design session) — see the ruling comment below. Founding ticket of Version 24.12, closing Phase 2 (itemization structure) of the V24 plan.
...
Design ruling (2026-08-06, V24.12 design session): the repair-kit wiring design is complete — seven rulings, recorded at the moment of decision.

**Diagnosis (current state):** the seeded `repair-kit` consumable carries `effect: None`, so `use repair kit` prints the generic "Nothing happens." warn. The `durability_restore` EffectComponent type has existed since migration 0004 with zero effect definitions using it; its v23.3-authored clause ("watch the repair kit fizz to no useful effect") is a dead placeholder. The kit is cart-only (Vnd-9, Mother Tansy) at the authored 15 cp cart price, but `base_value` is still the model default 1 (the #201 trap — sale pays the 1 cp floor). The vendor repair machinery (§6.6: cost = value × missing × 50%, success = 20% + durability × 75%, restore to 100%) is fully live and untouched by this design.

**Rulings:**

1. **Target selection:** automatic, most-damaged-first among everything the character owns (carried + equipped — the repair pool, #22). No new grammar; `use [N] repair kit` stands. Broken (0%) items are ineligible targets and are skipped.
2. **No roll:** a kit always succeeds — consumed, guaranteed restore. The success roll remains vendor repair's mechanic; the kit's cost is the kit itself plus the partial restore.
3. **Potency:** restore = 15 + 10 × Mk durability points (Mk 1 = +25, Mk 2 = +35), clamped at 100. Draught-Law shape: `durability_restore` component with magnitude computed from base 15, per-Mk factor 10.
4. **Broken gear:** a kit refuses a broken (0%) item — beyond a field patch (warn voice, its own authored line). Repairers keep the 0% monopoly; letting gear break stays a real cost. If the only damaged items owned are broken, `use` refuses.
5. **Combat:** using a durability-restore consumable is refused in combat — the gate keys on the effect component, never the item name; its own authored warn line, consistent with repair's no-mending-in-a-fight doctrine.
6. **Economy:** `base_value` = 15, the draught standard (#164 precedent) — cart buy 15 cp stands, sale 5 cp. Sourcing stays **cart-only**: no loot-table entries this release (the v24.2 faucet ruling is not reopened; loot presence may ride a later release, ruled against real data).
7. **Sequences (`use N`):** per-item path, re-targeting most-damaged-first after each kit. Zero need refuses (warn; the kit is NOT consumed); a sequence stops the moment nothing damaged remains (the #61 fulfilled-purpose doctrine's repair analogue) with the standard only-had-N / shortfall reporting. The fizz clause is replaced by a real patch clause naming the repaired item, with a `(+N durability)` annotation.

**Release wiring:** founding ticket of milestone Version 24.12 (branch `version_24_12`); brief `Shyland_V24.12_Brief_1_Repair_Kit_Wiring.md` to be committed on the branch; GDD §6/§9/§12 updated with "(v24.12, pending implementation)" markers. Singleton wiring release per the scope law — Phase 2 (itemization structure) closes with it.


**KnightOfNight** — 2026-08-07:
Ruling addendum (2026-08-06, V24.12 design session): **the dying state refuses the kit too — nothing but healing while dying** (operator-ruled). Using a durability-restore consumable is refused while dying, exactly as in combat: same effect-component-keyed gate, its own authored warn line. Ruling 5 now reads "refused in combat and while dying." GDD §6.5 field-repair block and the §9 state-gating matrix (dying line) updated accordingly.
