...
| 251 | All config command setters should write both the cached attribute and the DB row | KnightOfNight | triaged, commands | Version 24.30 | 2026-08-15 |
...
**KnightOfNight** — 2026-08-15:
**Design ruling (2026-08-15, V24.30 design session): #251 is the founding ticket of Version 24.30.** Milestone `Version 24.30` created and assigned; branch `version_24_30` cut from main.

The session also ran the operator-directed **configuration-command audit** (consistency / correctness / bugs) across the whole family — `brief`, `echo`, `plunder`, `timestamps` — verified against `consumers.py` at the branch tip (commit 1e159e3). Findings:

1. **The setter split is exactly as this issue states.** `_set_brief_mode`, `_set_show_timestamps`, `_set_echo_mode` write only the DB row; `_set_plunder_mode` writes both (consumers.py 4083–4097).
2. **A third instance of the caller-cooperation fragility:** `cmd_timestamps`' set path (line 3030) calls `_set_show_timestamps` directly and **never assigns the cached attribute at all** — it is correct today only because the subsequent `get_character_fresh()` (line 3031) happens to replace `self.character` wholesale. The ruled fix makes the setter own the cache, which closes this path by construction.
3. **No live staleness bug exists today:** every gameplay read of the four settings is a fresh fetch (`send_room_description` fresh-fetches before `_resolve_room_rendering`; `_status_payload` callers pass fresh chars; plunder re-reads by pk in `loot_utils.get_plunder` at combat end). The connect-time payload (lines 501–502) reads the just-fetched connect object. The risk is latent, exactly as the issue body says.
4. **Scope extension, same problem (not a widening):** with the setters owning both writes, the three caller-side cache assignments (`cmd_brief` 2994, `cmd_echo` 3004, `cmd_plunder` 3018) become redundant and are **removed** — leaving them would preserve the very caller-maintains-the-cache pattern this ruling kills. One writer: the setter.
5. **Behavior surface:** zero player-visible change; no model change, no migration; the v22 settings standard (six words, bare-reports, Usage line) is untouched. **No GDD text change** — this is internal correctness; GDD §9 remains accurate as written.

Brief: `Shyland_V24.30_Brief_1_Uniform_Config_Setters.md`, committed on `version_24_30`.

A related-but-separate finding (config set paths never fresh-fetch, so they miss the deleted-while-connected guard) is recorded on #243, where that family lives.
