...
| 33 | Shyland: persist detailed combat logs for balance analysis | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
...
| 191 | MC consumer: command-pattern watcher — mine player behavior for the next heal/loot-shaped improvements | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
...
| 259 | Sirius: AI felis sapiens companion — callable, may help, gives only when he wants to (unfinished design) | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
| 260 | MC chat policy: GDD §7.1 and §10.5 contradict — rule chat persistence, privacy, and retention before MC ships | KnightOfNight | monitoring-and-command, V25 | Version 25.0 | 2026-08-18 |
| 261 | The Command half of MC: how does a non-human actor take action in the world? (scoping) | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
| 262 | sudo AI watcher: an AI agent on the receiving end of sudo — reads the MC stream, and may or may not be allowed to answer | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
...
| 264 | Terminology: the firehose is now Monitoring and Command (MC) — sweep forward-looking doc references | KnightOfNight | monitoring-and-command, V25 | Version 25.0 | 2026-08-18 |
...
| 266 | MC kill switch: a single lever that silences every AI actor at once — ships no later than the first live actor (HIGH PRIORITY) | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
| 267 | MC egress: how remote consumers attach — Streams inside the trust boundary, WebSocket across it (auth, backpressure, resume) | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
| 268 | MC agent runtime & operations: hosting, supervision, credentials, cost governance for the AI actor fleet | KnightOfNight | monitoring-and-command, V25 |  | 2026-08-18 |
| 269 | V25.0 founding ticket: MC (Monitoring and Command) — the major design pass and version opening | KnightOfNight | monitoring-and-command, V25 | Version 25.0 | 2026-08-18 |
...
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-07-12 | Updated: 2026-08-18
- Blocked by: none
- Blocks: none
...
**KnightOfNight** — 2026-08-16:
**Explicitly unblocked — operator ruling 2026-08-16 (#260): nothing in the game is private; total capture ruled for all of MC.** No speech-policy question gates this ticket any longer (combat instrumentation never had a real privacy entanglement, and the ruling closes the question for the whole stream). Free to proceed whenever the release stream picks it up.

## Issue #37: MC monitoring: universal event logging — every command, every output, every event
...
**KnightOfNight** — 2026-08-16:
**Blocker resolved — operator ruling 2026-08-16, recorded on #260.** The "Required GDD ruling" named in this issue's body is made: **nothing in the game is private.** The sink captures everything — chat included, future DMs included (person-to-person visibility for players; full MC capture regardless) — with no category exclusions at the tap.

- The §10.5 reversal's GDD text lands on the V25 branch via a design session; the ruling itself does not wait for it and does not block planning or a brief here.
- **Retention stays in this issue's scope as a purely operational (volume/disk) question** — the privacy dimension is closed by the ruling.

Status: #32 shipped in v20, the chat ruling is made — **#37 has no open blockers.**


**KnightOfNight** — 2026-08-16:
**Technical note for the design session — where the tap sits changes what "one event" means.**

The designated tap (arch doc, the delivery choke point) is `SkylandConsumer.send_json` — a **per-connection** method, and `seq` is per-connection monotonic. A room broadcast reaching five players therefore passes five choke points and produces five envelopes with five different `seq` values. A sink attached at delivery records **one row per recipient, not one per event**.

That is not wrong — delivery-level capture is exactly right for "what did this player actually see" questions and for #191-style per-player behavior mining. But balance analysis and the canonical event record want **creation-level** capture: one row per event regardless of audience size. Options include tapping at creation/`group_send` as well as delivery, or recording delivery rows that carry a shared event id, or both layers explicitly. Worth deciding in the design/brief rather than discovering the multiplication in the data afterward.

Two adjacent facts, verified against the arch doc:

- The command-ingress tap (`receive_json`) is naturally singular — this note concerns the outbound side only.
- Events born in the **ticker container** reach players via the shared Redis channel layer and then the same per-connection delivery, so tick-engine broadcasts multiply identically. A creation-level tap has to exist in the ticker's process too, not just the consumer's.


**KnightOfNight** — 2026-08-16:
**Operator direction, 2026-08-16 (recorded from the ops session):** Redis is already in the stack, and changing how we use it to do more for MC is welcome — *"if there is more cool stuff we can do with it by changing how we use it, that's awesome too — especially things like firehose retention."* This deliberately opens the standing "Redis is exclusively the channel layer" rule for MC's design; the formal role-change ruling and the doc updates (CLAUDE.md / arch doc Redis lines) land with the release that ships it.

**The shape this suggests (for the design session — Redis Streams as MC's hot tier):**

- **Sink = `XADD` to a stream** (e.g. `mc:events`) at the creation-level tap points (consumer + ticker). Fire-and-forget inside a try/except: a Redis hiccup drops a log line, never a game action — additive-never-load-bearing holds by construction.
- **Every entry gets a stream id** (`<ms>-<seq>`) — the **global cursor** the per-connection `seq` can't be: agents resume from an id, replay is native (`XRANGE`), and ordering is total.
- **Consumer groups** give each listener — the Postgres persister, #191's pattern-watcher, #262's sudo watcher, #265's NPC responders — its own cursor, own pace, ack/pending semantics, and crash recovery (`XAUTOCLAIM`). Slow consumers never backpressure the game or each other.
- **Retention is a one-flag feature: `XADD … MAXLEN ~ N`** (or `MINID` for time-based trimming) — the stream is a bounded rolling window by construction. This is the "firehose retention" ask answered at the hot tier.
- **Two-tier retention:** Redis holds the hot window in RAM (bounded, fast, feeds live agents and replay-on-reconnect); a persister consumer group batch-drains to **Postgres as the durable record** (EBS disk, SQL analysis, long-horizon retention pruned by policy). RAM is the budget: at plausible sustained rates (~1–20 events/s, ~400 B/entry) a 1M-entry cap is ~hundreds of MB worst case — cap sized deliberately in the brief; the shared single-box memory is the constraint, and MAXLEN makes it a hard bound.

**Verified stack facts (docker-compose.yml, read 2026-08-16):** `redis:7-alpine` — Streams fully available; **no Redis Stack modules** (no RedisJSON/RediSearch — don't design against those); **no published ports** (Redis is unreachable from outside the compose network — remote agents can never speak Redis directly, and must not: the egress remains WebSocket-through-nginx with Django auth, with the server side reading the stream on the agent's behalf); **no volume** — today's Redis is ephemeral across container recreation, correct for a channel layer, and a deliberate decision point once the hot window has value (accept losing the window on rebuild — Postgres is the record — or add a volume/AOF).

Net: Streams solve buffering, replay, cursors, and multi-speed fan-out **inside** the trust boundary; the WebSocket solves transport **across** it; nginx/TLS/Django auth secure it exactly as they do for players.


**KnightOfNight** — 2026-08-18:
**Operator ruling — capture placement (2026-08-17):**

1. **The canonical MC sink taps at creation level.** One row per event, emitted where the event is born — in the Django consumer process and in the ticker's process (the tick engine creates events of its own). Total capture includes ingress: every inbound command is captured at `receive_json` (naturally one-row-per-event). Ingress capture is how `sudo`'s arguments reach a listener and is #191's primary food.
2. **Events carry their audience as a field.** The creation-level record notes at fan-out time who the event was addressed to, making per-player visibility ("what did P see") derivable by filtering on audience — no second tap, no per-recipient duplication.
3. **No delivery-level tap in 25.1.** The delivery choke point (`SkylandConsumer.send_json`) keeps its existing role — the `ts`/`seq` envelope stamp — and nothing more. A true delivery-confirmation tap may be added later only if a consumer demonstrates need.
4. **Cost acknowledged:** creation has no audited choke point today (events are born at many call sites). 25.1's work includes building one — a single emit helper every creation site calls, same "nothing may bypass it" discipline as the delivery envelope, fire-and-forget in try/except (a sink hiccup drops a log line, never a game action).

Consequence: the arch-doc :1375 rewrite (sequenced behind this ruling per #264) now unblocks — the choke point's text becomes "the envelope stamp; MC taps at creation." Wire-format details (stream id as global cursor, category, actor, room, audience, payload) are 25.1-brief work.

Ruled in conversation with Claude while working the V25 handoff's design-question list (§6 item 5).


**KnightOfNight** — 2026-08-18:
**Operator ruling — hot-tier architecture confirmed (2026-08-17):**

1. **Redis Streams is the MC hot tier.** Creation-tap events are `XADD`ed fire-and-forget; stream entry IDs are the global resume cursor and total order; every reader (Postgres persister, #191, future agents) attaches as its own consumer group with independent cursor, pace, and crash recovery — a slow or dead consumer never backpressures the game or its siblings. Alternatives considered and declined: Postgres-only (hot-path DB write + poll-based tailing), Redis pub/sub (no replay — a blinking consumer loses events), external brokers (new infrastructure for tens of KB/s).
2. **Two-tier retention confirmed.** Redis holds the bounded hot window (`XADD … MAXLEN ~ N`, cap sized in the 25.1 brief, RAM soft ceiling per #267); a persister consumer group batch-drains everything to **Postgres as the durable record** (SQL-queryable, policy-pruned).
3. **Redis stays volumeless — Postgres is durability.** A Redis recreation loses only the hot window, which refills. Consequence for #267's resume contract noted there.
4. **Design constraint:** plain `redis:7-alpine` Streams only — no Redis Stack modules (no RedisJSON/RediSearch); nothing may be designed against them.
5. **The formal Redis role change** (CLAUDE.md "exclusively the channel layer" line + arch-doc equivalents) rides 25.1's doc pass per the 2026-08-16 ruling on this issue.
6. **Dev-target fact for all MC briefs:** `local.py` uses `InMemoryChannelLayer` — Streams-based MC requires real Redis, so the containerized dev stack is the dev target from 25.1 on; bare-local is out for MC work.

Ruled in conversation with Claude while working the V25 handoff's design-question list (§6 item 6).
...
- Milestone: none
- Assignees: KnightOfNight
- Created: 2026-08-02 | Updated: 2026-08-18
- Blocked by: #37
- Blocks: none
...
## Issue #260: MC chat policy: GDD §7.1 and §10.5 contradict — rule chat persistence, privacy, and retention before MC ships

- State: open
- Author: KnightOfNight
- Labels: monitoring-and-command, V25
- Milestone: Version 25.0
- Assignees: KnightOfNight
- Created: 2026-08-16 | Updated: 2026-08-18
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/260

### Body

## Summary

**MC cannot ship without a chat ruling, and the GDD currently says two contradictory things.**

- **GDD §7.1:** all channels are *"logged server-side for moderation."*
- **GDD §10.5:** chat messages are *"never persisted."*

**Reality matches §10.5** — nothing is logged today. So one of these two sections is not merely out of date, it is the opposite of the shipped game.

#37 already flags this as a required ruling in its body. It is broken out here because it is a **policy decision about real players' speech**, not an implementation detail of a logging feature, and it deserves its own ticket, its own conversation, and its own record.

## Why this is load-bearing, not paperwork

**There are real human players on production** (since 2026-07-26). Whatever is decided here applies to speech those people have already produced under the current §10.5 understanding, and to speech they produce from the moment MC ships.

MC's whole premise is *"every command, every output, every event."* Chat is both:
- a **command** (`say`, and whatever channels exist), and
- **output** (it passes the same ts/seq delivery choke point every other message does — the tap #37 attaches to).

So the default behavior of a naive implementation is **capturing all player speech**, permanently, because it arrives at the tap like everything else. Not deciding is itself a decision, and it is the most invasive one.

## The decision surface

- **Do it or don't.** Persist chat, or exclude it at the tap. Excluding is cheap and specific — the envelope carries a category, so speech categories can be dropped before the sink.
- **If persisted: for how long?** A retention window is the main lever. "Forever" and "30 days" are very different products.
- **Who can read it?** The operator, admins in `admins.shyland`, an AI agent, analysis tooling — each is a separate grant.
- **Do agents get it?** Distinct from human access and probably the sharper question. An AI watcher reading player conversation is a different thing from an operator grepping a log during a moderation incident.
- **Do players know?** Disclosure, and where it lives — the `readme` command (#219) is an obvious surface, and there may need to be something at account creation.
- **Redaction / deletion.** Can a player ask for their speech to be removed? Does character deletion (shipped v24.27, admin-only) take their chat log with it? Today deletion is genuinely complete; a chat log would be the first thing that outlives a deleted character.
- **Does the moderation case actually need it?** §7.1's justification is moderation. It is worth checking whether the moderation need is real and current, or an aspiration written before there were players — the answer changes the cost/benefit entirely.

## A distinction worth preserving

**Chat is not the same as combat rolls.** #33 (combat-log persistence) is instrumentation of *mechanics* and carries none of this weight — nobody has a privacy interest in a dice roll. It would be a shame for the chat question to slow the balance-analysis work, and it would be worse for chat to ride in silently because it was bundled with something uncontroversial. **These should be able to ship independently**, which argues for excluding speech at the tap by default and revisiting deliberately.

That is a recommendation, not a ruling.

## Outcome required

A design session ruling that:

1. States the policy plainly.
2. **Corrects the GDD** — §7.1 and §10.5 must agree afterward, whichever way it goes. Note this is *not* an ops erratum: it needs a ruling first, so it is design content and lands on a version branch, never directly on main.
3. Gives #37 an unambiguous instruction about what the sink does with speech categories.

## Relationships

- **#37** — blocked by this in practice; #37's own body names the ruling as a prerequisite.
- **#33** — should be explicitly *unblocked* by this, per the distinction above.
- **#219** (`readme` command) — a possible disclosure surface.
- **#37/#191** and the AI-actor issues — the agent-access question above.

## Notes

Filed at operator direction, 2026-08-16, while scoping the **MC (Monitoring and Command)** major. Nothing ruled here. No labels or milestone (grouping labels are a design session's call).
...
Recorded under ruling immediacy, from the same-day ops session. The operator's words, near-verbatim:

> I'm ruling now. **Nothing in the game is private** — not even if we add direct-messaging later. DMs will go person to person, no other players will see them, but it will all go through the firehose. This is a change to the game design. I don't want any conditionals or any blockers to using everything in the game as data to making the game better, including chats. And there's one other big benefit: we can make AI respond to chats for NPCs.

("Firehose" spoken; the MC rename stands — read as the MC stream.)

## What this settles

1. **Total capture.** Everything in the game is data for making the game better. The MC sink at the ts/seq delivery choke point captures **every category, speech included** — no exclusions at the tap, no conditionals, no blockers. The exclude-speech-by-default recommendation in this issue's body is **rejected** by ruling.
2. **§10.5 is reversed; §7.1's direction stands — and widens.** "Chat messages are never persisted" is no longer the design. Capture is not merely for moderation: it is for balance, analysis, and AI, without qualification.
3. **Privacy model, stated plainly: player-visibility is not system-privacy.** A future DM is private *between players* — person to person, no other player sees it — but never private *from the game*. If/when DMs exist, they flow through MC like everything else. This is design-ahead: recorded now, binding on any future DM design.
4. **Agent access: yes, to all of it.** "Everything as data" includes AI consumers. Operator-named benefit: **AI responding to chats for NPCs** — a new consumer class (speech-producing, so it lands on the Command half, #261).
5. **Retention is now a purely operational question** — volume and disk, not privacy. Sizing it remains in #37's scope as an engineering decision; there is no privacy-driven deletion requirement.

## What remains on this ticket

- **The GDD harmonization** — §7.1/§10.5 and any other text asserting non-persistence. Design content on an unshipped system: it lands on the **V25 version branch via a design session** (GDD-first), never on main. This issue stays open until that text lands.
- **Disclosure to players** (the #219 `readme` surface) was a decision-surface item in the body. The ruling imposes no requirement; it is left to the design session as a non-blocking courtesy question, not a condition on anything.

## Effect on siblings

- **#37**: its "Required GDD ruling" blocker is resolved — noted there. No open blockers remain on #37.
- **#33**: explicitly unblocked — noted there.
- **#261/#262/#263/#259**: agents get the full stream; the "do agents get chat" question is answered yes.


**KnightOfNight** — 2026-08-18:
**Operator-blessed shape for the 25.0 GDD harmonization (2026-08-17):**

- A new §10 subsection — "Monitoring and Command (MC)" per the #264 house style — becomes the doctrine's single home: every event category flows through the MC sink, speech included, DMs included, no exclusions at the tap; capture serves balance, analysis, and AI, not just moderation; agents receive the full stream; retention is operational, not a privacy mechanism.
- §10.5's "Never persisted" list (chat, combat log) is reversed; §10.5 and §7.1 ("All channels are logged server-side for moderation") get one-line edits that point at the new subsection rather than restating policy.
- 25.0 text is worded as **policy now, mechanism design-ahead** (the sink itself arrives 25.1) — consistent with this issue's ruling that the harmonization lands with 25.0.
- The DM clause ("private between players, never from the game") lives as one sentence in the MC subsection; §7.1's Tell row gets a short cross-reference, not its own caveat prose.
- Player-facing disclosure is **out of scope** here — it rides #219's `readme` surface, non-blocking per the 2026-08-16 ruling.

Blessed in conversation with Claude while working the V25 handoff's design-question list (§6 item 2). Authoring happens on the version branch, GDD-first, in 25.0's doc pass.
...
## Issue #262: sudo AI watcher: an AI agent on the receiving end of sudo — reads the MC stream, and may or may not be allowed to answer
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/262

### Body

## Summary

An **AI agent on the receiving end of `sudo`** — the watcher players are already speaking to.

`sudo <anything>` shipped in v22 (#112) and is **half a feature by design**: the command echoes, the game never responds, and the arguments go nowhere. GDD §9 already names the missing half — *"The arguments' journey to a listener arrives with the firehose (#33/#37)"* — and GDD §12 says the same: *"`sudo`'s listener arrives with it."* This issue is that listener, and specifically the case where the listener is an **AI agent** rather than a log the operator reads later.

Filed at operator direction, 2026-08-16, as part of scoping the **MC (Monitoring and Command)** major — the system formerly called the firehose.

## Why this is the natural first AI actor

`sudo` is the cheapest possible place to put an AI agent in the game:

- The **ingress already exists and is stealth-gated** — `sudo` is invisible to non-members of the `admins.shyland` Group, so an agent here is reachable only by admins. No player-facing surface, no exposure, no griefing vector.
- The **audience is one person**. Failure modes are "the operator gets a bad answer," not "the world breaks."
- It is a **pure request/response shape** — no persistence, no turn order, no combat interaction. Every hard problem in #NN_COMMAND (the Command half of MC) is deferred.

## The one thing it collides with

**Today's ruling is that `sudo` never responds** — "no output, no acknowledgment, by design" (GDD §9.1, footnoted in the v22 command chart). An agent that answers **reverses a shipped ruling**. That reversal is a design decision, not an implementation detail, and it has to be made explicitly rather than discovered when the first reply appears in someone's pane.

Both directions are live options:
- **Keep the silence.** The agent reads `sudo` traffic and acts elsewhere — a digest, an alert, a change in the world — never a reply in the pane. The shipped ruling stands untouched.
- **Open a response channel.** `sudo` becomes a conversation. Needs a category, a voice (whose? not the world's ambient voice), and a rule for what happens when the agent is slow or down.

## Open questions for the design session

- **Read what, exactly?** Only `sudo` command text, or the whole MC stream with `sudo` as the address line?
- **Response or not** — the ruling above.
- **What can it *do*?** If the answer is more than "answer," this depends on **#261** (the Command half of MC) — an agent that can only speak needs nothing new; an agent that can act needs the whole actuation path.
- **Failure posture.** MC's standing rule is that logging is additive and never load-bearing. An agent is a live dependency in a way a log sink is not: a hung or absent agent must degrade to today's exact behavior (silence), never to an error or a stall.
- **Which model, and where it runs** — in-process, a worker, or an external service. Latency and failure posture follow from this.
- **Cost and rate limiting.** An admin typing `sudo` in a loop is a billing surface.

## Relationships

- **Depends on the Monitoring half of MC (#37)** for the stream, and #37's own blocker (#32, the ts/seq envelope) shipped in v20 — the tap point is live.
- **Sibling of #259 (Sirius)** and the player pets/companions issue: three AI-actor shapes, possibly one system. Whether they share a substrate is unruled.
- **Related to #191** — the command-pattern watcher is the same "agent listening to the stream" shape, aimed at design insight rather than admin interaction.

## Notes

Thin capture of operator direction. Nothing here is ruled; no labels or milestone (grouping labels are a design session's call). The response/silence question above is the first thing a design session should settle, because everything else is shaped by it.


### Comments (2)

**KnightOfNight** — 2026-08-18:
**Operator ruling — the silence reversal, and sudo the noun (2026-08-17):**

1. **"sudo" names the agent.** Not merely the command a player types — **sudo is the watcher itself**, referred to as a noun (styled lowercase, as the command is). Players speak to sudo; sudo may speak back.
2. **The shipped v22 silence ruling (#112, GDD §9 "the game never responds") is reversed, deliberately and conditionally:** *sudo may answer if running and able to parse the request.* It may respond with **immediate fulfillment of the request**, or it may **ask for more information**. Silence remains the norm and is never an error — not running, unable to parse, or simply not answering all look identical to the shipped game.
3. **Consequences:**
   - "May fulfill the request" means sudo has hands, not just a voice — what sudo is *authorized* to do (its action scope, rate, guardrails) is #262 design work inside the #261 arc, governed by the front-door and additive-never-load-bearing principles ruled there.
   - The kill-switch off-state is byte-identical to the shipped game: killed, sudo reverts to echo-and-silence exactly as v22 shipped it. This makes #262 the cleanest possible full-loop test of #266.
   - The GDD §9 "never responds" text does **not** change in 25.0's sweep — it stays true until sudo ships; the rewrite is design-ahead text riding #262's release.
4. **#262 is confirmed as V25's first live actor**, closing the slot the queue ruling (#269, item 4) left open at 25.5+.

Ruled in conversation with Claude while working the V25 handoff's design-question list (§6 item 8).


**KnightOfNight** — 2026-08-18:
Cross-reference (2026-08-17): ruled on #265 — **sudo's voice is not in-world speech.** The watcher is an out-of-world surface with a persona; it does not ride the say-color attributed-speech surface and does not wait on #265's generated-speech category rules. Rendering (category, color, voice) is this issue's design work.
...
## Issue #264: Terminology: the firehose is now Monitoring and Command (MC) — sweep forward-looking doc references
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/264

### Body

## Summary

**Operator ruling, 2026-08-16: "the firehose" is now "Monitoring and Command" — MC.** The major formerly scoped as universal event logging is now explicitly two halves, and the name says so.

This is the doc-side follow-through: forward-looking references to "the firehose" need to become MC. **It cannot be done as ops work on main** — the firehose is unshipped design, so under the erratum test (docs wrong about *shipped* behavior = ops; anything unshipped or needing a ruling = design content) this belongs to a design session on the V25 version branch.

Filed so the sweep does not get lost between the ruling and the release.

## What was already done (clerical, ops lane, 2026-08-16)

- GitHub label `firehose-logging` → **`monitoring-and-command`** (renamed in place; existing assignments on #37, #33, #191 preserved).
- Issue titles containing "firehose" retitled; a rename note added as a comment on each.

## What is NOT done — the design-session sweep

Forward-looking prose in versioned documents. Each needs a human read, not a blind find-and-replace, because several of these sentences describe a *tap point that shipped* pointing at a *system that has not*:

**GDD source (`docs/shyland/gdd/`)**
- `section_12_future_systems.md:15` — the future-systems row, currently *"The firehose (#33, #37) … `sudo`'s listener arrives with it."*
- `section_09_player_command_reference.md:152` — the `echo` settings paragraph: *"server behavior, timestamps, and the future firehose are untouched."*
- `section_09_player_command_reference.md:162` — the `sudo` entry: *"The arguments' journey to a listener arrives with the firehose (#33/#37)."*
- `section_10_technical_architecture.md:44` — the output envelope: *"the designated future firehose tap."*

**Architecture doc (`Shyland_Architecture_v24.md`)**
- `:1375` — the delivery choke point: *"the designated tap for the Firehose Logging milestone (#37/#33)."*
- `:1379` — *"the firehose tap is untouched."*
- `:1416` — *"the firehose and every server behavior are untouched."*

Arch-doc factual corrections can ride the ops lane under the #242 technical-writer test, but these are **terminology following a design ruling**, so they belong with the GDD edits in the same motion rather than split across two lanes.

**Project instructions (`Shyland_Project_Instructions_v37.md`)**
- `:126` — the capability-label example list names `firehose-logging`; the label has been renamed, so this line is now factually stale.
- `:276` — the Tech Stack close: *"the future firehose tap."*

These two are process-doc text and are arguably ops-lane, but a new instructions edition is written when **process rules** change and this changes none. Cleanest as a small edit riding whichever session next produces a `v38`, or folded into the V25 design session's branch work.

## Explicitly out of scope: the changelog

`_01_version_history.md` rows for **v20.0** and **v22.0** say "firehose." **Leave them.** They are the historical record of what those releases said at the time, and rewriting shipped changelog rows to match later terminology falsifies the record. Git history is the provenance for the rename; the changelog is the provenance for what v20 and v22 believed.

## Naming detail still open

The operator gave both the long form ("monitoring and command") and the short ("MC"). Worth a one-line ruling on house style — whether prose says "MC," "the MC system," or spells it out on first use per document — before seven files get edited in seven voices.

## Relationships

- **#37** — the Monitoring half.
- **#33**, **#191** — monitoring instrumentation and the first consumer.
- The **Command half** scoping ticket — the reason the name changed at all.

## Notes

Filed at operator direction, 2026-08-16. No labels or milestone (grouping labels are a design session's call). Small, mechanical, and easy to lose — which is why it has a number.


### Comments (3)

**KnightOfNight** — 2026-08-16:
Scope adjacency for the design session: the same branch pass that does this sweep should also **retheme the "V25 = new zones" references** — instructions v37 (Release Model + Zones Reference lines), GDD `section_02_world_model.md` ("the V25 zones…", "…when a V25 gate first asks for it") — since V25 is now MC (operator ruling 2026-08-16) and zones move to a later major. Forward-looking references only; shipped changelog rows stay as historical record. Enumerated with the rest of the handoff: `docs/shyland/Shyland_V25_MC_Design_Session_Handoff.md` (committed to main, feb809b). Founding ticket for the version: #269.

**KnightOfNight** — 2026-08-18:
**Operator ruling — MC house style (2026-08-17):**

First mention per document is "Monitoring and Command (MC)," thereafter "MC"; no article when MC stands alone ("events flow to MC"), normal article in compounds ("the MC sink," "the MC stream"); full form title-cased with lowercase "and" (Monitoring and Command); slug/label is `monitoring-and-command` only; no other variants (no "M&C," "Monitoring & Command," etc.). "V25 (MC)" names the major; "MC" unqualified names the system.

This unblocks the seven-file terminology sweep this issue tracks. Ruled in conversation with Claude while working the V25 handoff's design-question list (§6 item 1); the sweep itself still lands on the version branch per standing law.


**KnightOfNight** — 2026-08-18:
**Operator rulings — sweep + zones retheme mechanics (2026-08-17):**

1. **Zones references go unpinned.** "Version 25 = new zones" and kin re-point to "a future zones major" — no major number is named for zones (majors are named when scoped; "V26 = zones" would be a promise the tracker doesn't back). Instructions :118's example pair becomes "(Version 24 = new-zone-prep; Version 25 = MC — Monitoring and Command)". GDD §2's "the V25 zones" / "a V25 gate" and arch :1156's "(V25-era)" reword to the unpinned form.
2. **Instructions v37 is edited in place on the version branch** — no new edition (editions mark process-rule changes; this changes none). The four affected lines (:118, :126 label example, :276 firehose tap, :337 zones reference) ride 25.0's doc pass and merge to main with the release.
3. **Sequencing rule for arch :1375:** the "designated tap for the Firehose Logging milestone" passage is rewritten only *after* the creation-vs-delivery capture placement is ruled (#37, agenda item 5) — the sweep must not pre-decide a design question in prose. The other eight references rename freely.
4. **Changelog rows confirmed untouched** — v20/v22 "firehose" and v24.14/24.15 "V25" rows are the historical record; git history is the rename's provenance.

Ruled in conversation with Claude while working the V25 handoff's design-question list (§6 item 3). All edits land on the version branch in 25.0's doc pass.
...
## Issue #266: MC kill switch: a single lever that silences every AI actor at once — ships no later than the first live actor (HIGH PRIORITY)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/266

### Body

## Summary

**One lever that silences every AI actor at once.**

**Operator-ruled HIGH PRIORITY, 2026-08-16** — filed at operator direction while scoping the MC major. Recommended reading of that priority (for the design session to confirm): **the switch ships no later than the first live AI actor.** Never after.

## Why one lever

The day an NPC says something it shouldn't — or an agent misbehaves in any way, in a world with real players — response time is the whole game. By the time MC matures there will be at least four actor shapes (`sudo` watcher #262, pets #263, NPC chat responders #265, Sirius #259), each with its own runtime and its own failure modes. Hunting through four per-actor controls during an incident is exactly the wrong shape. One lever, one motion, everything quiet.

Per-actor toggles are a fine *addition* later. They are not this issue. This issue is the master.

## Scope: it kills actors, not capture

The switch silences **agent actuation and agent speech** — everything AI does *to* the world. It does **not** stop MC capture: monitoring is additive, harmless by law, and ruled total (#260 — everything is data). Data keeps flowing in; nothing AI flows out.

## The design principle: kill at the game side, not the agent side

A kill switch that asks agents to please stop is cooperative — useless against the failure modes that matter (hung agent, rogue agent, compromised agent, agent you can't reach). The reliable kill lives at the **game-side choke points**:

- **Egress:** stop forwarding the MC stream to agent consumers (players are unaffected).
- **Ingress:** refuse agent-originated actions and speech at the door (#261's actuation path checks the switch before honoring anything).

Flipped there, it works even if every agent process on earth is hung. Agent-side graceful shutdown can exist as a courtesy layer on top.

## Open questions for the design session

- **Mechanism.** Candidates: a config row on the existing pattern (the v24.30 uniform config setters — DB row + cache), a Redis key, an admin command, or several in concert. An admin command (`admins.shyland`-gated, like `sudo`/`last`) is the obvious operator-facing surface; whatever backs it must be readable from both the django and ticker processes.
- **Fail posture.** If the switch state can't be read, actuation should **fail closed** (agents stay silent) — the game degrading to "no AI" is the shipped game, which is always safe. Recommended, not ruled.
- **Per-actor degraded behavior when killed.** NPC responders and the `sudo` watcher go silent — in-character and indistinguishable from today. Sirius doesn't come — also in-character. **A pet mid-combat whose brain is off is the hard case** — #263's failure-posture question, which this switch forces to be answered early rather than discovered.
- **Who can flip it.** `admins.shyland` at minimum; whether it's also reachable out-of-band (shell helper, admin site) for the day the game surface itself is the problem.
- **Legibility.** Does the switch state announce itself anywhere (admin-visible status), and is flipping it an MC event? (It should be — the record should show when the world's AI went quiet and who did it.)

## Relationships

- **#261** — the actuation path this switch gates; the ingress check is part of that design.
- **#267 (MC egress)** — the stream-forwarding this switch cuts.
- **#262 / #263 / #265 / #259** — every actor ships behind this lever.
- **#236** (`wall`) — sibling admin-broadcast control, different purpose.

## Notes

Filed at operator direction, 2026-08-16, with the priority ruling recorded above. Queue order formally lives in design rulings (v30) — the HIGH PRIORITY here is the operator's word on the tracker for the design session to sequence accordingly.


### Comments (1)

**KnightOfNight** — 2026-08-18:
Classification ruled (2026-08-17, v35 litmus): **NOT an action item** — the handoff's hedge is resolved the other way. This is the founding ticket of 25.4 and closes with it. The arc-like part — every future actor must wire into the switch — is a **standing invariant**, not ticket work: "no actor goes live without kill-switch integration" ships as permanent doctrine text with 25.4 and binds every later actor brief, the way the soulbind and bar-law invariants bind theirs.

## Issue #267: MC egress: how remote consumers attach — Streams inside the trust boundary, WebSocket across it (auth, backpressure, resume)
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/267

### Body

## Summary

**MC egress: how remote consumers attach to the stream.** #37 owns capture, storage, and query; #261 owns acting. The pipe between the store and remote readers — live attach, auth, backpressure, resume — is this issue.

Filed at operator direction, 2026-08-16, after the feasibility discussion (recorded on #37).

## The shape already sketched (operator direction + verified stack facts)

Operator direction (2026-08-16, on #37): expanding Redis's role for MC is welcome — Streams as the hot tier. Verified against `docker-compose.yml` the same day: `redis:7-alpine` (Streams available, no Redis Stack modules), **no published ports** (only nginx faces the network), no volume (ephemeral today).

The contract that falls out:

- **Streams inside the trust boundary.** The sink `XADD`s to a stream at the creation-level taps (django consumer + ticker — both already share the Redis network). `MAXLEN ~` bounds the hot window; a persister consumer group batch-drains to Postgres as the durable record.
- **WebSocket across the boundary.** Remote agents never speak Redis (it is not internet-grade and is deliberately unexposed). They connect exactly like players: WSS through nginx, Django auth, a new MC consumer endpoint whose server side reads the stream on their behalf. Same front door, wider subscription.
- **Stream ids are the resume cursor.** Per-connection `seq` cannot be a resume token; the `XADD` id (`<ms>-<n>`) is global and totally ordered. An agent reconnects with "everything since id X" and replays from the hot window; older than the window means the durable store (query, not stream).
- **Backpressure never reaches the game.** `XADD` is fire-and-forget in a try/except; consumer groups give every reader its own cursor and pace; a slow WS client gets dropped or trimmed-past, never buffered-for at the game's expense. Live is best-effort; the store is truth.

## Open questions for the design session

- **Agent identity and authn.** Service accounts as Django users? A dedicated Group on the `admins.shyland` pattern (e.g. `agents.shyland`)? Token vs session auth over WSS? One identity per agent (attribution in the record wants this).
- **Authorization granularity.** Full stream for all authenticated agents (per #260, everything is data — but *access* is still a grant), or scoped subscriptions (categories, rooms)?
- **Wire format.** The enveloped event as JSON plus the stream id; schema versioning from day one.
- **Duplex or read-only.** Does the same connection eventually carry agent actions under #261's rules, or is actuation a separate ingress? (Speech-only actors, #265, make duplex tempting; the kill-switch ingress check applies either way.)
- **Replay limits.** How far back a live connection may replay before being redirected to the durable store's query surface.
- **Dev parity.** Playtests run on the dev stack; note `local.py` uses `InMemoryChannelLayer` — a Streams-based MC needs real Redis wherever it runs, so the dev-stack (containerized, has Redis) is the target and bare-local is explicitly out.

## Relationships

- **#37** — capture and the store this egress reads; the creation-vs-delivery tap note there feeds the wire format.
- **#261** — the write half; the duplex question above.
- **#266 (the kill switch)** — egress forwarding is one of its two game-side cut points.
- **#191 / #262 / #265** — the consumers this exists for.
- **#223** — monitoring the monitors, adjacent.

## Notes

Filed at operator direction, 2026-08-16. The Redis role-change ruling and doc updates (CLAUDE.md / arch-doc Redis lines) land with the release that ships this, per standing law. No milestone; grouping label `monitoring-and-command` per operator direction.


### Comments (2)

**KnightOfNight** — 2026-08-16:
**Operator note, 2026-08-16 (recorded from the ops session) — the RAM ceiling is soft:**

> I'm OK adding RAM to the instance for Redis, or maybe offloading Redis to an AWS-native service so it scales on its own — but I bet we're a few users away from that.

What this means for the design here:

- **The hot-window budget is not a hard design constraint.** Size `MAXLEN` for what the consumers actually want (replay depth, agent catch-up), not for fear of the box — if the window wants more RAM, vertical scale is pre-approved in principle, and ElastiCache is the named escalation path beyond that.
- **But build for the current reality.** The expectation is we're a few users away from needing any of it: the compose-stack `redis:7-alpine` stays, and nothing gets architected *for* ElastiCache now.
- **The one cheap future-proofing worth taking:** keep the Redis endpoint a configuration value rather than a baked-in container name on the MC paths, so "offload to ElastiCache" is someday an `.env` change plus a ruling, not a refactor. (An in-VPC ElastiCache endpoint also stays consistent with the never-expose-Redis law — it would be as unreachable from the internet as the container is.)
- **Cross-refs:** #37 carries the original Redis-direction note and the volume arithmetic; #214 (AWS work) is where any actual ElastiCache move would be planned if the day comes.


**KnightOfNight** — 2026-08-18:
**Cross-reference from the hot-tier confirmation (#37, 2026-08-17):** the operator confirmed Redis stays **volumeless** — Postgres is the durable record; a Redis container recreation loses only the hot window. Binding consequence for this issue's resume/replay contract: **agents must not assume stream IDs survive a Redis restart.** Deep replay goes to Postgres; the stream serves recent-window catchup. Design the resume cursor semantics accordingly in the 25.3 brief.
...
## Issue #269: V25.0 founding ticket: MC (Monitoring and Command) — the major design pass and version opening
...
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/269

### Body

## Summary

**The founding ticket for Version 25.0 — the opening release of the V25 major: MC (Monitoring and Command).**

Written at operator direction (2026-08-16) as the top-level MC ticket for the version's opening design session. The full handoff from the scoping discussion is committed at `docs/shyland/Shyland_V25_MC_Design_Session_Handoff.md` — read it alongside this ticket.

## Why a new ticket, and not #37

The founding ticket must **close with its release** (the closeout entry gate demands the milestone closed N/N; the v35 litmus — closes-with vs outlives — is law). Operator expectation, stated 2026-08-16 and recorded here:

> V25.0 ships nothing to dev or prod except the version bump — we won't be able to close something real like "the firehose works." 25.1 will see the first real change.

So #37 (the sink itself) cannot found 25.0 — it ships with a later point release and would outlive the milestone. **This ticket's deliverable is the design pass itself**, which is real work that 25.0 genuinely completes.

## Closes when (the 25.0 deliverables)

1. **The coherent MC design pass is done** — the major's opening design session(s) have ruled across the MC constellation (#37, #33, #191, #259–#268), with rulings recorded on their issues per ruling immediacy.
2. **The V25 queue order is ruled** — which ticket founds 25.1, 25.2, … (recorded as design rulings; order never lives in the tracker structure).
3. **The 25.0 GDD/doc text has landed on the version branch** — the §7.1/§10.5 harmonization (#260's remaining work), the firehose→MC terminology sweep (#264), and the retheme of "V25 = new zones" references to the new major plan.
4. **The 25.0 release ships** — version bump per the standing lifecycle, plus the major-opening mechanics at closeout (`GDD_MAJOR` bump, monolith rename to `Shyland_GDD_v25.md`, old monolith `git rm`'d).

## What V25 is

**"MC Shyland in the house"** (operator, 2026-08-16). Monitoring and Command: the game becomes fully observable (every command, every output, every event — total capture ruled on #260), and gains the beginnings of a sanctioned path for non-human actors — the `sudo` watcher, NPC chat responders, pets, and Sirius — with the kill switch (#266, HIGH PRIORITY) shipping no later than the first live actor. New zones move to a later major.

## Notes

Filed from the 2026-08-16 ops session. Milestone assignment (`Version 25.0`, born with the design session) and the `V25` label creation are the design session's own acts, not performed here.


### Comments (4)

**KnightOfNight** — 2026-08-18:
**Operator ruling — V25 queue order (2026-08-17):**

The release ladder 25.1–25.4 is adopted as the ruled spine, dependency-forced:

- **25.1 — #37**: the sink (taps, Redis Stream hot tier, Postgres persister, retention caps).
...
- **25.4 — #266**: the kill switch — in place before any actor goes live, satisfying the #266 gate exactly.

**Deliberately unruled:** the first live actor at 25.5+ (#262 vs #265) — the pick waits on the `sudo`-silence question (#262) and the AI-speech/pooled-speech question (#265), still ahead on the design-question list. The ladder reads "25.5+ = first actor, identity ruled after those."

**#220** (shared-NPC combat) stays unsequenced — non-MC, gates only #263 (pets), can be taken up in parallel at the operator's choosing. **#261 and #268** get no rungs — v35 action-item shape; slices ride briefs, ops closes at arc end. **#259** (Sirius) waits on the actor substrate.

Milestone mechanics unchanged per v30: no milestones created ahead; each `Version 25.N` milestone is born with its release's design session. This comment is the order-of-record.

Ruled in conversation with Claude while working the V25 handoff's design-question list (§6 item 4).


**KnightOfNight** — 2026-08-18:
**Queue-order addendum (2026-08-17):** the open 25.5+ slot is now ruled — **the first live actor is #262, sudo** (silence reversal ruled on #262 same day). The ladder's spine reads 25.1 #37 → 25.2 #33 → 25.3 #267 → 25.4 #266 → 25.5 #262. Later actors (#265, #191-as-consumer, #263 behind #220, #259 behind the substrate) remain unsequenced.


**KnightOfNight** — 2026-08-18:
**Operator ruling — label & milestone mechanics (2026-08-17), completing the handoff's design-question list:**

1. **`V25` label: all fourteen constellation tickets** — #269, #37, #33, #191, #260, #261, #262, #263, #264, #265, #266, #267, #268, #259 — including the gated pair (#259, #263: a gate delays a ticket, it doesn't move it to another major) and the v35 action items (#261, #268: labels are permanent membership with no closeout gate; it is milestones arcs must never join). The five adjacent non-MC tickets (#220, #223, #236, #219, #214) stay unlabeled. Standing direction: new MC filings get `monitoring-and-command` + `V25` when triaged in.
2. **`Version 25.0` milestone holds exactly #269, #260, #264** — all three close with 25.0's doc pass. Nothing else joins; 25.1's milestone is born with 25.1's design session per v30.
3. **Execution belongs to the opening design session** (v30: a milestone is born with its release's design session): verify V24.31 end state, create `version_25_0` + worktree, create/apply label and milestone, author the ruled doc pass (#260 shape, #264 sweep, ladder), write the 25.0 brief. The major's first-session re-triage duty is substantively discharged by the 2026-08-17 ruling set.

**The design-question list (§6 of the committed handoff) is now fully worked — all eleven items ruled 2026-08-17.** Ruling locations: house style + sweep mechanics on #264; harmonization shape on #260; queue spine + first actor on #269 (this issue); capture placement + hot-tier architecture on #37 (resume consequence on #267); Command-half scope + spine principles on #261; the silence reversal + sudo-the-noun on #262; generated-speech doctrine on #265; v35 classifications on #261/#268/#266. Tracker is authoritative; the conversation was operator-attended throughout.


**KnightOfNight** — 2026-08-18:
Brief committed (2026-08-17): `docs/shyland/Shyland_V25.0_Brief_1_MC_Major_Opening.md` on `version_25_0` @ fdeeee7 (doc pass landed at 5da479f). Thin by construction — version-start + a five-reference comment/docstring-only firehose→MC sweep in game code (consumers.py ×3, envelope.py, models.py; the two designated-tap docstrings get the corrected creation-level sentence per the #37 ruling), no new tests, no migration, no seed. v36 technical claims verified at writing time. Milestone `Version 25.0` closes 3/3 via the brief's §9 gate.
