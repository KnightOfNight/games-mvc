...
| 235 | New player setting: 'plunder' (default off) — auto-loot rights-held corpses at combat end | KnightOfNight | triaged, V24, commands | Version 24.29 | 2026-08-15 |
...
| 250 | _set_echo_mode assigns the wrong cached attribute — echo writes to self.character.show_timestamps | KnightOfNight | bug, emergent, triaged, V24, commands | Version 24.29 | 2026-08-15 |
...
- State: open
- Author: KnightOfNight
- Labels: triaged, V24, commands
- Milestone: Version 24.29
- Assignees: KnightOfNight
...
**Design ruling (2026-08-15, V24.29 design session — operator-confirmed). This issue founds Version 24.29;** milestone assigned, GDD text landed on `version_24_29`.

**The trigger, anchored to shipped code.** `plunder` fires at the transition to genuinely out of combat — every NPC in the fight dead — and at no other moment. That instant already exists in the engine as the point where `Combat has ended.` is delivered: three sites in `run_tick_engine.py`, all firing on the same "no living NPCs remain" condition (the loop-head self-heal close from #218, the M2M sibling-session close, and the killer's own last-kill close). Defining plunder as *"fires wherever `Combat has ended.` is delivered to a character"* is the whole trigger rule — one anchor, no new condition — and it excludes flee and death for free: flee's end path lives in `consumers.py` and emits no such line, and a dying character is removed from the session before it can close. The two trigger rulings above (miss-never-queue; no mid-fight per-kill loot) are satisfied by construction.

**Four rulings settled this session:**

1. **Silence contract — plunder is silent unless it plunders.** With no corpses, or none this character has rights to, plunder emits *nothing*. The typed command's refusals (`There is nothing to loot here.`, `That is not your kill; you may not loot it.`) belong to the typed command and are never spoken on plunder's behalf.
2. **Positive case is byte-identical to `loot`.** When plunder does sweep, the output is the sweep's output verbatim — per-item loot lines, coin lines, summary line, and the carry-capacity refusal when the player fills up mid-sweep (a shortfall is a consequence and must be seen, #132). A plunder is indistinguishable from a `loot` the player typed.
3. **Plunder does not require a connected player.** Quit is allowed in combat and combat continues after quit, so a fight that ends for a logged-out character plunders on their behalf. The loot is theirs by the same rights predicate either way; the output simply has no pane to land in.
4. **Field:** `plunder_mode = models.BooleanField(default=False)` on `Character`, matching the `brief_mode` / `echo_mode` / `show_timestamps` family. Migration required.

**Economy consequence — RATIFIED, no watch (operator, 2026-08-15):** *"autoloot is accepted as a rule and as a factor in the economy. no watching, no other updates."* Note for the record: the consequence-analysis comment above proposed watching via #164, but #164 closed with V24.2 and its Income Law is unaffected regardless — the V24.1 fight-cost survey computed expected income from the seeded drop rows, i.e. it already assumed the loot gets collected. Plunder raises the *realization rate* toward the number the model was always built on; it adds no faucet. What is genuinely new is income per unit of player attention, and of the levers named earlier only #70 (Longevity's first drain) remains open — it wants that mechanic for its own design reasons.

**The one architectural constraint for the brief.** The sweep (`_loot_sweep`, `consumers.py`) is a method on `SkylandConsumer` using `self.output` and consumer-bound helpers; the trigger is in the **tick engine**, a separate process with no consumer — and, per ruling 3, sometimes no connected player at all. The brief therefore extracts the sweep into a transport-agnostic helper that performs the mutations and *returns* message tuples, called by both `cmd_loot` and the engine (whose message accumulator is already `(char_pk, text, category, status)` — the right shape). Behavior of typed `loot` must not change.

**GDD text landed** on `version_24_29` with `(v24.29, pending implementation)` markers: §9.1 command chart (settings row, footnote 1, Added v24.29), the Settings Standard (three → four commands, `plunder off` default), the authoritative passage under §9.1 Corpses and Loot, §9.3's boolean-command rule, and a cross-reference sentence on §5.9's corpse paragraph.


**KnightOfNight** — 2026-08-15:
**Brief committed (2026-08-15, V24.29 design session).** `docs/shyland/Shyland_V24.29_Brief_1_Plunder_And_Verification_Harness.md` on `version_24_29` at `52f1568` — born committed per CLAUDE.md Rule 4. Actionable when the operator directs it by name to an implementation session.

The brief carries #235 (plunder, founding), #250 (the emergent `_set_echo_mode` fix, operator-ruled in) and #249 Part 2 (the verification harness) as three separate commits sharing no code.
...
## Issue #250: _set_echo_mode assigns the wrong cached attribute — echo writes to self.character.show_timestamps

- State: open
- Author: KnightOfNight
- Labels: bug, emergent, triaged, V24, commands
- Milestone: Version 24.29
- Assignees: KnightOfNight
- Created: 2026-08-15 | Updated: 2026-08-15
- Blocked by: none
- Blocks: none
- URL: https://github.com/KnightOfNight/games-mvc/issues/250

### Body

## The defect

`SkylandConsumer._set_echo_mode` (`django/src/apps/shyland/consumers.py`, ~:4087) assigns the wrong cached attribute:

```python
@database_sync_to_async
def _set_echo_mode(self, value):
    Character.objects.filter(pk=self.character_pk).update(echo_mode=value)
    self.character.show_timestamps = value      # <-- should be echo_mode
```

The database write is correct. The in-memory line sets `show_timestamps` where it means `echo_mode`, so every `echo on` / `echo off` silently corrupts the consumer's cached `self.character.show_timestamps`.

## Why it is not currently player-visible

Both readers of that attribute happen to be safe today:

- **`:498`** — the connect-time `verbs` payload reads `self.character.show_timestamps`, but it fires during `connect()`, before any command can dirty the cache.
- **`:3450`** — `_status_payload` reads `char.show_timestamps` from a **fresh** DB fetch (`get_character_fresh()`), not the cached object.

So the corrupt value is written and never read. Nothing is wrong for players right now.

## Why it still matters

It is a wrong-attribute assignment sitting in the settings family — a landmine, not a curiosity. It becomes a live bug the moment anything reads the cached `self.character.show_timestamps` after a command has run, and the correctness of `timestamps` is exactly the kind of thing a future change would reasonably read from the cached character. It is also the template a developer would copy when adding a new setting, which is how it was found: the V24.29 design session hit it while specifying `_set_plunder_mode` for #235.

Note the sibling setters are correct — `_set_brief_mode` and `_set_show_timestamps` write only the DB row, and `cmd_brief` / `cmd_echo` update the cached attribute themselves after the setter returns. The stray line in `_set_echo_mode` is redundant as well as wrong: `cmd_echo` already does `self.character.echo_mode = value`.

## Fix

Delete the stray line, or correct it to `self.character.echo_mode = value`. Deleting is preferred — it matches `_set_brief_mode` and `_set_show_timestamps`, and `cmd_echo` already maintains the cached value.

## Disposition

Found by the V24.29 design session (2026-08-15) while specifying the `plunder` setting (#235). Operator ruled it into **V24.29 Brief 1** as a related ticket — emergent and important — rather than queuing it.
