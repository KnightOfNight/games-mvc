...
| 37 | MC monitoring: universal event logging — every command, every output, every event | KnightOfNight | monitoring-and-command, V25 | Version 25.1 | 2026-08-18 |
...
| 271 | Presence Redis client hardcodes its endpoint — bypasses REDIS_HOST, blocker to MC configurable-endpoint provision | KnightOfNight | monitoring-and-command, V25 | Version 25.1 | 2026-08-18 |
...
**KnightOfNight** — 2026-08-18:
**Operator ruling — the event record (2026-08-18, V25.1 design session):**

1. **Taxonomy — 25.1 captures four event kinds:**
   - `cmd` — one inbound command at `receive_json`, accepted or rejected; raw payload plus resolved verb and outcome.
   - `out` — one outbound game event at its creation site (consumer and ticker alike); **all client message types fold in** — `output`, `status` (bar sync), and `redirect` are captured with their client type recorded, no carve-outs (total capture, #260).
   - `connect` / `disconnect` — presence transitions at the consumer's connect/close paths.
   - Combat internals remain #33 (25.2) per the ruled ladder on #269.
2. **Record shape — flat fields plus JSON:** stream entries carry flat top-level fields every reader filters on — `kind`, `actor` (name + id; NPC/system identity for world events), `room` (id; empty when roomless), `audience` (the ruled fan-out field: JSON list of recipient character ids resolved at fan-out time) — plus one `data` JSON field holding the payload (command args, output text + client category, …). The stream id (`<ms>-<n>`) is both timestamp and global cursor — no separate `ts` field at the hot tier; the Postgres row stores the stream id verbatim as its unique cursor column plus an indexed timestamp derived from it.
3. **The no-FK rule (binding):** the durable record never ForeignKeys into live tables. `actor` and `room` are loose ids plus denormalized names. Reseeds are enforce-exact and delete rows; an FK from the log would cascade-delete history or block reseeds. The log is a record of what happened, not a live relation.

Wire-format field details beyond this (exact key names, encoding) are 25.1-brief work. Ruled in conversation; GDD §10.11 text lands on `version_25_1` this session.


**KnightOfNight** — 2026-08-18:
**Operator ruling — sink mechanism: emit helper, retention, persister (2026-08-18, V25.1 design session):**

1. **The emit helper is the audited creation choke point.** New module `apps/shyland/mc.py`, single entry point `mc_emit(kind, actor, room, audience, data)`. Builds the record and `XADD`s it; the entire body is fire-and-forget inside try/except — a sink failure drops the record, never raises into game code. Two client paths under one module: async Redis client for the consumer's context, sync client for the ticker's process, shared record-building code. Redis endpoint from settings via env var (default: the compose `redis` host) — the one-cheap-future-proofing ruled on #267. **Discipline mirrors the delivery envelope: every creation site calls `mc_emit`; nothing may bypass it.** Client-library wiring details are brief work, verified against `requirements.txt` at writing time (#252).
2. **Retention, both tiers:**
   - **Hot tier: `XADD … MAXLEN ~ 100000`**, env-configurable (`MC_STREAM_MAXLEN`). ~40 MB at ~400 B/entry — well inside the #267 soft RAM ceiling; covers many hours at realistic sustained rates and ~10 minutes at absolute peak burst, ample for persister lag and reconnect replay. Revisited when agents arrive (25.3+) — a config change, not a release.
   - **Durable tier: no pruning in 25.1.** Deliberate statement: durable retention is unbounded *pending an operational trigger*, not by oversight. Row count stays visible; the pruning policy ships when data shows need.
3. **The persister is its own container.** New compose service `mc-persister`, same image as `django`/`ticker`, running new management command `run_mc_persister` (the ticker's proven pattern). Consumer group `persister` on the stream: batched `XREADGROUP` → `bulk_create` → `XACK`, `XAUTOCLAIM` at startup for crash recovery. A persister crash loses nothing (the stream buffers to MAXLEN) and can never backpressure the game or a future agent.
4. **Shared-surface edit explicitly authorized (CLAUDE.md Rule 2):** the operator confirms the `docker-compose.yml` change adding the `mc-persister` service. The 25.1 brief directs the implementation session to make it.

Ruled in conversation; GDD §10.11 text lands on `version_25_1` this session.


**KnightOfNight** — 2026-08-18:
**Operator ruling — query access, the 25.1 slice (2026-08-18, V25.1 design session):**

1. **Django admin, read-only.** The durable-record model registers in the admin with add/change/delete all disabled; list display and filters on kind, actor, room, timestamp; search over the payload. Principle made explicit: **the record is append-only truth — nobody edits history, not even admins.**
2. **SQL is the math engine.** Shell/dbshell against the indexed table is the analysis surface for 25.1. No bespoke reporting, export, or aggregate tooling ships in this release — #191 and later consumers are the real analysis clients; query tooling waits for real questions.

This closes the design decisions for the 25.1 sink. Remaining session work: GDD §10.11 text on `version_25_1`, then the brief (technical claims verified at writing time per #252).


**KnightOfNight** — 2026-08-18:
**Verification correction (2026-08-18, V25.1 design session, #252 pass):** the mechanism ruling above says "async Redis client for the consumer's context, sync client for the ticker's process." Verified against the code at brief-writing time: the ticker is a fully async process (`run_tick_engine.py:65` — `asyncio.run(self.tick_loop())`), and its two outbound helpers (`send_to_player:1714`, `broadcast_to_room:1739`) are `async def`. **One async Redis client path serves both processes** — the two-client provision is unnecessary and the brief specifies the simpler single-path form. No operator decision is disturbed: the substance (one module, one entry point, fire-and-forget) stands.

Adjacent verified facts the brief builds on: `redis>=5.0` is already a direct dependency (`requirements.txt:9`); `REDIS_HOST` already exists as a settings-level env with default `redis` (`base.py:82`) — the configurable-endpoint provision rides it; the ticker's outbound already funnels through exactly two creation helpers, and the consumer's personal output funnels through `send_output`/`output`/`send_report_lines` plus a bounded set of direct `send_json` sites (27 total) and 10 `group_send` sites — the emit-site inventory is enumerable and the brief enumerates it.


**KnightOfNight** — 2026-08-18:
**Operator ruling — the protocol-chrome boundary (2026-08-18, V25.1 design session):**

Three WebSocket message flows are pure client mechanics and are **excluded from capture** as protocol chrome: the `ping`/`pong` keepalive nonce, and the connect-time `verbs` list. Tab-`complete` requests are **captured** as `cmd`-kind records — they are player intent and #191's "what are players fumbling for" food. The boundary is a ruled line stated in GDD §10.11, not an erosion of total capture: chrome carries zero game information, and presence is properly captured by the `connect`/`disconnect` kinds.

**Also ruled same session:** the presence client's hardcoded Redis endpoint (`consumers.py:458`) is **a blocker to MC's success** — filed as #271, joined to milestone Version 25.1 as a dependency of this ticket, fixed by the 25.1 brief (settings-constant fix shape recorded there).


**KnightOfNight** — 2026-08-18:
**Ruling sweep straggler (2026-08-18, V25.1 design session end ritual — late catch):** consolidating the session's shared-surface authorizations (CLAUDE.md Rule 2) so all live on the tracker, not only in the brief:

1. **`docker-compose.yml`** — the `mc-persister` service (recorded in the mechanism ruling above).
2. **`game_mvc/settings/base.py`** — operator-authorized additive constants: `REDIS_HOST` (named constant; the two inline env reads re-point at it — the #271 fix shape) and `MC_STREAM_MAXLEN` (default 100000).
3. **CLAUDE.md** — the ruled Redis role-change language confirmed verbatim: the Redis bullet becomes "the Django Channels channel layer, the Shyland presence keys, and the MC event stream (hot tier — Streams, bounded window). Not a general-purpose cache." plus the `mc-persister` row in the infrastructure table.

All three land via the implementation session executing `Shyland_V25.1_Brief_1_MC_Sink.md` (§5.1, §5.8, §5.10), committed at `6b71d67` on `version_25_1`.
...
## Issue #271: Presence Redis client hardcodes its endpoint — bypasses REDIS_HOST, blocker to MC configurable-endpoint provision
...
- Blocked by: none
- Blocks: #37
- URL: https://github.com/KnightOfNight/games-mvc/issues/271

### Body

## Summary

The consumer's presence client hardcodes its Redis endpoint: `consumers.py:458` reads

```python
self.redis = aioredis.from_url("redis://redis:6379")
```

while every other Redis consumer in the stack routes through the `REDIS_HOST` env var — the channel layer (`game_mvc/settings/base.py:82`, default `redis`) and `DEFENDER_REDIS_URL` (`base.py:124`). If `REDIS_HOST` ever points anywhere else (the ElastiCache escalation named on #267, or any endpoint move), presence — single-session enforcement, the online set, `who`/room-occupancy filtering — silently splits from the channel layer onto a dead endpoint.

Found during the V25.1 design session's #252 verification pass (2026-08-18), while confirming the MC sink's configurable-endpoint provision (#267: the Redis endpoint stays configurable as the one cheap future-proofing).

## Operator ruling (2026-08-18, V25.1 design session)

Filed at operator direction: **this is a blocker to MC's success** — the configurable-endpoint provision is void while a core consumer bypasses it. Joins Version 25.1 as a dependency of #37 and is fixed by the 25.1 brief.

## Fix shape

Expose the already-existing env read as a settings constant (`REDIS_HOST = env('REDIS_HOST', default='redis')` in `base.py`), build the presence URL from it, and point the existing inline `env('REDIS_HOST', …)` reads at the same constant. No behavior change at any current deployment (the value is identical everywhere today); the MC sink's client uses the same constant from birth.
