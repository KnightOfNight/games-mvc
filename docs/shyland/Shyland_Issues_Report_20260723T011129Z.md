...
| 27 | Research: passive regen ticks landing after combat engagement | KnightOfNight |  | Version 23 | 2026-07-23 |
...
| 117 | shyland: stub tests.py shadows tests/ package — breaks whole-app test discovery | KnightOfNight | bug | Version 23 | 2026-07-22 |
...
| 137 | Corpse decay orphans unlooted contents — ItemInstance rows leak with no location | KnightOfNight | bug | Version 23 | 2026-07-22 |
| 138 | Bound zero-value items have no disposal path — starter kit junk is stuck in inventory forever | KnightOfNight | bug | Version 23 | 2026-07-22 |
...
| 141 | text cleanup | KnightOfNight |  | Version 23 | 2026-07-23 |
...
## V23 research spike — findings (code facts only; no intent ruling)

### Verdict: (c) — the observed lines are not passive regen at all

The passive regen pass cannot have produced the logged lines, for two independent reasons: it has **never emitted text** (silent status-only, since its introduction in v14), and it **excludes in-combat characters**. The logged line `You feel your body recover. (+25 Vitality)` is the message of the **instant `restore_vitality` effect component** — i.e. a consumable heal (a Mk 1 Healing Draught restores exactly 25), reachable only via the player's `use` command. The two lines landing between "moves to attack!" and first blows are explained by combat round cadence: rounds resolve only every 3rd session tick, leaving a multi-second window after engagement in which player-initiated heals print.

### 1. The passive regen pass

Location: Phase 4 of `process_effects`, `django/src/apps/shyland/management/commands/run_tick_engine.py:1249-1302`. Tick ordering is `run_tick_engine.py:54-60`:
...
async def process_tick(self, tick_number):
    ...
    await self.process_effects(tick_number)      # Phase 4 = passive regen
    await self.process_dying_ladder()
    await self.process_dialogue_delivery()
```

Selection query, `run_tick_engine.py:1253-1269`:
...
**The combat-session exclusion exists**: `run_tick_engine.py:1267`. It has been there since the pass was introduced (commit `f4d212e`, "v14: passive out-of-combat regeneration" — commit message: *"silent per-tick Vitality and Longevity regen for characters not dying and not in active combat … no text message sent to the player"*).

**The pass sends no text.** Its only output is a bare status payload, `run_tick_engine.py:1298-1302`:
...
Arithmetic corroboration: the regen formula is `heal = ceil((vitality_max - vitality_current) / VITALITY_REGEN_SECS)` with `VITALITY_REGEN_SECS = 120` (`models.py:14`, `run_tick_engine.py:1281-1283`). Producing +25 in one tick would require a Vitality deficit of ~2,900 — not a real character. And it never prints a number regardless.

### 2. Where the logged text actually lives

`django/src/apps/shyland/effect_utils.py:97-100`, the instant-component applier:
...
`git log -S` shows this string has only ever existed here (introduced v12, commit `0ba36ef`); it has never been in the tick engine. Standalone, it is reachable only through the consumable path: `use` command → `do_apply_effect` (`consumers.py:3570-3572`) → output loop `consumers.py:1404-1406` ("You use a …" then the effect line). The other caller, NPC on-hit effects (`combat_utils.py:370-395`), appends effect names inline to the attack line — never a standalone line.

Magnitude match: the seeded Healing Draught's `restore_vitality` component has `magnitude_base 20.0, magnitude_scaling 5.0` (`seed_world.py:3151-3158`); `computed_magnitude = base + scaling × mk_tier` (`models.py:376-377`) ⇒ **Mk 1 = exactly 25**.

### 3. The engagement-to-first-blows gap (why the timing looked like a race)

Walk-in aggro runs in the consumer, not the tick engine: on room entry the engagement lines print first, then the session is created (`consumers.py:629-642`; same pattern in flee-into-aggro, `consumers.py:2215-2227`):

```python
await self.send_output(f"... snarls and moves to attack!", 'combat')
...
session = await self.start_combat(aggro_npcs, first_attacker='npc')
```

First blows then wait for a round boundary: `COMBAT_ROUND_TICKS = 3` (`models.py:6`) and `run_tick_engine.py:263` (`if tick_counter % COMBAT_ROUND_TICKS != 0: … continue`) — so up to ~3 seconds pass between engagement and the first exchange. Two `use` commands fit comfortably in that window, and their output is exactly the logged lines.

On hypothesis (b) specifically: **within the tick engine there is no ordering race.** The respawn-tick aggro engagement (v21 #17: `process_npc_respawn` → `engage_respawned_aggro`, `run_tick_engine.py:757-763, 844-943`) runs at step 3, *before* the regen pass at step 4 — a character engaged mid-tick is already session-joined when regen candidates are selected. Walk-in aggro runs in the Daphne process concurrently with the ticker, so a sub-second interleave (session created between candidate selection and regen apply) is theoretically possible and at most one tick wide — but it could only yield a silent status update, never the observed text.

### 4. Per-bar note

- **Vitality & Longevity**: both regenerate in the same Phase 4 pass, same in-combat exclusion, both silent; rates differ (`VITALITY_REGEN_SECS = 120`, `LONGEVITY_REGEN_SECS = 3600`, `models.py:14-15`).
- **Acuity**: not part of Phase 4 at all. It drifts toward baseline in Phase 2 (`run_tick_engine.py:1129-1168`), every tick, silent, and **does not check combat membership** — its only exclusion is characters carrying an active acuity-shift effect component (`run_tick_engine.py:1136-1145`). If out-of-combat-only ever becomes the ruling, Acuity drift is governed by a different (absent) check than the two regen bars.

### 5. Still current vs. v19

The mechanism is **unchanged from v19 to today**. The v19-era Phase 4 (commit `8ab51cd`, file lines 940-993) has the identical query, the identical exclusion (`if not char.combat_sessions.filter(is_active=True).exists()`, v19 line 958), and the identical silent status-only send. Since v19: v20 `b8073bc` added the `in_combat` field to status payloads (display only), v21 #17 added respawn-tick aggro at tick step 3 (before regen — no race introduced), and v22 B6 (#135) touched only Phase 3 effect-expiry messaging. None altered regen selection, ordering relative to engagement, or messaging.

---

*Not addressed here by design: whether in-combat healing via consumables (or the Acuity-drift asymmetry above) is intended. Classification stays with the design chat.*
...
- Milestone: Version 23
- Assignees: KnightOfNight
- Created: 2026-07-18 | Updated: 2026-07-22
- Blocked by: none
- Blocks: none
