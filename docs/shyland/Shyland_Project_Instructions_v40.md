# Shyland Project Instructions v40

**Status: ACTIVE — approved by the operator 2026-09-16. Supersedes Shyland_Project_Instructions_v39.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v40 edition is the playtest-delivery pass (cut at the V26.2 closeout, 2026-09-16, on the flag recorded in the V26.2 Brief 1 closeout report): operator playtests are delivered interactively — one step at a time with ready-to-paste code, never a full-checklist dump — codified as the Playtest delivery rule under Brief Writing Rules. (v39 the three-bars truth pass — the Three Bars section updated to shipped v26.1 reality, no process rule changes; v38 the playtest-format pass — every operator playtest checklist is explicit numbered run/expect/decision sequences, never prose; v37 the daemon-inspection pass; v36 the technical-coherence pass; v35 the action-item pass; v34 the interim-retirement pass; v33 the prod-verification ruling pass; v32 the gift-language pass; v31 the seed-prod pass; v30 the label model; v29 the release model; v28 the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
...
- **The action-item paradigm (operator-ruled 2026-08-15, proven on #249; codified v35):** a **long-lived ticket whose work is deliberately partitioned across releases** — a process build, a multi-part arc — **never joins a release milestone**, even when a release ships one of its parts. Milestones hold the founding ticket plus same-release dependencies only, and the closeout entry gate demands the milestone closed N/N: a ticket that outlives the release would deadlock the closeout of a release that did its job. Instead, the release-bound slice enters the release as **ordinary brief scope**, recorded by a **ruling comment on the ticket** naming the carrier release; the ticket accumulates its parts as comments and **closes in an ops session** when the arc completes. **Litmus:** closes with the release → dependency, milestone member; outlives the release → comments and labels only, never the milestone. (#249's Part 2 rode V24.29 this way while #250, which closed with the release, correctly joined the milestone.)
...
- **Technical coherence (#252, operator-ruled 2026-08-15): a brief never asserts an unverified fact about existing code.** Every structural claim — a function's body or shape, a tuple's members, a call site's behavior, field names — is **verified against the code at writing time**, and the brief records that it was ("confirmed against the models" — the style V24.28 §7.2 used is the norm, not the exception). **Recall is not a source**; every claim is one file-read from correct. Before commit, the design session gives the brief **one end-to-end read for self-consistency** — a brief must not contradict itself. (Born from V24.29 Brief 1: four unverified claims, one of which shipped #251's non-uniform setter family.)
...
- **Executor checkpoint (#248, operator-ruled 2026-08-15): no step enters a PENDING DEPLOY-TIME ACTIONS block without a named executor** — a sanctioned posture-setting target (`make deploy-prod`, `make seed-prod`, `make verify-prod`), the seed's own verification pass, or **explicitly the operator**. A brief may not write a prod-side step no session can run. Classification rule: a **permanent data-shape invariant** ("nothing sits outside its rung") belongs in **seed verification** — it rides `make seed-prod`, executes on production for free, and re-running forever is a feature; one-time surveys do not go in the forever-pass. **Read-only prod inspection steps name `make verify-prod VERIFY=verify_<name>` as executor (#249, live since V24.29):** the named command is a `verify_*` management command the release itself ships — a `VerificationCommand` subclass (`apps/shyland/verification.py`; forced-rollback atomic block, exit code as the outcome signal, report-never-repair), dev-tested in-release via `make verify VERIFY=verify_<name>`
...
- **Playtest-step format (operator-ruled 2026-09-13): every playtest step is an explicit numbered run/expect/decision sequence, never prose.** Each step gives (a) the exact command or action to run, (b) the exact expected observation together with the precondition that makes it expected, and (c) an explicit fork — stop-and-report vs continue. Compressed prose descriptions of a playtest are not checklists; a step the operator cannot execute and adjudicate mechanically is not done being written. Applies to briefs and to closeout-report recipe rewrites alike.
- **Playtest delivery (operator-ruled 2026-09-16): playtests are administered interactively in-session, never as a full-checklist dump.** The session opens with a brief summary of the steps, then collects the information needed to build exact ready-to-paste commands (character name and the like), then delivers **one step at a time** — each with its ready-to-paste code — with the operator reporting the observation back between steps; each step's own stop-and-report vs continue fork governs from there. The brief's written checklist (the playtest-step format above) remains the artifact of record; interactive delivery is how it runs.
...
- **Longevity** — the slow burn, a pure fuel tank since v26.1: it gates, it never scales (the three-tanks doctrine, GDD §4.4). **Its first drain shipped in v26.1 (#70):** a contested flee attempt costs 25% of `longevity_max`, exact-or-refused (below the cost the refusal is free — no fuel, no flee, no cooldown). Recovers slowly — nominal 15 minutes from zero (`LONGEVITY_REGEN_SECS = 900`, the regime rule); the Stamina Potion is its Healing-Draught-mirror restorative. Future abilities draw it as mana fuel at smaller percentages.
