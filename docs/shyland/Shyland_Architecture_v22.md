...
- **`get_item_suffix(item)`** — the per-item info suffix: `— +N carry capacity` for bags, `— N% durability` / `— BROKEN` for gear that wears, empty otherwise (moved here from the consumer's old `_item_suffix`).
...
|---|---|---|
| Stamped | `combat` (+ the brief-5 family `combat-hit-out`, `combat-crit-out`, `combat-hit-in`, `combat-miss` — Section 4.16) | Hits, misses, crits, damage lines, engagement/flee lines |
...
| Miss, either direction (v22 recolor) | `combat-miss` | `var(--muted)` = `#6b6b80` |
| Gains and good outcomes: kill / XP / level / death_message / pickups / heal-to-full / loot lines / "Combat has ended." (v22; amendment 1 #124) | `reward` | `var(--loot-color)` = `var(--success-color)` = `#4caf7d` |
...
| System / ambient (decay, connection) | `system`, `room` | `var(--muted)` = `#6b6b80` |
