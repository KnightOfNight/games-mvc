...
| Player's outgoing hit (and generic `combat`) | `combat-hit-out`, `combat` | `#C4453F` |
| Player's outgoing critical | `combat-crit-out` | `#E24B4A`, bold |
| Incoming hit (NPC hits the player) | `combat-hit-in` | `#E0724A` |
| Incoming critical (v22, DD §2 — dormant until NPC crits occur; wired) | `combat-crit-in` | `#F08A50`, bold |
| Miss, either direction (v22 recolor) | `combat-miss` | `var(--muted)` = `#6b6b80` |
| Gains: kill / XP / level / death_message / pickups / heal-to-full (v22) | `reward` | `var(--loot-color)` = `#4caf7d` |
| Transactional success prose (v22, DD §6) | `success` | `var(--value-color)` = `#E8E4D8` |
| CLI error (v22, #121 — the amber defect is dead) | `error` | `var(--error)` = `#cc4444` |
...
**Rarity scale — flag block only.** The client colorizes the #48 flag block inside any message text (`appendFlaggedText()`, a presentational span-wrapper over the unchanged composed text): Common `#9C9A90`, Uncommon `#5FA8D3`, Rare `#B387E8`, Epic `#D8B45A`, Legendary `#E0724A`, Artifact `#E24B4A`; brackets, commas, and `Bound`/`Unbound` render `#9C9A90` (bind word renamed in v21 brief 1, #60 — the client's `FLAG_RE` changed in lockstep). The flag block keeps this colorization in every context, including inside v21's value-colored report lines. Composition itself (Section 4.6) is untouched.
