...
### 4.14 Command grammar, dispatch guard, and tab completion (v20 brief 3 — #22, #3, #21, #19, #20, #23, #45, #48)

**The resolver (`command_grammar.py`).** The single authoritative reference matcher. Every command that takes an object reference resolves through `resolve(verb, args, candidates)` — the old `parse_item_noun`, the `startswith` matchers in `cmd_buy`/`cmd_sell`/`cmd_repair`, and `parse_npc_noun` are gone; no second matcher survives. (`parse_corpse_noun` remains for corpse targeting only.)

*Grammar:* `<verb> [all | N] [rarity] [noun]`, plus the retained index form `[rarity] N.noun`. `all` = every matching instance; `N` = exactly N, all-or-nothing (fewer available refuses with a count, e.g. `You only have 3.`); `rarity` is a closed vocabulary (`common uncommon rare epic legendary artifact`) applied as an **instance filter before matching and selection**. The noun may be omitted only when a rarity qualifier is present (`sell all common`); a bare `sell all` refuses with a usage line. **Authoring law, enforced by the seed verify:** no `ItemDefinition` or `NpcDefinition` name begins with a rarity word, so a leading rarity token is never a name token.

*Matching:* the reference's tokens must prefix-match the tokens of the item's **player-visible name with tier** (`get_display_name_with_tier`, lowercased — suppressed-suffix items carry no `mk N` tokens; unidentified items match on their mystery name) as an **ordered subsequence** (`axe`, `battle axe`, `axe 1`, `b a` all match `battle axe mk 1`; `rap` does not match `carapace`; `d h` does not match `healing draught`). Case-insensitive. A token with no match falls back through plural forms in order: strip `es`, strip `s`, `ves`→`fe`, `ies`→`y`.

*Ambiguity and selection:* matches spanning multiple distinct definitions refuse with a one-line disambiguation list (`Which do you mean: …?`) — never guess across definitions; the one exception is an explicit `all` with no noun (`sell all common`, `loot all`), where the player asked for everything. Within one definition, selection is deterministic per verb: `sell`/`drop` pick lowest rarity → most damaged → oldest; `equip` picks highest rarity → best condition → oldest; every other verb picks oldest (age = `created_at`, pk tiebreak). `N.noun` indexes age order, stable across calls and input order. **Equipped items never resolve for `sell`/`drop`, including under `all`;** if the only matches were equipped: `You'll have to unequip it first.`

*Candidate scoping (per verb, supplied by the consumer):* `use` → carried consumables; `equip` → carried equippables (`valid_slots` non-empty); `unequip` → equipped items; `sell`/`drop` → carried items (equipped excluded in-resolver so the refusal message can fire); `buy` → the room vendor's `VendorEntry` rows (matched on definition name + tier via `entry_display_name`, rarity `common`); `pickup` → ground items here; `loot` → the targeted corpse's contents; `examine` → carried + ground (falls through to the NPC/corpse matchers on not-found); `repair` → all carried (incl. equipped); `attack`/`kill` → live room NPCs, same ordered token-prefix matching but **no all/N/plurals/rarity** (`You can only attack one target.`); the `N.noun` index is retained for NPCs. The per-verb policy table (`POLICIES`) also owns the refusal message strings, which keep the existing voice.

*Test suite:* `tests/test_command_grammar.py` implements the brief's authoritative 30-case table plus added coverage (54 cases) over in-memory model instances; `tests/test_dispatch_guard.py` covers the guard and the DB-free completion slices. Run with the other suites via explicit module paths (Section 8's `tests.py` collision note still applies).

**Command adoption (#3, #21; loot semantics re-ruled by amendment 1, #62).** All eleven object commands resolve through the module. **Loot** has three paths: bare `loot` and `loot <corpse> [item|all]` target a single corpse exactly as before (including the `… is already empty.` line); **`loot all` is a room sweep** — every corpse in the room the character may loot, per-item and coin lines exactly as single-corpse looting emits them, empty corpses making no individual noise, ending with one summary message (`Looted 3 corpses; 1 carried nothing worth taking.` — singular at 1, second clause omitted at 0); and item references without a named corpse (`loot all hides`, `loot 2 hides`, `loot hide`) resolve over the **union of all lootable corpse contents in the room**, quantifiers still meaning items (all-or-nothing `N`), coin drops coming along from each corpse an item is taken from. Ownership rules are enforced per corpse in every path — the sweep is a convenience over corpse-by-corpse looting, never a way to take loot that is not the player's. `sell all <noun>` / `sell N <noun>` / `sell all <rarity>` emit each sale line plus one total (`Sold 3 items for 2 silvers, 4 coppers.`), skipping zero-value items with a skip summary (`(2 worthless items skipped.)`); single-item sells keep the `That's not worth anything to me.` refusal. `buy N` checks funds and carry capacity up front for the whole quantity and `do_buy(entry, character, qty)` performs the entire purchase in **one transaction** — never partial. A verification-time fix rode along: `check_corpse_empty_and_delete` now queries the table directly instead of `corpse.contents.exists()`, which read a stale `prefetch_related` cache and left emptied corpses undeleted until a second loot or decay.

**Bulk-operation messaging rule (amendment 1, #63 — binding).** Bulk operations narrate as **streams of per-event messages plus a summary message, never joined batches**: each per-item line (a sale, a looted item, a coin take) is its own trip through the delivery choke point with its own `ts`/`seq` and stamped category, and the summary line (and the zero-value skip summary, where applicable) is likewise its own message. One trip through the choke point = one envelope = one stamp — a multi-sentence single message under one timestamp is a bug. Applies to all bulk-sell forms and the loot sweep; `buy N` intentionally remains one message because the atomic all-or-nothing purchase is a single event. `repair all` predated the rule and joined its lines; **Brief 4 Amendment 1 (#74) fixed it identically** (one message per attempt — success, failure, or afford-stop — plus a separate summary) and completed an **all-vintage emitter audit**: every other multi-item output path was verified clean — bulk sell and the loot sweep (fixed previously under #63/#62), `buy N` (exempt, atomic), per-corpse decay broadcasts and per-level level-up messages (already per-event), single-event messages with enumerating riders (death's broken-items list, effect riders on a hit line, equip/unequip blocker refusals), and the `report`/`room-render` renderings (inventory, wallet, who, help, stats, vendor list, examine, travel listing, room block), which are single-message state reports by design (#56), not bulk operations.

**Dispatch table and guard (#20).** `receive_json`'s verb chain is now `COMMAND_TABLE` — verb/alias → (handler, takes-args) — which is the source of truth for implemented commands (per CLAUDE.md) and the origin of the connect-time verb list. The entire dispatch is wrapped in a catch-all: any handler exception logs the full traceback server-side (`shyland.commands` logger), sends the player one `error`-category line (`Something went wrong with that command.`), and leaves the connection alive.

**Movement blocked in combat (#23).** All six directional moves and `travel` refuse during an active combat session — `You can't just walk away from a fight — flee!` (`error` category), before any movement side effects. `flee` is unchanged and remains the only exit from combat by movement.

**Timestamps preference (#45).** `Character.show_timestamps` (Section 4.1) + the `timestamps on|off` command (explicit value required; bare `timestamps` → `Usage: timestamps on|off`). The toggle writes the field, pushes a fresh `status` payload **before** the confirmation line so the confirmation already renders under the new preference. The preference reaches the client in the connect-time `verbs` message (so the very first stamped lines honor it, including on reconnect and other devices) and in every consumer-built `status` payload (now produced by one `_status_payload()` helper). The client gates the `tsSpan()` prefix on `showTimestamps && !UNSTAMPED_CATEGORIES.has(category)` — the preference governs stamped categories only; envelope fields are always present.

**Item-line composition (#48).** See Section 4.6: `compose_item_line()` renders every item line as `<name with tier>[ xN]  <suffix>  [<Rarity>, Bound|Unbound]` (bind word renamed in v21 brief 1, #60; composition otherwise unchanged). Inventory grouping (consumables by definition/tier/rarity) feeds its count through the helper's `xN`; the room render listed ground items under an interim `On the ground:` section, which v20 brief 5 absorbed into the `What's here?` section (Section 4.16) — composed lines, identical lines collapsed to `xN`, unchanged.
...
**Player help (#84 — v21 brief 1).** `cmd_help` renders from a class-level `HELP_COMMANDS` table (command, description) with computed em-dash column padding, so alignment survives edits. Ruled content: the **movement line is static** — `Movement: north (n), south (s), east (e), west (w), up (u), down (d)` — help documents the command set, not the current room (the old per-room exit filtering is gone); the `say` entry is one line (`speak to players here; NPCs may listen too` — the v19 brief-9 second line is folded in); the `brief on | off` command is documented (it existed but was missing from help; #83 closed into this); every command taking item arguments references **`<item selection>`**, and one trailing `Item selection:` section explains the grammar once (name/prefix, `2.axe` index, `3 axes` quantity, `all axes`, rarity filters) — the old `Item syntax:` block is gone; a blank line precedes the closing `Tab completes commands and item names.` note; and **every bracketed alternative uses spaces around the pipe** (`[<item selection> | all]`, `brief on | off`, `timestamps on | off`) — one style, no exceptions. **Amendment 1 (#84 completion):** help renders in the structured key/value form (Section 4.16) — `Movement:` is one key+value line, `Commands:` and `Item selection:` are key-only headers, every other line is a value line — section headers in key-color, content in value-color; and the command lines are **sorted alphabetically by leading command word at render** (table order is irrelevant). The aligned em-dash description columns are preserved within the value lines.
