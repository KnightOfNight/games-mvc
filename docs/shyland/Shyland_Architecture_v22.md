...
**Travel listing (interim, B4 deferral).** The destination listing sorts ascending by straight-line map-space distance from the player (same-zone; other-zone entries follow, by name), every entry labeled `(shard)` (checkpoint) or `(sphere)` (obelisk). Nothing else about travel changed; the travel redesign belongs to a future zones-and-travel version.
...
**Stats pane content (Amendment 1 — #71, #72).** The pane header is **`Character.name` verbatim** — byte-for-byte, no `text-transform` or case styling anywhere in its CSS/JS path — server-supplied as `character_name` in every state-sync payload (consumer and tick engine), never client-derived; it turns combat-red with the rest of the section. The V/A/L text lines are replaced by: **Vitality and Longevity ratio bars** (track in the pane's dark neutral at the fight panel's 8 px bar height; fills `var(--success-color)` — the loot-message green verbatim, full strength, per the v22 B2 amendment 2 repaint; numerals alongside in dim text, right-aligned, matching the `stats` command's values) and the **Acuity band gauge** — not a ratio: a fixed 0.0–2.0 track (uniform across Origins for v1), the character's Origin optimal band (`acuity_band_low`..`acuity_band_high`, already server-supplied in state sync) as a **solid `var(--success-color)` block** (amendment 2 ended the band's translucency era — no alpha anywhere on it), and current acuity as a `var(--say-color)` gold position tick clamped to the track, numeral alongside (one decimal, matching `stats`). Numerals remain real text (the accessible values); tracks and gauge are decorative (`aria-hidden`). The first UI surface that teaches the band mechanic rather than reporting a number.
