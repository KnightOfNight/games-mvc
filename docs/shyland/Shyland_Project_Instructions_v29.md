# Shyland Project Instructions v29

**Status: ACTIVE — approved by the operator 2026-07-29. Supersedes Shyland_Project_Instructions_v28.md.**

These instructions govern every Claude Code session doing Shyland work, of every type. Read them at the start of any Shyland session. **The document's version is an independent counter, decoupled from the game version (operator ruling 2026-07-27):** every refresh that changes process rules produces the next-numbered edition, whenever that happens; the highest-numbered edition is always the active one. This decoupling applies to the instructions doc only — the GDD and architecture doc remain in game-version lockstep. This v29 edition is the release-model pass (#173): a constant stream of point releases — one release type, the major number riding `N.0` when something big starts, the `Version N` milestone as the ordered feature queue, the cadence/B-labels/brief-cap retired, and the closeout marker-sweep self-check. (v28 added the playtest-disposition gate; v27 the GDD-errata rule; v26 the end rituals; v25 the closeout session type.)
...
- **There is no end-of-major ritual.** Every release closes itself completely; a major ends by implication when the next major's `N.0` ships. When the major's feature list empties, its `Version N` milestone closes — the shipping release's changelog row notes it, and that's the whole event.
- **Numbering:** M is a **minor version number, not a decimal** — 21.5 < 21.15, ordering numeric on M.
- **Milestones:** `Version N` holds the major's planned features **only**, in operator-ruled order — the queue the release stream drains. Each release gets its own `Version N.M` milestone with its founding ticket. Names are exactly `Version N` / `Version N.M`. The first design session of a new major re-triages `Version N`'s residents against the theme (off-theme issues leave — unmilestoned or to capability milestones like Healing Economy — and ship as ordinary point releases whenever chosen).
...
- **"Defer" means:** not now, plus a GitHub issue (in a future major's `Version N` list, a capability milestone, or unmilestoned for someday).
...
- The **per-release scope law** (one founding ticket, one brief) and the **`Version N` queue** bind design sessions during planning; a major's first design session re-triages the queue against the theme and rules its order
