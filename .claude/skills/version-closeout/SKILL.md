...
Authority: the Workflow section of the highest-numbered `Shyland_Project_Instructions_vN.md` — read it before starting; if it and this skill disagree, the instructions doc wins and the disagreement is reported. **Every release runs this same ritual** (release milestones are always `Version N.M` — the only milestones that exist; major membership is a permanent `VN` label, never a milestone — #175). An **`N.0` release** — a major's first — additionally runs the major-opening mechanics in step 5.
